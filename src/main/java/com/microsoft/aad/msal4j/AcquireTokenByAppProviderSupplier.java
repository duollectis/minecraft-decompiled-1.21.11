...
class AcquireTokenByAppProviderSupplier extends AuthenticationResultSupplier {
   ...
   AcquireTokenByAppProviderSupplier(
      AbstractClientApplicationBase clientApplication, ClientCredentialRequest clientCredentialRequest, AppTokenProviderParameters appTokenProviderParameters
   ) {
   ...
   private static void validateAndUpdateTokenProviderResult(TokenProviderResult tokenProviderResult) {
      ...
      if (0L == tokenProviderResult.getRefreshInSeconds()) {
         long expireInSeconds = tokenProviderResult.getExpiresInSeconds();
         if (expireInSeconds >= 7200L) {
   ...
   AuthenticationResult execute() throws Exception {
      ...
      TokenRequestExecutor tokenRequestExecutor = new TokenRequestExecutor(
         this.clientCredentialRequest.application().authenticationAuthority, this.msalRequest, this.clientApplication.serviceBundle()
      );
      this.clientApplication
         .tokenCache
         .saveTokens(tokenRequestExecutor, authenticationResult, this.clientCredentialRequest.application().authenticationAuthority.host);
   ...
   public AuthenticationResult fetchTokenUsingAppTokenProvider(AppTokenProviderParameters appTokenProviderParameters) throws ExecutionException, InterruptedException {
      ...
      try {
         ...
         throw new MsalAzureSDKException(var4);
      }
      ...
      return AuthenticationResult.builder()
         ...
         .metadata(
            AuthenticationResultMetadata.builder()
               .refreshOn(tokenProviderResult.getRefreshInSeconds() > 0L ? tokenProviderResult.getRefreshInSeconds() : 0L)
               .build()
         )
         .build();
   }
