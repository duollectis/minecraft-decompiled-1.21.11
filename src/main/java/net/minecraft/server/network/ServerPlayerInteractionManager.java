...
public class ServerPlayerInteractionManager {
	...
	private static final double REACH_DISTANCE_BONUS = 1.0;
	...
	public boolean changeGameMode(GameMode gameMode) {
		if (gameMode == this.gameMode) {
			...
			this.world
					.getServer()
					.getPlayerManager()
					.sendToAll(new PlayerListS2CPacket(PlayerListS2CPacket.Action.UPDATE_GAME_MODE, this.player));
	...
	private float continueMining(BlockState state, BlockPos pos, int failedStartMiningTime) {
		...
		float f = state.calcBlockBreakingDelta(this.player, this.player.getEntityWorld(), pos) * (i + 1);
		int j = (int) (f * 10.0F);
			...
			if (action == PlayerActionC2SPacket.Action.START_DESTROY_BLOCK) {
				...
				if (pos.equals(this.miningPos)) {
					...
					if (!blockStatex.isAir()) {
						float
								g =
								blockStatex.calcBlockBreakingDelta(this.player, this.player.getEntityWorld(), pos) * (j
										+ 1
								);
						if (g >= 0.7F) {
							this.mining = false;
		...
		if (!this.player.getMainHandStack().canMine(blockState, this.world, pos, this.player)) {
			...
			if (block instanceof OperatorBlock && !this.player.isCreativeLevelTwoOp()) {
				...
				if (SharedConstants.BLOCK_BREAK) {
					LOGGER.info(
							"server broke {} {} -> {}",
							new Object[]{pos, blockState2, this.world.getBlockState(pos)}
					);
				}
	...
	public ActionResult interactItem(ServerPlayerEntity player, World world, ItemStack stack, Hand hand) {
		if (this.gameMode == GameMode.SPECTATOR) {
			...
			if (itemStack == stack && itemStack.getCount() == i && itemStack.getMaxUseTime(player) <= 0
					&& itemStack.getDamage() == j) {
				return actionResult;
			}
			else if (actionResult instanceof ActionResult.Fail && itemStack.getMaxUseTime(player) > 0
					&& !player.isUsingItem()) {
				return actionResult;
			}
			else {
	...
	public ActionResult interactBlock(
		...
		if (!blockState.getBlock().isEnabled(world.getEnabledFeatures())) {
			...
			NamedScreenHandlerFactory
					namedScreenHandlerFactory =
					blockState.createScreenHandlerFactory(world, blockPos);
			...
			if (!bl2) {
				ActionResult
						actionResult =
						blockState.onUseWithItem(player.getStackInHand(hand), world, player, hand, hitResult);
