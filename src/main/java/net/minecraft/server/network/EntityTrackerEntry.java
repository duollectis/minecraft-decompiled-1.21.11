...
public class EntityTrackerEntry {
   ...
   public EntityTrackerEntry(
      ServerWorld world, Entity entity, int tickInterval, boolean alwaysUpdateVelocity, EntityTrackerEntry.TrackerPacketSender packetSender
   ) {
      this.world = world;
   ...
   public void tick() {
      ...
      if (!list.equals(this.lastPassengers)) {
         this.packetSender
            .sendToListenersIf(new EntityPassengersSetS2CPacket(this.entity), player -> list.contains(player) == this.lastPassengers.contains(player));
                  ...
                  new MoveMinecartAlongTrackS2CPacket(
                     ...
                     List.of(
                        new ExperimentalMinecartController.Step(
                           this.entity.getEntityPos(), this.entity.getVelocity(), this.entity.getYaw(), this.entity.getPitch(), 1.0F
                        )
                     )
                  )
               );
   ...
   private static void sendInBundles(ServerPlayerEntity player, List<Packet<? super ClientPlayPacketListener>> packets) {
   ...
   public interface TrackerPacketSender {
      ...
      void sendToListenersIf(Packet<? super ClientPlayPacketListener> packet, Predicate<ServerPlayerEntity> predicate);
   }
}
