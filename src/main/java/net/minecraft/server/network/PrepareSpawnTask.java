...
public class PrepareSpawnTask implements ServerPlayerConfigurationTask {
   ...
   public void sendPacket(Consumer<Packet<?>> sender) {
      try (ErrorReporter.Logging logging = new ErrorReporter.Logging(LOGGER)) {
         ...
         ServerPlayerEntity.SavePos savePos = optional.<ServerPlayerEntity.SavePos>flatMap(view -> view.read(ServerPlayerEntity.SavePos.CODEC))
      ...
      public PrepareSpawnTask.@Nullable PlayerSpawn tryFinish() {
         if (!this.spawnPos.isDone()) {
            ...
            if (this.chunkLoadingFuture == null) {
               ...
               this.chunkCounter
                  .load(
                     this.world, () -> this.chunkLoadingFuture = this.world.getChunkManager().addChunkLoadingTicket(ChunkTicketType.PLAYER_SPAWN, chunkPos, 3)
                  );
