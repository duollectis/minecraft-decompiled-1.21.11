...
public class ServerPlayerEntity extends PlayerEntity {
         ...
         .build(
            new CacheLoader<Component<?>, Integer>() {
               ...
               public Integer load(Component<?> component) {
                  return ((HashCode)component.encode(this.hashOps)
   ...
   protected void readCustomData(ReadView view) {
      ...
      this.sculkShriekerWarningManager = view.<SculkShriekerWarningManager>read("warden_spawn_tracker", SculkShriekerWarningManager.CODEC)
      ...
      this.setLeftShoulderNbt(view.<NbtCompound>read("ShoulderEntityLeft", NbtCompound.CODEC).orElseGet(NbtCompound::new));
      this.setRightShoulderNbt(view.<NbtCompound>read("ShoulderEntityRight", NbtCompound.CODEC).orElseGet(NbtCompound::new));
   }
   ...
   public void readRootVehicle(ReadView view) {
      ...
      if (!optional.isEmpty()) {
         ...
         Entity entity = EntityType.loadEntityWithPassengers(
            optional.get().getReadView("Entity"), serverWorld, SpawnReason.LOAD, entityx -> !serverWorld.tryLoadEntity(entityx) ? null : entityx
         );
         if (entity != null) {
   ...
   private static Optional<ServerPlayerEntity.RespawnPos> findRespawnPosition(ServerWorld world, ServerPlayerEntity.Respawn respawn, boolean bl) {
      ...
      if (block instanceof RespawnAnchorBlock && (bl2 || blockState.get(RespawnAnchorBlock.CHARGES) > 0) && RespawnAnchorBlock.isUsable(world, blockPos)) {
         ...
         if (!bl2 && bl && optional.isPresent()) {
            world.setBlockState(blockPos, blockState.with(RespawnAnchorBlock.CHARGES, blockState.get(RespawnAnchorBlock.CHARGES) - 1), 3);
         }
            ...
            if (bl2) {
               this.setSpawnPoint(
                  new ServerPlayerEntity.Respawn(
                     WorldProperties.SpawnPoint.create(this.getEntityWorld().getRegistryKey(), pos, this.getYaw(), this.getPitch()), false
                  ),
                  true
               );
            }
   ...
   public void requestTeleport(double destX, double destY, double destZ) {
      this.networkHandler
         .requestTeleport(
            new EntityPosition(new Vec3d(destX, destY, destZ), Vec3d.ZERO, 0.0F, 0.0F), PositionFlag.combine(PositionFlag.DELTA, PositionFlag.ROT)
         );
   }
