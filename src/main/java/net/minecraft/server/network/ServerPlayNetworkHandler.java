...
public class ServerPlayNetworkHandler
	...
	private static final int MAX_FLOATING_TICKS = 80;
	private static final int MIN_MOVE_PACKETS_PER_TICK = 5;
	public static final int LOADING_TICKS_TIMEOUT = 60;
	...
	private static final Text
			...
			Text.translatable("chat.disabled.invalid_command_signature").formatted(Formatting.RED);
	...
	public void tick() {
		...
		if (this.server.isPaused() || !this.tickMovement()) {
			...
			if (this.player.getLastActionTime() > 0L
					...
					&& Util.getMeasuringTimeMs() - this.player.getLastActionTime()
					> TimeUnit.MINUTES.toMillis(this.server.getPlayerIdleTimeout())
	...
	private boolean tickMovement() {
		...
		this.player.updatePositionAndAngles(
				this.lastTickX,
				this.lastTickY,
				this.lastTickZ,
				this.player.getYaw(),
		...
		if (this.topmostRiddenEntity != this.player
				&& this.topmostRiddenEntity.getControllingPassenger() == this.player) {
	...
	public boolean accepts(Packet<?> packet) {
		return super.accepts(packet) ? true : this.requestedReconfiguration && this.connection.isOpen()
	...
	private static boolean isMovementInvalid(double x, double y, double z, float yaw, float pitch) {
		return Double.isNaN(x) || Double.isNaN(y) || Double.isNaN(z) || !Floats.isFinite(pitch)
				|| !Floats.isFinite(yaw);
	}
			...
			if (entity != this.player && entity.getControllingPassenger() == this.player
				...
				if (p - o > 100.0 && !this.isHost()) {
					LOGGER.warn(
							...
							new Object[]{entity.getStringifiedName(), this.player.getStringifiedName(), l, m, n}
					);
				...
				if (p > 0.0625) {
					...
					LOGGER.warn(
							...
							new Object[]{entity.getStringifiedName(), this.player.getStringifiedName(), Math.sqrt(p)}
					);
				}

				if (bl2 && serverWorld.isSpaceEmpty(entity, box) || this.isEntityNotCollidingWithBlocks(
						serverWorld,
						entity,
						box,
						g,
	...
	public void onRecipeCategoryOptions(RecipeCategoryOptionsC2SPacket packet) {
		...
		this.player
				.getRecipeBook()
				.setCategoryOptions(packet.getCategory(), packet.isGuiOpen(), packet.isFilteringCraftable());
	}
	...
	public void onRequestCommandCompletions(RequestCommandCompletionsC2SPacket packet) {
		...
		ParseResults<ServerCommandSource>
				parseResults =
				this.server.getCommandManager().getDispatcher().parse(stringReader, this.player.getCommandSource());
		this.server
				.getCommandManager()
				.getDispatcher()
		...
		if (!this.player.isCreativeLevelTwoOp()) {
			...
			if (commandBlockExecutor != null) {
				...
				BlockState
						blockState3 =
						blockState2
								.with(CommandBlock.FACING, direction)
								.with(CommandBlock.CONDITIONAL, packet.isConditional());
				...
				if (!StringHelper.isEmpty(string)) {
					this.player
							.sendMessage(
									Text.translatable(
											this.player.getEntityWorld().areCommandBlocksEnabled()
											? "advMode.setCommand.success" : "advMode.setCommand.disabled", string
									)
							);
				}
			}
			...
			if (commandBlockExecutor != null) {
				...
				if (!StringHelper.isEmpty(string)) {
					this.player.sendMessage(Text.translatable(
							bl ? "advMode.setCommand.success" : "advMode.setCommand.disabled",
							string
					));
				}
			}
		...
		if (this.player.currentScreenHandler instanceof BeaconScreenHandler beaconScreenHandler) {
			if (!this.player.currentScreenHandler.canUse(this.player)) {
				LOGGER.debug(
						...
						this.player,
						this.player.currentScreenHandler
				);
				return;
			}
	...
	public void onUpdateStructureBlock(UpdateStructureBlockC2SPacket packet) {
		...
		if (this.player.isCreativeLevelTwoOp()) {
			...
			if (this.player
					.getEntityWorld()
					.getBlockEntity(blockPos) instanceof StructureBlockBlockEntity structureBlockBlockEntity) {
	...
	public void onTestInstanceBlockAction(TestInstanceBlockActionC2SPacket packet) {
		...
		if (this.player.isCreativeLevelTwoOp()
				&& this.player
				.getEntityWorld()
				.getBlockEntity(blockPos) instanceof TestInstanceBlockEntity testInstanceBlockEntity) {
			if (packet.action() != TestInstanceBlockActionC2SPacket.Action.QUERY
					&& packet.action() != TestInstanceBlockActionC2SPacket.Action.INIT) {
				...
				Registry<TestInstance>
						registry =
						this.player.getRegistryManager().getOrThrow(RegistryKeys.TEST_INSTANCE);
				Optional<RegistryEntry.Reference<TestInstance>>
						optional =
						packet.data().test().flatMap(registry::getOptional);
				Text text;
				if (optional.isPresent()) {
					text = optional.get().value().getDescription();
				}
				else {
				...
				if (packet.action() == TestInstanceBlockActionC2SPacket.Action.QUERY) {
					optional2 = packet.data()
					                  .test()
					                  .flatMap(registryKey -> TestInstanceBlockEntity.getStructureSize(
							                  this.player.getEntityWorld(),
							                  (RegistryKey<TestInstance>) registryKey
					                  ));
				}
				else {
	...
	private void addBook(FilteredMessage title, List<FilteredMessage> pages, int slotId) {
		...
		if (itemStack.contains(DataComponentTypes.WRITABLE_BOOK_CONTENT)) {
			...
			List<RawFilteredPair<Text>> list = pages.stream()
			                                        .<RawFilteredPair<Text>>map(page -> this
					                                        .toRawFilteredPair(page)
					                                        .map(Text::literal))
			                                        .toList();
			itemStack2.set(
					DataComponentTypes.WRITTEN_BOOK_CONTENT,
					new WrittenBookContentComponent(
							this.toRawFilteredPair(title),
							this.player.getStringifiedName(),
							0,
							list,
							true
					)
	...
	public void onQueryBlockNbt(QueryBlockNbtC2SPacket packet) {
		...
		if (this.player.getPermissions().hasPermission(DefaultPermissions.GAMEMASTERS)) {
			...
			NbtCompound
					nbtCompound =
					blockEntity != null ? blockEntity.createNbt(this.player.getRegistryManager()) : null;
			this.sendPacket(new NbtQueryResponseS2CPacket(packet.getTransactionId(), nbtCompound));
		}
	}
						...
						if (this.player.hasVehicle()) {
							...
							if (this.player.isSleeping()) {
								...
								if (serverWorld.getTickManager().shouldTick()) {
									...
									if (this.shouldCheckMovement(bl)) {
										...
										if (p - o > r * q) {
											LOGGER.warn(
													"{} moved too quickly! {},{},{}",
													new Object[]{this.player.getStringifiedName(), l, m, n}
											);
											this.requestTeleport(
													this.player.getX(),
													this.player.getY(),
								...
								if (this.player.noClip
										|| this.player.isSleeping()
										|| (!bl4 || !serverWorld.isSpaceEmpty(this.player, box))
										&& !this.isEntityNotCollidingWithBlocks(
										serverWorld,
										this.player,
										box,
										d,
									...
									if (packet.isOnGround() || this.player.hasLandedInFluid()
											|| this.player.isClimbing() || this.player.isSpectator() || bl || bl5) {
										this.player.tryClearCurrentExplosion();
									}

									this.player.increaseTravelMotionStats(
											this.player.getX() - i,
											this.player.getY() - j,
											this.player.getZ() - k
									);
	...
	private boolean shouldCheckMovement(boolean elytra) {
		if (this.isHost()) {
			...
			return !gameRules.getValue(GameRules.PLAYER_MOVEMENT_CHECK) ? false : !elytra || gameRules.getValue(
	...
	private boolean isEntityNotCollidingWithBlocks(
			WorldView world,
			Entity entity,
			Box box,
			double newX,
				...
				case STAB:
					if (this.player.isSpectator()) {
						...
						PiercingWeaponComponent
								piercingWeaponComponent =
								itemStack.get(DataComponentTypes.PIERCING_WEAPON);
				...
				case STOP_DESTROY_BLOCK:
					this.player
							...
							.processBlockBreakingAction(
									...
									action,
									packet.getDirection(),
									this.player.getEntityWorld().getTopYInclusive(),
									packet.getSequence()
							);
	...
	private static boolean canPlace(ServerPlayerEntity player, ItemStack stack) {
		if (stack.isEmpty()) {
			...
			return (item instanceof BlockItem
					|| item instanceof BucketItem bucketItem && bucketItem.getFluid() != Fluids.EMPTY
			)
		...
		if (this.canInteractWithGame()) {
			...
			if (itemStack.isItemEnabled(serverWorld.getEnabledFeatures())) {
				...
				if (this.player.canInteractWithBlockAt(blockPos, 1.0)) {
					...
					double d = 1.0000001;
					if (Math.abs(vec3d2.getX()) < 1.0000001 && Math.abs(vec3d2.getY()) < 1.0000001
							&& Math.abs(vec3d2.getZ()) < 1.0000001) {
						Direction direction = blockHitResult.getSide();
						this.player.updateLastActionTime();
						int i = this.player.getEntityWorld().getTopYInclusive();
						if (blockPos.getY() <= i) {
							if (this.requestedTeleportPos == null && serverWorld.canEntityModifyAt(
									this.player,
									blockPos
							)) {
								ActionResult
								...
								if (actionResult.isAccepted()) {
									Criteria.ANY_BLOCK_USE.trigger(
											this.player,
								...
								if (direction == Direction.UP && !actionResult.isAccepted() && blockPos.getY() >= i
										&& canPlace(this.player, itemStack)) {
									Text text = Text.translatable("build.tooHigh", i).formatted(Formatting.RED);
									this.player.sendMessageToClient(text, true);
								}
								else if (actionResult instanceof ActionResult.Success success
						...
						LOGGER.warn(
								...
								new Object[]{this.player.getGameProfile().name(), vec3d, blockPos}
						);
					}
				}
			}
	...
	public void onPlayerInteractItem(PlayerInteractItemC2SPacket packet) {
		...
		if (this.canInteractWithGame()) {
			...
			if (!itemStack.isEmpty() && itemStack.isItemEnabled(serverWorld.getEnabledFeatures())) {
				...
				if (this.player.interactionManager.interactItem(
						this.player,
						serverWorld,
						itemStack,
						hand
				) instanceof ActionResult.Success success
						&& success.swingSource() == ActionResult.SwingSource.SERVER) {
					this.player.swingHand(hand, true);
				}
			}
	...
	private void cleanUp() {
		...
		this.server
				...
				.broadcast(
						Text
								.translatable("multiplayer.player.left", this.player.getDisplayName())
	...
	public void onUpdateSelectedSlot(UpdateSelectedSlotC2SPacket packet) {
		...
		if (packet.getSelectedSlot() >= 0 && packet.getSelectedSlot() < PlayerInventory.getHotbarSize()) {
			if (this.player.getInventory().getSelectedSlot() != packet.getSelectedSlot()
					&& this.player.getActiveHand() == Hand.MAIN_HAND) {
				this.player.clearActiveItem();
			}
		...
		if (!optional.isEmpty()) {
			this.validateMessage(
					packet.chatMessage(), false, () -> {
						...
						CompletableFuture<FilteredMessage>
								completableFuture =
								this.filterText(signedMessage.getSignedContent());
						...
						this.messageChainTaskQueue.append(
								completableFuture, filtered -> {
									SignedMessage
											signedMessage2 =
											signedMessage.withUnsignedContent(text).withFilterMask(filtered.mask());
	...
	private void handleCommandExecution(ChatCommandSignedC2SPacket packet, LastSeenMessageList lastSeenMessages) {
		...
		parseResults =
				CommandManager.withCommandSource(
						parseResults,
						source -> source.withSignedArguments(signedCommandArguments, this.messageChainTaskQueue)
				);
	...
	private void handleMessageChainException(MessageChain.MessageChainException exception) {
		LOGGER.warn(
				...
				this.player.getGameProfile().name(),
				exception.getMessageText().getString()
		);
	...
	private <S> Map<String, SignedMessage> collectArgumentMessages(
		...
		if (list.isEmpty()) {
			...
			for (ArgumentSignatureDataMap.Entry entry : list) {
				...
				MessageBody
						messageBody =
						new MessageBody(parsedArgument.value(), packet.timestamp(), packet.salt(), lastSeenMessages);
	...
	private <S> Map<String, SignedMessage> toUnsignedSignatures(List<SignedArgumentList.ParsedArgument<S>> arguments)
	throws MessageChain.MessageChainException {
	...
	private static <S> MessageChain.MessageChainException createInvalidCommandSignatureException(
			String command,
			List<ArgumentSignatureDataMap.Entry> actual,
			List<SignedArgumentList.ParsedArgument<S>> expected
	) {
		String string = actual.stream().map(ArgumentSignatureDataMap.Entry::name).collect(Collectors.joining(", "));
		String
				string2 =
				expected.stream().map(SignedArgumentList.ParsedArgument::getNodeName).collect(Collectors.joining(", "));
		LOGGER.error(
				"Signed command mismatch between server and client ('{}'): got [{}] from client, but expected [{}]",
				new Object[]{command, string, string2}
		);
	...
	private void validateMessage(String message, boolean bl, Runnable runnable) {
		if (hasIllegalCharacter(message)) {
			...
			this.sendPacket(new GameMessageS2CPacket(
					Text
							.translatable("chat.disabled.options")
	...
	private SignedMessage getSignedMessage(ChatMessageC2SPacket packet, LastSeenMessageList lastSeenMessages)
	throws MessageChain.MessageChainException {
		MessageBody
				messageBody =
				new MessageBody(packet.chatMessage(), packet.timestamp(), packet.salt(), lastSeenMessages);
	...
	private void handleDecoratedMessage(SignedMessage message) {
		this.server
				.getPlayerManager()
				.broadcast(message, this.player, MessageType.params(MessageType.CHAT, this.player));
		this.checkForSpam();
	}
							...
							new PlayerInteractEntityC2SPacket.Handler() {
								private void processInteract(Hand hand, ServerPlayNetworkHandler.Interaction action) {
									...
									if (itemStack.isItemEnabled(serverWorld.getEnabledFeatures())) {
										...
										if (action.run(
												ServerPlayNetworkHandler.this.player,
												entity,
												hand
										) instanceof ActionResult.Success success) {
											ItemStack
													itemStack3 =
													success.shouldIncrementStat() ? itemStack2 : ItemStack.EMPTY;
											Criteria.PLAYER_INTERACTED_WITH_ENTITY.trigger(
													ServerPlayNetworkHandler.this.player,
													itemStack3,
													entity
											);
								...
								public void interactAt(Hand hand, Vec3d pos) {
									this.processInteract(
											hand,
											(player, entityxx, handx) -> entityxx.interactAt(player, pos, handx)
									);
								}
								...
								public void attack() {
									if (!(entity instanceof ItemEntity)
											...
											&& !(entity instanceof PersistentProjectileEntity persistentProjectileEntity
											&& !persistentProjectileEntity.isAttackable()
									)) {
										ItemStack
												itemStack =
												ServerPlayNetworkHandler.this.player.getStackInHand(Hand.MAIN_HAND);
										if (itemStack.isItemEnabled(serverWorld.getEnabledFeatures())) {
											if (!ServerPlayNetworkHandler.this.player.isBelowMinimumAttackCharge(
													itemStack,
													5
											)) {
			...
			case PERFORM_RESPAWN:
				if (this.player.notInAnyWorld) {
					...
					this.player =
							this.server
									.getPlayerManager()
									.respawnPlayer(this.player, true, Entity.RemovalReason.CHANGED_DIMENSION);
					this.syncWithPlayerPosition();
					this.markRespawned();
					...
					this.player =
							this.server
									.getPlayerManager()
									.respawnPlayer(this.player, false, Entity.RemovalReason.KILLED);
					this.syncWithPlayerPosition();
					this.markRespawned();
					if (this.server.isHardcore()) {
						...
						this.player
								.getEntityWorld()
								.getGameRules()
								.setValue(GameRules.SPECTATORS_GENERATE_CHUNKS, false, this.server);
					}
				}
				break;
			case REQUEST_STATS:
				...
				if (!this.player.currentScreenHandler.isValid(i)) {
					LOGGER.debug(
							...
							new Object[]{
									this.player.getStringifiedName(),
									i,
									this.player.currentScreenHandler.slots.size()
							}
					);
				}
				else {
					...
					while (var4.hasNext()) {
						Entry<ItemStackHash> entry = (Entry<ItemStackHash>) var4.next();
						this.player.currentScreenHandler.setReceivedHash(
								entry.getIntKey(),
								(ItemStackHash) entry.getValue()
						);
					}
					...
					if (this.player.getRecipeBook().isUnlocked(recipeEntry.id())) {
						if (this.player.currentScreenHandler instanceof AbstractRecipeScreenHandler abstractRecipeScreenHandler) {
							if (recipeEntry.value().getIngredientPlacement().hasNoPlacement()) {
								LOGGER.debug(
										"Player {} tried to place impossible recipe {}",
										this.player,
							...
							if (postFillAction == AbstractRecipeScreenHandler.PostFillAction.PLACE_GHOST_RECIPE) {
								this.sendPacket(new CraftFailedResponseS2CPacket(
										this.player.currentScreenHandler.syncId,
										serverRecipe.display().display()
								));
							}
						}
					}
	...
	public void onUpdateSign(UpdateSignC2SPacket packet) {
		...
		this
				.filterTexts(list)
				.thenAcceptAsync(texts -> this.onSignUpdate(packet, (List<FilteredMessage>) texts), this.server);
	}
	...
	public void onClientOptions(ClientOptionsC2SPacket packet) {
		...
		if (this.player.isModelPartVisible(PlayerModelPart.HAT) != bl) {
			this.server
					.getPlayerManager()
					.sendToAll(new PlayerListS2CPacket(PlayerListS2CPacket.Action.UPDATE_HAT, this.player));
		}
	}
	...
	public void onUpdateDifficulty(UpdateDifficultyC2SPacket packet) {
		...
		if (!this.player.getPermissions().hasPermission(DefaultPermissions.GAMEMASTERS) && !this.isHost()) {
			LOGGER.warn(
					...
					this.player.getGameProfile().name(),
					packet.difficulty().getTranslatableName()
			);
		}
		else {
		...
		if (!this.requestedReconfiguration) {
			...
			this.connection
					.transitionInbound(
							...
							new ServerConfigurationNetworkHandler(
									this.server,
									this.connection,
									this.createClientData(this.player.getClientOptions())
							)
					);
		}
	}
		...
		this.messageChainTaskQueue.append(() -> {
			...
			this.server
					...
					.sendToAll(new PlayerListS2CPacket(
							EnumSet.of(PlayerListS2CPacket.Action.INITIALIZE_CHAT),
							List.of(this.player)
					));
		});
	}
