...
public record ServerLinks(List<ServerLinks.Entry> entries) {
	...
	public enum Known {
		...
		private static final IntFunction<ServerLinks.Known> FROM_ID = ValueLists.createIndexToValueFunction(
				(ServerLinks.Known known) -> known.id, values(), ValueLists.OutOfBoundsHandling.ZERO
		);
		public static final PacketCodec<ByteBuf, ServerLinks.Known>
				CODEC =
				PacketCodecs.indexed(FROM_ID, known -> known.id);
	...
	public record StringifiedEntry(Either<ServerLinks.Known, Text> type, String link) {
		...
		public static final PacketCodec<ByteBuf, ServerLinks.StringifiedEntry> CODEC = PacketCodec.tuple(
				ServerLinks.TYPE_CODEC,
				ServerLinks.StringifiedEntry::type,
				PacketCodecs.STRING,
				ServerLinks.StringifiedEntry::link,
				ServerLinks.StringifiedEntry::new
		);
	}
}
