...
public class ServerNetworkIo {
   ...
   public SocketAddress bindLocal() {
      ...
      synchronized (this.channels) {
         channelFuture = ((ServerBootstrap)((ServerBootstrap)new ServerBootstrap().channel(NetworkingBackend.local().getServerChannelClass()))
               .childHandler(
                     ...
                     protected void initChannel(Channel channel) {
                        ...
                        if (SharedConstants.FAKE_LATENCY_MS > 0) {
                           channelPipeline.addLast(
                              "latency", new ServerNetworkIo.DelayingChannelInboundHandler(SharedConstants.FAKE_LATENCY_MS, SharedConstants.FAKE_JITTER_MS)
                           );
                        }
