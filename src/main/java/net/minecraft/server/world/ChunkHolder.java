...
public class ChunkHolder extends AbstractChunkHolder {
	...
	private static final CompletableFuture<OptionalChunk<WorldChunk>>
			UNLOADED_WORLD_CHUNK_FUTURE =
			CompletableFuture.completedFuture(UNLOADED_WORLD_CHUNK);
	...
	public void combinePostProcessingFuture(CompletableFuture<?> postProcessingFuture) {
		if (this.postProcessingFuture.isDone()) {
			...
			this.postProcessingFuture =
					this.postProcessingFuture.thenCombine(
							(CompletionStage<? extends Object>) postProcessingFuture,
							(object, object2) -> null
					);
		}
	}
	...
	protected void combineSavingFuture(CompletableFuture<?> savingFuture) {
		if (this.savingFuture.isDone()) {
			...
			this.savingFuture =
					this.savingFuture.thenCombine(
							(CompletionStage<? extends Object>) savingFuture,
							(object, thenResult) -> null
					);
		}
	}
	...
	public boolean markForBlockUpdate(BlockPos pos) {
		...
		if (worldChunk == null) {
			...
			if (shortSet == null) {
				...
				this.blockUpdatesBySection[i] = shortSet;
			}
		...
		if (this.hasPendingUpdates()) {
			...
			if (!this.skyLightUpdateBits.isEmpty() || !this.blockLightUpdateBits.isEmpty()) {
				List<ServerPlayerEntity>
						list =
						this.playersWatchingChunkProvider.getPlayersWatchingChunk(this.pos, true);
				if (!list.isEmpty()) {
			...
			if (this.pendingBlockUpdates) {
				List<ServerPlayerEntity>
						list =
						this.playersWatchingChunkProvider.getPlayersWatchingChunk(this.pos, false);
						...
						if (!list.isEmpty()) {
							...
							if (shortSet.size() == 1) {
								...
								ChunkDeltaUpdateS2CPacket
										chunkDeltaUpdateS2CPacket =
										new ChunkDeltaUpdateS2CPacket(chunkSectionPos, shortSet, chunkSection);
								this.sendPacketToPlayers(list, chunkDeltaUpdateS2CPacket);
								chunkDeltaUpdateS2CPacket.visitUpdates((pos, state) -> this.tryUpdateBlockEntityAt(
										list,
										world,
										pos,
										state
	...
	private void increaseLevel(
			...
			CompletableFuture<OptionalChunk<WorldChunk>> chunkFuture,
			Executor executor,
			ChunkLevelType target
	) {
