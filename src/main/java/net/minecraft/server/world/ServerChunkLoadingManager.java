...
public class ServerChunkLoadingManager extends VersionedChunkStorage implements ChunkHolder.PlayersWatchingChunkProvider, ChunkLoadingManager {
   ...
   private NbtCompound updateChunkNbt(NbtCompound nbt) {
      return this.updateChunkNbt(nbt, -1, getContextNbt(this.world.getRegistryKey(), this.getChunkGenerator().getCodecKey()));
   }
   ...
   protected void tickEntityMovement() {
      ...
      while (var3.hasNext()) {
         ...
         if (bl || entityTracker.entity.velocityDirty || this.levelManager.shouldTickEntities(chunkSectionPos2.toChunkPos().toLong())) {
      ...
      if (!list.isEmpty()) {
         ...
         while (var3.hasNext()) {
            ServerChunkLoadingManager.EntityTracker entityTrackerx = (ServerChunkLoadingManager.EntityTracker)var3.next();
