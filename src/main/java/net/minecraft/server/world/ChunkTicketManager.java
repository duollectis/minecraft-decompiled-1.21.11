...
public class ChunkTicketManager extends PersistentState {
   ...
   private static ChunkTicketManager create(List<Pair<ChunkPos, ChunkTicket>> tickets) {
      ...
      for (Pair<ChunkPos, ChunkTicket> pair : tickets) {
         ...
         List<ChunkTicket> list = (List<ChunkTicket>)long2ObjectOpenHashMap.computeIfAbsent(chunkPos.toLong(), l -> new ObjectArrayList(4));
      ...
      while (objectIterator.hasNext()) {
         ...
         while (iterator.hasNext()) {
            ...
            if (predicate.test(chunkTicket, l)) {
               if (transferTo != null) {
                  List<ChunkTicket> list = (List<ChunkTicket>)transferTo.computeIfAbsent(l, pos -> new ObjectArrayList(((List)entry.getValue()).size()));
