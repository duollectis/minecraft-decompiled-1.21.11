...
public class ServerLightingProvider extends LightingProvider implements AutoCloseable {
	...
	public static final int MAX_QUEUED_TASKS = 1000;
	...
	public int doLightUpdates() {
		throw (UnsupportedOperationException) Util.getFatalOrPause(new UnsupportedOperationException(
				"Ran automatically on a different thread!"));
	}
				...
				pos.x, pos.z, () -> 0, ServerLightingProvider.Stage.PRE_UPDATE, Util.debugRunnable(
						() -> {
							...
							for (int i = this.world.getBottomSectionCoord();
							     i <= this.world.getTopSectionCoord();
							     i++) {
	...
	public void setSectionStatus(ChunkSectionPos pos, boolean notReady) {
		this.enqueue(
				...
				Util.debugRunnable(
						...
						() -> "updateSectionStatus " + pos + " " + notReady
				)
		);
	}
	...
	public CompletableFuture<Chunk> initializeLight(Chunk chunk, boolean bl) {
		...
		this.enqueue(
				chunkPos.x, chunkPos.z, ServerLightingProvider.Stage.PRE_UPDATE, Util.debugRunnable(
						() -> {
	...
	private void runTasks() {
		...
		for (j = 0; objectListIterator.hasNext() && j < i; j++) {
			Pair<ServerLightingProvider.Stage, Runnable>
					pair =
					(Pair<ServerLightingProvider.Stage, Runnable>) objectListIterator.next();
