...
public class ServerChunkManager extends ChunkManager {
	...
	public @Nullable Chunk getChunk(int x, int z, ChunkStatus leastStatus, boolean create) {
		if (Thread.currentThread() != this.serverThread) {
			return CompletableFuture
					.<Chunk>supplyAsync(() -> this.getChunk(x, z, leastStatus, create), this.mainThreadExecutor)
					.join();
		}
		else {
			...
			if (chunk2 == null && create) {
				throw (IllegalStateException) Util.getFatalOrPause(new IllegalStateException(
						"Chunk not there when requested: " + optionalChunk.getError()));
			}
			else {
	...
	public CompletableFuture<OptionalChunk<Chunk>> getChunkFutureSyncOnMainThread(
			int chunkX,
			int chunkZ,
	...
	private CompletableFuture<OptionalChunk<Chunk>> getChunkFuture(
			int chunkX,
			int chunkZ,
			ChunkStatus leastStatus,
			boolean create
		...
		return this.isMissingForLevel(chunkHolder, i) ? AbstractChunkHolder.UNLOADED_FUTURE
		                                              : chunkHolder.load(leastStatus, this.chunkLoadingManager);
	}
	...
	public boolean isTickingFutureReady(long pos) {
		if (!this.world.shouldTickBlocksInChunk(pos)) {
			...
			return chunkHolder == null ? false : chunkHolder
			                                     .getTickingFuture()
	...
	public CompletableFuture<?> addChunkLoadingTicket(ChunkTicketType ticketType, ChunkPos pos, int radius) {
		if (!ticketType.isForLoading()) {
			...
			throw new IllegalStateException(
					"Ticket type " + ticketType + " can expire before it loads, cannot fetch asynchronously");
		}
		else {
	...
	@Debug
	public SpawnHelper.@Nullable Info getSpawnInfo() {
