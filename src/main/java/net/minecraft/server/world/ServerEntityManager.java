...
public class ServerEntityManager<T extends EntityLike> implements AutoCloseable {
	...
	private void unloadChunks() {
		this.pendingUnloads.removeIf(pos -> this.trackingStatuses.get(pos) != EntityTrackingStatus.HIDDEN ? true
		                                                                                                  : this.unload(
				                                                                                                  pos));
	}
	...
	public void dump(Writer writer) throws IOException {
		...
		this.cache
				...
				.forEach(
						chunkPos -> {
							ServerEntityManager.Status
									status =
									(ServerEntityManager.Status) this.managedStatuses.get(chunkPos);
							this.cache
									...
									.forEach(
											sectionPos -> {
												EntityTrackingSection<T>
														...
														this.cache.findTrackingSection(sectionPos);
	...
	class Listener implements EntityChangeListener {
				...
				if (!this.section.remove(this.entity)) {
					ServerEntityManager.LOGGER
							.warn(
									...
									new Object[]{this.entity, ChunkSectionPos.from(this.sectionPos), l}
							);
				}
		...
		private void updateLoadStatus(EntityTrackingStatus oldStatus, EntityTrackingStatus newStatus) {
			...
			EntityTrackingStatus
					entityTrackingStatus2 =
					ServerEntityManager.getNeededLoadStatus(this.entity, newStatus);
			...
			if (!this.section.remove(this.entity)) {
				ServerEntityManager.LOGGER
						.warn(
								...
								new Object[]{this.entity, ChunkSectionPos.from(this.sectionPos), reason}
						);
			}

			EntityTrackingStatus
					entityTrackingStatus =
					ServerEntityManager.getNeededLoadStatus(this.entity, this.section.getStatus());
			if (entityTrackingStatus.shouldTick()) {
				ServerEntityManager.this.stopTicking(this.entity);
			}
