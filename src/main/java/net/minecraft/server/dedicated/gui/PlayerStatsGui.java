...
public class PlayerStatsGui extends JComponent {
	...
	private void update() {
		...
		this.lines[0] =
				"Memory use: " + l / 1024L / 1024L + " mb (" + Runtime.getRuntime().freeMemory() * 100L / Runtime
						.getRuntime()
						.maxMemory() + "% free)";
		this.lines[1] =
				"Avg tick: " + AVG_TICK_FORMAT.format(
						(double) this.server.getAverageNanosPerTick() / TimeHelper.MILLI_IN_NANOS) + " ms";
		this.memoryUsePercentage[this.memoryUsePercentagePos++ & 0xFF] =
				(int) (l * 100L / Runtime.getRuntime().maxMemory());
		this.repaint();
	}
