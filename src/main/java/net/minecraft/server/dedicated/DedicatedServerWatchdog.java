...
public class DedicatedServerWatchdog implements Runnable {
	...
	private static final long WATCHDOG_INTERVAL_MS = 10000L;
	private static final int WATCHDOG_THREAD_PRIORITY = 1;
			...
			if (n > this.maxTickTime) {
				LOGGER.error(
						...
						String.format(
								Locale.ROOT,
								"%.2f",
								this.server.getTickManager().getMillisPerTick() / (float) TimeHelper.SECOND_IN_MILLIS
						)
				);
				...
				crashReportSection.add(
						...
						() -> this.server
								.getSaveProperties()
								.getGameRules()
								.getRuleValueName(GameRules.RANDOM_TICK_SPEED)
				);
				crashReportSection.add(
						"Level stats",
