...
public class AllowlistRpcDispatcher {
	...
	public static List<RpcPlayer> get(ManagementHandlerDispatcher dispatcher) {
		return dispatcher.getAllowlistHandler()
		                 ...
		                 .map(player -> RpcPlayer.of(player.getKey()))
		                 .toList();
	}

	public static List<RpcPlayer> add(
	...
	public static List<RpcPlayer> remove(
			ManagementHandlerDispatcher dispatcher,
			List<RpcPlayer> players,
			ManagementConnectionId remote
	) {
	...
	public static List<RpcPlayer> set(
			ManagementHandlerDispatcher dispatcher,
			List<RpcPlayer> players,
			ManagementConnectionId remote
	) {
		...
		set2
				...
				.forEach(player -> dispatcher.getAllowlistHandler().remove(player, remote));
		set
				.stream()
				.filter(player -> !set2.contains(player))
				.forEach(player -> dispatcher.getAllowlistHandler().add(new WhitelistEntry(player), remote));
		dispatcher.getAllowlistHandler().kickUnlisted(remote);
		return get(dispatcher);
	}
}
