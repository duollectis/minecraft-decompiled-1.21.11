      ...
      public JsonElement handle(ManagementHandlerDispatcher dispatcher, @Nullable JsonElement parameters, ManagementConnectionId remote) {
         if (parameters != null && (parameters.isJsonArray() || parameters.isJsonObject())) {
            if (this.info.params().isEmpty()) {
               ...
               Params object = (Params)this.info.params().get().schema().codec().parse(JsonOps.INSTANCE, jsonElement2).getOrThrow(RpcException::new);
               ...
               if (this.info.result().isEmpty()) {
                  ...
                  return (JsonElement)this.info.result().get().schema().codec().encodeStart(JsonOps.INSTANCE, object2).getOrThrow(RpcEncodingException::new);
               }
            }
         } else {
         ...
         if (parameters == null || parameters.isJsonArray() && parameters.getAsJsonArray().isEmpty()) {
            if (this.info.params().isPresent()) {
               ...
               if (this.info.result().isEmpty()) {
                  ...
                  return (JsonElement)this.info.result().get().schema().codec().encodeStart(JsonOps.INSTANCE, object).getOrThrow(RpcException::new);
               }
            }
         } else {
