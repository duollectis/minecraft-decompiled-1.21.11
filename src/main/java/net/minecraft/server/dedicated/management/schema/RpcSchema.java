...
public record RpcSchema<T>(
   ...
   public static final Codec<RpcSchema<?>> CODEC = (Codec<RpcSchema<?>>) (Codec<?>) Codec.<RpcSchema<?>>recursive(
         ...
         codec -> RecordCodecBuilder.<RpcSchema<?>>create(
            instance -> instance.group(
                  ...
                  Codecs.listOrSingle(Codec.STRING).optionalFieldOf("type", List.of()).forGetter(schema -> schema.type),
                  codec.optionalFieldOf("items").forGetter(schema -> (Optional) schema.items),
                  Codec.unboundedMap(Codec.STRING, codec).optionalFieldOf("properties", Map.of()).forGetter(schema -> (Map) schema.properties),
                  Codec.STRING.listOf().optionalFieldOf("enum", List.of()).forGetter(schema -> schema.enumValues)
               )
               .apply(instance, (ref, type, items, properties, enumValues) -> null)
         )
      )
      .validate(schema -> schema == null
   ...
   public RpcSchema<T> copy() {
      return new RpcSchema<>(
         ...
         this.properties.entrySet().stream().collect(Collectors.toMap(Entry::getKey, entry -> entry.getValue().copy())),
         this.enumValues,
         this.codec
      );
   }
   ...
   public static <E extends Enum<E> & StringIdentifiable> RpcSchema<E> ofEnum(Supplier<E[]> values, Codec<E> codec) {
      List<String> list = Stream.<Enum>of((Enum[])values.get()).map(value -> ((StringIdentifiable)value).asString()).toList();
