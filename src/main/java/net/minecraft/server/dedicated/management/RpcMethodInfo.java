...
public record RpcMethodInfo<Params, Result>(
		String description,
		Optional<RpcRequestParameter<Params>> params,
		Optional<RpcResponseResult<Result>> result
) {

	public RpcMethodInfo(
			String description,
			@Nullable RpcRequestParameter<Params> param,
			@Nullable RpcResponseResult<Result> result
	) {
	...
	public record Entry<Params, Result>(Identifier name, RpcMethodInfo<Params, Result> contents) {
		...
		@SuppressWarnings("unchecked")
		public static final Codec<RpcMethodInfo.Entry<?, ?>>
				CODEC =
				(Codec<RpcMethodInfo.Entry<?, ?>>) (Codec<?>) RpcMethodInfo.Entry.<Object, Object>createCodec();
