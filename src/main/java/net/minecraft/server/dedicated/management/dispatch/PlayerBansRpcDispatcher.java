...
public class PlayerBansRpcDispatcher {
	...
	public static List<PlayerBansRpcDispatcher.RpcEntry> add(
			...
			List<PlayerBansRpcDispatcher.RpcEntry> players,
		...
		List<CompletableFuture<Optional<PlayerBansRpcDispatcher.ConfigEntry>>> list = players.stream()
		                                                                                     .map(
				                                                                                     player -> dispatcher
						                                                                                     ...
						                                                                                     .thenApply(
								                                                                                     playerEntry -> playerEntry.map(
										                                                                                     player::toConfigEntry))
		                                                                                     )
		                                                                                     .toList();
		...
		for (Optional<PlayerBansRpcDispatcher.ConfigEntry> optional : Util.combineSafe(list).join()) {
			if (!optional.isEmpty()) {
				...
				ServerPlayerEntity
						serverPlayerEntity =
						dispatcher.getPlayerListHandler().getPlayer(optional.get().player().id());
	...
	public static List<PlayerBansRpcDispatcher.RpcEntry> clear(
			ManagementHandlerDispatcher dispatcher,
			ManagementConnectionId remote
	) {
	...
	public static List<PlayerBansRpcDispatcher.RpcEntry> set(
		...
		List<CompletableFuture<Optional<PlayerBansRpcDispatcher.ConfigEntry>>> list = players.stream()
		                                                                                     ...
		                                                                                     .toList();
		Set<PlayerBansRpcDispatcher.ConfigEntry>
				set =
				Util.combineSafe(list).join().stream().flatMap(Optional::stream).collect(Collectors.toSet());
		...
		set2
				.stream()
				.filter(player -> !set.contains(player))
				.forEach(player -> dispatcher.getBanHandler().removePlayer(player.player(), remote));
	...
	record ConfigEntry(PlayerConfigEntry player, @Nullable String reason, String source, Optional<Instant> expires) {
		...
		BannedPlayerEntry toBannedPlayerEntry() {
			return new BannedPlayerEntry(
					new PlayerConfigEntry(this.player().id(), this.player().name()),
					null,
					this.source(),
	...
	public record RpcEntry(
		...
		private static PlayerBansRpcDispatcher.RpcEntry of(PlayerBansRpcDispatcher.ConfigEntry entry) {
			return new PlayerBansRpcDispatcher.RpcEntry(
					RpcPlayer.of(entry.player()),
					Optional.ofNullable(entry.reason()),
					Optional.of(entry.source()),
					entry.expires()
			);
		}
