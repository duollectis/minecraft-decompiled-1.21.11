...
public class ServerPropertiesHandler extends AbstractPropertiesHandler<ServerPropertiesHandler> {
	...
	public final AbstractPropertiesHandler<ServerPropertiesHandler>.PropertyAccessor<Boolean>
			allowFlight =
			this.booleanAccessor("allow-flight", false);
	public final AbstractPropertiesHandler<ServerPropertiesHandler>.PropertyAccessor<String>
			motd =
			this.stringAccessor("motd", "A Minecraft Server");
	...
	public final AbstractPropertiesHandler<ServerPropertiesHandler>.PropertyAccessor<Boolean>
			forceGameMode =
			this.booleanAccessor("force-gamemode", false);
	public final AbstractPropertiesHandler<ServerPropertiesHandler>.PropertyAccessor<Boolean>
			enforceWhitelist =
			this.booleanAccessor("enforce-whitelist", false);
	public final AbstractPropertiesHandler<ServerPropertiesHandler>.PropertyAccessor<Difficulty>
			difficulty =
			this.accessor(
					"difficulty",
					combineParser(Difficulty::byId, Difficulty::byName),
					Difficulty::getName,
					Difficulty.EASY
			);
	public final AbstractPropertiesHandler<ServerPropertiesHandler>.PropertyAccessor<GameMode> gameMode = this.accessor(
			"gamemode",
			combineParser(GameMode::byIndex, GameMode::byId),
			gameMode -> gameMode.getId(),
			GameMode.SURVIVAL
	);
	...
	public final String
			managementServerKeystorePassword =
			this.getString("management-server-tls-keystore-password", "");
	public final String managementServerAllowedOrigin = this.getString("management-server-allowed-origins", "");
	public final @Nullable Boolean
			announcePlayerAchievements =
			this.getDeprecatedBoolean("announce-player-achievements");
	...
	public final AbstractPropertiesHandler<ServerPropertiesHandler>.PropertyAccessor<Integer>
			spawnProtection =
			this.intAccessor("spawn-protection", 16);
	public final AbstractPropertiesHandler<ServerPropertiesHandler>.PropertyAccessor<LeveledPermissionPredicate>
			opPermissionLevel =
			this.accessor(
					"op-permission-level",
	...
	public final AbstractPropertiesHandler<ServerPropertiesHandler>.PropertyAccessor<Integer>
			viewDistance =
			this.intAccessor("view-distance", 10);
	public final AbstractPropertiesHandler<ServerPropertiesHandler>.PropertyAccessor<Integer>
			simulationDistance =
			this.intAccessor("simulation-distance", 10);
	public final AbstractPropertiesHandler<ServerPropertiesHandler>.PropertyAccessor<Integer>
			maxPlayers =
			this.intAccessor("max-players", 20);
	...
	public final int
			maxWorldSize =
			this.transformedParseInt(
					"max-world-size",
					maxWorldSize -> MathHelper.clamp(maxWorldSize, 1, 29999984),
					29999984
			);
	...
	public final AbstractPropertiesHandler<ServerPropertiesHandler>.PropertyAccessor<Boolean>
			enableStatus =
			this.booleanAccessor("enable-status", true);
	public final AbstractPropertiesHandler<ServerPropertiesHandler>.PropertyAccessor<Boolean>
			hideOnlinePlayers =
			this.booleanAccessor(
	...
	public final AbstractPropertiesHandler<ServerPropertiesHandler>.PropertyAccessor<Integer>
			...
			this.accessor(
					...
					value -> MathHelper.clamp(Integer.parseInt(value), 10, 1000),
					100
			);
	...
	public final AbstractPropertiesHandler<ServerPropertiesHandler>.PropertyAccessor<Integer>
			playerIdleTimeout =
			this.intAccessor("player-idle-timeout", 0);
	public final AbstractPropertiesHandler<ServerPropertiesHandler>.PropertyAccessor<Integer>
	...
	public final AbstractPropertiesHandler<ServerPropertiesHandler>.PropertyAccessor<Boolean>
			whiteList =
			this.booleanAccessor("white-list", false);
	...
	public AbstractPropertiesHandler<ServerPropertiesHandler>.PropertyAccessor<Boolean>
			acceptsTransfers =
			this.booleanAccessor("accepts-transfers", false);
	...
	public ServerPropertiesHandler(Properties properties) {
		...
		this.worldGenProperties = new ServerPropertiesHandler.WorldGenProperties(
				this.get(
						"generator-settings",
						generatorSettings -> JsonHelper.deserialize(
								!generatorSettings.isEmpty() ? generatorSettings : "{}"),
						new JsonObject()
				),
				this.get(
						"level-type",
		...
		this.dataPackSettings = parseDataPackSettings(
				this.getString(
						...
						String.join(",", DataConfiguration.SAFE_MODE.dataPacks().getEnabled())
				),
				this.getString(
						"initial-disabled-packs",
						String.join(",", DataConfiguration.SAFE_MODE.dataPacks().getDisabled())
				)
		);
	}
			...
			try {
				...
				return (Text) TextCodecs.CODEC
						...
						.resultOrPartial(error -> LOGGER.warn(
								"Failed to parse resource pack prompt '{}': {}",
								prompt,
								error
						))
						.orElse(null);
		...
		public DimensionOptionsRegistryHolder createDimensionsRegistryHolder(RegistryWrapper.WrapperLookup registries) {
			...
			RegistryEntry.Reference<WorldPreset> reference = registryWrapper.getOptional(WorldPresets.DEFAULT)
			                                                                ...
			                                                                .orElseThrow(() -> new IllegalStateException(
					                                                                "Invalid datapack contents: can't find default preset"));
			RegistryEntry<WorldPreset> registryEntry = Optional.ofNullable(Identifier.tryParse(this.levelType))
			                                                   .map(levelTypeId -> RegistryKey.of(
					                                                   RegistryKeys.WORLD_PRESET,
					                                                   levelTypeId
			                                                   ))
			                                                   .or(() -> Optional.ofNullable(LEVEL_TYPE_TO_PRESET_KEY.get(
					                                                   this.levelType)))
			                                                   .flatMap(registryWrapper::getOptional)
			                                                   .orElseGet(() -> {
				                                                   ServerPropertiesHandler.LOGGER.warn(
			...
			DimensionOptionsRegistryHolder
					dimensionOptionsRegistryHolder =
					registryEntry.value().createDimensionsRegistryHolder();
			if (registryEntry.matchesKey(WorldPresets.FLAT)) {
				...
				Optional<FlatChunkGeneratorConfig> optional = FlatChunkGeneratorConfig.CODEC
						...
						.resultOrPartial(msg -> ServerPropertiesHandler.LOGGER.error(msg));
