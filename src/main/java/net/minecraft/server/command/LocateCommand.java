...
public class LocateCommand {
	...
	private static Optional<? extends RegistryEntryList.ListBacked<Structure>> getStructureListForPredicate(
		...
		return (Optional<? extends RegistryEntryList.ListBacked<Structure>>) predicate.getKey()
		                                                                              .map(
				                                                                              key -> structureRegistry
						                                                                              .getOptional(key)
						                                                                              .map(entry -> RegistryEntryList.of(
								                                                                              entry)),
				                                                                              structureRegistry::getOptional
		                                                                              );
	}

	private static int executeLocateStructure(
		...
		RegistryEntryList<Structure>
				registryEntryList =
				(RegistryEntryList<Structure>) getStructureListForPredicate(predicate, registry)
	...
	private static int executeLocateBiome(
			ServerCommandSource source,
	...
	private static int executeLocatePoi(
			ServerCommandSource source,
	...
	public static int sendCoordinates(
		...
		String string = (String) predicate.getEntry()
		                                  .map(
				                                  entry -> predicate.asString(),
				                                  tag -> predicate.asString() + " ("
						                                  + ((RegistryEntry) result.getSecond()).getIdAsString() + ")"
		                                  );
