...
public class DataCommand {
	...
	private static final SimpleCommandExceptionType
			...
			new SimpleCommandExceptionType(Text.translatable("commands.data.merge.failed"));
	...
	private static final SimpleCommandExceptionType
			...
			new SimpleCommandExceptionType(Text.translatable("commands.data.get.multiple"));
	...
	private static final Dynamic2CommandExceptionType
			...
			new Dynamic2CommandExceptionType(
					(startIndex, endIndex) -> Text.stringifiedTranslatable(
	...
	public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
		LiteralArgumentBuilder<ServerCommandSource>
				...
				(LiteralArgumentBuilder<ServerCommandSource>) CommandManager.literal("data")
									...
									(builder, modifier) -> builder.then(
									                              ...
									                              .then(
											                              CommandManager.literal("set")
											                                            .then(modifier.create((context, sourceNbt, path, elements) -> path.put(
													                                            sourceNbt,
													                                            (NbtElement) Iterables.getLast(
															                                            elements)
											                                            )))
									                              )
									                              .then(CommandManager
											                              ...
											                              .then(modifier.create((context, element, path, elements) -> {
												                              ...
												                              for (NbtElement nbtElement : elements) {
													                              if (NbtPathArgumentType.NbtPath.isTooDeep(
															                              nbtElement,
															                              0
													                              )) {
													                              ...
													                              if (!(nbtElement instanceof NbtCompound nbtCompound2)) {
														                              throw MODIFY_EXPECTED_OBJECT_EXCEPTION.create(
																                              nbtElement);
													                              }
	...
	private static List<NbtElement> mapValues(List<NbtElement> list, DataCommand.Processor processor)
			...
			objectType.addArgumentsToBuilder(
					...
					builder -> {
						ArgumentBuilder<ServerCommandSource, ?>
								argumentBuilder =
								CommandManager.argument("targetPath", NbtPathArgumentType.nbtPath());
									...
									operation -> objectType2.addArgumentsToBuilder(
											...
											builderx -> builderx.executes(
													                    context -> executeModify(
															                    ...
															                    objectType,
															                    operation,
															                    mapValues(getValues(context, objectType2), value -> value)
													                    )
											                    )
											                    .then(
													                    ((RequiredArgumentBuilder) CommandManager
								...
								modifier -> CommandManager
										...
										.then(CommandManager
												...
												.executes(context -> {
													List<NbtElement>
															list =
															Collections.singletonList(NbtElementArgumentType.getNbtElement(
																	context,
																	"value"
															));
	...
	private static List<NbtElement> getValues(
	...
	private static List<NbtElement> getValuesByPath(
			CommandContext<ServerCommandSource> context,
			DataCommand.ObjectType objectType
	) throws CommandSyntaxException {
		DataCommandObject dataCommandObject = objectType.getObject(context);
