...
public class SpawnArmorTrimsCommand {
   ...
   public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
      dispatcher.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("spawn_armor_trims")
   ...
   private static int execute(ServerCommandSource source, PlayerEntity player, RegistryKey<ArmorTrimPattern> pattern) {
      return execute(
         source, player, Stream.of(source.getServer().getRegistryManager().getOrThrow(RegistryKeys.TRIM_PATTERN).getOptional(pattern).orElseThrow())
      );
   }
   ...
   private static List<RegistryEntry.Reference<Item>> getArmorItems(RegistryWrapper<Item> itemRegistry) {
      ...
      itemRegistry.streamEntries()
         .forEach(
            entry -> {
               EquippableComponent equippableComponent = entry.value().getComponents().get(DataComponentTypes.EQUIPPABLE);
