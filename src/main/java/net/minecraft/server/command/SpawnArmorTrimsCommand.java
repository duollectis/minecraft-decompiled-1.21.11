...
public class SpawnArmorTrimsCommand {
	...
	public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
		dispatcher.register(
				(LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) CommandManager
						.literal("spawn_armor_trims")
						.requires(CommandManager.requirePermissionLevel(CommandManager.GAMEMASTERS_CHECK))
				)
						.then(
								CommandManager.literal("*_lag_my_game")
						...
						.then(
								CommandManager
										.argument(
												"pattern",
												RegistryKeyArgumentType.registryKey(RegistryKeys.TRIM_PATTERN)
										)
										.executes(
												context -> execute(
														(ServerCommandSource) context.getSource(),
	...
	private static int execute(ServerCommandSource source, PlayerEntity player) {
		return execute(
				source,
				player,
				source.getServer().getRegistryManager().getOrThrow(RegistryKeys.TRIM_PATTERN).streamEntries()
		);
	}
	...
	private static int execute(
			...
			PlayerEntity player,
			Stream<RegistryEntry.Reference<ArmorTrimPattern>> patterns
	) {
		...
		List<RegistryEntry.Reference<ArmorTrimMaterial>> list2 = serverWorld.getRegistryManager()
		                                                                    ...
		                                                                    .streamEntries()
		                                                                    .sorted(Comparator.comparing(material -> MATERIAL_INDEX_GETTER.applyAsInt(
				                                                                    material.registryKey())))
		                                                                    .toList();
		List<RegistryEntry.Reference<Item>>
				list3 =
				getArmorItems(serverWorld.getRegistryManager().getOrThrow(RegistryKeys.ITEM));
		BlockPos blockPos = player.getBlockPos().offset(player.getHorizontalFacing(), 5);
		double d = 3.0;

		for (int i = 0; i < list2.size(); i++) {
			RegistryEntry.Reference<ArmorTrimMaterial> reference = list2.get(i);

			for (int j = 0; j < list.size(); j++) {
				RegistryEntry.Reference<ArmorTrimPattern> reference2 = list.get(j);
				...
				for (int k = 0; k < list3.size(); k++) {
					...
					EquippableComponent
							equippableComponent =
							Objects.requireNonNull(itemStack.get(DataComponentTypes.EQUIPPABLE));
	...
	private static List<RegistryEntry.Reference<Item>> getArmorItems(RegistryWrapper<Item> itemRegistry) {
		...
		itemRegistry.streamEntries()
		            .forEach(
				            entry -> {
					            EquippableComponent
							            equippableComponent =
							            entry.value().getComponents().get(DataComponentTypes.EQUIPPABLE);
					            if (equippableComponent != null
							            && equippableComponent.slot().getType() == EquipmentSlot.Type.HUMANOID_ARMOR
							            && equippableComponent.assetId().isPresent()) {
