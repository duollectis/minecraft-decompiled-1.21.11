...
public class RaidCommand {
	...
	public static void register(
		...
		dispatcher.register(
				(LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) CommandManager
						...
						.then(CommandManager
								.literal("check")
								.executes(context -> executeCheck((ServerCommandSource) context.getSource())))
				)
						.then(
								CommandManager.literal("sound")
								              .then(
										              CommandManager
												              .argument("type", TextArgumentType.text(registryAccess))
												              .executes(
														              context -> executeSound(
																              (ServerCommandSource) context.getSource(),
						...
						.then(CommandManager
								.literal("spawnleader")
								.executes(context -> executeSpawnLeader((ServerCommandSource) context.getSource())))
				)
						.then(
								CommandManager.literal("setomen")
								              .then(
										              CommandManager.argument("level", IntegerArgumentType.integer(0))
										                            .executes(context -> executeSetOmen(
												                            (ServerCommandSource) context.getSource(),
												                            IntegerArgumentType.getInteger(
														                            context,
														                            "level"
						...
						.then(CommandManager
								.literal("glow")
								.executes(context -> executeGlow((ServerCommandSource) context.getSource())))
		);
	}
	...
	private static int executeSetOmen(ServerCommandSource source, int level) throws CommandSyntaxException {
		...
		if (raid != null) {
			...
			if (level > i) {
				...
				source.sendFeedback(
						() -> Text.literal("Changed village's raid omen level from " + j + " to " + level),
						false
				);
			}
		}
	...
	private static int executeSpawnLeader(ServerCommandSource source) {
		...
		if (raiderEntity == null) {
			...
			raiderEntity.equipStack(
					EquipmentSlot.HEAD,
					Raid.createOminousBanner(source.getRegistryManager().getOrThrow(RegistryKeys.BANNER_PATTERN))
			);
			raiderEntity.setPosition(source.getPosition().x, source.getPosition().y, source.getPosition().z);
			raiderEntity.initialize(
					source.getWorld(),
					source.getWorld().getLocalDifficulty(BlockPos.ofFloored(source.getPosition())),
					SpawnReason.COMMAND,
					null
			);
