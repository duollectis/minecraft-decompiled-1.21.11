...
public class StopwatchCommand {
   ...
   public static final SuggestionProvider<ServerCommandSource> STOPWATCH_SUGGESTION_PROVIDER = (context, builder) -> CommandSource.suggestIdentifiers(
      ((ServerCommandSource)context.getSource()).getServer().getStopwatchPersistentState().keys(), builder
   );
                        ...
                        CommandManager.literal("create")
                           .then(
                              CommandManager.argument("id", IdentifierArgumentType.identifier())
                                 .executes(
                                    context -> executeCreate((ServerCommandSource)context.getSource(), IdentifierArgumentType.getIdentifier(context, "id"))
                                 )
                           )
                     ))
                  .then(
                     CommandManager.literal("query")
                        .then(
                           ((RequiredArgumentBuilder)CommandManager.argument("id", IdentifierArgumentType.identifier())
                              ...
                              .executes(
                                 context -> executeQuery((ServerCommandSource)context.getSource(), IdentifierArgumentType.getIdentifier(context, "id"), 1.0)
                              )
                        )
                  ))
               .then(
                  CommandManager.literal("restart")
                     .then(
                        CommandManager.argument("id", IdentifierArgumentType.identifier())
                           .suggests(STOPWATCH_SUGGESTION_PROVIDER)
                           .executes(context -> executeRestart((ServerCommandSource)context.getSource(), IdentifierArgumentType.getIdentifier(context, "id")))
                     )
               ))
            .then(
               CommandManager.literal("remove")
                  .then(
                     CommandManager.argument("id", IdentifierArgumentType.identifier())
                        .suggests(STOPWATCH_SUGGESTION_PROVIDER)
                        .executes(context -> executeRemove((ServerCommandSource)context.getSource(), IdentifierArgumentType.getIdentifier(context, "id")))
                  )
            )
      );
   }
