...
public class ChaseCommand {
	...
	public static BiMap<String, RegistryKey<World>>
			DIMENSIONS =
			ImmutableBiMap.of("o", World.OVERWORLD, "n", World.NETHER, "e", World.END);
	...
	public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
		dispatcher.register(
				(LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) CommandManager
						...
						.then(CommandManager
								.literal("stop")
								.executes(context -> stop((ServerCommandSource) context.getSource())))
		);
	}
	...
	private static int startServer(ServerCommandSource source, String ip, int port) {
		if (isRunning(source)) {
			...
			try {
				server.start();
				source.sendFeedback(
						() -> Text.literal("Chase server is now running on port " + port
								+ ". Clients can follow you using /chase follow <ip> <port>"), false
				);
			}
