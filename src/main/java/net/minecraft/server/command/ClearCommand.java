	...
	private static int execute(
		...
		for (ServerPlayerEntity serverPlayerEntity : targets) {
			i +=
					serverPlayerEntity
							.getInventory()
							.remove(item, maxCount, serverPlayerEntity.playerScreenHandler.getCraftingInput());
		...
		if (i == 0) {
			...
			if (maxCount == 0) {
				if (targets.size() == 1) {
					...
					source.sendFeedback(
							() -> Text.translatable("commands.clear.test.multiple", j, targets.size()),
							true
					);
				}
			}
				...
				source.sendFeedback(
						() -> Text.translatable("commands.clear.success.multiple", j, targets.size()),
						true
				);
			}
