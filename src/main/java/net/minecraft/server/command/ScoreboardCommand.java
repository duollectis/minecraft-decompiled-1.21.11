...
public class ScoreboardCommand {
	...
	private static final Dynamic2CommandExceptionType PLAYERS_GET_NULL_EXCEPTION = new Dynamic2CommandExceptionType(
			(objective, target) -> Text.stringifiedTranslatable(
				...
				(LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) CommandManager
						...
						.then(
								((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) CommandManager
										...
										.then(CommandManager
												.literal("list")
												.executes(context -> executeListObjectives((ServerCommandSource) context.getSource())))
								)
										.then(
								...
								((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) CommandManager
										...
										.then(
												((LiteralArgumentBuilder) CommandManager.literal("list")
												                                        .executes(context -> executeListPlayers(
														                                        (ServerCommandSource) context.getSource()))
												)
														.then(
																CommandManager
																														              ...
																														              CommandManager
																																              ...
																																              .executes(
																																		              context -> executeOperation(
																																				              ...
																																				              OperationArgumentType.getOperation(
																																						              context,
																																						              "operation"
																																				              ),
																																				              ScoreHolderArgumentType.getScoreboardScoreHolders(
	...
	private static ArgumentBuilder<ServerCommandSource, ?> makeNumberFormatArguments(
			...
			ScoreboardCommand.NumberFormatCommandExecutor executor
	) {
		return argumentBuilder
				.then(CommandManager
						.literal("blank")
						.executes(context -> executor.run(context, BlankNumberFormat.INSTANCE)))
				.then(CommandManager
						.literal("fixed")
						.then(CommandManager
								.argument("contents", TextArgumentType.text(registryAccess))
								.executes(context -> {
				...
				.then(CommandManager
						.literal("styled")
						.then(CommandManager
								.argument("style", StyleArgumentType.style(registryAccess))
								.executes(context -> {
			...
			if (scoreboardObjective.getCriterion() == ScoreboardCriterion.TRIGGER) {
				...
				for (ScoreHolder scoreHolder : targets) {
					ReadableScoreboardScore
							readableScoreboardScore =
							scoreboard.getScore(scoreHolder, scoreboardObjective);
	...
	private static int executeGet(ServerCommandSource source, ScoreHolder scoreHolder, ScoreboardObjective objective)
		...
		if (readableScoreboardScore == null) {
			...
			source.sendFeedback(
					() -> Text.translatable(
							...
							readableScoreboardScore.getScore(),
		...
		if (targets.size() == 1) {
			...
			source.sendFeedback(
					() -> Text.translatable(
							...
							targetObjective.toHoverableText(),
							getNextDisplayName(targets),
							j
					),
					true
			);
	...
	private static int executeEnable(
			ServerCommandSource source,
			Collection<ScoreHolder> targets,
		...
		if (collection.isEmpty()) {
			...
			source.sendFeedback(
					() -> Text.translatable(
							...
							collection.size(),
							Texts.join(collection, ScoreHolder::getStyledDisplayName)
					),
					false
			);
		}
	...
	private static int executeListScores(ServerCommandSource source, ScoreHolder scoreHolder) {
		Object2IntMap<ScoreboardObjective>
				object2IntMap =
				source.getServer().getScoreboard().getScoreHolderObjectives(scoreHolder);
	...
	private static int executeClearDisplay(ServerCommandSource source, ScoreboardDisplaySlot slot)
	throws CommandSyntaxException {
		Scoreboard scoreboard = source.getServer().getScoreboard();
	...
	private static int executeSetDisplay(
			ServerCommandSource source,
	...
	private static int executeModifyObjective(
			ServerCommandSource source,
			ScoreboardObjective objective,
			Text displayName
	) {
	...
	private static int executeModifyDisplayAutoUpdate(
			ServerCommandSource source,
			ScoreboardObjective objective,
			boolean enable
	) {
	...
	private static int executeModifyObjectiveFormat(
			ServerCommandSource source,
			ScoreboardObjective objective,
		...
		if (collection.isEmpty()) {
			...
			source.sendFeedback(
					() -> Text.translatable(
							...
							collection.size(),
							Texts.join(collection, ScoreboardObjective::toHoverableText)
					),
					false
			);
		}
