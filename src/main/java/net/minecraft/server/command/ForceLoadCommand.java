...
public class ForceLoadCommand {
	...
	private static final Dynamic2CommandExceptionType QUERY_FAILURE_EXCEPTION = new Dynamic2CommandExceptionType(
			(chunkPos, registryKey) -> Text.stringifiedTranslatable(
				...
				(LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) CommandManager
						...
						.then(
								((LiteralArgumentBuilder) CommandManager.literal("remove")
										...
										.then(CommandManager
												.literal("all")
												.executes(context -> executeRemoveAll((ServerCommandSource) context.getSource())))
						)
				)
						.then(
										...
										.then(
												CommandManager.argument("pos", ColumnPosArgumentType.columnPos())
												              .executes(context -> executeQuery(
														              (ServerCommandSource) context.getSource(),
														              ColumnPosArgumentType.getColumnPos(context, "pos")
												              ))
										)
						)
		);
	...
	private static int executeQuery(ServerCommandSource source) {
		...
		if (i > 0) {
			String
					...
					Joiner
							.on(", ")
							.join(longSet.stream().sorted().map(ChunkPos::new).map(ChunkPos::toString).iterator());
			if (i == 1) {
				source.sendFeedback(
						() -> Text.translatable(
	...
	private static int executeRemoveAll(ServerCommandSource source) {
		...
		RegistryKey<World> registryKey = serverWorld.getRegistryKey();
		LongSet longSet = serverWorld.getForcedChunks();
		longSet.forEach(chunkPos -> serverWorld.setChunkForced(
				ChunkPos.getPackedX(chunkPos),
				ChunkPos.getPackedZ(chunkPos),
				false
		));
		source.sendFeedback(
				() -> Text.translatable("commands.forceload.removed.all", Text.of(registryKey.getValue())),
				true
		);
		return 0;
	}

	private static int executeChange(ServerCommandSource source, ColumnPos from, ColumnPos to, boolean forceLoaded)
