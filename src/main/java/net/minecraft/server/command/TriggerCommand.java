...
public class TriggerCommand {
				...
				(LiteralArgumentBuilder) CommandManager.literal("trigger")
				                                       .then(
						                                       ((RequiredArgumentBuilder) ((RequiredArgumentBuilder) CommandManager
								                                       ...
								                                       .suggests((context, builder) -> suggestObjectives(
	...
	private static int executeSet(
		...
		source.sendFeedback(
				() -> Text.translatable("commands.trigger.set.success", objective.toHoverableText(), value),
				true
		);
		return value;
	}
	...
	private static int executeSimple(
		...
		source.sendFeedback(
				() -> Text.translatable("commands.trigger.simple.success", objective.toHoverableText()),
				true
		);
		return i;
	}

	private static ScoreAccess getScore(Scoreboard scoreboard, ScoreHolder scoreHolder, ScoreboardObjective objective)
	throws CommandSyntaxException {
