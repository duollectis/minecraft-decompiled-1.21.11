...
public class ScheduleCommand {
	...
	private static final SimpleCommandExceptionType
			SAME_TICK_EXCEPTION =
			new SimpleCommandExceptionType(Text.translatable("commands.schedule.same_tick"));
	...
	private static final SimpleCommandExceptionType
			MACRO_EXCEPTION =
			new SimpleCommandExceptionType(Text.stringifiedTranslatable("commands.schedule.macro"));
				...
				(LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) CommandManager
										              ...
										              CommandManager
												              ...
												              .then(
														              ((RequiredArgumentBuilder) ((RequiredArgumentBuilder) CommandManager
																              .argument("time", TimeArgumentType.time())
																              .executes(
																		              context -> execute(
																				              (ServerCommandSource) context.getSource(),
																				              CommandFunctionArgumentType.getFunctionOrTag(
																						              context,
																						              "function"
																				              ),
						...
						.then(
								CommandManager.literal("clear")
								              .then(
										              CommandManager
												              .argument("function", StringArgumentType.greedyString())
												              .suggests(SUGGESTION_PROVIDER)
												              .executes(context -> clearEvent(
														              (ServerCommandSource) context.getSource(),
	...
	private static int execute(
		...
		if (time == 0) {
			...
			Timer<MinecraftServer>
					timer =
					source.getServer().getSaveProperties().getMainWorldProperties().getScheduledEvents();
