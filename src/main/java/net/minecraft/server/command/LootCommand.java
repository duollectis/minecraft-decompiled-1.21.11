...
public class LootCommand {
								                                 ...
								                                 CommandManager.literal("fish")
								                                               .then(
										                                               CommandManager
												                                               .argument(
														                                               "loot_table",
														                                               RegistryEntryArgumentType.lootTable(
												                                               ...
												                                               .then(
														                                               ((RequiredArgumentBuilder) ((RequiredArgumentBuilder) ((RequiredArgumentBuilder) CommandManager
																                                               ...
																                                               .executes(
																		                                               context -> executeFish(
																				                                               context,
																				                                               RegistryEntryArgumentType.getLootTable(
																						                                               context,
																						                                               "loot_table"
																				                                               ),
	...
	private static <T extends ArgumentBuilder<ServerCommandSource, T>> T addTargetArguments(
			T rootArgument,
				                       ...
				                       ((LiteralArgumentBuilder) CommandManager.literal("replace")
						                       ...
						                       .then(
								                       CommandManager.literal("block")
								                                     .then(
										                                     CommandManager
												                                     .argument("targetPos", BlockPosArgumentType.blockPos())
												                                     .then(
														                                     sourceConstructor.construct(
																                                                      CommandManager.argument(
																		                                                      "slot",
				                       ...
				                       CommandManager.literal("give")
				                                     .then(
						                                     sourceConstructor.construct(
								                                     ...
								                                     (context, stacks, messageSender) -> executeGive(
	...
	private static int executeInsert(
	...
	private static void replace(
		...
		for (int i = 0; i < stackCount; i++) {
			...
			if (stackReference != null && stackReference.set(itemStack.copy())) {
	...
	private static void sendDroppedFeedback(
			ServerCommandSource source,
			List<ItemStack> stacks,
	...
	private static int executeMine(
		...
		if (optional.isEmpty()) {
			...
			return constructor.accept(
					context,
					list,
					stacks -> sendDroppedFeedback(serverCommandSource, stacks, optional.get())
			);
		}
	}

	private static int executeKill(
			CommandContext<ServerCommandSource> context,
			Entity entity,
			LootCommand.Target constructor
	) throws CommandSyntaxException {
		...
		if (optional.isEmpty()) {
			...
			LootTable
					lootTable =
					serverCommandSource.getServer().getReloadableRegistries().getLootTable(optional.get());
	...
	private static int executeLoot(
			CommandContext<ServerCommandSource> context,
	...
	private static int getFeedbackMessageSingle(
			CommandContext<ServerCommandSource> context,
			RegistryEntry<LootTable> lootTable,
			LootWorldContext lootContextParameters,
			LootCommand.Target constructor
	) throws CommandSyntaxException {
		ServerCommandSource serverCommandSource = (ServerCommandSource) context.getSource();
