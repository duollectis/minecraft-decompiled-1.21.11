...
public class FetchProfileCommand {
   public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
      dispatcher.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("fetchprofile")
                  .requires(CommandManager.requirePermissionLevel(CommandManager.GAMEMASTERS_CHECK)))
               .then(
                  CommandManager.literal("name")
                     .then(
                        CommandManager.argument("name", StringArgumentType.greedyString())
                           .executes(context -> executeName((ServerCommandSource)context.getSource(), StringArgumentType.getString(context, "name")))
                     )
               ))
            .then(
               CommandManager.literal("id")
                  .then(
                     CommandManager.argument("id", UuidArgumentType.uuid())
                        .executes(context -> executeId((ServerCommandSource)context.getSource(), UuidArgumentType.getUuid(context, "id")))
                  )
            )
      );
   }
                        ...
                        source.sendFeedback(
                           () -> {
                              Text text2 = Texts.join(
                                 List.of(
                                    Text.translatable("commands.fetchprofile.copy_component")
                                       .styled(style -> style.withClickEvent(new ClickEvent.CopyToClipboard(string2))),
                                    Text.translatable("commands.fetchprofile.give_item")
                                       .styled(
                                          style -> style.withClickEvent(new ClickEvent.RunCommand("give @s minecraft:player_head[profile=" + string2 + "]"))
                                       ),
                                    Text.translatable("commands.fetchprofile.summon_mannequin")
                                       .styled(
                                          style -> style.withClickEvent(new ClickEvent.RunCommand("summon minecraft:mannequin ~ ~ ~ {profile:" + string2 + "}"))
                                       ),
