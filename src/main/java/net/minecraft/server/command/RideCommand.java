...
public class RideCommand {
	...
	private static final SimpleCommandExceptionType
			RIDE_LOOP_EXCEPTION =
			new SimpleCommandExceptionType(Text.translatable("commands.ride.mount.failure.loop"));
								...
								((RequiredArgumentBuilder) CommandManager
										...
										.then(
												CommandManager.literal("dismount")
												              .executes(context -> executeDismount(
														              (ServerCommandSource) context.getSource(),
	...
	throws CommandSyntaxException {
		...
		else if (rider.getEntityWorld() != vehicle.getEntityWorld()) {
			throw WRONG_DIMENSION_EXCEPTION.create();
		}
		else if (!rider.startRiding(vehicle, true, true)) {
