...
public class WorldBorderCommand {
	...
	public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
		dispatcher.register(
				(LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) CommandManager
								...
								CommandManager.literal("center")
								              .then(
										              CommandManager.argument("pos", Vec2ArgumentType.vec2())
										                            .executes(context -> executeCenter(
												                            (ServerCommandSource) context.getSource(),
												                            Vec2ArgumentType.getVec2(context, "pos")
										                            ))
								              )
						)
				)
						...
						.then(CommandManager
								.literal("get")
								.executes(context -> executeGet((ServerCommandSource) context.getSource())))
				)
						.then(
		...
		if (worldBorder.getDamagePerBlock() == damagePerBlock) {
			...
			source.sendFeedback(
					() -> Text.translatable(
							...
							String.format(Locale.ROOT, "%.2f", damagePerBlock)
					), true
			);
	...
	private static int executeWarningTime(ServerCommandSource source, int time) throws CommandSyntaxException {
		...
		if (worldBorder.getWarningTime() == time) {
			...
			source.sendFeedback(
					() -> Text.translatable("commands.worldborder.warning.time.success", toSeconds(time)),
					true
			);
			return time;
		}
	...
	private static int executeWarningDistance(ServerCommandSource source, int distance) throws CommandSyntaxException {
		...
		if (worldBorder.getWarningBlocks() == distance) {
			...
			source.sendFeedback(
					() -> Text.translatable("commands.worldborder.warning.distance.success", distance),
					true
			);
			return distance;
		}
	...
	private static int executeGet(ServerCommandSource source) {
		...
		source.sendFeedback(
				() -> Text.translatable("commands.worldborder.get", String.format(Locale.ROOT, "%.0f", d)),
				false
		);
	...
	private static int executeSet(ServerCommandSource source, double distance, long time)
