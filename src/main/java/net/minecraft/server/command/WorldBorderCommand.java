...
public class WorldBorderCommand {
                              ...
                              CommandManager.literal("add")
                                 .then(
                                    ((RequiredArgumentBuilder)CommandManager.argument("distance", DoubleArgumentType.doubleArg(-5.999997E7F, 5.999997E7F))
                                       ...
                                       .then(
                                          CommandManager.argument("time", TimeArgumentType.time(0))
                                             .executes(
                                                context -> executeSet(
                                                   (ServerCommandSource)context.getSource(),
                                                   ((ServerCommandSource)context.getSource()).getWorld().getWorldBorder().getSize()
   ...
   private static int executeWarningTime(ServerCommandSource source, int time) throws CommandSyntaxException {
      ...
      if (worldBorder.getWarningTime() == time) {
         ...
         source.sendFeedback(() -> Text.translatable("commands.worldborder.warning.time.success", toSeconds(time)), true);
         return time;
      }
   }
   ...
   private static int executeSet(ServerCommandSource source, double distance, long time) throws CommandSyntaxException {
      ...
      if (d == distance) {
         ...
         if (time > 0L) {
            ...
            if (distance > d) {
               source.sendFeedback(() -> Text.translatable("commands.worldborder.set.grow", string, toSeconds(time)), true);
            } else {
               source.sendFeedback(() -> Text.translatable("commands.worldborder.set.shrink", string, toSeconds(time)), true);
            }
         } else {
