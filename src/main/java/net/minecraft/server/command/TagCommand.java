...
public class TagCommand {
	...
	private static final SimpleCommandExceptionType
			...
			new SimpleCommandExceptionType(Text.translatable("commands.tag.add.failed"));
	private static final SimpleCommandExceptionType
														              ...
														              CommandManager
																              ...
																              .suggests(
																		              (context, builder) -> CommandSource.suggestMatching(
																				              getTags(EntityArgumentType.getEntities(
																						              context,
																						              "targets"
																				              )),
																				              builder
	...
	throws CommandSyntaxException {
		...
		if (i == 0) {
			...
			if (targets.size() == 1) {
				...
				source.sendFeedback(
						() -> Text.translatable("commands.tag.add.success.multiple", tag, targets.size()),
						true
				);
			}
	...
	private static int executeList(ServerCommandSource source, Collection<? extends Entity> targets) {
		...
		if (targets.size() == 1) {
			...
			if (set.isEmpty()) {
				source.sendFeedback(
						() -> Text.translatable("commands.tag.list.single.empty", entity2.getDisplayName()),
						false
				);
			}
			else {
				source.sendFeedback(
						() -> Text.translatable(
								...
								entity2.getDisplayName(),
								set.size(),
								Texts.joinOrdered(set)
						), false
				);
			}
		}
