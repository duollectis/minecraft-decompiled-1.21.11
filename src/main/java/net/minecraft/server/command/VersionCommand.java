...
public class VersionCommand {
	...
	public static void acceptInfo(Consumer<Text> sender) {
		...
		sender.accept(Text.translatable(
				"commands.version.protocol",
				gameVersion.protocolVersion(),
				"0x" + Integer.toHexString(gameVersion.protocolVersion())
		));
		sender.accept(Text.translatable("commands.version.build_time", Text.of(gameVersion.buildTime())));
		sender.accept(Text.translatable(
				"commands.version.pack.resource",
				gameVersion.packVersion(ResourceType.CLIENT_RESOURCES).toString()
		));
		sender.accept(Text.translatable(
				"commands.version.pack.data",
				gameVersion.packVersion(ResourceType.SERVER_DATA).toString()
		));
