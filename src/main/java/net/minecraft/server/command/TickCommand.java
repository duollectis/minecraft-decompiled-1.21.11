...
public class TickCommand {
	...
	public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
		dispatcher.register(
				(LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) CommandManager
						...
						.then(CommandManager
								.literal("query")
								.executes(context -> executeQuery((ServerCommandSource) context.getSource())))
				)
						.then(
										              ...
										              CommandManager
												              ...
												              .suggests(
														              (context, suggestionsBuilder) -> CommandSource.suggestMatching(
																              new String[]{DEFAULT_TICK_RATE_STRING},
																              suggestionsBuilder
														              )
												              )
												              .executes(context -> executeRate(
						...
						.then(
								((LiteralArgumentBuilder) ((LiteralArgumentBuilder) CommandManager.literal("step")
										...
										.then(CommandManager
												.literal("stop")
												.executes(context -> executeStopStep((ServerCommandSource) context.getSource())))
								)
										.then(
						...
						.then(
								((LiteralArgumentBuilder) CommandManager.literal("sprint")
								                                        .then(CommandManager
										                                        .literal("stop")
										                                        .executes(context -> executeStopSprint((ServerCommandSource) context.getSource())))
								)
										.then(
												CommandManager.argument("time", TimeArgumentType.time(1))
												              .suggests((context, suggestionsBuilder) -> CommandSource.suggestMatching(
														              new String[]{"60s", "1d", "3d"},
														              suggestionsBuilder
												              ))
												              .executes(context -> executeSprint(
														              (ServerCommandSource) context.getSource(),
														              IntegerArgumentType.getInteger(context, "time")
												              ))
										)
						...
						.then(CommandManager
								.literal("unfreeze")
								.executes(context -> executeFreeze((ServerCommandSource) context.getSource(), false)))
				)
						.then(CommandManager
								.literal("freeze")
								.executes(context -> executeFreeze((ServerCommandSource) context.getSource(), true)))
		);
	}
	...
	private static int executeQuery(ServerCommandSource source) {
		...
		if (serverTickManager.isSprinting()) {
			...
			source.sendFeedback(
					() -> Text.translatable("commands.tick.query.rate.running", string2, string, string3),
					false
			);
		}
