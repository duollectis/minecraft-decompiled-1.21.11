...
public class TeamCommand {
                                    ...
                                    CommandManager.argument("prefix", TextArgumentType.text(registryAccess))
                                       .executes(
                                          context -> executeModifyPrefix(
                                             (ServerCommandSource)context.getSource(),
                                             TeamArgumentType.getTeam(context, "team"),
                                             TextArgumentType.parseTextArgument(context, "prefix")
                                          )
                                       )
                                 )
                           ))
                                 ...
                                 CommandManager.argument("suffix", TextArgumentType.text(registryAccess))
                                    .executes(
                                       context -> executeModifySuffix(
                                          (ServerCommandSource)context.getSource(),
                                          TeamArgumentType.getTeam(context, "team"),
                                          TextArgumentType.parseTextArgument(context, "suffix")
                                       )
                                    )
                              )
                        )
   ...
   private static int executeModifyNametagVisibility(ServerCommandSource source, Team team, AbstractTeam.VisibilityRule visibility) throws CommandSyntaxException {
      if (team.getNameTagVisibilityRule() == visibility) {
         ...
         source.sendFeedback(
            () -> Text.translatable("commands.team.option.nametagVisibility.success", team.getFormattedName(), visibility.getDisplayName()), true
         );
         return 0;
      }
   }
   ...
   private static int executeModifyCollisionRule(ServerCommandSource source, Team team, AbstractTeam.CollisionRule collisionRule) throws CommandSyntaxException {
      if (team.getCollisionRule() == collisionRule) {
         ...
         source.sendFeedback(
            () -> Text.translatable("commands.team.option.collisionRule.success", team.getFormattedName(), collisionRule.getDisplayName()), true
         );
         return 0;
      }
   }
