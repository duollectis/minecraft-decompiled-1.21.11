...
public class TeamCommand {
	...
	public static void register(
		...
		dispatcher.register(
				(LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) CommandManager
						.literal(
								"team"
						)
						...
						.then(
								CommandManager.literal("modify")
								              .then(
										              ((RequiredArgumentBuilder) ((RequiredArgumentBuilder) ((RequiredArgumentBuilder) ((RequiredArgumentBuilder) ((RequiredArgumentBuilder) ((RequiredArgumentBuilder) ((RequiredArgumentBuilder) ((RequiredArgumentBuilder) CommandManager
												              .argument(
	...
	private static int executeLeave(ServerCommandSource source, Collection<ScoreHolder> members) {
		...
		if (members.size() == 1) {
			source.sendFeedback(
					() -> Text.translatable("commands.team.leave.success.single", getMemberName(members)),
					true
			);
		}
		else {
	...
	private static int executeModifyNametagVisibility(
			ServerCommandSource source,
			Team team,
			AbstractTeam.VisibilityRule visibility
	...
	private static int executeModifyDeathMessageVisibility(
			ServerCommandSource source,
			Team team,
			AbstractTeam.VisibilityRule visibility
	) throws CommandSyntaxException {
	...
	private static int executeModifyCollisionRule(
			ServerCommandSource source,
			Team team,
	...
	private static int executeModifySeeFriendlyInvisibles(ServerCommandSource source, Team team, boolean allowed)
	throws CommandSyntaxException {
		if (team.shouldShowFriendlyInvisibles() == allowed) {
			...
			source.sendFeedback(
					() -> Text.translatable(
							"commands.team.option.seeFriendlyInvisibles." + (allowed ? "enabled" : "disabled"),
							team.getFormattedName()
					), true
			);
			return 0;
	...
	private static int executeModifyFriendlyFire(ServerCommandSource source, Team team, boolean allowed)
	throws CommandSyntaxException {
		if (team.isFriendlyFireAllowed() == allowed) {
			...
			source.sendFeedback(
					() -> Text.translatable(
							"commands.team.option.friendlyfire." + (allowed ? "enabled" : "disabled"),
							team.getFormattedName()
					),
					true
			);
	...
	private static int executeModifyDisplayName(ServerCommandSource source, Team team, Text displayName)
	throws CommandSyntaxException {
		if (team.getDisplayName().equals(displayName)) {
			...
			source.sendFeedback(
					() -> Text.translatable("commands.team.option.name.success", team.getFormattedName()),
					true
			);
			return 0;
		}
	...
	private static int executeModifyColor(ServerCommandSource source, Team team, Formatting color)
	throws CommandSyntaxException {
	...
	private static int executeAdd(ServerCommandSource source, String team, Text displayName)
	throws CommandSyntaxException {
		Scoreboard scoreboard = source.getServer().getScoreboard();
	...
	private static int executeListMembers(ServerCommandSource source, Team team) {
		...
		if (collection.isEmpty()) {
			source.sendFeedback(
					() -> Text.translatable("commands.team.list.members.empty", team.getFormattedName()),
					false
			);
		}
		else {
		...
		if (collection.isEmpty()) {
			...
			source.sendFeedback(
					() -> Text.translatable(
							...
							collection.size(),
							Texts.join(collection, Team::getFormattedName)
					), false
			);
		}
