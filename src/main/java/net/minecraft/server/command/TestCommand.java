...
public class TestCommand {
	...
	public static final int DEFAULT_SEARCH_RADIUS = 15;
	public static final int DEFAULT_MAX_TESTS = 250;
	public static final int DEFAULT_TESTS_PER_ROW = 10;
	public static final int DEFAULT_ROTATION_STEPS = 100;
	private static final int MAX_SEARCH_RADIUS = 250;
	private static final int MAX_TESTS_COUNT = 1024;
	private static final int DEFAULT_ROTATION_STEP_COUNT = 3;
	private static final int DEFAULT_TEST_SPACING = 5;
	private static final int DEFAULT_TEST_PADDING = 5;
	private static final int DEFAULT_TEST_OFFSET = 5;
	...
	private static final Dynamic3CommandExceptionType
	...
	private static int reset(TestFinder finder) throws CommandSyntaxException {
		...
		int
				i =
				stream(finder.getCommandSource(), TestAttemptConfig.once(), finder)
						.map(state -> reset(finder.getCommandSource(), state))
						.toList()
						.size();
		if (i == 0) {
	...
	private static int clear(TestFinder finder) throws CommandSyntaxException {
		...
		List<TestInstanceBlockEntity> list = finder.findTestPos()
		                                           .flatMap(pos -> serverWorld
				                                           .getBlockEntity(pos, BlockEntityType.TEST_INSTANCE_BLOCK)
				                                           .stream())
		                                           .toList();
	...
	private static int start(TestFinder finder) {
		...
		Collection<GameTestState> collection = Stream.concat(
				                                             stream(serverCommandSource, TestAttemptConfig.once(), finder),
				                                             stream(serverCommandSource, TestAttemptConfig.once(), finder, 0)
		                                             )
		                                             .toList();
		...
		for (GameTestState gameTestState : collection) {
			for (BlockRotation blockRotation : BlockRotation.values()) {
				...
				GameTestBatch
						...
						Batches.create(
								collection3,
								gameTestState.getInstance().getEnvironment(),
								blockRotation.ordinal()
						);
	...
	private static int start(TestFinder finder, TestAttemptConfig config, int rotationSteps, int testsPerRow) {
		...
		BlockPos blockPos = getStructurePos(serverCommandSource);
		Collection<GameTestState> collection = Stream.concat(
				                                             stream(serverCommandSource, config, finder), stream(serverCommandSource, config, finder, rotationSteps)
		                                             )
		                                             .toList();
		if (collection.isEmpty()) {
			...
			serverCommandSource.sendFeedback(
					() -> Text.translatable("commands.test.run.running", collection.size()),
					false
			);
			TestRunContext testRunContext = TestRunContext.Builder.ofStates(collection, serverWorld)
			                                                      .initialSpawner(new TestStructurePlacer(
					                                                      blockPos,
					                                                      testsPerRow,
					                                                      false
			                                                      ))
		      ...
		      .forEach(
				      pos -> {
					      if (finder
							      .getCommandSource()
							      .getWorld()
							      .getBlockEntity(pos) instanceof TestInstanceBlockEntity testInstanceBlockEntity) {
						      ...
						      finder
								      .getCommandSource()
								      .sendFeedback(
										      () -> Text.translatable("commands.test.locate.found", text, l),
										      false
								      );
		...
		return builder.executes(context -> start(finderGetter.apply(context), TestAttemptConfig.once(), 0, 8))
		              .then(
				              ((RequiredArgumentBuilder) CommandManager
						              .argument("numberOfTimes", IntegerArgumentType.integer(0))
						              .executes(
								              context -> start(
	...
	private static ArgumentBuilder<ServerCommandSource, ?> testAttemptAndPlacementConfig(
		...
		return testAttemptConfig(
				...
				builder2 -> builder2.then(
						((RequiredArgumentBuilder) CommandManager
								.argument("rotationSteps", IntegerArgumentType.integer())
								.executes(
										context -> start(
												finderGetter.apply(context),
												new TestAttemptConfig(
	...
	public static void register(
		...
		LiteralArgumentBuilder<ServerCommandSource>
				...
				(LiteralArgumentBuilder<ServerCommandSource>) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) CommandManager
						.literal(
								"test"
						)
								              ...
								              .then(
										              testAttemptAndPlacementConfig(
												              CommandManager.argument(
														              "tests",
														              RegistrySelectorArgumentType.selector(
																              registryAccess,
																              RegistryKeys.TEST_INSTANCE
														              )
												              ),
												              context -> TestFinder.builder()
												                                   .selector(
														                                   context,
														                                   RegistrySelectorArgumentType.getEntries(
																                                   context,
																                                   "tests"
														                                   )
												                                   )
												              ...
												              .then(
														              CommandManager
																              ...
																              .executes(
																		              context -> start(
																				              TestFinder.builder()
																				                        .repeat(IntegerArgumentType.getInteger(
																						                        context,
																						                        "amount"
																				                        ))
																				                        .selector(
						...
						.then(testAttemptConfig(
								CommandManager.literal("runthese"),
								TestFinder.builder()::allStructures
						))
				)
						...
						.then(
								testAttemptAndPlacementConfig(
										CommandManager.literal("runfailed").then(argumentBuilder),
										TestFinder.builder()::failed
								)
						)
				)
						.then(
						...
						.then(CommandManager
								.literal("resetclosest")
								.executes(context -> reset(TestFinder.builder().nearest(context))))
				)
						.then(CommandManager
								.literal("resetthese")
								.executes(context -> reset(TestFinder.builder().allStructures(context))))
				)
						.then(CommandManager
								.literal("resetthat")
								.executes(context -> reset(TestFinder.builder().targeted(context))))
				)
						.then(CommandManager
								.literal("clearthat")
								.executes(context -> clear(TestFinder.builder().targeted(context))))
				)
						.then(CommandManager
								.literal("clearthese")
								.executes(context -> clear(TestFinder.builder().allStructures(context))))
				)
						.then(
		...
		if (SharedConstants.isDevelopment) {
			literalArgumentBuilder =
					(LiteralArgumentBuilder<ServerCommandSource>) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) literalArgumentBuilder.then(
							...
							.then(CommandManager
									.literal("exportclosest")
									.executes(context -> export(TestFinder.builder().nearest(context))))
					)
							.then(CommandManager
									.literal("exportthese")
									.executes(context -> export(TestFinder.builder().allStructures(context))))
					)
							.then(CommandManager
									.literal("exportthat")
									.executes(context -> export(TestFinder.builder().targeted(context))));
		}

		dispatcher.register(literalArgumentBuilder);
	}

	public static CompletableFuture<Suggestions> suggestTestFunctions(
			CommandContext<ServerCommandSource> context,
			SuggestionsBuilder builder
	) {
		Stream<String> stream = ((ServerCommandSource) context.getSource())
				.getRegistryManager()
				.getOrThrow(RegistryKeys.TEST_FUNCTION)
				.streamEntries()
	...
	private static Stream<GameTestState> stream(
			ServerCommandSource source,
			TestAttemptConfig config,
			TestInstanceBlockFinder finder
	) {
	...
	private static Stream<GameTestState> stream(
		...
		return finder.findTests()
		             .filter(instance -> checkStructure(source, instance.value().getStructure()))
		             .map(
				             instance -> new GameTestState(
						             (RegistryEntry.Reference<TestInstance>) instance,
						             TestInstanceUtil.getRotation(rotationSteps),
						             source.getWorld(),
						             config
				             )
		             );
		...
		if (serverWorld.getBlockEntity(pos) instanceof TestInstanceBlockEntity testInstanceBlockEntity) {
			Optional<RegistryEntry.Reference<TestInstance>> optional = testInstanceBlockEntity.getTestKey()
			                                                                                  .flatMap(source
					                                                                                  .getRegistryManager()
					                                                                                  .getOrThrow(
							                                                                                  RegistryKeys.TEST_INSTANCE)::getOptional);
			if (optional.isEmpty()) {
				source.sendError(Text.translatable(
						"commands.test.error.non_existant_test",
						testInstanceBlockEntity.getTestName()
				));
				return Optional.empty();
			}
			else {
				RegistryEntry.Reference<TestInstance> reference = optional.get();
				GameTestState
						gameTestState =
						new GameTestState(reference, testInstanceBlockEntity.getRotation(), serverWorld, config);
				gameTestState.setTestBlockPos(pos);
				return !checkStructure(source, gameTestState.getStructure()) ? Optional.empty()
				                                                             : Optional.of(gameTestState);
			}
		}
		else {
			source.sendError(Text.translatable(
	...
	private static int executeCreate(ServerCommandSource source, Identifier id, int x, int y, int z)
	throws CommandSyntaxException {
		if (x <= 48 && y <= 48 && z <= 48) {
			...
			BlockPos
					.stream(blockPos2, blockPos3)
					.forEach(pos -> serverWorld.setBlockState(pos, Blocks.BEDROCK.getDefaultState()));
			source.sendFeedback(
					() -> Text.translatable(
	...
	private static int executePos(ServerCommandSource source, String variableName) throws CommandSyntaxException {
		...
		if (optional.isEmpty()) {
		...
		else if (serverWorld.getBlockEntity(optional.get()) instanceof TestInstanceBlockEntity testInstanceBlockEntity) {
			...
			MutableText
					text =
					Text.translatable("commands.test.coordinates", blockPos3.getX(), blockPos3.getY(), blockPos3.getZ())
					    .setStyle(
							    Style.EMPTY
									    .withBold(true)
	...
	private static int executeExport(ServerCommandSource source, RegistryEntry<TestInstance> instance) {
		return !TestInstanceBlockEntity.exportData(
				source.getWorld(),
				instance.value().getStructure(),
				source::sendMessage
		) ? 0 : 1;
	}
		...
		private void onFinished() {
			if (this.tests.isDone()) {
				this.source.sendFeedback(
						() -> Text
								.translatable("commands.test.summary", this.tests.getTestCount())
								.formatted(Formatting.WHITE), true
				);
				if (this.tests.failed()) {
					this.source.sendError(Text.translatable(
							"commands.test.summary.failed",
							this.tests.getFailedRequiredTestCount()
					));
				}
				else {
					this.source.sendFeedback(
							() -> Text
									.translatable("commands.test.summary.all_required_passed")
									.formatted(Formatting.GREEN), true
					);
				}

				if (this.tests.hasFailedOptionalTests()) {
					this.source.sendMessage(Text.translatable(
							"commands.test.summary.optional_failed",
							this.tests.getFailedOptionalTestCount()
					));
				}
			}
		}
