...
public class CloneCommand {
	...
	private static final SimpleCommandExceptionType
			OVERLAP_EXCEPTION =
			new SimpleCommandExceptionType(Text.translatable("commands.clone.overlap"));
	...
	private static final SimpleCommandExceptionType
			FAILED_EXCEPTION =
			new SimpleCommandExceptionType(Text.translatable("commands.clone.failed"));
	...
	public static void register(
		...
		dispatcher.register(
				(LiteralArgumentBuilder) ((LiteralArgumentBuilder) ((LiteralArgumentBuilder) CommandManager
						...
						.then(createSourceArgs(
								commandRegistryAccess,
								context -> ((ServerCommandSource) context.getSource()).getWorld()
						))
				)
						.then(
	...
	private static ArgumentBuilder<ServerCommandSource, ?> createSourceArgs(
			CommandRegistryAccess commandRegistryAccess,
			ArgumentGetter<CommandContext<ServerCommandSource>, ServerWorld> worldGetter
	) {
	...
	private static CloneCommand.DimensionalPos createDimensionalPos(
	...
	private static ArgumentBuilder<ServerCommandSource, ?> createDestinationArgs(
		...
		ArgumentGetter<CommandContext<ServerCommandSource>, CloneCommand.DimensionalPos>
				argumentGetter =
				context -> createDimensionalPos(
				                            ...
				                            .then(
						                            createModeArgs(
								                            ...
								                            CommandManager.argument(
										                            "filter",
										                            BlockPredicateArgumentType.blockPredicate(registries)
								                            )
						                            )
				                            )
		              );
	...
	private static int execute(
		...
		if (!mode.allowsOverlap() && serverWorld == serverWorld2 && blockBox2.intersects(blockBox)) {
			...
			else if (!serverWorld.isRegionLoaded(blockPos, blockPos2) || !serverWorld2.isRegionLoaded(
					blockPos3,
					blockPos4
			)) {
				...
				try {
					...
					for (int l = blockBox.getMinZ(); l <= blockBox.getMaxZ(); l++) {
						for (int m = blockBox.getMinY(); m <= blockBox.getMaxY(); m++) {
							for (int n = blockBox.getMinX(); n <= blockBox.getMaxX(); n++) {
								...
								CachedBlockPosition
										cachedBlockPosition =
										new CachedBlockPosition(serverWorld, blockPos6, false);
								...
								if (filter.test(cachedBlockPosition)) {
									...
									if (blockEntity != null) {
										...
										CloneCommand.BlockEntityInfo blockEntityInfo = new CloneCommand.BlockEntityInfo(
												nbtWriteView.getNbt(), blockEntity.getComponents()
										);
										list2.add(new CloneCommand.BlockInfo(
												blockPos7,
												blockState,
												blockEntityInfo,
												serverWorld2.getBlockState(blockPos7)
										));
										deque.addLast(blockPos6);
									}
									else if (!blockState.isOpaqueFullCube() && !blockState.isFullCube(
											serverWorld,
											blockPos6
									)) {
										list3.add(new CloneCommand.BlockInfo(
												blockPos7,
												blockState,
												null,
												serverWorld2.getBlockState(blockPos7)
										...
										list.add(new CloneCommand.BlockInfo(
												blockPos7,
												blockState,
												null,
												serverWorld2.getBlockState(blockPos7)
						...
						if (blockInfox.blockEntityInfo != null && blockEntity2 != null) {
							blockEntity2.readComponentlessData(
									NbtReadView.create(
											...
											serverWorld2.getRegistryManager(),
											blockInfox.blockEntityInfo.nbt
									)
							);
					...
					if (!strict) {
						for (CloneCommand.BlockInfo blockInfox : list5) {
							serverWorld2.onStateReplacedWithCommands(
									blockInfox.pos,
									blockInfox.previousStateAtDestination
							);
						}
					}
					...
					serverWorld2
							...
							.scheduleTicks(serverWorld.getBlockTickScheduler(), blockBox, blockPos5);
				}
	...
	record BlockInfo(
			BlockPos pos,
			BlockState state,
			CloneCommand.@Nullable BlockEntityInfo blockEntityInfo,
