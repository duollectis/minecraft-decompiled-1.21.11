...
public class SpreadPlayersCommand {
	...
	private static final Dynamic4CommandExceptionType FAILED_TEAMS_EXCEPTION = new Dynamic4CommandExceptionType(
			(pilesCount, x, z, maxSpreadDistance) -> Text.stringifiedTranslatable(
	...
	private static final Dynamic2CommandExceptionType INVALID_HEIGHT_EXCEPTION = new Dynamic2CommandExceptionType(
			(maxY, worldBottomY) -> Text.stringifiedTranslatable(
	...
	private static int execute(
		...
		if (maxY < i) {
			...
			SpreadPlayersCommand.Pile[]
					piles =
					makePiles(random, respectTeams ? getPileCountRespectingTeams(targets) : targets.size(), d, e, f, g);
	...
	private static void spread(
		...
		if (i >= 10000) {
			if (respectTeams) {
				...
				throw FAILED_ENTITIES_EXCEPTION.create(
						piles.length,
						center.x,
						center.y,
						String.format(Locale.ROOT, "%.2f", d)
	...
	private static double teleport(
		...
		for (Entity entity : entities) {
			...
			entity.teleport(
					world,
					MathHelper.floor(pile.x) + 0.5,
					pile.getY(world, maxY),
					MathHelper.floor(pile.z) + 0.5,
					Set.of(),
					entity.getYaw(),
					entity.getPitch(),
					true
	...
	private static SpreadPlayersCommand.Pile[] makePiles(
			Random random,
			int count,
			double minX,
			double minZ,
