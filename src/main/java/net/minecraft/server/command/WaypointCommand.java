...
public class WaypointCommand {
   public static void register(CommandDispatcher<ServerCommandSource> dispatcher, CommandRegistryAccess registryAccess) {
      dispatcher.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("waypoint")
            ...
            .then(
               CommandManager.literal("modify")
                  .then(
                     ((RequiredArgumentBuilder)CommandManager.argument("waypoint", EntityArgumentType.entity())
                           .then(
                              ((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("color")
                                       .then(
                                             ...
                                             CommandManager.argument("color", HexColorArgumentType.hexColor())
                                                .executes(
                                                   context -> executeColor(
                                                      (ServerCommandSource)context.getSource(),
                                                      WaypointArgument.getWaypoint(context, "waypoint"),
                                                      HexColorArgumentType.getArgbColor(context, "color")
                                                   )
                                                )
                                          )
                                    ))
                                 .then(
                                    CommandManager.literal("reset")
                                       .executes(
                                          context -> executeReset((ServerCommandSource)context.getSource(), WaypointArgument.getWaypoint(context, "waypoint"))
                                       )
                                 )
                           ))
                        .then(
                           ((LiteralArgumentBuilder)CommandManager.literal("style")
                                 .then(
                                    CommandManager.literal("reset")
                                       .executes(
                        ...
                        return livingEntity.getStyledDisplayName()
                           ...
                           .styled(
                              style -> style.withClickEvent(
                                 ...
                                 .withColor(waypoint.getWaypointConfig().color.orElse(-1))
                           );
                     } else {
