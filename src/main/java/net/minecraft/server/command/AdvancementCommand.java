...
public class AdvancementCommand {
																                            ...
																                            ((RequiredArgumentBuilder) CommandManager
																		                            ...
																		                            .executes(
																				                            context -> executeAdvancement(
																						                            ...
																						                            select(
																								                            context,
																								                            RegistryKeyArgumentType.getAdvancementEntry(
																										                            context,
																										                            "advancement"
																								                            ),
		...
		if (i == 0) {
			if (selection.size() == 1) {
				if (targets.size() == 1) {
					...
					throw GENERIC_EXCEPTION.create(
							Text.translatable(
									operation.getCommandPrefix() + ".one.to.many.failure",
									Advancement.getNameFromIdentity(selection.iterator().next()),
									targets.size()
							)
					);
				...
				throw GENERIC_EXCEPTION.create(
						Text.translatable(
								operation.getCommandPrefix() + ".many.to.one.failure",
								selection.size(),
								targets.iterator().next().getDisplayName()
						)
				);
				...
				throw GENERIC_EXCEPTION.create(Text.translatable(
						operation.getCommandPrefix() + ".many.to.many.failure",
						selection.size(),
						targets.size()
				));
			}
			...
			if (selection.size() == 1) {
				if (targets.size() == 1) {
					...
					source.sendFeedback(
							() -> Text.translatable(
									operation.getCommandPrefix() + ".one.to.many.success",
									Advancement.getNameFromIdentity(selection.iterator().next()),
									targets.size()
							),
							true
				...
				source.sendFeedback(
						() -> Text.translatable(
								operation.getCommandPrefix() + ".many.to.one.success",
								selection.size(),
								targets.iterator().next().getDisplayName()
						),
						true
				...
				source.sendFeedback(
						() -> Text.translatable(
								operation.getCommandPrefix() + ".many.to.many.success",
								selection.size(),
								targets.size()
						), true
				);
		...
		if (!advancement2.criteria().containsKey(criterion)) {
			...
			if (i == 0) {
				if (targets.size() == 1) {
					...
					throw GENERIC_EXCEPTION.create(
							Text.translatable(
									operation.getCommandPrefix() + ".criterion.to.many.failure",
									criterion,
									Advancement.getNameFromIdentity(advancement),
									targets.size()
							)
				...
				if (targets.size() == 1) {
					...
					source.sendFeedback(
							() -> Text.translatable(
									operation.getCommandPrefix() + ".criterion.to.many.success",
									criterion,
									Advancement.getNameFromIdentity(advancement),
									targets.size()
							),
	...
	private static List<AdvancementEntry> select(
		...
		AdvancementManager
				advancementManager =
				((ServerCommandSource) context.getSource()).getServer().getAdvancementLoader().getManager();
	...
	static enum Operation {
		GRANT("grant") {
			...
			@Override
			protected boolean processEachCriterion(
