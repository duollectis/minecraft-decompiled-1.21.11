...
public class Texts {
	...
	public static Text withStyle(Text text, Style style) {
		if (style.isEmpty()) {
			...
			if (style2.isEmpty()) {
				...
				return (Text) (style2.equals(style) ? text : text.copy().setStyle(style2.withParent(style)));
			}
		}
	}
	...
	public static MutableText parse(@Nullable ServerCommandSource source, Text text, @Nullable Entity sender, int depth)
	throws CommandSyntaxException {
		if (depth > 100) {
			return text.copy();
		}
