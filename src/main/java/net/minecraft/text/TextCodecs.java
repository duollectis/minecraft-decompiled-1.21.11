...
public class TextCodecs {
	...
	public static final PacketCodec<RegistryByteBuf, Optional<Text>>
			...
			UNLIMITED_REGISTRY_PACKET_CODEC.collect(
	...
	public static Codec<Text> withJsonLengthLimit(int maxLength) {
		return new Codec<Text>() {
				...
				return TextCodecs.CODEC
						...
						.flatMap(
								pair -> this.isTooLarge(ops, (Text) pair.getFirst())
								        ? DataResult.error(() -> "Component was too large: greater than max size "
								                                 + maxLength)
			...
			private <T> boolean isTooLarge(DynamicOps<T> ops, Text text) {
				...
				return dataResult.isSuccess() && JsonHelper.isTooLarge(
			...
			private static <T> DynamicOps<JsonElement> toJsonOps(DynamicOps<T> ops) {
				return (DynamicOps<JsonElement>) (ops instanceof RegistryOps<T> registryOps ? registryOps.withDelegate(
