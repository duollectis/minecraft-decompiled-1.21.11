...
public class NbtTextContent implements TextContent {
   ...
   private NbtTextContent(String rawPath, NbtPathArgumentType.@Nullable NbtPath path, boolean interpret, Optional<Text> separator, NbtDataSource dataSource) {
      ...
      if (source != null && this.path != null) {
         ...
         if (this.interpret) {
            ...
            return stream.flatMap(nbt -> {
               try {
                  Text textx = (Text)TextCodecs.CODEC.parse(registryOps, nbt).getOrThrow();
            ...
            return Texts.parse(source, this.separator, sender, depth)
               .map(textx -> stream2.map(Text::literal).reduce((accumulator, current) -> accumulator.append(textx).append(current)).orElseGet(Text::empty))
               .orElseGet(() -> Text.literal(stream2.collect(Collectors.joining(", "))));
         }
      } else {
         return Text.empty();
      }
