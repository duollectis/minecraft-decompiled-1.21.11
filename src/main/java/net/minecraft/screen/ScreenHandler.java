...
public abstract class ScreenHandler {
	...
	protected static void checkDataCount(PropertyDelegate data, int expectedCount) {
		...
		if (i < expectedCount) {
			throw new IllegalArgumentException(
					"Container data count " + i + " is smaller than expected " + expectedCount);
		}
	}
	...
	protected Slot addSlot(Slot slot) {
		...
		this.trackedSlots.add(
				this.syncHandler != null ? this.syncHandler.createTrackedSlot() : TrackedSlot.ALWAYS_IN_SYNC);
		return slot;
	}
	...
	public void onSlotClick(int slotIndex, int button, SlotActionType actionType, PlayerEntity player) {
		try {
			...
			crashReportSection.add(
					"Menu Type",
					() -> this.type != null ? Registries.SCREEN_HANDLER.getId(this.type).toString() : "<no type>"
			);
	...
	private void internalOnSlotClick(int slotIndex, int button, SlotActionType actionType, PlayerEntity player) {
		...
		if (actionType == SlotActionType.QUICK_CRAFT) {
					...
					for (Slot slot2 : this.quickCraftSlots) {
						...
						if (slot2 != null
							...
							int
									n =
									Math.min(
											calculateStackSize(this.quickCraftSlots, this.quickCraftButton, itemStack2)
													+ l, m
									);
							k -= n - l;
		...
		else if ((actionType == SlotActionType.PICKUP || actionType == SlotActionType.QUICK_MOVE) && (button == 0
				|| button == 1
		)) {
				...
				if (!this.handleSlotClick(player, clickType, slot, itemStack, itemStack4)) {
					if (itemStack.isEmpty()) {
						...
						if (itemStack4.isEmpty()) {
							...
							Optional<ItemStack>
									optional2 =
									slot.tryTakeStackRange(
											itemStack.getCount(),
											itemStack4.getMaxCount() - itemStack4.getCount(),
											player
									);
		...
		else if (actionType == SlotActionType.CLONE && player.isInCreativeMode() && this.getCursorStack().isEmpty()
				&& slotIndex >= 0) {
			...
			if (!itemStack2.isEmpty() && (!slot3x.hasStack() || !slot3x.canTakeItems(player))) {
				...
				for (int o = 0; o < 2; o++) {
					for (int q = k;
					     q >= 0 && q < this.slots.size() && itemStack2.getCount() < itemStack2.getMaxCount();
					     q += p) {
						...
						if (slot4.hasStack()
							...
							if (o != 0 || itemStack6.getCount() != itemStack6.getMaxCount()) {
								ItemStack
										itemStack7 =
										slot4.takeStackRange(
												itemStack6.getCount(),
												itemStack2.getMaxCount() - itemStack2.getCount(),
												player
										);
	...
	private boolean handleSlotClick(
		...
		return cursorStack.isItemEnabled(featureSet) && cursorStack.onStackClicked(slot, clickType, player)
		       ? true
		       : stack.isItemEnabled(featureSet) && stack.onClicked(
				       cursorStack,
				       slot,
				       clickType,
				       player,
				       this.getCursorStackReference()
		       );
	}
