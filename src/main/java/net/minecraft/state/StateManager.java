...
public class StateManager<O, S extends State<O, S>> {
	...
	protected StateManager(
		...
		stream.forEach(entries -> {
			Reference2ObjectArrayMap<Property<?>, Comparable<?>>
					...
					new Reference2ObjectArrayMap(entries.size());
		...
		return (MapCodec<S>) Codec
				.mapPair(
						...
						property
								.getValueCodec()
								.fieldOf(key)
								.orElseGet(value -> {}, () -> property.createValue(defaultStateGetter.get()))
				)
				.xmap(
	...
	public String toString() {
		return MoreObjects.toStringHelper(this)
		                  ...
		                  .add(
				                  "properties",
				                  this.properties.values().stream().map(Property::getName).collect(Collectors.toList())
		                  )
		                  .toString();
	}
			...
			if (!StateManager.VALID_NAME_PATTERN.matcher(string).matches()) {
				...
				if (collection.size() <= 1) {
					throw new IllegalArgumentException(
							this.owner + " attempted use property " + string + " with <= 1 possible values");
				}
				else {
					for (T comparable : collection) {
						...
						if (!StateManager.VALID_NAME_PATTERN.matcher(string2).matches()) {
							throw new IllegalArgumentException(
									this.owner + " has property: " + string + " with invalidly named value: "
											+ string2);
						}
					}
