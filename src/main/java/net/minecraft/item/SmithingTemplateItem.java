...
/**
 * Предмет «Шаблон кузнеца». Хранит тексты подсказок и текстуры пустых слотов
 * для отображения в интерфейсе кузнечного стола. Фабричные методы {@link #of}
 * и {@link #createNetheriteUpgrade} создают предустановленные варианты шаблонов.
 */
public class SmithingTemplateItem extends Item {
	...
	private static final Text INGREDIENTS_TEXT = Text
		.translatable(Util.createTranslationKey("item", Identifier.ofVanilla("smithing_template.ingredients")))
		.formatted(TITLE_FORMATTING);

	private static final Text APPLIES_TO_TEXT = Text
		.translatable(Util.createTranslationKey("item", Identifier.ofVanilla("smithing_template.applies_to")))
		.formatted(TITLE_FORMATTING);

	private static final Text SMITHING_TEMPLATE_TEXT = Text
		.translatable(Util.createTranslationKey("item", Identifier.ofVanilla("smithing_template")))
		.formatted(TITLE_FORMATTING);

	private static final Text ARMOR_TRIM_APPLIES_TO_TEXT = Text
		.translatable(Util.createTranslationKey("item", Identifier.ofVanilla("smithing_template.armor_trim.applies_to")))
		.formatted(DESCRIPTION_FORMATTING);

	private static final Text ARMOR_TRIM_INGREDIENTS_TEXT = Text
		.translatable(Util.createTranslationKey("item", Identifier.ofVanilla("smithing_template.armor_trim.ingredients")))
		.formatted(DESCRIPTION_FORMATTING);
	...
	private static final Text NETHERITE_UPGRADE_APPLIES_TO_TEXT = Text
		.translatable(Util.createTranslationKey("item", Identifier.ofVanilla("smithing_template.netherite_upgrade.applies_to")))
		.formatted(DESCRIPTION_FORMATTING);

	private static final Text NETHERITE_UPGRADE_INGREDIENTS_TEXT = Text
		.translatable(Util.createTranslationKey("item", Identifier.ofVanilla("smithing_template.netherite_upgrade.ingredients")))
		.formatted(DESCRIPTION_FORMATTING);
	...
	/**
	 * Создаёт шаблон для нанесения украшений на броню.
	 */
	...
	/**
	 * Создаёт шаблон для улучшения снаряжения до незерита.
	 */
	...
	public void appendTooltip(
		...
		textConsumer.accept(ScreenTexts.space().append(appliesToText));
		textConsumer.accept(INGREDIENTS_TEXT);
		textConsumer.accept(ScreenTexts.space().append(ingredientsText));
	}

	public Text getBaseSlotDescription() {
		return baseSlotDescriptionText;
	}

	public Text getAdditionsSlotDescription() {
		return additionsSlotDescriptionText;
	}

	public List<Identifier> getEmptyBaseSlotTextures() {
		return emptyBaseSlotTextures;
	}

	public List<Identifier> getEmptyAdditionsSlotTextures() {
		return emptyAdditionsSlotTextures;
	}
}
