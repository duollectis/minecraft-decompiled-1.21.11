...
public class ItemGroups {
				...
				ItemGroup.create(ItemGroup.Row.TOP, 3)
				         ...
				         .entries(
						         (displayContext, entries) -> {
							         ...
							         displayContext.lookup()
							                       ...
							                       .ifPresent(
									                       registryWrapper -> addPaintings(
											                       ...
											                       registryEntry -> registryEntry.isIn(
							         ...
							         entries.add(Items.MAGENTA_BANNER);
							         entries.add(Items.PINK_BANNER);
							         entries.add(Raid.createOminousBanner(displayContext
									         .lookup()
						         ...
						         (displayContext, entries) -> {
							         if (displayContext.hasPermissions()) {
								         ...
								         for (TestBlockMode testBlockMode : TestBlockMode.values()) {
									         entries.add(TestBlock.applyBlockStateToStack(
	...
	private static void addMaxLevelEnchantedBooks(
		...
		registryWrapper.streamEntries()
		               .map(enchantmentEntry -> EnchantmentHelper.getEnchantedBookWith(new EnchantmentLevelEntry(
				               enchantmentEntry,
	...
	private static void addAllLevelEnchantedBooks(
		...
		registryWrapper.streamEntries()
		               .flatMap(
				               enchantmentEntry -> IntStream
						               .rangeClosed(
								               enchantmentEntry.value().getMinLevel(),
								               enchantmentEntry.value().getMaxLevel()
						               )
						               .mapToObj(level -> EnchantmentHelper.getEnchantedBookWith(new EnchantmentLevelEntry(
								               enchantmentEntry,
								               level
						               )))
		               )
	...
	private static void addInstruments(
			ItemGroup.Entries entries,
			RegistryWrapper<Instrument> registryWrapper,
			Item item,
	...
	private static void updateEntries(ItemGroup.DisplayContext displayContext) {
		stream()
				...
				.forEach(group -> group.updateEntries(displayContext));
		stream()
				.filter(group -> group.getType() != ItemGroup.Type.CATEGORY)
				.forEach(group -> group.updateEntries(displayContext));
	}

	public static boolean updateDisplayContext(
			FeatureSet enabledFeatures,
