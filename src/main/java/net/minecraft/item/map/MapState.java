...
public class MapState extends PersistentState {
      ...
      for (int i = 0; i < this.updateTrackers.size(); i++) {
         ...
         if (!playerEntity.isRemoved() && (playerEntity.getInventory().contains(predicate) || stack.isInFrame())) {
            if (!stack.isInFrame() && playerEntity.getEntityWorld().getRegistryKey() == this.dimension && this.showDecorations) {
               this.addDecoration(
                  MapDecorationTypes.PLAYER, playerEntity.getEntityWorld(), string, playerEntity.getX(), playerEntity.getZ(), playerEntity.getYaw(), null
               );
            }
         } else {
   ...
   private MapState.@Nullable Marker getMarker(RegistryEntry<MapDecorationType> type, @Nullable WorldAccess world, double rotation, float dx, float dz) {
