...
public interface ProjectileItem {
	...
	default void initializeProjectile(
	...
	public record Settings(
			...
			float uncertainty,
			float power,
			OptionalInt overrideDispenseEvent
	) {
		...
		public static class Builder {
			...
			private ProjectileItem.PositionFunction
					positionFunction =
					(pointer, direction) -> DispenserBlock.getOutputLocation(
			...
			public ProjectileItem.Settings build() {
				return new ProjectileItem.Settings(
						this.positionFunction,
						this.uncertainty,
						this.power,
						this.overrideDispenserEvent
				);
			}
		}
	}
