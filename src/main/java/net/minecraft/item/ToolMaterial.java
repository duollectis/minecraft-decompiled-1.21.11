...
/**
 * Материал инструмента: определяет прочность, скорость добычи, бонус к урону,
 * ценность зачарования, теги неправильных блоков и ремонтные материалы.
 * Используется в {@link Item.Settings} для настройки инструментов и оружия.
 */
public record ToolMaterial(
	...
	private Item.Settings applyBaseSettings(Item.Settings settings) {
		return settings.maxDamage(durability).repairable(repairItems).enchantable(enchantmentValue);
	}
	...
	public Item.Settings applyToolSettings(
		...
		return applyBaseSettings(settings)
			.component(
				...
				new ToolComponent(
					List.of(
						ToolComponent.Rule.ofNeverDropping(blockLookup.getOrThrow(incorrectBlocksForDrops)),
						ToolComponent.Rule.ofAlwaysDropping(blockLookup.getOrThrow(effectiveBlocks), speed)
					),
					1.0F,
					1,
					true
			...
			.attributeModifiers(createToolAttributeModifiers(attackDamage, attackSpeed))
	...
	public Item.Settings applySwordSettings(Item.Settings settings, float attackDamage, float attackSpeed) {
		...
		return applyBaseSettings(settings)
				...
				new ToolComponent(
					List.of(
						...
						ToolComponent.Rule.of(
							blockLookup.getOrThrow(BlockTags.SWORD_INSTANTLY_MINES),
							Float.MAX_VALUE
						),
						ToolComponent.Rule.of(
							blockLookup.getOrThrow(BlockTags.SWORD_EFFICIENT),
							1.5F
						)
					),
					1.0F,
			...
			.attributeModifiers(createSwordAttributeModifiers(attackDamage, attackSpeed))
