...
public class FilledMapItem extends Item {
	...
	public static ItemStack createMap(
		...
		MapIdComponent
				mapIdComponent =
				allocateMapId(world, x, z, scale, showIcons, unlimitedTracking, world.getRegistryKey());
	...
	public void updateColors(World world, Entity entity, MapState state) {
		if (world.getRegistryKey() == state.dimension && entity instanceof PlayerEntity) {
			...
			playerUpdateTracker.updateTick++;
			BlockPos.Mutable mutable = new BlockPos.Mutable();
			BlockPos.Mutable mutable2 = new BlockPos.Mutable();
			boolean bl = false;

			for (int o = l - n + 1; o < l + n; o++) {
				if ((o & 15) == (playerUpdateTracker.updateTick & 15) || bl) {
					bl = false;
					double d = 0.0;
					...
					for (int p = m - n - 1; p < m + n; p++) {
						if (o >= 0 && p >= -1 && o < 128 && p < 128) {
							...
							WorldChunk
									worldChunk =
									world.getChunk(
											ChunkSectionPos.getSectionCoord(r),
											ChunkSectionPos.getSectionCoord(s)
									);
										...
										for (int v = 0; v < i; v++) {
											...
											if (w <= world.getBottomY()) {
												...
												if (w > world.getBottomY() && !blockState.getFluidState().isEmpty()) {
													...
													while (x > world.getBottomY() && !blockState2
															.getFluidState()
															.isEmpty());

													blockState =
															this.getFluidStateIfVisible(world, blockState, mutable);
												}
											}
	...
	private BlockState getFluidStateIfVisible(World world, BlockState state, BlockPos pos) {
		...
		return !fluidState.isEmpty() && !state.isSideSolidFullSquare(world, pos, Direction.UP)
		       ? fluidState.getBlockState() : state;
	}
