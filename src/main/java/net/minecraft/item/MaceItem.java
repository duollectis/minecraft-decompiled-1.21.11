...
public class MaceItem extends Item {
	...
	public void postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
		if (shouldDealAdditionalDamage(attacker)) {
			...
			if (target.isOnGround()) {
				...
				SoundEvent
						soundEvent =
						attacker.fallDistance > 5.0 ? SoundEvents.ITEM_MACE_SMASH_GROUND_HEAVY
		...
		if (damageSource.getSource() instanceof LivingEntity livingEntity) {
			if (!shouldDealAdditionalDamage(livingEntity)) {
				...
				return livingEntity.getEntityWorld() instanceof ServerWorld serverWorld
				       ? (float) (g + EnchantmentHelper.getSmashDamagePerFallenBlock(
						serverWorld,
						livingEntity.getWeaponStack(),
						target,
						damageSource,
						0.0F
				) * f
	...
	private static void knockbackNearbyEntities(World world, Entity attacker, Entity attacked) {
		...
		world
				.getEntitiesByClass(
						LivingEntity.class,
						attacked.getBoundingBox().expand(3.5),
				...
				.forEach(entity -> {
					...
					if (d > 0.0) {
						...
						if (entity instanceof ServerPlayerEntity serverPlayerEntity) {
							serverPlayerEntity.networkHandler.sendPacket(new EntityVelocityUpdateS2CPacket(
									serverPlayerEntity));
						}
					}
				});
