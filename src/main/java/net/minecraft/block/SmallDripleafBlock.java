...
public class SmallDripleafBlock extends TallPlantBlock implements Fertilizable, Waterloggable {
	...
	protected boolean canPlantOnTop(BlockState floor, BlockView world, BlockPos pos) {
		return floor.isIn(BlockTags.SMALL_DRIPLEAF_PLACEABLE)
				|| world.getFluidState(pos.up()).isEqualAndStill(Fluids.WATER) && super.canPlantOnTop(
				floor,
				world,
				pos
		);
	...
	public @Nullable BlockState getPlacementState(ItemPlacementContext ctx) {
		...
		return blockState != null
		       ? withWaterloggedState(
				ctx.getWorld(),
				ctx.getBlockPos(),
				blockState.with(FACING, ctx.getHorizontalPlayerFacing().getOpposite())
		)
		       : null;
	}
	...
	public void onPlaced(
		...
		if (!world.isClient()) {
			...
			BlockState blockState = TallPlantBlock.withWaterloggedState(
					world,
					blockPos,
					this.getDefaultState().with(HALF, DoubleBlockHalf.UPPER).with(FACING, state.get(FACING))
			);
