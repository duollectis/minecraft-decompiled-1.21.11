...
public class MultifaceGrower {
	...
	public static final MultifaceGrower.GrowType[] GROW_TYPES = new MultifaceGrower.GrowType[]{
			MultifaceGrower.GrowType.SAME_POSITION,
			MultifaceGrower.GrowType.SAME_PLANE,
			MultifaceGrower.GrowType.WRAP_AROUND
	};
	...
	public boolean canGrow(BlockState state, BlockView world, BlockPos pos, Direction direction) {
		return Direction
				.stream()
				.anyMatch(direction2 -> this
						.getGrowPos(state, world, pos, direction, direction2, this.growChecker::canGrow)
						.isPresent());
	}
	...
	public Optional<MultifaceGrower.GrowPos> getGrowPos(
			...
			Direction oldDirection,
			Direction newDirection,
			MultifaceGrower.GrowPosPredicate predicate
	) {
		if (newDirection.getAxis() == oldDirection.getAxis()) {
			return Optional.empty();
		}
		else if (this.growChecker.canGrow(state)
				|| this.growChecker.hasDirection(state, oldDirection) && !this.growChecker.hasDirection(
				state,
				newDirection
		)) {
	...
	public Optional<MultifaceGrower.GrowPos> place(
			WorldAccess world,
			MultifaceGrower.GrowPos pos,
			boolean markForPostProcessing
	) {
		BlockState blockState = world.getBlockState(pos.pos());
		return this.growChecker.place(world, pos, blockState, markForPostProcessing) ? Optional.of(pos)
		                                                                             : Optional.empty();
	}
	...
	public interface GrowChecker {
		...
		default boolean place(
				WorldAccess world,
				MultifaceGrower.GrowPos growPos,
				BlockState state,
				boolean markForPostProcessing
		) {
	...
	enum GrowType {
		...
		WRAP_AROUND {
			@Override
			public MultifaceGrower.GrowPos getGrowPos(BlockPos pos, Direction newDirection, Direction oldDirection) {
				return new MultifaceGrower.GrowPos(
						pos.offset(newDirection).offset(oldDirection),
						newDirection.getOpposite()
				);
			}
		};

		public abstract MultifaceGrower.GrowPos getGrowPos(
				BlockPos pos,
				Direction newDirection,
				Direction oldDirection
		);
	...
	public static class LichenGrowChecker implements MultifaceGrower.GrowChecker {
		...
		@Override
		public @Nullable BlockState getStateWithDirection(
				BlockState state,
				BlockView world,
				BlockPos pos,
				Direction direction
		...
		protected boolean canGrow(
			...
			return state.isAir() || state.isOf(this.lichen) || state.isOf(Blocks.WATER) && state
					.getFluidState()
					.isStill();
		}
