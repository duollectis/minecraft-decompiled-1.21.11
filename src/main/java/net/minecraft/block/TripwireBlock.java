...
public class TripwireBlock extends Block {
	...
	public static final MapCodec<TripwireBlock> CODEC = RecordCodecBuilder.mapCodec(
			instance -> instance
					.group(
							Registries.BLOCK.getCodec().fieldOf("hook").forGetter(block -> block.hookBlock),
							createSettingsCodec()
					)
	...
	public BlockState onBreak(World world, BlockPos pos, BlockState state, PlayerEntity player) {
		if (!world.isClient() && !player.getMainHandStack().isEmpty() && player.getMainHandStack().isOf(Items.SHEARS)) {
			world.setBlockState(pos, state.with(DISARMED, true), 260);
	...
	private void updatePowered(World world, BlockPos pos) {
		...
		List<? extends Entity>
				list =
				world.getOtherEntities(null, blockState.getOutlineShape(world, pos).getBoundingBox().offset(pos));
	...
	private void updatePowered(World world, BlockPos pos, List<? extends Entity> entities) {
		...
		if (bl2) {
			world.scheduleBlockTick(new BlockPos(pos), this, SCHEDULED_TICK_DELAY);
		}
	}
