...
public class RedstoneWireBlock extends Block {
	...
	private static final int[] COLORS = Util.make(
			new int[16], colors -> {
				for (int i = 0; i <= 15; i++) {
					float f = i / 15.0F;
	...
	private static final float PARTICLE_SPAWN_CHANCE = 0.2F;
	...
	private Function<BlockState, VoxelShape> createShapeFunction() {
		...
		Map<Direction, VoxelShape>
				map =
				VoxelShapes.createHorizontalFacingShapeMap(Block.createCuboidZShape(10.0, 0.0, 1.0, 0.0, 8.0));
		Map<Direction, VoxelShape>
				map2 =
				VoxelShapes.createHorizontalFacingShapeMap(Block.createCuboidZShape(10.0, 16.0, 0.0, 1.0));
				...
				state -> {
					...
					for (Entry<Direction, EnumProperty<WireConnection>> entry : DIRECTION_TO_WIRE_CONNECTION_PROPERTY.entrySet()) {
						voxelShape2 = switch ((WireConnection) state.get(entry.getValue())) {
							case UP ->
									VoxelShapes.union(voxelShape2, map.get(entry.getKey()), map2.get(entry.getKey()));
							case SIDE -> VoxelShapes.union(voxelShape2, map.get(entry.getKey()));
	...
	protected BlockState getStateForNeighborUpdate(
		...
		if (direction == Direction.DOWN) {
			...
			return wireConnection.isConnected() == state
					.get(DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(direction))
					.isConnected() && !isFullyConnected(state)
			       ...
			       : this.getPlacementState(
					       ...
					       .with(DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(direction), wireConnection),
					       pos
			       );
		}
	}
	...
	private WireConnection getRenderConnectionType(BlockView world, BlockPos pos, Direction direction, boolean bl) {
		...
		if (bl) {
			boolean
					bl2 =
					blockState.getBlock() instanceof TrapdoorBlock || this.canRunOnTop(world, blockPos, blockState);
		...
		if (this.wiresGivePower && direction != Direction.DOWN) {
			...
			if (i == 0) {
				...
				return direction != Direction.UP
						       && !this
						.getPlacementState(world, state, pos)
						.get(DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(direction.getOpposite()))
						.isConnected()
				       ? 0
				       : i;
			}
	...
	protected static boolean connectsTo(BlockState state, @Nullable Direction dir) {
		if (state.isOf(Blocks.REDSTONE_WIRE)) {
			...
			return state.isOf(Blocks.OBSERVER) ? dir == state.get(ObserverBlock.FACING)
			                                   : state.emitsRedstonePower() && dir != null;
		}
	}
