...
public interface SculkSpreadable {
		...
		public boolean spread(
			...
			if (directions == null) {
				return ((SculkVeinBlock) Blocks.SCULK_VEIN)
						...
						.grow(world.getBlockState(pos), world, pos, markForPostProcessing) > 0L;
			}
			else if (!directions.isEmpty()) {
				return !state.isAir() && !state.getFluidState().isOf(Fluids.WATER) ? false : SculkVeinBlock.place(
						world,
						pos,
						state,
						directions
	...
	default boolean canSpreadAtSamePosition(WorldAccess world, BlockPos pos, Random random) {
		return false;
	}
	...
	default boolean spread(
		...
		return ((MultifaceGrowthBlock) Blocks.SCULK_VEIN).getGrower().grow(state, world, pos, markForPostProcessing)
				> 0L;
	}
