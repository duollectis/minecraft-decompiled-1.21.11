...
public class RespawnAnchorBlock extends Block {
	...
	protected ActionResult onUseWithItem(
		...
		if (isChargeItem(stack) && canCharge(state)) {
			...
			return (ActionResult) (
					hand == Hand.MAIN_HAND && isChargeItem(player.getStackInHand(Hand.OFF_HAND)) && canCharge(state)
					? ActionResult.PASS
		...
		if (state.get(CHARGES) == 0) {
			...
			if (!isUsable(serverWorld, pos)) {
				...
				if (player instanceof ServerPlayerEntity serverPlayerEntity) {
					...
					ServerPlayerEntity.Respawn respawn2 = new ServerPlayerEntity.Respawn(
							WorldProperties.SpawnPoint.create(serverWorld.getRegistryKey(), pos, 0.0F, 0.0F), false
					);
	...
	private void explode(BlockState state, ServerWorld world, BlockPos explodedPos) {
		...
		boolean
				bl =
				Direction.Type.HORIZONTAL.stream().map(explodedPos::offset).anyMatch(pos -> hasStillWater(pos, world));
	...
	private static Optional<Vec3d> findRespawnPosition(
			EntityType<?> entity,
			CollisionView world,
			BlockPos pos,
