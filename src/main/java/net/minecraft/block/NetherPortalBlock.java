...
public class NetherPortalBlock extends Block implements Portal {
	...
	private static final Map<Direction.Axis, VoxelShape>
			SHAPES_BY_AXIS =
			VoxelShapes.createHorizontalAxisShapeMap(Block.createColumnShape(4.0, 16.0, 0.0, 16.0));
	...
	protected void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
		if (world.shouldSpawnMonsters()
				&& world
				.getEnvironmentAttributes()
				.getAttributeValue(EnvironmentAttributes.NETHER_PORTAL_SPAWNS_PIGLIN_GAMEPLAY, pos)
	...
	public int getPortalDelay(ServerWorld world, Entity entity) {
		return entity instanceof PlayerEntity playerEntity
		       ? Math.max(
				...
				world.getGameRules()
				     .getValue(
						     playerEntity.getAbilities().invulnerable ? GameRules.PLAYERS_NETHER_PORTAL_CREATIVE_DELAY
	...
	private @Nullable TeleportTarget getOrCreateExitPortalTarget(
			ServerWorld world,
			Entity entity,
			BlockPos pos,
			BlockPos scaledPos,
		...
		if (optional.isPresent()) {
			...
			rectangle = BlockLocating.getLargestRectangle(
					...
					Direction.Axis.Y,
					21,
					posx -> world.getBlockState(posx) == blockState
			);
			postDimensionTransition =
					TeleportTarget.SEND_TRAVEL_THROUGH_PORTAL_PACKET.then(entityx -> entityx.addPortalChunkTicketAt(
							blockPos));
		}
		else {
			Direction.Axis axis = entity.getEntityWorld().getBlockState(pos).getOrEmpty(AXIS).orElse(Direction.Axis.X);
	...
	private static TeleportTarget getExitPortalTarget(
		...
		if (blockState.contains(Properties.HORIZONTAL_AXIS)) {
			...
			BlockLocating.Rectangle rectangle = BlockLocating.getLargestRectangle(
					...
					Direction.Axis.Y,
					21,
					posx -> entity.getEntityWorld().getBlockState(posx) == blockState
			);
