...
public class FluidBlock extends Block implements FluidDrainable {
	...
	private static final Codec<FlowableFluid> FLUID_CODEC = Registries.FLUID
			.getCodec()
			.comapFlatMap(
					fluid -> fluid instanceof FlowableFluid flowableFluid ? DataResult.success(flowableFluid)
					                                                      : DataResult.error(() ->
					                                                                         "Not a flowing fluid: "
					                                                                         + fluid),
					fluid -> fluid
	...
	public static final MapCodec<FluidBlock> CODEC = RecordCodecBuilder.mapCodec(
			instance -> instance
					.group(FLUID_CODEC.fieldOf("fluid").forGetter(block -> block.fluid), createSettingsCodec())
					.apply(instance, FluidBlock::new)
	);
