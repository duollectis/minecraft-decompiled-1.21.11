...
/**
 * {@code VaultServerData}.
 */
public class VaultServerData {

	public static final String SERVER_DATA_KEY = "server_data";
	public static Codec<VaultServerData> codec = RecordCodecBuilder.create(
			instance -> instance.group(
					                    Uuids.LINKED_SET_CODEC
							                    .lenientOptionalFieldOf("rewarded_players", Set.of())
							                    .forGetter(data -> data.rewardedPlayers),
					                    Codec.LONG
							                    .lenientOptionalFieldOf("state_updating_resumes_at", 0L)
							                    .forGetter(data -> data.stateUpdatingResumesAt),
					                    ItemStack.CODEC
							                    .listOf()
							                    .lenientOptionalFieldOf("items_to_eject", List.of())
							                    .forGetter(data -> data.itemsToEject),
					                    Codec.INT
							                    .lenientOptionalFieldOf("total_ejections_needed", 0)
	...
	public ItemStack getItemToDisplay() {
		return this.itemsToEject.isEmpty() ? ItemStack.EMPTY : Objects.requireNonNullElse(
				this.itemsToEject.get(
						this.itemsToEject.size() - 1), ItemStack.EMPTY
		);
	}
	...
	public float getEjectSoundPitchModifier() {
		return this.totalEjectionsNeeded == 1
		       ? 1.0F
		       : 1.0F - MathHelper.getLerpProgress(
