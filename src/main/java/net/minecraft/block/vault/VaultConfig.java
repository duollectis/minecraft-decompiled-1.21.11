...
public record VaultConfig(
	...
	public static Codec<VaultConfig> codec = RecordCodecBuilder.<VaultConfig>create(
			                                                           instance -> instance.group(
					                                                                               ...
					                                                                               Codec.DOUBLE
							                                                                               .lenientOptionalFieldOf("activation_range", DEFAULT.activationRange())
							                                                                               .forGetter(VaultConfig::activationRange),
					                                                                               Codec.DOUBLE
							                                                                               .lenientOptionalFieldOf("deactivation_range", DEFAULT.deactivationRange())
	...
	private static DataResult<VaultConfig> validate(VaultConfig config) {
		return config.activationRange > config.deactivationRange
		       ? DataResult.error(
				() -> "Activation range must (" + config.activationRange + ") be less or equal to deactivation range ("
