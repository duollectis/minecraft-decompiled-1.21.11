...
public abstract class LeavesBlock extends Block implements Waterloggable {
	...
	private static final int DECAY_TICK_DELAY = 1;
	...
	protected boolean isSideInvisible(BlockState state, BlockState stateFrom, Direction direction) {
		return !cutoutLeaves && stateFrom.getBlock() instanceof LeavesBlock ? true : super.isSideInvisible(
				state,
				stateFrom,
				direction
		);
	...
	private static void spawnWaterParticle(
			World world,
			BlockPos pos,
			Random random,
			BlockState state,
	...
	public BlockState getPlacementState(ItemPlacementContext ctx) {
		...
		BlockState
				blockState =
				this.getDefaultState().with(PERSISTENT, true).with(WATERLOGGED, fluidState.getFluid() == Fluids.WATER);
