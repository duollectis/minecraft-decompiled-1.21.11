...
public class FlowerPotBlock extends Block {
	...
	public static final MapCodec<FlowerPotBlock> CODEC = RecordCodecBuilder.mapCodec(
			instance -> instance
					.group(
							Registries.BLOCK.getCodec().fieldOf("potted").forGetter(block -> block.content),
							createSettingsCodec()
					)
	...
	protected ActionResult onUseWithItem(
		...
		BlockState
				blockState =
				(stack.getItem() instanceof BlockItem blockItem ? CONTENT_TO_POTTED.getOrDefault(
						blockItem.getBlock(),
						Blocks.AIR
				) : Blocks.AIR
				)
	...
	public BlockState getToggledState(BlockState state) {
		if (state.isOf(Blocks.POTTED_OPEN_EYEBLOSSOM)) {
			...
			return state.isOf(Blocks.POTTED_CLOSED_EYEBLOSSOM) ? Blocks.POTTED_OPEN_EYEBLOSSOM.getDefaultState()
			                                                   : state;
		}
	}
}
