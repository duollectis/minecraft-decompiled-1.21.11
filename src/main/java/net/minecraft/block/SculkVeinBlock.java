...
public class SculkVeinBlock extends MultifaceGrowthBlock implements SculkSpreadable {
	...
	private final MultifaceGrower
			allGrowTypeGrower =
			new MultifaceGrower(new SculkVeinBlock.SculkVeinGrowChecker(MultifaceGrower.GROW_TYPES));
	private final MultifaceGrower
			samePositionOnlyGrower =
			new MultifaceGrower(new SculkVeinBlock.SculkVeinGrowChecker(MultifaceGrower.GrowType.SAME_POSITION));
	...
	public int spread(
		...
		if (shouldConvertToBlock && this.convertToBlock(spreadManager, world, cursor.getPos(), random)) {
			...
			return random.nextInt(spreadManager.getSpreadChance()) == 0 ? MathHelper.floor(cursor.getCharge() * 0.5F)
			                                                            : cursor.getCharge();
		}
	}
	...
	public static boolean veinCoversSculkReplaceable(WorldAccess world, BlockState state, BlockPos pos) {
		if (!state.isOf(Blocks.SCULK_VEIN)) {
			...
			for (Direction direction : DIRECTIONS) {
				if (hasDirection(state, direction) && world
						.getBlockState(pos.offset(direction))
						.isIn(BlockTags.SCULK_REPLACEABLE)) {
					return true;
				}
			}
	...
	class SculkVeinGrowChecker extends MultifaceGrower.LichenGrowChecker {
		...
		public boolean canGrow(BlockView world, BlockPos pos, BlockPos growPos, Direction direction, BlockState state) {
			...
			if (!blockState.isOf(Blocks.SCULK) && !blockState.isOf(Blocks.SCULK_CATALYST)
				...
				if (!fluidState.isEmpty() && !fluidState.isOf(Fluids.WATER)) {
					...
					return state.isIn(BlockTags.FIRE) ? false : state.isReplaceable() || super.canGrow(
							world,
							pos,
							growPos,
							direction,
