...
public class InfestedBlock extends Block {
	...
	protected void onStacksDropped(
		...
		if (world.getGameRules().getValue(GameRules.DO_TILE_DROPS) && !EnchantmentHelper.hasAnyEnchantmentsIn(
				tool,
	...
	private static BlockState copyProperties(
			Map<BlockState, BlockState> stateMap,
			BlockState fromState,
			Supplier<BlockState> toStateSupplier
	) {
		return stateMap.computeIfAbsent(
				fromState, infestedState -> {
					BlockState blockState = toStateSupplier.get();

					for (Property property : infestedState.getProperties()) {
						blockState =
								blockState.contains(property) ? blockState.with(property, infestedState.get(property))
								                              : blockState;
					}
