...
public class WallHangingSignBlock extends AbstractSignBlock {
	...
	private static final Map<Direction.Axis, VoxelShape>
			COLLISION_SHAPES_BY_AXIS =
			VoxelShapes.createHorizontalAxisShapeMap(
					Block.createColumnShape(16.0, 4.0, 14.0, 16.0)
			);
	private static final Map<Direction.Axis, VoxelShape>
			OUTLINE_SHAPES_BY_AXIS =
			VoxelShapes.createHorizontalAxisShapeMap(
					VoxelShapes.union(
							COLLISION_SHAPES_BY_AXIS.get(Direction.Axis.Z),
	...
	public boolean canAttachAt(BlockState state, WorldView world, BlockPos pos) {
		...
		return this.canAttachTo(world, state, pos.offset(direction), direction2) || this.canAttachTo(
				world,
				state,
