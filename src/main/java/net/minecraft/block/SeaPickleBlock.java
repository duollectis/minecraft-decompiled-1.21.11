...
public class SeaPickleBlock extends PlantBlock implements Fertilizable, Waterloggable {
	...
	protected boolean canReplace(BlockState state, ItemPlacementContext context) {
		return !context.shouldCancelInteraction() && context.getStack().isOf(this.asItem()) && state.get(PICKLES) < 4
		...
		for (int o = 0; o < 5; o++) {
			for (int p = 0; p < j; p++) {
				...
				for (int r = q - 2; r < q; r++) {
					...
					if (!blockPos.equals(pos) && random.nextInt(6) == 0 && world
						...
						if (blockState.isIn(BlockTags.CORAL_BLOCKS)) {
							world.setBlockState(
									blockPos,
									Blocks.SEA_PICKLE.getDefaultState().with(PICKLES, random.nextInt(4) + 1),
									3
							);
						}
					}
