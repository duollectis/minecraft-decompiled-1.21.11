...
public class SculkSensorBlock extends BlockWithEntity implements Waterloggable {
	...
	public void onSteppedOn(World world, BlockPos pos, BlockState state, Entity entity) {
		if (!world.isClient()
				...
				.accepts(serverWorld, pos, GameEvent.STEP, GameEvent.Emitter.of(state))) {
			sculkSensorBlockEntity
					.getEventListener()
					.forceListen(serverWorld, GameEvent.STEP, GameEvent.Emitter.of(entity), entity.getEntityPos());
		}
	...
	public static void tryResonate(@Nullable Entity sourceEntity, World world, BlockPos pos, int frequency) {
		for (Direction direction : Direction.values()) {
			...
			if (blockState.isIn(BlockTags.VIBRATION_RESONATORS)) {
				world.emitGameEvent(
						Vibrations.getResonation(frequency),
						blockPos,
						GameEvent.Emitter.of(sourceEntity, blockState)
				);
	...
	public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
		if (getPhase(state) == SculkSensorPhase.ACTIVE) {
			...
			if (direction != Direction.UP && direction != Direction.DOWN) {
				double
						d =
						pos.getX() + 0.5 + (direction.getOffsetX() == 0 ? 0.5 - random.nextDouble()
						                                                : direction.getOffsetX() * 0.6
						);
				double e = pos.getY() + 0.25;
				double
						f =
						pos.getZ() + 0.5 + (direction.getOffsetZ() == 0 ? 0.5 - random.nextDouble()
