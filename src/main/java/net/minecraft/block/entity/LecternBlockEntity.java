...
public class LecternBlockEntity extends BlockEntity implements Clearable, NamedScreenHandlerFactory {
   ...
   private ServerCommandSource getCommandSource(@Nullable PlayerEntity player, ServerWorld world) {
      ...
      return new ServerCommandSource(
         CommandOutput.DUMMY, vec3d, Vec2f.ZERO, world, LeveledPermissionPredicate.GAMEMASTERS, string, text, world.getServer(), player
      );
   }
   ...
   protected void readData(ReadView view) {
      super.readData(view);
      this.book = view.<ItemStack>read("Book", ItemStack.CODEC).map(itemStack -> this.resolveBook(itemStack, null)).orElse(ItemStack.EMPTY);
      this.pageCount = getPageCount(this.book);
