...
/**
 * {@code SculkShriekerWarningManager}.
 */
public class SculkShriekerWarningManager {
	...
	public static OptionalInt warnNearbyPlayers(ServerWorld world, BlockPos pos, ServerPlayerEntity player) {
		if (isWardenNearby(world, pos)) {
			...
			if (list
					.stream()
					.anyMatch(nearbyPlayer -> nearbyPlayer
				...
				Optional<SculkShriekerWarningManager> optional = list.stream()
				                                                     .flatMap(playerx -> playerx
						                                                     .getSculkShriekerWarningManager()
						                                                     .stream())
				                                                     .max(Comparator.comparingInt(
						                                                     SculkShriekerWarningManager::getWarningLevel));
				if (optional.isPresent()) {
					...
					list.forEach(
							nearbyPlayer -> nearbyPlayer
									.getSculkShriekerWarningManager()
									.ifPresent(warningManager -> warningManager.copy(sculkShriekerWarningManager))
					);
	...
	private static List<ServerPlayerEntity> getPlayersInRange(ServerWorld world, BlockPos pos) {
		...
		return world.getPlayers(player -> !player.isSpectator() && player.getEntityPos().isInRange(vec3d, 16.0)
				&& player.isAlive());
	}
