...
public class CreakingHeartBlockEntity extends BlockEntity {
         ...
         if (!world.getBlockState(pos).isIn(BlockTags.PALE_OAK_LOGS)) {
            ...
            for (Direction direction : Util.copyShuffled(Direction.values(), world.random)) {
               ...
               if (blockState.isOf(Blocks.RESIN_CLUMP) && !MultifaceBlock.hasDirection(blockState, direction2)) {
                  world.setBlockState(blockPos, blockState.with(MultifaceBlock.getProperty(direction2), true), 3);
   ...
   protected void readData(ReadView view) {
      super.readData(view);
      view.<UUID>read("creaking", Uuids.INT_STREAM_CODEC).ifPresentOrElse(this::setCreakingPuppetFromUuid, this::clearCreakingPuppet);
   }
