...
/**
 * Блок стеклянной панели (и железных прутьев). Соединяется с соседними блоками
 * по горизонтали, если те являются сплошными или тоже панелями/стенами.
 */
public class PaneBlock extends HorizontalConnectingBlock {
	...
	public BlockState getPlacementState(ItemPlacementContext ctx) {
		...
		return this.getDefaultState()
		           ...
		           .with(
				           ...
				           this.connectsTo(
						           blockState3,
						           blockState3.isSideSolidFullSquare(blockView, blockPos4, Direction.EAST)
				           )
		           )
		           .with(
				           EAST,
				           this.connectsTo(
						           blockState4,
						           blockState4.isSideSolidFullSquare(blockView, blockPos5, Direction.WEST)
				           )
		           )
	...
	protected boolean isSideInvisible(BlockState state, BlockState stateFrom, Direction direction) {
		if (stateFrom.isOf(this)
				|| stateFrom.isIn(BlockTags.BARS) && state.isIn(BlockTags.BARS)
				&& stateFrom.contains(FACING_PROPERTIES.get(direction.getOpposite()))) {
