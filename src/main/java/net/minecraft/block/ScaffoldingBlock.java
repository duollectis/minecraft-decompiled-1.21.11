...
public class ScaffoldingBlock extends Block implements Waterloggable {
	...
	private static final VoxelShape NORMAL_OUTLINE_SHAPE = VoxelShapes.union(
			...
			VoxelShapes.createHorizontalFacingShapeMap(Block.createCuboidShape(0.0, 0.0, 0.0, 2.0, 16.0, 2.0))
			           .values()
			           .stream()
	...
	private static final VoxelShape BOTTOM_OUTLINE_SHAPE = VoxelShapes.union(
			...
			COLLISION_SHAPE,
			VoxelShapes.createHorizontalFacingShapeMap(Block.createCuboidZShape(16.0, 0.0, 2.0, 0.0, 2.0))
			           .values()
			           .stream()
			           .reduce(VoxelShapes.empty(), VoxelShapes::union)
	);
	...
	protected VoxelShape getCollisionShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
		if (context.isPlacement()) {
			...
			return state.get(DISTANCE) != 0 && state.get(BOTTOM) && context.isAbove(OUTLINE_SHAPE, pos, true)
			       ? COLLISION_SHAPE : VoxelShapes.empty();
		}
	}
