...
public class ShulkerBoxBlock extends BlockWithEntity {
			...
			instance -> instance
					.group(
							DyeColor.CODEC
									...
									.forGetter(block -> Optional.ofNullable(block.color)),
							createSettingsCodec()
					)
	...
	public static final Map<Direction, VoxelShape>
			SHAPES_BY_DIRECTION =
			VoxelShapes.createFacingShapeMap(Block.createCuboidZShape(16.0, 0.0, 1.0));
	...
	private static boolean canOpen(BlockState state, World world, BlockPos pos, ShulkerBoxBlockEntity entity) {
		if (entity.getAnimationStage() != ShulkerBoxBlockEntity.AnimationStage.CLOSED) {
			...
			Box
					box =
					ShulkerEntity
							.calculateBoundingBox(1.0F, state.get(FACING), 0.0F, 0.5F, pos.toBottomCenterPos())
							.contract(1.0E-6);
	...
	protected List<ItemStack> getDroppedStacks(BlockState state, LootWorldContext.Builder builder) {
		...
		if (blockEntity instanceof ShulkerBoxBlockEntity shulkerBoxBlockEntity) {
			builder = builder.addDynamicDrop(
					CONTENTS_DYNAMIC_DROP_ID, consumer -> {
	...
	protected VoxelShape getSidesShape(BlockState state, BlockView world, BlockPos pos) {
		return world.getBlockEntity(pos) instanceof ShulkerBoxBlockEntity shulkerBoxBlockEntity
		       ...
		       ? SHAPES_BY_DIRECTION.get(state.get(FACING).getOpposite())
