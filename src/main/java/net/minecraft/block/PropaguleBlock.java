...
public class PropaguleBlock extends SaplingBlock implements Waterloggable {
	...
	public static final MapCodec<PropaguleBlock> CODEC = RecordCodecBuilder.mapCodec(
			instance -> instance
					.group(
							SaplingGenerator.CODEC.fieldOf("tree").forGetter(block -> block.generator),
							createSettingsCodec()
					)
	...
	private static final VoxelShape[]
			SHAPES_BY_AGE =
			Block.createShapeArray(4, age -> Block.createColumnShape(2.0, MIN_Y_BY_AGE[age], 16.0));
	...
	protected VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
		int i = state.get(HANGING) ? state.get(AGE) : 4;
		return SHAPES_BY_AGE[i].offset(state.getModelOffset(pos));
	}

	@Override
	protected boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
		return isHanging(state) ? world.getBlockState(pos.up()).isOf(Blocks.MANGROVE_LEAVES)
