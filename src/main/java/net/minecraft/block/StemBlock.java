...
public class StemBlock extends PlantBlock implements Fertilizable {
	...
	public static final MapCodec<StemBlock> CODEC = RecordCodecBuilder.mapCodec(
			instance -> instance.group(
					                    ...
					                    RegistryKey
							                    .createCodec(RegistryKeys.BLOCK)
							                    .fieldOf("attached_stem")
							                    .forGetter(block -> block.attachedStemBlock),
	...
	private static final VoxelShape[]
			SHAPES_BY_AGE =
			Block.createShapeArray(7, age -> Block.createColumnShape(2.0, 0.0, 2 + age * 2));
	...
	public StemBlock(
			...
			RegistryKey<Block> attachedStemBlock,
		...
		if (world.getBaseLightLevel(pos, 0) >= 9) {
			...
			if (random.nextInt((int) (25.0F / f) + 1) == 0) {
				...
				if (i < 7) {
					...
					if (world.getBlockState(blockPos).isAir() && (blockState.isOf(Blocks.FARMLAND) || blockState.isIn(
							BlockTags.DIRT)
					)) {
						...
						if (optional.isPresent() && optional2.isPresent()) {
							...
							world.setBlockState(
									pos,
									optional2.get().getDefaultState().with(HorizontalFacingBlock.FACING, direction)
							);
						}
					}
				}
