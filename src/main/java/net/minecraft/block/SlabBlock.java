...
public class SlabBlock extends Block implements Waterloggable {
	...
	public @Nullable BlockState getPlacementState(ItemPlacementContext ctx) {
		...
		if (blockState.isOf(this)) {
			...
			return direction != Direction.DOWN && (direction == Direction.UP || !(ctx.getHitPos().y - blockPos.getY()
					> 0.5
			)
			)
			       ? blockState2
	...
	public boolean canFillWithFluid(
		...
		return state.get(TYPE) != SlabType.DOUBLE ? Waterloggable.super.canFillWithFluid(
				filler,
				world,
				pos,
				state,
