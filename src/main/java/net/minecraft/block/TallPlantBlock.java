...
public class TallPlantBlock extends PlantBlock {
	...
	public @Nullable BlockState getPlacementState(ItemPlacementContext ctx) {
		...
		return blockPos.getY() < world.getTopYInclusive() && world.getBlockState(blockPos.up()).canReplace(ctx)
	...
	public void onPlaced(
		...
		world.setBlockState(
				blockPos,
				withWaterloggedState(world, blockPos, this.getDefaultState().with(HALF, DoubleBlockHalf.UPPER)),
				3
		);
	}
	...
	public static void placeAt(WorldAccess world, BlockState state, BlockPos pos, @Block.SetBlockStateFlag int flags) {
		...
		world.setBlockState(
				blockPos,
				withWaterloggedState(world, blockPos, state.with(HALF, DoubleBlockHalf.UPPER)),
				flags
		);
	}
	...
	protected static void onBreakInCreative(World world, BlockPos pos, BlockState state, PlayerEntity player) {
		...
		if (doubleBlockHalf == DoubleBlockHalf.UPPER) {
			...
			if (blockState.isOf(state.getBlock()) && blockState.get(HALF) == DoubleBlockHalf.LOWER) {
				BlockState
						blockState2 =
						blockState.getFluidState().isOf(Fluids.WATER) ? Blocks.WATER.getDefaultState()
						                                              : Blocks.AIR.getDefaultState();
