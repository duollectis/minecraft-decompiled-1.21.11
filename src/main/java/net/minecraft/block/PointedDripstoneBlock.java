...
public class PointedDripstoneBlock extends Block implements Falling, Waterloggable {
	...
	private static final int TIP_SEARCH_RANGE = 11;
	private static final int STALACTITE_DRIP_SEARCH_RANGE = 2;
	private static final float MIN_DRIP_FLUID_LEVEL = 0.02F;
	private static final float MAX_DRIP_FLUID_LEVEL = 0.12F;
	private static final int SUPPORT_SEARCH_RANGE = 11;
	private static final float WATER_DRIP_CHANCE = 0.17578125F;
	private static final float LAVA_DRIP_CHANCE = 0.05859375F;
	private static final double MIN_PROJECTILE_SPEED = 0.6;
	private static final float FALLING_STALACTITE_DAMAGE_PER_BLOCK = 1.0F;
	private static final int FALLING_STALACTITE_MAX_DAMAGE = 40;
	private static final int FALLING_STALACTITE_MIN_HEIGHT = 6;
	private static final float STALAGMITE_FALL_DAMAGE_BONUS = 2.5F;
	private static final int STALAGMITE_FALL_DAMAGE_MULTIPLIER = 2;
	private static final float MAX_STALACTITE_GROWTH_SPEED = 5.0F;
	private static final float STALACTITE_GROW_CHANCE = 0.011377778F;
	...
	private static void createParticle(World world, BlockPos pos, BlockState state, Fluid fluid, BlockPos fluidPos) {
		...
		ParticleEffect particleEffect = getParticleEffect(world, fluid, fluidPos);
		world.addParticleClient(particleEffect, e, f, g, 0.0, 0.0, 0.0);
	}

	private static @Nullable BlockPos getTipPos(
			BlockState state,
			WorldAccess world,
			BlockPos pos,
			int range,
		...
		if (isTip(state, allowMerged)) {
			...
			BiPredicate<BlockPos, BlockState>
					biPredicate =
					(posx, statex) -> statex.isOf(Blocks.POINTED_DRIPSTONE)
							&& statex.get(VERTICAL_DIRECTION) == direction;
			return searchInDirection(
					world,
					pos,
					direction.getDirection(),
					biPredicate,
					statex -> isTip(statex, allowMerged),
					range
			).orElse(null);
		}
	}
	...
	private static Optional<BlockPos> getSupportingPos(World world, BlockPos pos, BlockState state, int range) {
		...
		BiPredicate<BlockPos, BlockState>
				biPredicate =
				(posx, statex) -> statex.isOf(Blocks.POINTED_DRIPSTONE) && statex.get(VERTICAL_DIRECTION) == direction;
		return searchInDirection(
				world,
				pos,
				direction.getOpposite().getDirection(),
				biPredicate,
	...
	private static boolean canPlaceAtWithDirection(WorldView world, BlockPos pos, Direction direction) {
		...
		BlockState blockState = world.getBlockState(blockPos);
		return blockState.isSideSolidFullSquare(world, blockPos, direction) || isPointedDripstoneFacingDirection(
				blockState,
				direction
		);
	}
	...
	public static Fluid getDripFluid(ServerWorld world, BlockPos pos) {
		return getFluid(world, pos, world.getBlockState(pos))
	...
	private static Optional<PointedDripstoneBlock.DrippingFluid> getFluid(World world, BlockPos pos, BlockState state) {
		return !isPointingDown(state) ? Optional.empty() : getSupportingPos(world, pos, state, 11).map(posx -> {
			...
			BlockState blockState = world.getBlockState(blockPos);
			Fluid fluid;
			if (blockState.isOf(Blocks.MUD) && !world
			                                    .getEnvironmentAttributes()
			                                    .getAttributeValue(
					                                    EnvironmentAttributes.WATER_EVAPORATES_GAMEPLAY,
					                                    blockPos
			                                    )) {
				fluid = Fluids.WATER;
			}
	...
	private static boolean canGrow(BlockState dripstoneBlockState, BlockState waterState) {
		return dripstoneBlockState.isOf(Blocks.DRIPSTONE_BLOCK) && waterState.isOf(Blocks.WATER) && waterState
				.getFluidState()
				.isStill();
	}
	...
	private static ParticleEffect getParticleEffect(World world, Fluid fluid, BlockPos pos) {
		if (fluid.matchesType(Fluids.EMPTY)) {
			return world
					.getEnvironmentAttributes()
					.getAttributeValue(EnvironmentAttributes.DEFAULT_DRIPSTONE_PARTICLE_VISUAL, pos);
		}
		else {
			return fluid.isIn(FluidTags.LAVA) ? ParticleTypes.DRIPPING_DRIPSTONE_LAVA
