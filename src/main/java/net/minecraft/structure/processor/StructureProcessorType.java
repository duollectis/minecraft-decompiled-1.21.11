...
public interface StructureProcessorType<P extends StructureProcessor> {
	...
	Codec<StructureProcessorList>
			LIST_CODEC =
			CODEC.listOf().xmap(StructureProcessorList::new, StructureProcessorList::getList);

	Codec<StructureProcessorList>
			PROCESSORS_CODEC =
			Codec.withAlternative(LIST_CODEC.fieldOf("processors").codec(), LIST_CODEC);

	Codec<RegistryEntry<StructureProcessorList>>
			REGISTRY_CODEC =
			RegistryElementCodec.of(RegistryKeys.PROCESSOR_LIST, PROCESSORS_CODEC);

	StructureProcessorType<BlockIgnoreStructureProcessor>
			BLOCK_IGNORE =
			register("block_ignore", BlockIgnoreStructureProcessor.CODEC);

	StructureProcessorType<BlockRotStructureProcessor>
			BLOCK_ROT =
			register("block_rot", BlockRotStructureProcessor.CODEC);

	StructureProcessorType<GravityStructureProcessor> GRAVITY = register("gravity", GravityStructureProcessor.CODEC);

	StructureProcessorType<JigsawReplacementStructureProcessor>
			JIGSAW_REPLACEMENT =
			register("jigsaw_replacement", JigsawReplacementStructureProcessor.CODEC);
	...
	StructureProcessorType<BlockAgeStructureProcessor>
			BLOCK_AGE =
			register("block_age", BlockAgeStructureProcessor.CODEC);
	...
	StructureProcessorType<ProtectedBlocksStructureProcessor>
			PROTECTED_BLOCKS =
			register("protected_blocks", ProtectedBlocksStructureProcessor.CODEC);
