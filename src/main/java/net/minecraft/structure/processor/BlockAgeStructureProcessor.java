...
public class BlockAgeStructureProcessor extends StructureProcessor {
   ...
   private @Nullable BlockState processBlocks(Random random) {
      if (random.nextFloat() >= 0.5F) {
         ...
         BlockState[] blockStates2 = new BlockState[]{
            Blocks.MOSSY_STONE_BRICKS.getDefaultState(), randomStairProperties(random, Blocks.MOSSY_STONE_BRICK_STAIRS)
         };
   ...
   private @Nullable BlockState processStairs(BlockState blockState, Random random) {
      if (random.nextFloat() >= 0.5F) {
         ...
         BlockState[] blockStates = new BlockState[]{
            Blocks.MOSSY_STONE_BRICK_STAIRS.getStateWithProperties(blockState), Blocks.MOSSY_STONE_BRICK_SLAB.getDefaultState()
         };
   ...
   private @Nullable BlockState processSlabs(BlockState blockState, Random random) {
      return random.nextFloat() < this.mossiness ? Blocks.MOSSY_STONE_BRICK_SLAB.getStateWithProperties(blockState) : null;
   }

   private @Nullable BlockState processWalls(BlockState blockState, Random random) {
      return random.nextFloat() < this.mossiness ? Blocks.MOSSY_STONE_BRICK_WALL.getStateWithProperties(blockState) : null;
   }
