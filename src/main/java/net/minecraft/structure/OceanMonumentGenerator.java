...
public class OceanMonumentGenerator {
	...
	public static class Base extends OceanMonumentGenerator.Piece {
		...
		private static final int MONUMENT_SIZE = 61;
		...
		private List<OceanMonumentGenerator.PieceSetting> setupPieces(Random random) {
			...
			this.coreRoomPieceSetting.neighbors[Direction.EAST.getIndex()].neighbors[Direction.NORTH.getIndex()].used =
					true;
			this.coreRoomPieceSetting.neighbors[Direction.UP.getIndex()].used = true;
			this.coreRoomPieceSetting.neighbors[Direction.EAST.getIndex()].neighbors[Direction.UP.getIndex()].used =
					true;
			this.coreRoomPieceSetting.neighbors[Direction.NORTH.getIndex()].neighbors[Direction.UP.getIndex()].used =
					true;
			this.coreRoomPieceSetting.neighbors[Direction.EAST.getIndex()].neighbors[Direction.NORTH.getIndex()].neighbors[Direction.UP.getIndex()].used =
					true;
			...
			while (var34.hasNext()) {
				...
				while (r < 2 && m < 5) {
					...
					if (pieceSetting5.neighborPresences[n]) {
						int o = Direction.byIndex(n).getOpposite().getIndex();
						pieceSetting5.neighborPresences[n] = false;
						pieceSetting5.neighbors[n].neighborPresences[o] = false;
						if (pieceSetting5.hasEntranceConnection(q++)
								&& pieceSetting5.neighbors[n].hasEntranceConnection(q++)) {
							r++;
						}
						else {
		...
		public void generate(
			...
			if (this.setting.roomIndex / LEVEL_TWO_INDEX_BOUND > 0) {
				this.generateVerticalConnection(
						...
						8,
						0,
						pieceSetting.neighborPresences[Direction.DOWN.getIndex()]
				);
				this.generateVerticalConnection(
						world,
						chunkBox,
						0,
						0,
						pieceSetting2.neighborPresences[Direction.DOWN.getIndex()]
				);
			}
	...
	static class DoubleXRoomFactory implements OceanMonumentGenerator.PieceFactory {
		...
		public boolean canGenerate(OceanMonumentGenerator.PieceSetting setting) {
			return setting.neighborPresences[Direction.EAST.getIndex()]
					&& !setting.neighbors[Direction.EAST.getIndex()].used;
		}

		@Override
		public OceanMonumentGenerator.Piece generate(
				Direction direction,
				OceanMonumentGenerator.PieceSetting setting,
				Random random
		) {
			setting.used = true;
	...
	static class DoubleXYRoomFactory implements OceanMonumentGenerator.PieceFactory {
		...
		@Override
		public boolean canGenerate(OceanMonumentGenerator.PieceSetting setting) {
			if (setting.neighborPresences[Direction.EAST.getIndex()]
					&& !setting.neighbors[Direction.EAST.getIndex()].used
					&& setting.neighborPresences[Direction.UP.getIndex()]
					&& !setting.neighbors[Direction.UP.getIndex()].used) {
				OceanMonumentGenerator.PieceSetting pieceSetting = setting.neighbors[Direction.EAST.getIndex()];
				return pieceSetting.neighborPresences[Direction.UP.getIndex()]
						&& !pieceSetting.neighbors[Direction.UP.getIndex()].used;
			}
			else {
				return false;
			}
		...
		public void generate(
			...
			if (this.setting.roomIndex / LEVEL_TWO_INDEX_BOUND > 0) {
				this.generateVerticalConnection(
						...
						0,
						0,
						this.setting.neighborPresences[Direction.DOWN.getIndex()]
				);
			}
	...
	static class DoubleYRoomFactory implements OceanMonumentGenerator.PieceFactory {
		...
		@Override
		public boolean canGenerate(OceanMonumentGenerator.PieceSetting setting) {
			return setting.neighborPresences[Direction.UP.getIndex()]
					&& !setting.neighbors[Direction.UP.getIndex()].used;
		}
	...
	static class DoubleYZRoomFactory implements OceanMonumentGenerator.PieceFactory {
		...
		@Override
		public boolean canGenerate(OceanMonumentGenerator.PieceSetting setting) {
			if (setting.neighborPresences[Direction.NORTH.getIndex()]
					&& !setting.neighbors[Direction.NORTH.getIndex()].used
					&& setting.neighborPresences[Direction.UP.getIndex()]
					&& !setting.neighbors[Direction.UP.getIndex()].used) {
	...
	static class DoubleZRoomFactory implements OceanMonumentGenerator.PieceFactory {
		...
		@Override
		public boolean canGenerate(OceanMonumentGenerator.PieceSetting setting) {
			return setting.neighborPresences[Direction.NORTH.getIndex()]
					&& !setting.neighbors[Direction.NORTH.getIndex()].used;
		}
	...
	protected abstract static class Piece extends StructurePiece {
		...
		protected static final boolean DEFAULT_FILL_AIR = true;
		...
		private static BlockBox createBox(
				Direction orientation,
				OceanMonumentGenerator.PieceSetting setting,
				int x,
				int y,
		...
		protected void generateVerticalConnection(
				StructureWorldAccess world,
				BlockBox box,
				int x,
				int z,
		...
		protected void spawnElderGuardian(StructureWorldAccess world, BlockBox box, int x, int y, int z) {
			...
			if (box.contains(blockPos)) {
				ElderGuardianEntity
						elderGuardianEntity =
						EntityType.ELDER_GUARDIAN.create(world.toServerWorld(), SpawnReason.STRUCTURE);
				if (elderGuardianEntity != null) {
					elderGuardianEntity.heal(elderGuardianEntity.getMaxHealth());
					elderGuardianEntity.refreshPositionAndAngles(
							blockPos.getX() + 0.5,
							blockPos.getY(),
							blockPos.getZ() + 0.5,
							0.0F,
					...
					elderGuardianEntity.initialize(
							world,
							world.getLocalDifficulty(elderGuardianEntity.getBlockPos()),
	...
	interface PieceFactory {
		...
		OceanMonumentGenerator.Piece generate(
				Direction direction,
				OceanMonumentGenerator.PieceSetting setting,
				Random random
		);
	...
	public static class SimpleRoom extends OceanMonumentGenerator.Piece {
		...
		public void generate(
			...
			boolean bl = this.roomType != 0
					&& random.nextBoolean()
					&& !this.setting.neighborPresences[Direction.DOWN.getIndex()]
					&& !this.setting.neighborPresences[Direction.UP.getIndex()]
	...
	static class SimpleRoomTopFactory implements OceanMonumentGenerator.PieceFactory {
		...
		@Override
		public boolean canGenerate(OceanMonumentGenerator.PieceSetting setting) {
			return !setting.neighborPresences[Direction.WEST.getIndex()]
					&& !setting.neighborPresences[Direction.EAST.getIndex()]
					&& !setting.neighborPresences[Direction.NORTH.getIndex()]
					&& !setting.neighborPresences[Direction.SOUTH.getIndex()]
					&& !setting.neighborPresences[Direction.UP.getIndex()];
		}
