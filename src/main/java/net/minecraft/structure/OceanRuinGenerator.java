...
public class OceanRuinGenerator {
	...
	private static StructureProcessor createArchaeologyStructureProcessor(
			Block baseBlock,
	...
	private static void addCluster(
		...
		BlockPos
				blockPos2 =
				StructureTemplate
						.transformAround(new BlockPos(15, 0, 15), BlockMirror.NONE, rotation, BlockPos.ORIGIN)
						.add(blockPos);
	...
	private static void addPieces(
		...
		switch (structure.biomeTemperature) {
			...
			default:
				...
				holder.addPiece(new OceanRuinGenerator.Piece(
						manager,
						identifier,
						pos,
						rotation,
						integrity,
						structure.biomeTemperature,
						large
				));
				break;
			case COLD:
	...
	public static class Piece extends SimpleStructurePiece {
		...
		private Piece(
			...
			super(
					...
					nbt,
					holder,
					identifier -> createPlacementData(rotation, integrity, biomeType)
			);
			this.integrity = integrity;
			this.biomeType = biomeType;
			this.large = large;
		...
		private static StructurePlacementData createPlacementData(
				BlockRotation rotation,
				float integrity,
				OceanRuinStructure.BiomeTemperature temperature
		) {
		...
		public static OceanRuinGenerator.Piece fromNbt(
				StructureTemplateManager structureTemplateManager,
				NbtCompound nbt
		) {
			BlockRotation blockRotation = nbt.<BlockRotation>get("Rot", BlockRotation.ENUM_NAME_CODEC).orElseThrow();
			float f = nbt.getFloat("Integrity", 0.0F);
			OceanRuinStructure.BiomeTemperature biomeTemperature = nbt.<OceanRuinStructure.BiomeTemperature>get(
					                                                          "BiomeType", OceanRuinStructure.BiomeTemperature.ENUM_NAME_CODEC
			                                                          )
			                                                          .orElseThrow();
		...
		protected void handleMetadata(
			...
			if ("chest".equals(metadata)) {
				world.setBlockState(
						...
						Blocks.CHEST
								.getDefaultState()
								.with(ChestBlock.WATERLOGGED, world.getFluidState(pos).isIn(FluidTags.WATER)),
						2
				);
				...
				if (blockEntity instanceof ChestBlockEntity) {
					((ChestBlockEntity) blockEntity)
							.setLootTable(
									this.large ? LootTables.UNDERWATER_RUIN_BIG_CHEST
									           : LootTables.UNDERWATER_RUIN_SMALL_CHEST, random.nextLong()
							);
				}
			}
		...
		private int getGenerationY(BlockPos start, BlockView world, BlockPos end) {
			...
			for (BlockPos blockPos : BlockPos.iterate(start, end)) {
				...
				for (FluidState fluidState = world.getFluidState(mutable);
				     (blockState.isAir() || fluidState.isIn(FluidTags.WATER) || blockState.isIn(BlockTags.ICE))
						     && o > world.getBottomY() + 1;
