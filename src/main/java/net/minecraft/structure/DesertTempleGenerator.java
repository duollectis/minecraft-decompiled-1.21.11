...
public class DesertTempleGenerator extends ShiftableStructurePiece {
	...
	public void generate(
		...
		if (this.adjustToMinHeight(world, -random.nextInt(3))) {
			...
			BlockState
					blockState2 =
					Blocks.SANDSTONE_STAIRS.getDefaultState().with(StairsBlock.FACING, Direction.SOUTH);
	...
	private void generateBasementRoof(
			StructureWorldAccess world,
			BlockBox chunkBox,
			int startX,
			int y,
		...
		this.basementMarkerPos =
				new BlockPos(this.applyXTransform(j, k), this.applyYTransform(y), this.applyZTransform(j, k));
	}
