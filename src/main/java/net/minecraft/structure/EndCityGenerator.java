...
public class EndCityGenerator {
         ...
         if (depth > 8) {
            ...
            if (i == 0) {
               piece = EndCityGenerator.addPiece(
                  pieces, EndCityGenerator.createPiece(manager, piece, new BlockPos(-1, 4, -1), "base_roof", blockRotation, true)
               );
            } else if (i == 1) {
               piece = EndCityGenerator.addPiece(
               ...
               piece = EndCityGenerator.addPiece(
                  pieces, EndCityGenerator.createPiece(manager, piece, new BlockPos(-1, 8, -1), "third_roof", blockRotation, true)
               );
               EndCityGenerator.createPart(manager, EndCityGenerator.SMALL_TOWER, depth + 1, piece, null, pieces, random);
            }
   ...
   static final EndCityGenerator.Part SMALL_TOWER = new EndCityGenerator.Part() {
      ...
      public boolean create(StructureTemplateManager manager, int depth, EndCityGenerator.Piece root, BlockPos pos, List<StructurePiece> pieces, Random random) {
         ...
         EndCityGenerator.Piece piece = EndCityGenerator.addPiece(
            pieces,
            EndCityGenerator.createPiece(manager, root, new BlockPos(3 + random.nextInt(2), -3, 3 + random.nextInt(2)), "tower_base", blockRotation, true)
         );
   ...
   public static class Piece extends SimpleStructurePiece {
      ...
      public Piece(StructureTemplateManager manager, NbtCompound nbt) {
         super(
            ...
            nbt,
            manager,
            id -> createPlacementData(nbt.getBoolean("OW", false), nbt.<BlockRotation>get("Rot", BlockRotation.ENUM_NAME_CODEC).orElseThrow())
         );
      }
