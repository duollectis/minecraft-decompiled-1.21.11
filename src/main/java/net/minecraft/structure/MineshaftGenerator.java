...
/**
 * {@code MineshaftGenerator}.
 */
public class MineshaftGenerator {

	private static final int MIN_CORRIDOR_LENGTH = 3;
	private static final int MIN_CORRIDOR_WIDTH = 3;
	private static final int MAX_CORRIDOR_LENGTH = 5;
	private static final int MAX_CHAIN_DEPTH = 20;
	private static final int MAX_PIECE_COUNT = 50;
	private static final int MAX_CORRIDOR_WIDTH = 8;
	public static final int MAX_SHAFT_LENGTH = 50;

	private static MineshaftGenerator.@Nullable MineshaftPart pickPiece(
		...
		if (i >= 80) {
			BlockBox
					blockBox =
					MineshaftGenerator.MineshaftCrossing.getBoundingBox(holder, random, x, y, z, orientation);
			if (blockBox != null) {
			...
			BlockBox
					blockBox =
					MineshaftGenerator.MineshaftCorridor.getBoundingBox(holder, random, x, y, z, orientation);
			if (blockBox != null) {
	...
	static MineshaftGenerator.@Nullable MineshaftPart pieceGenerator(
		...
		if (chainLength > 8) {
		...
		else if (Math.abs(x - start.getBoundingBox().getMinX()) <= 80
			...
			MineshaftGenerator.MineshaftPart
					mineshaftPart =
					pickPiece(holder, random, x, y, z, orientation, chainLength + 1, type);
	...
	public static class MineshaftCorridor extends MineshaftGenerator.MineshaftPart {
		...
		public static @Nullable BlockBox getBoundingBox(
				StructurePiecesHolder holder,
				Random random,
				int x,
				int y,
		...
		protected boolean addChest(
			...
			if (boundingBox.contains(blockPos) && world.getBlockState(blockPos).isAir() && !world
				...
				ChestMinecartEntity
						chestMinecartEntity =
						EntityType.CHEST_MINECART.create(world.toServerWorld(), SpawnReason.CHUNK_GENERATION);
				if (chestMinecartEntity != null) {
					chestMinecartEntity.initPosition(
							blockPos.getX() + 0.5,
							blockPos.getY() + 0.5,
							blockPos.getZ() + 0.5
					);
		...
		protected void fillSupportBeam(
				StructureWorldAccess world,
				BlockState state,
				int x,
				int y,
				...
				for (boolean bl2 = true; bl || bl2; j++) {
					...
					if (bl2) {
						...
						if (!bl3 && this.sideCoversSmallSquare(world, mutable, blockState)) {
							...
							fillColumn(world, Blocks.IRON_CHAIN.getDefaultState(), mutable, i + 2, i + j);
							return;
						}
		...
		private void addCobwebsUnderground(
				StructureWorldAccess world,
				BlockBox box,
				Random random,
				float threshold,
		...
		private boolean hasSolidNeighborBlocks(
				StructureWorldAccess world,
				BlockBox box,
				int x,
				int y,
	...
	public static class MineshaftCrossing extends MineshaftGenerator.MineshaftPart {
		...
		public MineshaftCrossing(NbtCompound nbt) {
			...
			this.direction = nbt.<Direction>get("D", Direction.HORIZONTAL_QUARTER_TURNS_CODEC).orElse(Direction.SOUTH);
		}
		...
		private void generateCrossingPillar(
				StructureWorldAccess world,
				BlockBox boundingBox,
				int x,
				int minY,
	...
	abstract static class MineshaftPart extends StructurePiece {
		...
		protected boolean canAddBlock(WorldView world, int x, int y, int z, BlockBox box) {
			...
			return !blockState.isOf(this.mineshaftType.getPlanks().getBlock())
					...
					&& !blockState.isOf(Blocks.IRON_CHAIN);
		}
	...
	public static class MineshaftRoom extends MineshaftGenerator.MineshaftPart {
		...
		public MineshaftRoom(NbtCompound nbt) {
			...
			this.entrances.addAll(nbt.<List<BlockBox>>get("Entrances", BlockBox.CODEC.listOf()).orElse(List.of()));
		}
