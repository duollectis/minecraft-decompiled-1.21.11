...
public class StrongholdGenerator {
	...
	private static final int MIN_PIECE_SIZE = 3;
	private static final int MAX_PIECE_COUNT = 50;
	private static final int MAX_CHAIN_DEPTH = 10;
	private static final boolean GENERATE_PORTAL_ROOM = true;
	...
	static @Nullable Class<? extends StrongholdGenerator.Piece> activePieceType;
	private static int totalWeight;
	static final StrongholdGenerator.StoneBrickRandomizer
			STONE_BRICK_RANDOMIZER =
			new StrongholdGenerator.StoneBrickRandomizer();
	...
	private static StrongholdGenerator.@Nullable Piece createPiece(
	...
	private static StrongholdGenerator.@Nullable Piece pickPiece(
		...
		if (!checkRemainingPieces()) {
			...
			if (activePieceType != null) {
				StrongholdGenerator.Piece
						piece =
						createPiece(activePieceType, holder, random, x, y, z, orientation, chainLength);
				activePieceType = null;
				if (piece != null) {
					return piece;
				}
			...
			while (i < 5) {
				...
				for (StrongholdGenerator.PieceData pieceData : possiblePieces) {
					...
					if (j < 0) {
						...
						StrongholdGenerator.Piece
								piece2 =
								createPiece(pieceData.pieceType, holder, random, x, y, z, orientation, chainLength);
						if (piece2 != null) {
			...
			return blockBox != null && blockBox.getMinY() > 1 ? new StrongholdGenerator.SmallCorridor(
					chainLength,
					blockBox,
					orientation
			) : null;
	...
	static @Nullable StructurePiece pieceGenerator(
			StrongholdGenerator.Start start,
			StructurePiecesHolder holder,
			Random random,
			int x,
	...
	public static class ChestCorridor extends StrongholdGenerator.Piece {
		...
		public static StrongholdGenerator.@Nullable ChestCorridor create(
	...
	public static class Corridor extends StrongholdGenerator.Piece {
		...
		public static StrongholdGenerator.@Nullable Corridor create(
				StructurePiecesHolder holder, Random random, int x, int y, int z, Direction orientation, int chainLength
		) {
			BlockBox blockBox = BlockBox.rotated(x, y, z, -1, -1, 0, 5, 5, 7, orientation);
			return isInBounds(blockBox) && holder.getIntersecting(blockBox) == null
	...
	public static class FiveWayCrossing extends StrongholdGenerator.Piece {
		...
		public void generate(
			...
			this.addBlock(
					world,
					Blocks.WALL_TORCH.getDefaultState().with(WallTorchBlock.FACING, Direction.SOUTH),
					6,
					5,
					6,
					chunkBox
	...
	public static class LeftTurn extends StrongholdGenerator.Turn {
		...
		public static StrongholdGenerator.@Nullable LeftTurn create(
				StructurePiecesHolder holder, Random random, int x, int y, int z, Direction orientation, int chainLength
		) {
	...
	public static class Library extends StrongholdGenerator.Piece {
		...
		protected static final int DOOR_COUNT = 6;
		...
		public void generate(
			...
			for (int l = 1; l <= 13; l++) {
				if ((l - 1) % 4 == 0) {
					...
					this.addBlock(
							world,
							Blocks.WALL_TORCH.getDefaultState().with(WallTorchBlock.FACING, Direction.EAST),
							2,
							3,
							l,
							chunkBox
			...
			if (this.tall) {
				...
				for (int m = 0; m <= 2; m++) {
					this.addBlock(
							world,
							Blocks.OAK_FENCE.getDefaultState().with(FenceBlock.SOUTH, true).with(FenceBlock.WEST, true),
							8 + m,
							6,
							12 - m,
							chunkBox
	...
	abstract static class Piece extends StructurePiece {
		...
		public Piece(StructurePieceType structurePieceType, NbtCompound nbtCompound) {
			...
			this.entryDoor =
					nbtCompound
							.<StrongholdGenerator.Piece.EntranceType>get(
									"EntryDoor",
									StrongholdGenerator.Piece.EntranceType.CODEC
							)
							.orElseThrow();
		}
		...
		protected void generateEntrance(
			...
			switch (type) {
				...
				case WOOD_DOOR:
					...
					this.addBlock(
							world,
							Blocks.OAK_DOOR.getDefaultState().with(DoorBlock.HALF, DoubleBlockHalf.UPPER),
							x + 1,
							y + 1,
							z,
							boundingBox
				...
				case GRATES:
					...
					this.addBlock(
							world,
							Blocks.IRON_BARS.getDefaultState().with(PaneBlock.WEST, true),
							x,
							y,
							z,
							boundingBox
					...
					this.addBlock(
							world,
							Blocks.IRON_BARS.getDefaultState().with(PaneBlock.EAST, true).with(PaneBlock.WEST, true),
							x,
							y + 2,
							z,
							boundingBox
					...
					this.addBlock(
							world,
							Blocks.IRON_BARS.getDefaultState().with(PaneBlock.EAST, true),
							x + 2,
							y + 1,
							z,
							boundingBox
				...
				case IRON_DOOR:
					...
					this.addBlock(
							world,
							Blocks.IRON_DOOR.getDefaultState().with(DoorBlock.HALF, DoubleBlockHalf.UPPER),
							x + 1,
							y + 1,
							z,
							boundingBox
					...
					this.addBlock(
							world,
							Blocks.STONE_BUTTON.getDefaultState().with(ButtonBlock.FACING, Direction.NORTH),
							x + 2,
							y + 1,
							z + 1,
							boundingBox
					...
					this.addBlock(
							world,
							Blocks.STONE_BUTTON.getDefaultState().with(ButtonBlock.FACING, Direction.SOUTH),
							x + 2,
							y + 1,
							z - 1,
							boundingBox
		...
		protected @Nullable StructurePiece fillForwardOpening(
				StrongholdGenerator.Start start,
				StructurePiecesHolder holder,
				Random random,
				int leftRightOffset,
		...
		protected @Nullable StructurePiece fillNWOpening(
				StrongholdGenerator.Start start,
				StructurePiecesHolder holder,
				Random random,
				int heightOffset,
		...
		protected @Nullable StructurePiece fillSEOpening(
				StrongholdGenerator.Start start,
				StructurePiecesHolder holder,
				Random random,
				int heightOffset,
		...
		protected static enum EntranceType {
			...
			@Deprecated
			public static final Codec<StrongholdGenerator.Piece.EntranceType>
					CODEC =
					Codecs.enumByName(StrongholdGenerator.Piece.EntranceType::valueOf);
		}
	}
	...
	public static class PortalRoom extends StrongholdGenerator.Piece {
		...
		public static StrongholdGenerator.@Nullable PortalRoom create(
				StructurePiecesHolder holder,
				int x,
				int y,
				int z,
		...
		public void generate(
			...
			BlockState
					blockState =
					Blocks.IRON_BARS.getDefaultState().with(PaneBlock.NORTH, true).with(PaneBlock.SOUTH, true);
			BlockState
					blockState2 =
	...
	public static class RightTurn extends StrongholdGenerator.Turn {
		...
		public static StrongholdGenerator.@Nullable RightTurn create(
				StructurePiecesHolder holder, Random random, int x, int y, int z, Direction orientation, int chainLength
		) {
			BlockBox blockBox = BlockBox.rotated(x, y, z, -1, -1, 0, 5, 5, 5, orientation);
			return isInBounds(blockBox) && holder.getIntersecting(blockBox) == null
	...
	public static class SmallCorridor extends StrongholdGenerator.Piece {
		...
		public SmallCorridor(int chainLength, BlockBox boundingBox, Direction orientation) {
			...
			this.setOrientation(orientation);
			this.length =
					orientation != Direction.NORTH && orientation != Direction.SOUTH ? boundingBox.getBlockCountX()
					                                                                 : boundingBox.getBlockCountZ();
		}
		...
		public static @Nullable BlockBox create(
				StructurePiecesHolder holder,
				Random random,
				int x,
				int y,
		...
		public void generate(
			...
			switch (this.roomType) {
				case 0:
					...
					this.addBlock(
							world,
							Blocks.WALL_TORCH.getDefaultState().with(WallTorchBlock.FACING, Direction.NORTH),
							5,
							3,
							6,
							chunkBox
	...
	public static class Stairs extends StrongholdGenerator.Piece {
		...
		public void generate(
			...
			BlockState
					blockState =
					Blocks.COBBLESTONE_STAIRS.getDefaultState().with(StairsBlock.FACING, Direction.SOUTH);
	...
	public static class Start extends StrongholdGenerator.SpiralStaircase {
		...
		public StrongholdGenerator.@Nullable PortalRoom portalRoom;
