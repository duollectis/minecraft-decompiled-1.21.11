...
/**
 * Универсальный критерий на основе тика — срабатывает каждый тик для игрока.
 * Используется для проверки местоположения, сна, геройства деревни и других
 * состояний, которые проверяются периодически, а не по событию.
 */
public class TickCriterion extends AbstractCriterion<TickCriterion.Conditions> {
	...
	public record Conditions(Optional<LootContextPredicate> player) implements AbstractCriterion.Conditions {
		...
		public static AdvancementCriterion<Conditions> createLocation(LocationPredicate.Builder location) {
			return Criteria.LOCATION.create(new Conditions(
					Optional.of(EntityPredicate.contextPredicateFromEntityPredicate(
							EntityPredicate.Builder.create().location(location)
					))
			));
		}
		...
		public static AdvancementCriterion<Conditions> createLocation(EntityPredicate.Builder entity) {
			return Criteria.LOCATION.create(new Conditions(
					Optional.of(EntityPredicate.asLootContextPredicate(entity.build()))
			));
		}
		...
		/**
		 * Создаёт критерий местоположения, проверяющий, что игрок стоит на конкретном блоке
		 * и носит конкретные ботинки.
		 */
		public static AdvancementCriterion<Conditions> createLocation(
			...
			return createLocation(
					EntityPredicate.Builder.create()
							.equipment(EntityEquipmentPredicate.Builder
									.create()
									.feet(ItemPredicate.Builder.create().items(itemRegistry, boots)))
							.steppingOn(LocationPredicate.Builder
									.create()
									.block(BlockPredicate.Builder
											.create()
											.blocks(blockRegistry, steppingOn)))
			);
		}
	}
}
