...
/**
 * {@code BeeNestDestroyedCriterion}.
 */
	...
	public record Conditions(
		...
		public static final Codec<BeeNestDestroyedCriterion.Conditions> CODEC = RecordCodecBuilder.create(
				instance -> instance.group(
						                    EntityPredicate.LOOT_CONTEXT_PREDICATE_CODEC
								                    ...
								                    .forGetter(BeeNestDestroyedCriterion.Conditions::player),
						                    Registries.BLOCK
								                    .getEntryCodec()
								                    .optionalFieldOf("block")
								                    .forGetter(BeeNestDestroyedCriterion.Conditions::block),
						                    ItemPredicate.CODEC
								                    .optionalFieldOf("item")
								                    .forGetter(BeeNestDestroyedCriterion.Conditions::item),
		...
		public boolean test(BlockState state, ItemStack stack, int count) {
			if (this.block.isPresent() && !state.isOf(this.block.get())) {
				...
				return this.item.isPresent() && !this.item.get().test(stack) ? false : this.beesInside.test(count);
			}
		}
	}
}
