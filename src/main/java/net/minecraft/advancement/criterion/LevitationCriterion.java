...
public class LevitationCriterion extends AbstractCriterion<LevitationCriterion.Conditions> {
   ...
   public record Conditions(Optional<LootContextPredicate> player, Optional<DistancePredicate> distance, NumberRange.IntRange duration)
      ...
      public boolean matches(ServerPlayerEntity player, Vec3d distance, int duration) {
         return this.distance.isPresent() && !this.distance.get().test(distance.x, distance.y, distance.z, player.getX(), player.getY(), player.getZ())
            ? false
