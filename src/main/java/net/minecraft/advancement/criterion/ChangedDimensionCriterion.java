...
/**
 * Критерий, срабатывающий при смене измерения игроком.
 */
public class ChangedDimensionCriterion extends AbstractCriterion<ChangedDimensionCriterion.Conditions> {
	...
	public void trigger(ServerPlayerEntity player, RegistryKey<World> from, RegistryKey<World> to) {
		trigger(player, conditions -> conditions.matches(from, to));
	}
	...
	public record Conditions(
		...
		public static final Codec<Conditions> CODEC = RecordCodecBuilder.create(
				instance -> instance.group(
						...
						RegistryKey.createCodec(RegistryKeys.WORLD)
								...
								.forGetter(Conditions::from),
						RegistryKey.createCodec(RegistryKeys.WORLD)
								.optionalFieldOf("to")
								.forGetter(Conditions::to)
				).apply(instance, Conditions::new)
		);

		public static AdvancementCriterion<Conditions> create() {
			return Criteria.CHANGED_DIMENSION.create(new Conditions(
		...
		public static AdvancementCriterion<Conditions> create(RegistryKey<World> from, RegistryKey<World> to) {
			return Criteria.CHANGED_DIMENSION.create(new Conditions(
					Optional.empty(), Optional.of(from), Optional.of(to)
			));
		}

		public static AdvancementCriterion<Conditions> to(RegistryKey<World> to) {
			return Criteria.CHANGED_DIMENSION.create(new Conditions(
					Optional.empty(), Optional.empty(), Optional.of(to)
			));
		}
		...
		public boolean matches(RegistryKey<World> from, RegistryKey<World> to) {
			...
			return this.to.isEmpty() || this.to.get() == to;
		}
	}
}
