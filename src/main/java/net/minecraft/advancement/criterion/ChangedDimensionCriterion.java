...
/**
 * {@code ChangedDimensionCriterion}.
 */
public class ChangedDimensionCriterion extends AbstractCriterion<ChangedDimensionCriterion.Conditions> {
	...
	public record Conditions(
		...
		public static final Codec<ChangedDimensionCriterion.Conditions> CODEC = RecordCodecBuilder.create(
				instance -> instance.group(
						                    EntityPredicate.LOOT_CONTEXT_PREDICATE_CODEC
								                    ...
								                    .forGetter(ChangedDimensionCriterion.Conditions::player),
						                    RegistryKey
								                    .createCodec(RegistryKeys.WORLD)
								                    .optionalFieldOf("from")
								                    .forGetter(ChangedDimensionCriterion.Conditions::from),
						                    RegistryKey
								                    .createCodec(RegistryKeys.WORLD)
								                    .optionalFieldOf("to")
								                    .forGetter(ChangedDimensionCriterion.Conditions::to)
				                    )
		...
		public static AdvancementCriterion<ChangedDimensionCriterion.Conditions> create() {
			return Criteria.CHANGED_DIMENSION.create(new ChangedDimensionCriterion.Conditions(
					Optional.empty(),
					Optional.empty(),
					Optional.empty()
			));
		...
		public boolean matches(RegistryKey<World> from, RegistryKey<World> to) {
			return this.from.isPresent() && this.from.get() != from ? false
			                                                        : !this.to.isPresent() || this.to.get() == to;
		}
	}
}
