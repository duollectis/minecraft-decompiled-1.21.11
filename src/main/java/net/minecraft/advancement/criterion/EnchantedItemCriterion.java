...
public class EnchantedItemCriterion extends AbstractCriterion<EnchantedItemCriterion.Conditions> {
   ...
   public record Conditions(Optional<LootContextPredicate> player, Optional<ItemPredicate> item, NumberRange.IntRange levels)
      ...
      public boolean matches(ItemStack stack, int levels) {
         return this.item.isPresent() && !this.item.get().test(stack) ? false : this.levels.test(levels);
      }
   }
}
