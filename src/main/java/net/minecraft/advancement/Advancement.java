...
public record Advancement(
   ...
   public void validate(ErrorReporter errorReporter, RegistryEntryLookup.RegistryLookup lookup) {
      this.criteria
         .forEach(
            (name, criterion) -> {
               LootContextPredicateValidator lootContextPredicateValidator = new LootContextPredicateValidator(
      ...
      public Advancement.Builder display(
         ...
         return this.display(
            new AdvancementDisplay(
               icon, title, description, Optional.ofNullable(background).map(AssetInfo.TextureAssetInfo::new), frame, showToast, announceToChat, hidden
            )
         );
      }
