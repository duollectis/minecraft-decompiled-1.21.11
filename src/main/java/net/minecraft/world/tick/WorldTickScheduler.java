...
public class WorldTickScheduler<T> implements QueryableTickScheduler<T> {
	...
	public void scheduleTick(OrderedTick<T> orderedTick) {
		...
		ChunkTickScheduler<T> chunkTickScheduler = (ChunkTickScheduler<T>) this.chunkTickSchedulers.get(l);
		if (chunkTickScheduler == null) {
	...
	private void addTickableTicks(long time, int maxTicks) {
		...
		while (this.isTickableTicksCountUnder(maxTicks)
				&& (chunkTickScheduler = this.tickableChunkTickSchedulers.poll()) != null) {
	...
	private void addTickableTicks(
			Queue<ChunkTickScheduler<T>> tickableChunkTickSchedulers,
			ChunkTickScheduler<T> chunkTickScheduler,
			long tick,
			int maxTicks
	) {
	...
	public boolean isQueued(BlockPos pos, T type) {
		ChunkTickScheduler<T>
				chunkTickScheduler =
				(ChunkTickScheduler<T>) this.chunkTickSchedulers.get(ChunkPos.toLong(pos));
	...
	private void visitChunks(BlockBox box, WorldTickScheduler.ChunkVisitor<T> visitor) {
		...
		for (int m = i; m <= k; m++) {
			for (int n = j; n <= l; n++) {
				...
				ChunkTickScheduler<T> chunkTickScheduler = (ChunkTickScheduler<T>) this.chunkTickSchedulers.get(o);
				if (chunkTickScheduler != null) {
	...
	public void clearNextTicks(BlockBox box) {
		...
		this.visitChunks(
				box, (chunkPos, chunkTickScheduler) -> {
					OrderedTick<T> orderedTick = chunkTickScheduler.peekNextTick();
	...
	public void scheduleTicks(WorldTickScheduler<T> scheduler, BlockBox box, Vec3i offset) {
		...
		scheduler.visitChunks(
				box,
				(chunkPos, chunkTickScheduler) -> chunkTickScheduler
						.getQueueAsStream()
						.filter(predicate)
						.forEach(list::add)
		);
		LongSummaryStatistics
				longSummaryStatistics =
				list.stream().mapToLong(OrderedTick::subTickOrder).summaryStatistics();
