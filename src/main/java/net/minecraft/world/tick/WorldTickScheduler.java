...
public class WorldTickScheduler<T> implements QueryableTickScheduler<T> {
   ...
   public boolean isQueued(BlockPos pos, T type) {
      ChunkTickScheduler<T> chunkTickScheduler = (ChunkTickScheduler<T>)this.chunkTickSchedulers.get(ChunkPos.toLong(pos));
   ...
   public void scheduleTicks(WorldTickScheduler<T> scheduler, BlockBox box, Vec3i offset) {
      ...
      list.forEach(
         tick -> this.scheduleTick(
            new OrderedTick<>(tick.type(), tick.pos().add(offset), tick.triggerTick(), tick.priority(), tick.subTickOrder() - l + m + 1L)
         )
      );
   }
