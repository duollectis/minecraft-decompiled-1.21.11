...
public class LevelProperties implements ServerWorldProperties, SaveProperties {
   ...
   public static <T> LevelProperties readProperties(
      ...
      return new LevelProperties(
         ...
         dynamic.get("spawn").read(WorldProperties.SpawnPoint.CODEC).result().orElse(WorldProperties.SpawnPoint.DEFAULT),
         l,
         ...
         dynamic.get("removed_features").asStream().flatMap(removedFeatures -> removedFeatures.asString().result().stream()).collect(Collectors.toSet()),
