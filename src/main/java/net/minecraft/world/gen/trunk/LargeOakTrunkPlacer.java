...
public class LargeOakTrunkPlacer extends TrunkPlacer {
		...
		if (!make && Objects.equals(startPos, branchPos)) {
			...
			for (int j = 0; j <= i; j++) {
				BlockPos
						blockPos2 =
						startPos.add(
								MathHelper.floor(0.5F + j * f),
								MathHelper.floor(0.5F + j * g),
								MathHelper.floor(0.5F + j * h)
						);
				if (make) {
					this.getAndSetState(
							world,
							...
							blockPos2,
							config,
							state -> state.withIfExists(PillarBlock.AXIS, this.getLogAxis(startPos, blockPos2))
					);
				}
	...
	private void makeBranches(
		...
		for (LargeOakTrunkPlacer.BranchPosition branchPosition : branchPositions) {
			...
			if (!blockPos.equals(branchPosition.node.getCenter()) && this.isHighEnough(
					treeHeight,
					i - startPos.getY()
			)) {
				this.makeOrCheckBranch(
						world,
						replacer,
						random,
						blockPos,
						branchPosition.node.getCenter(),
						true,
						config
				);
			}
