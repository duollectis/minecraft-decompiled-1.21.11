...
/**
 * Конфигурация карвера пещер, расширяющая {@link CarverConfig}
 * параметрами горизонтального/вертикального масштаба и уровня пола.
 */
public class CaveCarverConfig extends CarverConfig {
		...
		instance -> instance
			.group(
				...
				FloatProvider.VALUE_CODEC
					...
					.forGetter(config -> config.horizontalRadiusMultiplier),
				FloatProvider.VALUE_CODEC
					.fieldOf("vertical_radius_multiplier")
					.forGetter(config -> config.verticalRadiusMultiplier),
				FloatProvider
					.createValidatedCodec(-1.0F, 1.0F)
					.fieldOf("floor_level")
					.forGetter(config -> config.floorLevel)
			)
	...
	public CaveCarverConfig(
		...
		this(
			probability, y, yScale, lavaLevel,
			CarverDebugConfig.DEFAULT, replaceable,
