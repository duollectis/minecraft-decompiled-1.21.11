...
public class RavineCarver extends Carver<RavineCarverConfig> {
	...
	public boolean carve(
		...
		int l = (int) (i * ravineCarverConfig.shape.distanceFactor.get(random));
		int m = 0;
		this.carveRavine(
				carverContext,
				ravineCarverConfig,
	...
	private void carveRavine(
		...
		for (int i = branchStartIndex; i < branchCount; i++) {
			double d = 1.5 + MathHelper.sin(i * (float) Math.PI / branchCount) * width;
	...
	private double getVerticalScale(
		...
		return g * pitch * MathHelper.nextBetween(random, 0.75F, 1.0F);
	}
	...
	private boolean isPositionExcluded(
		...
		return (scaledRelativeX * scaledRelativeX + scaledRelativeZ * scaledRelativeZ) * horizontalStretchFactors[i - 1]
