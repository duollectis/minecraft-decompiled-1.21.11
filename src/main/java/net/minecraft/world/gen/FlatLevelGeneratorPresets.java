...
/**
 * {@code FlatLevelGeneratorPresets}.
 */
	...
	static class Registrar {
		...
		private void createAndRegister(
			...
			RegistryEntryLookup<StructureSet>
					registryEntryLookup =
					this.presetRegisterable.getRegistryLookup(RegistryKeys.STRUCTURE_SET);
			RegistryEntryLookup<PlacedFeature>
					registryEntryLookup2 =
					this.presetRegisterable.getRegistryLookup(RegistryKeys.PLACED_FEATURE);
			RegistryEntryLookup<Biome>
					registryEntryLookup3 =
					this.presetRegisterable.getRegistryLookup(RegistryKeys.BIOME);
			RegistryEntryList.Direct<StructureSet> direct = RegistryEntryList.of(
					structureSetKeys.stream().map(registryEntryLookup::getOrThrow).collect(Collectors.toList())
			);
			FlatChunkGeneratorConfig flatChunkGeneratorConfig = new FlatChunkGeneratorConfig(
					Optional.of(direct),
					registryEntryLookup3.getOrThrow(biome),
					FlatChunkGeneratorConfig.getLavaLakes(registryEntryLookup2)
			);
			if (hasFeatures) {
			...
			this.presetRegisterable.register(
					registryKey,
					new FlatLevelGeneratorPreset(icon.asItem().getRegistryEntry(), flatChunkGeneratorConfig)
			);
		}
