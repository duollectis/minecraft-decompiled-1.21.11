...
public class DensityFunctions {
   ...
   public static final double field_61474 = 0.390625;
   ...
   private static final double field_61480 = 4.0;
   ...
   private static void registerSlopedCheeseFunction(
      ...
      DensityFunction densityFunction3 = registerAndGetHolder(densityFunctionRegisterable, depthKey, applyHeightGradient(densityFunction));
   ...
   private static DensityFunction estimateUpperBound(DensityFunction density, double bottomDensity, double topDensity, double bottomY, double topY) {
      double d = (topY - bottomY) / (topDensity - bottomDensity);
      double e = bottomY - bottomDensity * d;
      return DensityFunctionTypes.add(DensityFunctionTypes.mul(density, DensityFunctionTypes.constant(d)), DensityFunctionTypes.constant(e));
   }
   ...
   public static NoiseRouter createSurfaceNoiseRouter(
      ...
      DensityFunction densityFunction9 = entryHolder(
         densityFunctionLookup, largeBiomes ? OFFSET_OVERWORLD_LARGE_BIOME : (amplified ? OFFSET_OVERWORLD_AMPLIFIED : OFFSET_OVERWORLD)
      );
      ...
      DensityFunction densityFunction14 = DensityFunctionTypes.min(
         densityFunction13, DensityFunctionTypes.mul(DensityFunctionTypes.constant(5.0), entryHolder(densityFunctionLookup, CAVES_ENTRANCES_OVERWORLD))
      );
      DensityFunction densityFunction15 = DensityFunctionTypes.rangeChoice(
         densityFunction13, -1000000.0, 1.5625, densityFunction14, createCavesFunction(densityFunctionLookup, noiseParametersLookup, densityFunction13)
      );
      DensityFunction densityFunction16 = DensityFunctionTypes.min(
         applyBlendDensity(applySurfaceSlides(amplified, densityFunction15)), entryHolder(densityFunctionLookup, CAVES_NOODLE_OVERWORLD)
      );
      ...
      DensityFunction densityFunction18 = verticalRangeChoice(
         densityFunction17, DensityFunctionTypes.noise(noiseParametersLookup.getOrThrow(NoiseParametersKeys.ORE_VEININESS), 1.5, 1.5), i, j, 0
      );
      float f = 4.0F;
      DensityFunction densityFunction19 = verticalRangeChoice(
            densityFunction17, DensityFunctionTypes.noise(noiseParametersLookup.getOrThrow(NoiseParametersKeys.ORE_VEIN_A), 4.0, 4.0), i, j, 0
         )
         .abs();
      DensityFunction densityFunction20 = verticalRangeChoice(
            densityFunction17, DensityFunctionTypes.noise(noiseParametersLookup.getOrThrow(NoiseParametersKeys.ORE_VEIN_B), 4.0, 4.0), i, j, 0
         )
         .abs();
   ...
   private static DensityFunction findTopSurface(DensityFunction offset, DensityFunction factor, boolean amplified) {
      ...
      DensityFunction densityFunction4 = DensityFunctionTypes.add(
         applySurfaceSlides(
            amplified,
            DensityFunctionTypes.add(
                  createInitialDensityFunction(densityFunction, applyHeightGradient(densityFunction2)), DensityFunctionTypes.constant(-0.703125)
               )
               .clamp(-64.0, 64.0)
         ),
         DensityFunctionTypes.constant(-0.390625)
      );
      return DensityFunctionTypes.findTopSurface(densityFunction4, densityFunction3, -64, GenerationShapeConfig.SURFACE.verticalCellBlockCount());
   }
