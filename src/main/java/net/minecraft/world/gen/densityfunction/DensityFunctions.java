...
/**
 * Фабрика и реестр всех функций плотности ванильного Minecraft.
 * Содержит константы ключей реестра, методы создания {@link NoiseRouter} для каждого измерения
 * (Overworld, Nether, End), а также вспомогательные функции для построения пещер, рельефа и руд.
 */
public class DensityFunctions {

	public static final float END_ISLAND_OFFSET = -0.50375F;
	private static final float ORE_VEIN_NOISE_SCALE = 0.08F;
	private static final double WORLD_BOTTOM_NOISE_OFFSET = 1.5;
	private static final double WORLD_TOP_NOISE_OFFSET = 1.5;
	private static final double SLOPED_CHEESE_SCALE = 1.5625;
	private static final double SLOPED_CHEESE_OFFSET = -0.703125;
	public static final double BOTTOM_SLIDE_SCALE = 0.390625;
	public static final int WORLD_BOTTOM_HEIGHT = 64;
	public static final long CELL_COUNT = 4096L;
	private static final int WORLD_BOTTOM_Y = -64;
	private static final int WORLD_TOP_HEIGHT = 320;
	private static final double WORLD_BOTTOM_OFFSET = 1.5;
	private static final double WORLD_TOP_OFFSET = -1.5;
	private static final int BOTTOM_RELATIVE_SLIDE_MAX_Y = 24;
	private static final double BOTTOM_SLIDE_FACTOR = 4.0;
	...
	public static RegistryEntry<? extends DensityFunction> bootstrap(Registerable<DensityFunction> densityFunctionRegisterable) {
		RegistryEntryLookup<DoublePerlinNoiseSampler.NoiseParameters> noiseLookup =
				densityFunctionRegisterable.getRegistryLookup(RegistryKeys.NOISE_PARAMETERS);
		RegistryEntryLookup<DensityFunction> functionLookup =
				densityFunctionRegisterable.getRegistryLookup(RegistryKeys.DENSITY_FUNCTION);

		densityFunctionRegisterable.register(ZERO, DensityFunctionTypes.zero());
		int minY = DimensionType.MIN_HEIGHT * 2;
		int maxY = DimensionType.MAX_COLUMN_HEIGHT * 2;
		densityFunctionRegisterable.register(Y, DensityFunctionTypes.yClampedGradient(minY, maxY, minY, maxY));
		DensityFunction densityFunction = registerAndGetHolder(
				densityFunctionRegisterable,
				SHIFT_X,
				DensityFunctionTypes.flatCache(DensityFunctionTypes.cache2d(DensityFunctionTypes.shiftA(
						noiseLookup.getOrThrow(NoiseParametersKeys.OFFSET))))
		);
		DensityFunction densityFunction2 = registerAndGetHolder(
				densityFunctionRegisterable,
				SHIFT_Z,
				DensityFunctionTypes.flatCache(DensityFunctionTypes.cache2d(DensityFunctionTypes.shiftB(
						noiseLookup.getOrThrow(NoiseParametersKeys.OFFSET))))
		);
		...
		densityFunctionRegisterable.register(
				...
				InterpolatedNoiseSampler.createBase3dNoiseFunction(0.25, 0.375, 80.0, 60.0, 8.0)
		);
		densityFunctionRegisterable.register(
				BASE_3D_NOISE_END,
				InterpolatedNoiseSampler.createBase3dNoiseFunction(0.25, 0.25, 80.0, 160.0, 4.0)
		);
		RegistryEntry<DensityFunction> registryEntry = densityFunctionRegisterable.register(
				...
				DensityFunctionTypes.flatCache(
						DensityFunctionTypes.shiftedNoise(
								...
								densityFunction2,
								0.25,
								noiseLookup.getOrThrow(NoiseParametersKeys.CONTINENTALNESS)
						)
				)
		);
		RegistryEntry<DensityFunction> registryEntry2 = densityFunctionRegisterable.register(
				...
				DensityFunctionTypes.flatCache(
						DensityFunctionTypes.shiftedNoise(
								...
								densityFunction2,
								0.25,
								noiseLookup.getOrThrow(NoiseParametersKeys.EROSION)
						)
				)
		);
		DensityFunction densityFunction3 = registerAndGetHolder(
				...
				DensityFunctionTypes.flatCache(
						DensityFunctionTypes.shiftedNoise(
								...
								densityFunction2,
								0.25,
								noiseLookup.getOrThrow(NoiseParametersKeys.RIDGE)
						)
				)
		);
		densityFunctionRegisterable.register(
				RIDGES_FOLDED_OVERWORLD,
				createRidgesFoldedOverworldFunction(densityFunction3)
		);
		DensityFunction densityFunction4 = DensityFunctionTypes.noise(noiseLookup.getOrThrow(NoiseParametersKeys.JAGGED), 1500.0, 0.0);
		...
		RegistryEntry<DensityFunction> registryEntry3 = densityFunctionRegisterable.register(
				...
				DensityFunctionTypes.flatCache(
						DensityFunctionTypes.shiftedNoise(
								...
								densityFunction2,
								0.25,
								noiseLookup.getOrThrow(NoiseParametersKeys.CONTINENTALNESS_LARGE)
						)
				)
		);
		RegistryEntry<DensityFunction> registryEntry4 = densityFunctionRegisterable.register(
				...
				DensityFunctionTypes.flatCache(
						DensityFunctionTypes.shiftedNoise(
								...
								densityFunction2,
								0.25,
								noiseLookup.getOrThrow(NoiseParametersKeys.EROSION_LARGE)
						)
				)
		);
		registerSlopedCheeseFunction(
		...
		densityFunctionRegisterable.register(
				...
				DensityFunctionTypes.add(
						DensityFunctionTypes.endIslands(0L),
						entryHolder(functionLookup, BASE_3D_NOISE_END)
				)
		);
		densityFunctionRegisterable.register(
				CAVES_SPAGHETTI_ROUGHNESS_FUNCTION_OVERWORLD,
				createCavesSpaghettiRoughnessOverworldFunction(noiseLookup)
		);
		densityFunctionRegisterable.register(
				CAVES_SPAGHETTI_2D_THICKNESS_MODULATOR_OVERWORLD,
				DensityFunctionTypes.cacheOnce(
						DensityFunctionTypes.noiseInRange(
								noiseLookup.getOrThrow(NoiseParametersKeys.SPAGHETTI_2D_THICKNESS),
								2.0,
								1.0,
								-0.6,
								-1.3
		...
		densityFunctionRegisterable.register(
				...
				createCavesSpaghetti2dOverworldFunction(functionLookup, noiseLookup)
		);
		densityFunctionRegisterable.register(
				CAVES_ENTRANCES_OVERWORLD,
				createCavesEntrancesOverworldFunction(functionLookup, noiseLookup)
		);
		densityFunctionRegisterable.register(
				CAVES_NOODLE_OVERWORLD,
				createCavesNoodleOverworldFunction(functionLookup, noiseLookup)
		);
		return densityFunctionRegisterable.register(
				CAVES_PILLARS_OVERWORLD,
				createCavePillarsOverworldFunction(noiseLookup)
		);
	}
	...
	private static void registerSlopedCheeseFunction(
		...
		var densityFunctionWrapper = new DensityFunctionTypes.Spline.DensityFunctionWrapper(continents);
		var densityFunctionWrapper2 = new DensityFunctionTypes.Spline.DensityFunctionWrapper(erosion);
		var densityFunctionWrapper3 = new DensityFunctionTypes.Spline.DensityFunctionWrapper(
				densityFunctionLookup.getOrThrow(RIDGES_OVERWORLD)
		);
		var densityFunctionWrapper4 = new DensityFunctionTypes.Spline.DensityFunctionWrapper(
		...
		DensityFunction densityFunction = registerAndGetHolder(
				...
				applyBlending(
						DensityFunctionTypes.add(
								DensityFunctionTypes.constant(END_ISLAND_OFFSET),
		...
		DensityFunction densityFunction3 = registerAndGetHolder(
				densityFunctionRegisterable,
				depthKey,
				applyHeightGradient(densityFunction)
		);
		...
		DensityFunction densityFunction6 = createInitialDensityFunction(
				densityFunction2,
				DensityFunctionTypes.add(densityFunction3, densityFunction5)
		);
		densityFunctionRegisterable.register(
				slopedCheeseKey,
				DensityFunctionTypes.add(densityFunction6, entryHolder(densityFunctionLookup, BASE_3D_NOISE_OVERWORLD))
		);
	}

	private static DensityFunction applyHeightGradient(DensityFunction function) {
		return DensityFunctionTypes.add(DensityFunctionTypes.yClampedGradient(-WORLD_BOTTOM_HEIGHT, WORLD_TOP_HEIGHT, WORLD_BOTTOM_NOISE_OFFSET, WORLD_TOP_OFFSET), function);
	}
	...
	private static DensityFunction entryHolder(
			RegistryEntryLookup<DensityFunction> densityFunctionRegisterable,
			RegistryKey<DensityFunction> key
	) {
	...
	private static DensityFunction createRidgesFoldedOverworldFunction(DensityFunction input) {
		return DensityFunctionTypes.mul(
				DensityFunctionTypes.add(
						DensityFunctionTypes.add(input.abs(), DensityFunctionTypes.constant(-0.6666666666666666)).abs(),
	...
	private static DensityFunction createCavesSpaghettiRoughnessOverworldFunction(
		...
		DensityFunction
				densityFunction =
				DensityFunctionTypes.noise(noiseParametersLookup.getOrThrow(NoiseParametersKeys.SPAGHETTI_ROUGHNESS));
		...
		return DensityFunctionTypes.cacheOnce(
				DensityFunctionTypes.mul(
						densityFunction2,
						DensityFunctionTypes.add(densityFunction.abs(), DensityFunctionTypes.constant(-0.4))
				)
		);
	}
	...
	private static DensityFunction createCavesEntrancesOverworldFunction(
		...
		DensityFunction densityFunction = DensityFunctionTypes.cacheOnce(
				DensityFunctionTypes.noise(
						noiseParametersLookup.getOrThrow(NoiseParametersKeys.SPAGHETTI_3D_RARITY),
						2.0,
						1.0
				)
		);
		...
		DensityFunction densityFunction3 = DensityFunctionTypes.weirdScaledSampler(
				densityFunction,
				noiseParametersLookup.getOrThrow(NoiseParametersKeys.SPAGHETTI_3D_1),
				DensityFunctionTypes.WeirdScaledSampler.RarityValueMapper.TYPE1
		);
		DensityFunction densityFunction4 = DensityFunctionTypes.weirdScaledSampler(
				densityFunction,
				noiseParametersLookup.getOrThrow(NoiseParametersKeys.SPAGHETTI_3D_2),
				DensityFunctionTypes.WeirdScaledSampler.RarityValueMapper.TYPE1
		);
		DensityFunction
				densityFunction5 =
				DensityFunctionTypes.add(DensityFunctionTypes.max(densityFunction3, densityFunction4), densityFunction2)
				                    .clamp(-1.0, 1.0);
		DensityFunction
				densityFunction6 =
				entryHolder(densityFunctionLookup, CAVES_SPAGHETTI_ROUGHNESS_FUNCTION_OVERWORLD);
		DensityFunction
				densityFunction7 =
				DensityFunctionTypes.noise(
						noiseParametersLookup.getOrThrow(NoiseParametersKeys.CAVE_ENTRANCE),
						0.75,
						0.5
				);
		DensityFunction densityFunction8 = DensityFunctionTypes.add(
				DensityFunctionTypes.add(densityFunction7, DensityFunctionTypes.constant(0.37)),
				DensityFunctionTypes.yClampedGradient(-10, 30, 0.3, 0.0)
		);
		return DensityFunctionTypes.cacheOnce(DensityFunctionTypes.min(
				densityFunction8,
				DensityFunctionTypes.add(densityFunction6, densityFunction5)
		));
	}
	...
	private static DensityFunction createCavesNoodleOverworldFunction(
		...
		DensityFunction densityFunction2 = verticalRangeChoice(
				densityFunction,
				DensityFunctionTypes.noise(noiseParametersLookup.getOrThrow(NoiseParametersKeys.NOODLE), 1.0, 1.0),
				-60,
				WORLD_TOP_HEIGHT,
				-1
		);
		DensityFunction densityFunction3 = verticalRangeChoice(
				densityFunction,
				DensityFunctionTypes.noiseInRange(
						noiseParametersLookup.getOrThrow(NoiseParametersKeys.NOODLE_THICKNESS),
						1.0,
						1.0,
						-0.05,
						-0.1
		...
		// 8/3 — масштаб шума для гребней нудл-пещер
		double noodleRidgeScale = 2.6666666666666665;
		DensityFunction densityFunction4 = verticalRangeChoice(
				densityFunction,
				DensityFunctionTypes.noise(
						noiseParametersLookup.getOrThrow(NoiseParametersKeys.NOODLE_RIDGE_A),
						noodleRidgeScale,
						noodleRidgeScale
				),
				-60,
		...
		DensityFunction densityFunction5 = verticalRangeChoice(
				densityFunction,
				DensityFunctionTypes.noise(
						noiseParametersLookup.getOrThrow(NoiseParametersKeys.NOODLE_RIDGE_B),
						noodleRidgeScale,
						noodleRidgeScale
				),
				-60,
		...
		DensityFunction densityFunction6 = DensityFunctionTypes.mul(
				DensityFunctionTypes.constant(WORLD_BOTTOM_NOISE_OFFSET),
				DensityFunctionTypes.max(densityFunction4.abs(), densityFunction5.abs())
		);
		return DensityFunctionTypes.rangeChoice(
				...
				DensityFunctionTypes.add(densityFunction3, densityFunction6)
		);
	}
	...
	private static DensityFunction createCavePillarsOverworldFunction(RegistryEntryLookup<DoublePerlinNoiseSampler.NoiseParameters> noiseParametersLookup) {
		DensityFunction densityFunction = DensityFunctionTypes.noise(
				noiseParametersLookup.getOrThrow(NoiseParametersKeys.PILLAR), 25.0, 0.3
		);
		DensityFunction densityFunction2 = DensityFunctionTypes.noiseInRange(
				noiseParametersLookup.getOrThrow(NoiseParametersKeys.PILLAR_RARENESS), 0.0, -2.0
		);
		DensityFunction densityFunction3 = DensityFunctionTypes.noiseInRange(
				noiseParametersLookup.getOrThrow(NoiseParametersKeys.PILLAR_THICKNESS), 0.0, 1.1
		);
	...
	private static DensityFunction createCavesSpaghetti2dOverworldFunction(
		...
		DensityFunction
				densityFunction =
				DensityFunctionTypes.noise(
						noiseParametersLookup.getOrThrow(NoiseParametersKeys.SPAGHETTI_2D_MODULATOR),
						2.0,
						1.0
				);
		DensityFunction densityFunction2 = DensityFunctionTypes.weirdScaledSampler(
				densityFunction,
				noiseParametersLookup.getOrThrow(NoiseParametersKeys.SPAGHETTI_2D),
				DensityFunctionTypes.WeirdScaledSampler.RarityValueMapper.TYPE2
		);
		DensityFunction densityFunction3 = DensityFunctionTypes.noiseInRange(
				noiseParametersLookup.getOrThrow(NoiseParametersKeys.SPAGHETTI_2D_ELEVATION),
				0.0,
				Math.floorDiv(-WORLD_BOTTOM_HEIGHT, 8),
				8.0
		);
		DensityFunction
				densityFunction4 =
				entryHolder(densityFunctionLookup, CAVES_SPAGHETTI_2D_THICKNESS_MODULATOR_OVERWORLD);
		DensityFunction
				densityFunction5 =
				DensityFunctionTypes
						.add(densityFunction3, DensityFunctionTypes.yClampedGradient(-WORLD_BOTTOM_HEIGHT, WORLD_TOP_HEIGHT, 8.0, -40.0))
						.abs();
		DensityFunction densityFunction6 = DensityFunctionTypes.add(densityFunction5, densityFunction4).cube();
		// 0.083 — коэффициент смешивания толщины спагетти-пещер
	...
	private static DensityFunction createCavesFunction(
		...
		DensityFunction
				densityFunction3 =
				DensityFunctionTypes.noise(noiseParametersLookup.getOrThrow(NoiseParametersKeys.CAVE_LAYER), 8.0);
		DensityFunction
				densityFunction4 =
				DensityFunctionTypes.mul(DensityFunctionTypes.constant(4.0), densityFunction3.square());
		DensityFunction
				densityFunction5 =
				DensityFunctionTypes.noise(
						noiseParametersLookup.getOrThrow(NoiseParametersKeys.CAVE_CHEESE),
						0.6666666666666666
				);
		...
		DensityFunction densityFunction8 = DensityFunctionTypes.min(
				DensityFunctionTypes.min(
						densityFunction7,
						entryHolder(densityFunctionLookup, CAVES_ENTRANCES_OVERWORLD)
				),
	...
	private static DensityFunction applyBlendDensity(DensityFunction density) {
		...
		return DensityFunctionTypes
				.mul(DensityFunctionTypes.interpolated(densityFunction), DensityFunctionTypes.constant(0.64))
				.squeeze();
	}

	private static DensityFunction estimateUpperBound(
		...
		double slope = (topY - bottomY) / (topDensity - bottomDensity);
		double intercept = bottomY - bottomDensity * slope;
		return DensityFunctionTypes.add(
				DensityFunctionTypes.mul(density, DensityFunctionTypes.constant(slope)),
				DensityFunctionTypes.constant(intercept)
		);
	}
	...
	public static NoiseRouter createSurfaceNoiseRouter(
		...
		DensityFunction
				densityFunction =
				DensityFunctionTypes.noise(noiseParametersLookup.getOrThrow(NoiseParametersKeys.AQUIFER_BARRIER), 0.5);
		DensityFunction
				densityFunction2 =
				DensityFunctionTypes.noise(
						noiseParametersLookup.getOrThrow(NoiseParametersKeys.AQUIFER_FLUID_LEVEL_FLOODEDNESS),
						0.67
				);
		...
		DensityFunction
				densityFunction4 =
				DensityFunctionTypes.noise(noiseParametersLookup.getOrThrow(NoiseParametersKeys.AQUIFER_LAVA));
		...
		DensityFunction densityFunction7 = DensityFunctionTypes.shiftedNoise(
				...
				densityFunction6,
				0.25,
				noiseParametersLookup.getOrThrow(
						largeBiomes ? NoiseParametersKeys.TEMPERATURE_LARGE : NoiseParametersKeys.TEMPERATURE)
		);
		DensityFunction densityFunction8 = DensityFunctionTypes.shiftedNoise(
				...
				0.25,
				noiseParametersLookup.getOrThrow(
						largeBiomes ? NoiseParametersKeys.VEGETATION_LARGE : NoiseParametersKeys.VEGETATION)
		);
		DensityFunction densityFunction9 = entryHolder(
				densityFunctionLookup,
				largeBiomes ? OFFSET_OVERWORLD_LARGE_BIOME : (amplified ? OFFSET_OVERWORLD_AMPLIFIED : OFFSET_OVERWORLD)
		);
		DensityFunction densityFunction10 = entryHolder(
				densityFunctionLookup,
				largeBiomes ? FACTOR_OVERWORLD_LARGE_BIOME : (amplified ? FACTOR_OVERWORLD_AMPLIFIED : FACTOR_OVERWORLD)
		);
		DensityFunction densityFunction11 = entryHolder(
				densityFunctionLookup,
				largeBiomes ? DEPTH_OVERWORLD_LARGE_BIOME : (amplified ? DEPTH_OVERWORLD_AMPLIFIED : DEPTH_OVERWORLD)
		);
		DensityFunction densityFunction12 = findTopSurface(densityFunction9, densityFunction10, amplified);
		DensityFunction densityFunction13 = entryHolder(
				densityFunctionLookup,
				largeBiomes ? SLOPED_CHEESE_OVERWORLD_LARGE_BIOME
				            : (amplified ? SLOPED_CHEESE_OVERWORLD_AMPLIFIED : SLOPED_CHEESE_OVERWORLD)
		);
		DensityFunction densityFunction14 = DensityFunctionTypes.min(
				densityFunction13,
				DensityFunctionTypes.mul(
		...
		DensityFunction densityFunction15 = DensityFunctionTypes.rangeChoice(
				densityFunction13,
				-1000000.0,
				SLOPED_CHEESE_SCALE,
				densityFunction14,
				createCavesFunction(densityFunctionLookup, noiseParametersLookup, densityFunction13)
		);
		DensityFunction densityFunction16 = DensityFunctionTypes.min(
				applyBlendDensity(applySurfaceSlides(amplified, densityFunction15)),
				entryHolder(densityFunctionLookup, CAVES_NOODLE_OVERWORLD)
		);
		DensityFunction densityFunction17 = entryHolder(densityFunctionLookup, Y);
		int oreVeinMinY = Stream
				.of(OreVeinSampler.VeinType.values())
				.mapToInt(veinType -> veinType.minY)
				.min()
				.orElse(-DimensionType.MIN_HEIGHT * 2);
		int oreVeinMaxY = Stream
				.of(OreVeinSampler.VeinType.values())
				.mapToInt(veinType -> veinType.maxY)
				.max()
				.orElse(-DimensionType.MIN_HEIGHT * 2);
		DensityFunction densityFunction18 = verticalRangeChoice(
				densityFunction17,
				DensityFunctionTypes.noise(
						noiseParametersLookup.getOrThrow(NoiseParametersKeys.ORE_VEININESS),
						WORLD_BOTTOM_NOISE_OFFSET,
						WORLD_BOTTOM_NOISE_OFFSET
				),
				oreVeinMinY,
				oreVeinMaxY,
		...
		DensityFunction densityFunction19 = verticalRangeChoice(
				densityFunction17,
				DensityFunctionTypes.noise(noiseParametersLookup.getOrThrow(NoiseParametersKeys.ORE_VEIN_A), 4.0, 4.0),
				oreVeinMinY,
				oreVeinMaxY,
				0
		).abs();
		DensityFunction densityFunction20 = verticalRangeChoice(
				densityFunction17,
				DensityFunctionTypes.noise(noiseParametersLookup.getOrThrow(NoiseParametersKeys.ORE_VEIN_B), 4.0, 4.0),
				oreVeinMinY,
				oreVeinMaxY,
				0
		).abs();
		DensityFunction densityFunction21 = DensityFunctionTypes.add(
				DensityFunctionTypes.constant(-ORE_VEIN_NOISE_SCALE),
				DensityFunctionTypes.max(densityFunction19, densityFunction20)
		);
		DensityFunction densityFunction22 = DensityFunctionTypes.noise(noiseParametersLookup.getOrThrow(NoiseParametersKeys.ORE_GAP));
		return new NoiseRouter(
				...
				entryHolder(
						densityFunctionLookup,
						largeBiomes ? CONTINENTS_OVERWORLD_LARGE_BIOME : CONTINENTS_OVERWORLD
				),
	...
	private static DensityFunction applySurfaceSlides(boolean amplified, DensityFunction density) {
		return applySlides(
				...
				384,
				amplified ? 16 : 80,
				amplified ? 0 : WORLD_BOTTOM_HEIGHT,
				-0.078125,
				0,
	...
	private static DensityFunction applyCavesSlides(
			RegistryEntryLookup<DensityFunction> densityFunctionLookup,
			int minY,
			int maxY
	) {
		return applySlides(
				entryHolder(densityFunctionLookup, BASE_3D_NOISE_NETHER),
				minY,
				maxY,
				BOTTOM_RELATIVE_SLIDE_MAX_Y,
				0,
	...
	public static NoiseRouter createNetherNoiseRouter(
		...
		return createMultiNoiseDependentNoiseRouter(
				...
				applyCavesSlides(densityFunctionLookup, 0, 128)
		);
	}
	...
	public static NoiseRouter createCavesNoiseRouter(
		...
		return createMultiNoiseDependentNoiseRouter(
				densityFunctionLookup,
				noiseParametersLookup,
				applyCavesSlides(densityFunctionLookup, -WORLD_BOTTOM_HEIGHT, 192)
		);
	}
	...
	public static NoiseRouter createFloatingIslandsNoiseRouter(
		...
		return createMultiNoiseDependentNoiseRouter(
				densityFunctionLookup,
				noiseParametersLookup,
				applyFloatingIslandsSlides(entryHolder(densityFunctionLookup, BASE_3D_NOISE_END), 0, 256)
		);
	}
	...
	/**
	 * Строит {@link NoiseRouter} для измерения Края (The End): использует функцию островов
	 * {@code endIslands} как источник рельефа и применяет к ней сглаживание и смешивание.
	 */
	...
	/** Возвращает заглушку {@link NoiseRouter}, где все каналы заполнены нулевыми функциями. */
	...
	private static DensityFunction findTopSurface(DensityFunction offset, DensityFunction factor, boolean amplified) {
		DensityFunction densityFunction = DensityFunctionTypes.cache2d(factor);
		DensityFunction densityFunction2 = DensityFunctionTypes.cache2d(offset);
		DensityFunction densityFunction3 = estimateUpperBound(
				DensityFunctionTypes.add(
						DensityFunctionTypes.mul(DensityFunctionTypes.constant(0.2734375), densityFunction.invert()),
						DensityFunctionTypes.mul(DensityFunctionTypes.constant(-1.0), densityFunction2)
				),
				WORLD_BOTTOM_NOISE_OFFSET,
				WORLD_TOP_OFFSET,
				-64.0,
		...
		densityFunction3 = densityFunction3.clamp(-40.0, 320.0);
		DensityFunction densityFunction4 = DensityFunctionTypes.add(
				applySurfaceSlides(
						amplified,
						DensityFunctionTypes.add(
								                    createInitialDensityFunction(densityFunction, applyHeightGradient(densityFunction2)),
								                    DensityFunctionTypes.constant(SLOPED_CHEESE_OFFSET)
						                    )
						                    .clamp(-64.0, 64.0)
				),
				DensityFunctionTypes.constant(-BOTTOM_SLIDE_SCALE)
		);
		return DensityFunctionTypes.findTopSurface(
				densityFunction4,
				densityFunction3,
				-WORLD_BOTTOM_HEIGHT,
				GenerationShapeConfig.SURFACE.verticalCellBlockCount()
		);
	}

	private static DensityFunction verticalRangeChoice(
			DensityFunction y,
		...
		return DensityFunctionTypes.interpolated(
				DensityFunctionTypes.rangeChoice(
						...
						maxInclusive + 1,
						whenInRange,
						DensityFunctionTypes.constant(whenOutOfRange)
				)
		);
	}
	...
	private static DensityFunction applySlides(
		...
		DensityFunction densityFunction2 = DensityFunctionTypes.yClampedGradient(
				minY + maxY - topRelativeMinY,
				minY + maxY - topRelativeMaxY,
				1.0,
				0.0
		);
		...
		DensityFunction densityFunction3 = DensityFunctionTypes.yClampedGradient(
				minY + bottomRelativeMinY,
				minY + bottomRelativeMaxY,
				0.0,
				1.0
		);
	...
	/** Масштабирует значения шума для пещер и тоннелей по ступенчатым порогам. */
	protected static final class CaveScaler {
		...
		protected static double scaleTunnels(double value) {
			...
			return value < 0.5 ? WORLD_BOTTOM_NOISE_OFFSET : 2.0;
		}
	}
}
