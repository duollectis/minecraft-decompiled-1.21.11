	...
	public static final Codec<RootSystemFeatureConfig> CODEC = RecordCodecBuilder.create(
			instance -> instance.group(
					                    ...
					                    Codec
							                    ...
							                    .forGetter(config -> config.requiredVerticalSpaceForTree),
					                    Codec.intRange(1, 64).fieldOf("root_radius").forGetter(config -> config.rootRadius),
					                    TagKey
							                    .codec(RegistryKeys.BLOCK)
							                    .fieldOf("root_replaceable")
							                    .forGetter(config -> config.rootReplaceable),
					                    BlockStateProvider.TYPE_CODEC
							                    .fieldOf("root_state_provider")
							                    .forGetter(config -> config.rootStateProvider),
					                    Codec
							                    .intRange(1, 256)
							                    .fieldOf("root_placement_attempts")
							                    .forGetter(config -> config.rootPlacementAttempts),
					                    Codec
							                    .intRange(1, 4096)
							                    .fieldOf("root_column_max_height")
							                    .forGetter(config -> config.maxRootColumnHeight),
					                    Codec.intRange(1, 64).fieldOf("hanging_root_radius").forGetter(config -> config.hangingRootRadius),
					                    Codec
							                    .intRange(1, 16)
							                    .fieldOf("hanging_roots_vertical_span")
							                    .forGetter(config -> config.hangingRootVerticalSpan),
					                    BlockStateProvider.TYPE_CODEC
							                    .fieldOf("hanging_root_state_provider")
							                    .forGetter(config -> config.hangingRootStateProvider),
					                    Codec
							                    .intRange(1, 256)
							                    .fieldOf("hanging_root_placement_attempts")
							                    .forGetter(config -> config.hangingRootPlacementAttempts),
					                    Codec
							                    .intRange(1, 64)
							                    .fieldOf("allowed_vertical_water_for_tree")
							                    .forGetter(config -> config.allowedVerticalWaterForTree),
