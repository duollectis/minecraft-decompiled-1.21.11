   ...
   public boolean generate(FeatureContext<GeodeFeatureConfig> context) {
      ...
      for (BlockPos blockPos3 : BlockPos.iterate(blockPos.add(i, i, i), blockPos.add(j, j, j))) {
         ...
         for (Pair<BlockPos, Integer> pair : list) {
            s += MathHelper.inverseSqrt(blockPos3.getSquaredDistance((Vec3i)pair.getFirst()) + ((Integer)pair.getSecond()).intValue()) + r;
         }

         for (BlockPos blockPos4 : list2) {
            t += MathHelper.inverseSqrt(blockPos3.getSquaredDistance(blockPos4) + geodeCrackConfig.crackPointOffset) + r;
         }
