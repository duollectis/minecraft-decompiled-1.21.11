...
public class PlacedFeatureIndexer {
	...
	public static <T> List<PlacedFeatureIndexer.IndexedFeatures> collectIndexedFeatures(
		...
		Comparator<IndexedFeature>
				comparator =
				Comparator.comparingInt(IndexedFeature::step).thenComparingInt(IndexedFeature::featureIndex);
		...
		for (T object : biomes) {
			...
			for (int j = 0; j < list2.size(); j++) {
				for (RegistryEntry<PlacedFeature> registryEntry : list2.get(j)) {
					...
					list.add(new IndexedFeature(
							object2IntMap.computeIfAbsent(
									placedFeature,
									feature -> mutableInt.getAndIncrement()
							), j, placedFeature
					));
				}
			}
