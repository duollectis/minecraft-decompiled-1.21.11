				...
				for (int t = 0; t < 16; t++) {
					for (int u = 0; u < 8; u++) {
						...
						if (bl) {
							...
							if (u < 4 && !blockState2.isSolid()
									&& structureWorldAccess.getBlockState(blockPos.add(s, u, t)) != blockState) {
								return false;
							}
						}
					}
					...
					for (int uxx = 0; uxx < 16; uxx++) {
						for (int v = 0; v < 8; v++) {
							...
							if (bl2 && (v < 4 || random.nextInt(2) != 0)) {
								...
								if (blockState4.isSolid()
										&& !blockState4.isIn(BlockTags.LAVA_POOL_STONE_CANNOT_REPLACE)) {
				...
				for (int t = 0; t < 16; t++) {
					for (int uxx = 0; uxx < 16; uxx++) {
						...
						if (structureWorldAccess
								.getBiome(blockPos4)
								.value()
								.canSetIce(structureWorldAccess, blockPos4, false)
