...
public class BlueIceFeature extends Feature<DefaultFeatureConfig> {
	...
	public boolean generate(FeatureContext<DefaultFeatureConfig> context) {
		...
		if (blockPos.getY() > structureWorldAccess.getSeaLevel() - 1) {
		...
		else if (!structureWorldAccess.getBlockState(blockPos).isOf(Blocks.WATER) && !structureWorldAccess
			...
			for (Direction direction : Direction.values()) {
				if (direction != Direction.DOWN && structureWorldAccess
						.getBlockState(blockPos.offset(direction))
						.isOf(Blocks.PACKED_ICE)) {
					bl = true;
					break;
				}
			}
			...
			if (!bl) {
				...
				for (int i = 0; i < 200; i++) {
					...
					if (k >= 1) {
						...
						if (blockState.isAir() || blockState.isOf(Blocks.WATER) || blockState.isOf(Blocks.PACKED_ICE)
								|| blockState.isOf(Blocks.ICE)) {
							for (Direction direction2 : Direction.values()) {
								BlockState
										blockState2 =
										structureWorldAccess.getBlockState(blockPos2.offset(direction2));
