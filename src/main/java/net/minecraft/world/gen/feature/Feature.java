...
/** Базовый класс для всех генераторов мировых объектов (деревья, руды, пещеры и т.д.). */
public abstract class Feature<FC extends FeatureConfig> {
	...
	public static final Feature<FallenTreeFeatureConfig> FALLEN_TREE = register("fallen_tree", new FallenTreeFeature(FallenTreeFeatureConfig.CODEC));
	...
	public boolean generateIfValid(
		...
		return world.isValidForSetBlock(pos) && generate(new FeatureContext<>(
				Optional.empty(),
				world,
				chunkGenerator,
				random,
