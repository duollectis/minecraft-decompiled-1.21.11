...
public class FossilFeatureConfig implements FeatureConfig {
	...
	public static final Codec<FossilFeatureConfig> CODEC = RecordCodecBuilder.create(
			instance -> instance.group(
					                    ...
					                    Identifier.CODEC
							                    .listOf()
							                    .fieldOf("overlay_structures")
							                    .forGetter(config -> config.overlayStructures),
					                    StructureProcessorType.REGISTRY_CODEC
							                    .fieldOf("fossil_processors")
							                    .forGetter(config -> config.fossilProcessors),
					                    StructureProcessorType.REGISTRY_CODEC
							                    .fieldOf("overlay_processors")
							                    .forGetter(config -> config.overlayProcessors),
					                    Codec
							                    .intRange(0, 7)
							                    .fieldOf("max_empty_corners_allowed")
							                    .forGetter(config -> config.maxEmptyCorners)
			                    )
