...
public class TreeFeature extends Feature<TreeFeatureConfig> {
	...
	private boolean generate(
		...
		BlockPos
				blockPos =
				config.rootPlacer.<BlockPos>map(rootPlacer -> rootPlacer.trunkOffset(pos, random)).orElse(pos);
		...
		if (m >= world.getBottomY() + 1 && n <= world.getTopYInclusive() + 1) {
			...
			if (o >= i || !optionalInt.isEmpty() && o >= optionalInt.getAsInt()) {
				if (config.rootPlacer.isPresent() && !config.rootPlacer
						.get()
						.generate(world, rootPlacerReplacer, random, pos, blockPos, config)) {
					return false;
				}
				else {
					List<FoliagePlacer.TreeNode>
							list =
							config.trunkPlacer.generate(world, trunkPlacerReplacer, random, o, blockPos, config);
					list.forEach(node -> config.foliagePlacer.generate(
							world,
							blockPlacer,
							random,
							config,
		...
		for (int i = 0; i <= height + 1; i++) {
			...
			for (int k = -j; k <= j; k++) {
				for (int l = -j; l <= j; l++) {
					...
					if (!config.trunkPlacer.canReplaceOrIsLog(world, mutable) || !config.ignoreVines && isVine(
							world,
							mutable
					)) {
						return i - 2;
		...
		if (bl && (!set2.isEmpty() || !set3.isEmpty())) {
			if (!treeFeatureConfig.decorators.isEmpty()) {
				TreeDecorator.Generator
						generator =
						new TreeDecorator.Generator(structureWorldAccess, biConsumer3, random, set2, set3, set);
	...
	private static VoxelSet placeLogsAndLeaves(
		...
		for (BlockPos blockPos : Lists.newArrayList(Sets.union(decorationPositions, rootPositions))) {
			if (box.contains(blockPos)) {
				voxelSet.set(
						blockPos.getX() - box.getMinX(),
						blockPos.getY() - box.getMinY(),
						blockPos.getZ() - box.getMinZ()
				);
			}
		}
				...
				if (box.contains(blockPos2)) {
					if (k != 0) {
						...
						setBlockStateWithoutUpdatingNeighbors(
								world,
								blockPos2,
								blockState.with(Properties.DISTANCE_1_7, k)
						);
					}

					voxelSet.set(
							blockPos2.getX() - box.getMinX(),
							blockPos2.getY() - box.getMinY(),
							blockPos2.getZ() - box.getMinZ()
					);
