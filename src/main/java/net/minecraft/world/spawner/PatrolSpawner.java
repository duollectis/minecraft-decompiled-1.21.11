							...
							if (i >= 1) {
								...
								if (!playerEntity.isSpectator()) {
									if (!world.isNearOccupiedPointOfInterest(playerEntity.getBlockPos(), 2)) {
										...
										BlockPos.Mutable
												mutable =
												playerEntity.getBlockPos().mutableCopy().move(j, 0, k);
										int l = 10;
										if (world.isRegionLoaded(
											...
											if (world
												...
												for (int n = 0; n < m; n++) {
													...
													mutable.setX(
															mutable.getX() + random.nextInt(5) - random.nextInt(5));
													mutable.setZ(
															mutable.getZ() + random.nextInt(5) - random.nextInt(5));
												}
											}
										}
									}
