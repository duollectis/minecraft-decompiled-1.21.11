...
/**
 * Регистратор ванильных типов измерений: Overworld, Overworld Caves, Nether и End.
 * Каждый тип описывает физические параметры измерения и его атрибуты окружения.
 */
public class DimensionTypeRegistrar {

	/**
	 * Регистрирует все ванильные типы измерений в указанном реестре.
	 * Overworld и Overworld Caves используют одинаковые атрибуты окружения,
	 * различаясь только наличием потолка.
	 *
	 * @param dimensionTypeRegisterable реестр для регистрации типов измерений
	 */
	public static void bootstrap(Registerable<DimensionType> dimensionTypeRegisterable) {
		RegistryEntryLookup<Timeline> timelineLookup =
			dimensionTypeRegisterable.getRegistryLookup(RegistryKeys.TIMELINE);
		...
		EnvironmentAttributeMap overworldAttributes = EnvironmentAttributeMap.builder()
			...
			.with(EnvironmentAttributes.CLOUD_HEIGHT_VISUAL, YLevels.OVERWORLD_CLOUD_HEIGHT)
		...
		dimensionTypeRegisterable.register(
			...
			new DimensionType(
				...
				false,
				1.0,
				YLevels.OVERWORLD_MIN_Y,
				YLevels.OVERWORLD_HEIGHT,
				YLevels.OVERWORLD_LOGICAL_HEIGHT,
				...
				overworldAttributes,
				timelineLookup.getOrThrow(TimelineTags.IN_OVERWORLD)
			)
		);
		...
		dimensionTypeRegisterable.register(
			...
			new DimensionType(
				...
				true,
				8.0,
				YLevels.NETHER_MIN_Y,
				YLevels.NETHER_HEIGHT,
				YLevels.NETHER_LOGICAL_HEIGHT,
				...
				EnvironmentAttributeMap.builder()
					...
					.build(),
				timelineLookup.getOrThrow(TimelineTags.IN_NETHER)
			)
		);
		...
		dimensionTypeRegisterable.register(
			...
			new DimensionType(
				...
				EnvironmentAttributeMap.builder()
					...
					.with(EnvironmentAttributes.RESPAWN_ANCHOR_WORKS_GAMEPLAY, false)
					.build(),
				timelineLookup.getOrThrow(TimelineTags.IN_END)
			)
		);
