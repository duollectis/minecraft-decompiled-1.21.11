...
public class NetherPortal {
	...
	public static Optional<NetherPortal> getNewPortal(
		...
		return getOrEmpty(
				world,
				pos,
				areaHelper -> areaHelper.isValid() && areaHelper.foundPortalBlocks == 0,
				firstCheckedAxis
		);
	}
	...
	public static Vec3d entityPosInPortal(
			...
			g =
					MathHelper.clamp(
							MathHelper.getLerpProgress(
									entityPos.getComponentAlongAxis(portalAxis) - f,
									0.0,
									d
							), 0.0, 1.0
					);
		...
		if (e > 0.0) {
			...
			f =
					MathHelper.clamp(
							MathHelper.getLerpProgress(
									entityPos.getComponentAlongAxis(axis) - blockPos.getComponentAlongAxis(axis),
									0.0,
									e
							), 0.0, 1.0
					);
		...
		if (!(dimensions.width() > 4.0F) && !(dimensions.height() > 4.0F)) {
			...
			VoxelShape
					...
					VoxelShapes.cuboid(Box
							.of(vec3d, dimensions.width(), 0.0, dimensions.width())
							.stretch(0.0, 1.0, 0.0)
							.expand(1.0E-6));
			Optional<Vec3d>
					optional =
					world.findClosestCollision(
							entity,
							voxelShape,
							vec3d,
							dimensions.width(),
