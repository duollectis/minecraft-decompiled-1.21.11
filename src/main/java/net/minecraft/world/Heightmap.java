...
public class Heightmap {
	...
	public static void populateHeightmaps(Chunk chunk, Set<Heightmap.Type> types) {
		if (!types.isEmpty()) {
			...
			int j = chunk.getTopYInclusive() + 1;
	...
	public void setTo(Chunk chunk, Heightmap.Type type, long[] values) {
		...
		if (ls.length == values.length) {
			...
			LOGGER.warn(
					...
					new Object[]{chunk.getPos(), ls.length, values.length}
			);
	...
	public static enum Type implements StringIdentifiable {
		...
		private static final IntFunction<Heightmap.Type> INDEX_MAPPER = ValueLists.createIndexToValueFunction(
				(Heightmap.Type type) -> type.index, values(), ValueLists.OutOfBoundsHandling.ZERO
		);
		public static final PacketCodec<ByteBuf, Heightmap.Type>
				PACKET_CODEC =
