...
public record Vibration(
			...
			instance -> instance.group(
			                    ...
			                    .apply(
					                    ...
					                    (event, distance, pos, uuid, projectileOwnerUuid) -> new Vibration(
							                    ...
							                    distance,
							                    pos,
							                    (UUID) uuid.orElse(null),
							                    (UUID) projectileOwnerUuid.orElse(null)
					                    )
			                    )
	);
	...
	private static @Nullable UUID getOwnerUuid(@Nullable Entity entity) {
		return entity instanceof ProjectileEntity projectileEntity && projectileEntity.getOwner() != null
		       ? projectileEntity.getOwner().getUuid() : null;
	}
