		...
		public static Codec<Vibrations.ListenerData> CODEC = RecordCodecBuilder.create(
				instance -> instance.group(
						                    Vibration.CODEC
								                    ...
								                    .forGetter(listenerData -> Optional.ofNullable(listenerData.vibration)),
						                    VibrationSelector.CODEC.fieldOf("selector").forGetter(Vibrations.ListenerData::getSelector),
						                    Codecs.NON_NEGATIVE_INT
								                    .fieldOf("event_delay")
								                    .orElse(0)
								                    .forGetter(Vibrations.ListenerData::getDelay)
				                    )
				                    .apply(
						                    instance,
						                    (vibration, selector, delay) -> new Vibrations.ListenerData(
								                    (Vibration) vibration.orElse(
										                    null), selector, delay, true
						                    )
				                    )
		);
			            ...
			            .ifPresent(
					            vibration -> {
						            ...
						            world.spawnParticles(
								            new VibrationParticleEffect(
										            callback.getPositionSource(),
										            listenerData.getDelay()
								            ), vec3d.x, vec3d.y, vec3d.z, 1, 0.0, 0.0, 0.0, 0.0
						            );
						            callback.onListen();
			...
			if (listenerData.shouldSpawnParticle()) {
				if (listenerData.vibration == null) {
					...
					boolean
							bl =
							world.spawnParticles(
									new VibrationParticleEffect(positionSource, i),
									e,
									f,
									g,
									1,
		...
		private static boolean areChunksTickingAround(World world, BlockPos pos) {
			...
			for (int i = chunkPos.x - 1; i <= chunkPos.x + 1; i++) {
				for (int j = chunkPos.z - 1; j <= chunkPos.z + 1; j++) {
					if (!world.shouldTickBlocksInChunk(ChunkPos.toLong(i, j))
							|| world.getChunkManager().getWorldChunk(i, j) == null) {
						return false;
					}
				}
			}
	...
	public static class VibrationListener implements GameEventListener {
		...
		public void forceListen(
			...
			this.receiver
					...
					.getPositionSource()
					.getPos(world)
					.ifPresent(pos -> this.listen(
							world,
							this.receiver.getVibrationListenerData(),
							event,
							emitter,
							emitterPos,
							pos
		...
		private static boolean isOccluded(World world, Vec3d emitterPos, Vec3d listenerPos) {
			...
			for (Direction direction : Direction.values()) {
				...
				if (world
						.raycast(new BlockStateRaycastContext(
								vec3d3,
								vec3d2,
