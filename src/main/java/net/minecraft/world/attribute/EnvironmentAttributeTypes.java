...
/**
 * {@code EnvironmentAttributeTypes}.
 */
public interface EnvironmentAttributeTypes {

	EnvironmentAttributeType<Boolean> BOOLEAN = register(
			"boolean", EnvironmentAttributeType.discrete(Codec.BOOL, EnvironmentAttributeModifier.BOOLEAN_MODIFIERS)
	);

	EnvironmentAttributeType<TriState>
			TRI_STATE =
			register("tri_state", EnvironmentAttributeType.discrete(TriState.CODEC));
	...
	EnvironmentAttributeType<MoonPhase>
			MOON_PHASE =
			register("moon_phase", EnvironmentAttributeType.discrete(MoonPhase.CODEC));

	EnvironmentAttributeType<Activity>
			ACTIVITY =
			register("activity", EnvironmentAttributeType.discrete(Registries.ACTIVITY.getCodec()));
	...
	EnvironmentAttributeType<ParticleEffect>
			PARTICLE =
			register("particle", EnvironmentAttributeType.discrete(ParticleTypes.TYPE_CODEC));
	...
	EnvironmentAttributeType<BackgroundMusic>
			BACKGROUND_MUSIC =
			register("background_music", EnvironmentAttributeType.discrete(BackgroundMusic.CODEC));

	EnvironmentAttributeType<AmbientSounds>
			AMBIENT_SOUNDS =
			register("ambient_sounds", EnvironmentAttributeType.discrete(AmbientSounds.CODEC));
