   ...
   private static <T> List<TrackEvaluator.Segment<T>> convertToSegments(Track<T> track, Optional<Integer> period) {
      ...
      if (list.size() == 1) {
         ...
         if (period.isPresent()) {
            ...
            list2.add(new TrackEvaluator.Segment<>(track, keyframe2, keyframe2.ticks() - period.get(), keyframe, keyframe.ticks()));
            addSegmentsOfKeyframe(track, list, list2);
            list2.add(new TrackEvaluator.Segment<>(track, keyframe2, keyframe2.ticks(), keyframe, keyframe.ticks() + period.get()));
         } else {
            addSegmentsOfKeyframe(track, list, list2);
         }
