...
public record SerializedChunk(
   ...
   public static SerializedChunk fromNbt(HeightLimitView world, PalettesFactory palettesFactory, NbtCompound nbt) {
      if (nbt.getString("Status").isEmpty()) {
         ...
         UpgradeData upgradeData = nbt.getCompound("UpgradeData").map(upgradeDatax -> new UpgradeData(upgradeDatax, world)).orElse(UpgradeData.NO_UPGRADE_DATA);
         boolean bl = nbt.getBoolean("isLightOn", false);
         BlendingData.Serialized serialized = nbt.<BlendingData.Serialized>get("blending_data", BlendingData.Serialized.CODEC).orElse(null);
         BelowZeroRetrogen belowZeroRetrogen = nbt.<BelowZeroRetrogen>get("below_zero_retrogen", BelowZeroRetrogen.CODEC).orElse(null);
         ...
         List<Tick<Block>> list = Tick.filter(nbt.<List<Tick<Block>>>get("block_ticks", BLOCK_TICKS_CODEC).orElse(List.of()), chunkPos);
         List<Tick<Fluid>> list2 = Tick.filter(nbt.<List<Tick<Fluid>>>get("fluid_ticks", FLUID_TICKS_CODEC).orElse(List.of()), chunkPos);
            ...
            if (!optional.isEmpty()) {
               ...
               if (n >= world.getBottomSectionCoord() && n <= world.getTopSectionCoord()) {
                  PalettedContainer<BlockState> palettedContainer = nbtCompound2.getCompound("block_states")
                     .map(
                        blockStates -> (PalettedContainer<BlockState>)codec2.parse(NbtOps.INSTANCE, blockStates)
                  ...
                  ReadableContainer<RegistryEntry<Biome>> readableContainer = nbtCompound2.getCompound("biomes")
                     .map(
                        biomes -> (ReadableContainer<RegistryEntry<Biome>>)codec.parse(NbtOps.INSTANCE, biomes)
                           .promotePartial(error -> logRecoverableError(chunkPos, n, error))
                           .getOrThrow(SerializedChunk.ChunkLoadingException::new)
                     )
               ...
               ChunkNibbleArray chunkNibbleArray = nbtCompound2.getByteArray("BlockLight").map(ChunkNibbleArray::new).orElse(null);
               ChunkNibbleArray chunkNibbleArray2 = nbtCompound2.getByteArray("SkyLight").map(ChunkNibbleArray::new).orElse(null);
   ...
   public static ChunkStatus getChunkStatus(@Nullable NbtCompound nbt) {
      return nbt != null ? nbt.<ChunkStatus>get("Status", ChunkStatus.CODEC).orElse(ChunkStatus.EMPTY) : ChunkStatus.EMPTY;
   }

   private static WorldChunk.@Nullable EntityLoader getEntityLoadingCallback(ServerWorld world, List<NbtCompound> entities, List<NbtCompound> blockEntities) {
            ...
            if (!optional.isEmpty()) {
               map.put(structure, new LongOpenHashSet(Arrays.stream(optional.get()).filter(packedPos -> {
                  ...
                  if (chunkPos2x.getChebyshevDistance(pos) > 8) {
                     LOGGER.warn("Found invalid structure reference [ {} @ {} ] for chunk {}.", new Object[]{identifier, chunkPos2x, pos});
                     return false;
                  } else {
                     return true;
                  }
