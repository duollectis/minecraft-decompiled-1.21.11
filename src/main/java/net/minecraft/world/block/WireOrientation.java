...
public class WireOrientation {
	...
	public static final PacketCodec<ByteBuf, WireOrientation>
			PACKET_CODEC =
			PacketCodecs.indexed(WireOrientation::fromOrdinal, WireOrientation::ordinal);
	private static final WireOrientation[] VALUES = Util.make(() -> {
		...
		initializeValuesArray(
				new WireOrientation(Direction.UP, Direction.NORTH, WireOrientation.SideBias.LEFT),
				wireOrientations
		);
	...
	private final Map<WireOrientation.SideBias, WireOrientation>
			siblingsBySideBias =
			new EnumMap<>(WireOrientation.SideBias.class);
	...
	private WireOrientation(Direction up, Direction front, WireOrientation.SideBias sideBias) {
		...
		this.directionsByPriority =
				List.of(
						this.front.getOpposite(),
						this.front,
						this.right,
						this.right.getOpposite(),
						this.up.getOpposite(),
						this.up
				);
		this.horizontalDirections =
				this.directionsByPriority
						.stream()
						.filter(directionx -> directionx.getAxis() != this.up.getAxis())
						.toList();
	...
	private static WireOrientation initializeValuesArray(WireOrientation prime, WireOrientation[] valuesOut) {
		if (valuesOut[prime.ordinal()] != null) {
			...
			for (WireOrientation.SideBias sideBias : WireOrientation.SideBias.values()) {
				prime.siblingsBySideBias.put(
						sideBias,
						initializeValuesArray(new WireOrientation(prime.up, prime.front, sideBias), valuesOut)
				);
			}
			...
			for (Direction direction : Direction.values()) {
				...
				prime.siblingsByFront.put(
						direction,
						initializeValuesArray(new WireOrientation(direction2, direction, prime.sideBias), valuesOut)
				);
			}
			...
			for (Direction direction : Direction.values()) {
				...
				prime.siblingsByUp.put(
						direction,
						initializeValuesArray(new WireOrientation(direction, direction2x, prime.sideBias), valuesOut)
				);
			}
