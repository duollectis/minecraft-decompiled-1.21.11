...
/**
 * {@code VanillaTerrainParametersCreator}.
 */
public class VanillaTerrainParametersCreator {
	...
	public static <C, I extends ToFloatFunction<C>> Spline<C, I> createFactorSpline(
		...
		return Spline.<C, I>builder(continents, IDENTITY)
		             .add(-0.19F, 3.95F)
		             .add(-0.15F, createContinentalOffsetSplineInner(erosion, ridges, ridgesFolded, 6.25F, true, IDENTITY))
		             .add(-0.1F, createContinentalOffsetSplineInner(erosion, ridges, ridgesFolded, 5.47F, true, toFloatFunction))
		             .add(0.03F, createContinentalOffsetSplineInner(erosion, ridges, ridgesFolded, 5.08F, true, toFloatFunction))
		             .add(0.06F, createContinentalOffsetSplineInner(erosion, ridges, ridgesFolded, 4.69F, false, toFloatFunction))
		             .build();
	}
	...
	public static <C, I extends ToFloatFunction<C>> Spline<C, I> createJaggednessSpline(
		...
		return Spline.<C, I>builder(continents, toFloatFunction)
		             .add(-0.11F, 0.0F)
		             .add(0.03F, createErosionRidgesSpline(erosion, ridges, ridgesFolded, 1.0F, 0.5F, 0.0F, 0.0F, toFloatFunction))
		             .add(0.65F, createErosionRidgesSpline(erosion, ridges, ridgesFolded, 1.0F, 1.0F, 1.0F, 0.0F, toFloatFunction))
		             .build();
	}
	...
	private static <C, I extends ToFloatFunction<C>> Spline<C, I> createErosionRidgesSpline(
		...
		float j = -0.5775F;
		Spline<C, I> spline = createRidgesFoldedSpline(ridges, ridgesFolded, f, h, amplifier);
		Spline<C, I> spline2 = createRidgesFoldedSpline(ridges, ridgesFolded, g, i, amplifier);
		return Spline
				.<C, I>builder(erosion, amplifier)
				.add(-1.0F, spline)
				.add(-0.78F, spline2)
				.add(-0.5775F, spline2)
				.add(-0.375F, 0.0F)
	...
	private static <C, I extends ToFloatFunction<C>> Spline<C, I> createRidgesFoldedSpline(
		...
		builder.add(h, 0.0F);
		if (g > 0.0F) {
			builder.add(j, createRidgesSpline(ridges, g, amplifier));
		}
		else {
			builder.add(j, 0.0F);
		}
		...
		if (f > 0.0F) {
			builder.add(1.0F, createRidgesSpline(ridges, f, amplifier));
		}
		else {
	...
	private static <C, I extends ToFloatFunction<C>> Spline<C, I> createContinentalOffsetSplineInner(
		...
		if (bl) {
			...
			Spline<C, I>
					spline3 =
					Spline.<C, I>builder(ridgesFolded, amplifier).add(-0.9F, f).add(-0.69F, spline2).build();
			builder.add(0.35F, f).add(0.45F, spline3).add(0.55F, spline3).add(0.62F, f);
		}
		else {
			Spline<C, I>
					spline2 =
					Spline.<C, I>builder(ridgesFolded, amplifier).add(-0.7F, spline).add(-0.15F, 1.37F).build();
			Spline<C, I>
					spline3 =
					Spline.<C, I>builder(ridgesFolded, amplifier).add(0.45F, spline).add(0.7F, 1.56F).build();
	...
	private static float computeSlope(float f, float g, float h, float i) {
		return (g - f) / (i - h);
	}
	...
	private static <C, I extends ToFloatFunction<C>> Spline<C, I> createRidgesFoldedOffsetSpline(
		...
		if (-0.65F < l && l < 1.0F) {
			...
			float n = computeSlope(i, k, -1.0F, 1.0F);
			if (bl) {
	...
	private static float computeJaggednessThreshold(float f) {
		float g = 1.17F;
		float h = 0.46082947F;
	...
	public static <C, I extends ToFloatFunction<C>> Spline<C, I> createContinentalOffsetSpline(
		...
		float l = 0.5F;
		float m = 0.5F;
		Spline<C, I> spline = createRidgesFoldedOffsetSpline(ridgesFolded, MathHelper.lerp(h, 0.6F, 1.5F), bl2, amplifier);
		Spline<C, I> spline2 = createRidgesFoldedOffsetSpline(ridgesFolded, MathHelper.lerp(h, 0.6F, 1.0F), bl2, amplifier);
		Spline<C, I> spline3 = createRidgesFoldedOffsetSpline(ridgesFolded, h, bl2, amplifier);
		Spline<C, I> spline4 = createRidgesOffsetSpline(
				ridgesFolded,
				continentalness - 0.15F,
				0.5F * h,
				MathHelper.lerp(0.5F, 0.5F, 0.5F) * h,
				0.5F * h,
				0.6F * h,
				0.5F,
				amplifier
		...
		Spline<C, I>
				spline5 =
				createRidgesOffsetSpline(ridgesFolded, continentalness, i * h, f * h, 0.5F * h, 0.6F * h, 0.5F, amplifier);
		Spline<C, I> spline6 = createRidgesOffsetSpline(ridgesFolded, continentalness, i, i, f, g, 0.5F, amplifier);
		Spline<C, I> spline7 = createRidgesOffsetSpline(ridgesFolded, continentalness, i, i, f, g, 0.5F, amplifier);
		Spline<C, I>
				spline8 =
				Spline
						.<C, I>builder(ridgesFolded, amplifier)
						.add(-1.0F, continentalness)
						.add(-0.4F, spline6)
						.add(0.0F, g + 0.07F)
						.build();
		Spline<C, I> spline9 = createRidgesOffsetSpline(ridgesFolded, -0.02F, j, j, f, g, 0.0F, amplifier);
		Spline.Builder<C, I> builder = Spline.<C, I>builder(erosion, amplifier)
		                                     .add(-0.85F, spline)
		                                     .add(-0.7F, spline2)
		                                     .add(-0.4F, spline3)
