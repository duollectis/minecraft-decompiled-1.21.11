...
public class MultiNoiseUtil {
					             ...
					             RecordCodecBuilder.<Pair<MultiNoiseUtil.NoiseHypercube, T>>create(
							                               instance -> instance.group(
									                                                   MultiNoiseUtil.NoiseHypercube.CODEC
											                                                   .fieldOf("parameters")
											                                                   .forGetter((Pair<MultiNoiseUtil.NoiseHypercube, T> p) -> p.getFirst()),
									                                                   entryCodec.forGetter((Pair<MultiNoiseUtil.NoiseHypercube, T> p) -> p.getSecond())
							                                                   )
	...
	public record NoiseHypercube(
		...
		public static final Codec<MultiNoiseUtil.NoiseHypercube> CODEC = RecordCodecBuilder.create(
				instance -> instance.group(
						                    MultiNoiseUtil.ParameterRange.CODEC
								                    .fieldOf("temperature")
								                    .forGetter(noiseHypercube -> noiseHypercube.temperature),
						                    MultiNoiseUtil.ParameterRange.CODEC
								                    .fieldOf("humidity")
								                    .forGetter(noiseHypercube -> noiseHypercube.humidity),
						                    MultiNoiseUtil.ParameterRange.CODEC
								                    .fieldOf("continentalness")
								                    .forGetter(noiseHypercube -> noiseHypercube.continentalness),
						                    MultiNoiseUtil.ParameterRange.CODEC
								                    .fieldOf("erosion")
								                    .forGetter(noiseHypercube -> noiseHypercube.erosion),
						                    MultiNoiseUtil.ParameterRange.CODEC
								                    .fieldOf("depth")
								                    .forGetter(noiseHypercube -> noiseHypercube.depth),
						                    MultiNoiseUtil.ParameterRange.CODEC
								                    .fieldOf("weirdness")
								                    .forGetter(noiseHypercube -> noiseHypercube.weirdness),
	...
	public record ParameterRange(long min, long max) {
		...
		public static MultiNoiseUtil.ParameterRange combine(
				MultiNoiseUtil.ParameterRange min,
				MultiNoiseUtil.ParameterRange max
		) {
			if (min.min() > max.max()) {
				throw new IllegalArgumentException("min > max: " + min + " " + max);
			}
		...
		public MultiNoiseUtil.ParameterRange combine(MultiNoiseUtil.@Nullable ParameterRange other) {
			return other == null ? this : new MultiNoiseUtil.ParameterRange(
	...
	protected static final class SearchTree<T> {
		...
		private final ThreadLocal<MultiNoiseUtil.SearchTree.@Nullable TreeLeafNode<T>>
				previousResultNode =
				new ThreadLocal<>();
			...
			if (entries.isEmpty()) {
				...
				if (i != 7) {
					...
					List<MultiNoiseUtil.SearchTree.TreeLeafNode<T>> list = entries.stream()
					                                                              .map(entry -> new MultiNoiseUtil.SearchTree.TreeLeafNode<>(
							                                                              (MultiNoiseUtil.NoiseHypercube) entry.getFirst(),
							                                                              entry.getSecond()
					                                                              ))
		...
		private static <T> void sortTree(
			...
			for (int i = 1; i < parameterNumber; i++) {
				comparator =
						comparator.thenComparing(createNodeComparator((currentParameter + i) % parameterNumber, abs));
			}
		...
		public T get(MultiNoiseUtil.NoiseValuePoint point, MultiNoiseUtil.NodeDistanceFunction<T> distanceFunction) {
			...
			MultiNoiseUtil.SearchTree.TreeLeafNode<T>
					treeLeafNode =
					this.firstNode.getResultingNode(ls, this.previousResultNode.get(), distanceFunction);
		...
		static final class TreeBranchNode<T> extends MultiNoiseUtil.SearchTree.TreeNode<T> {
			...
			protected MultiNoiseUtil.SearchTree.TreeLeafNode<T> getResultingNode(
				...
				long
						l =
						alternative == null ? Long.MAX_VALUE
				...
				for (MultiNoiseUtil.SearchTree.TreeNode<T> treeNode : this.subTree) {
					...
					if (l > m) {
						MultiNoiseUtil.SearchTree.TreeLeafNode<T>
								treeLeafNode2 =
								treeNode.getResultingNode(otherParameters, treeLeafNode, distanceFunction);
						long
								n =
								treeNode == treeLeafNode2 ? m : distanceFunction.getDistance(
										treeLeafNode2,
										otherParameters
								);
						if (l > n) {
		...
		abstract static class TreeNode<T> {
			...
			@Override
			public String toString() {
				return Arrays.toString((Object[]) this.parameters);
			}
		}
	}
}
