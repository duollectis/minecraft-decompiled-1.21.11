...
public class PointOfInterestStorage extends SerializingRegionBasedStorage<PointOfInterestSet, PointOfInterestSet.Serialized> {
	...
	public Stream<PointOfInterest> getInSquare(
		...
		return ChunkPos
				.stream(new ChunkPos(pos), i)
				.flatMap(chunkPos -> this.getInChunk(typePredicate, chunkPos, occupationStatus))
				.filter(poi -> {
					BlockPos blockPos2 = poi.getPos();
					return Math.abs(blockPos2.getX() - pos.getX()) <= radius
							&& Math.abs(blockPos2.getZ() - pos.getZ()) <= radius;
				});
	}
	...
	public Stream<PointOfInterest> getInCircle(
		...
		int i = radius * radius;
		return this
				.getInSquare(typePredicate, pos, radius, occupationStatus)
				.filter(poi -> poi.getPos().getSquaredDistance(pos) <= i);
	}
	...
	public Stream<BlockPos> getPositions(
		...
		return this
				.getInCircle(typePredicate, pos, radius, occupationStatus)
	...
	public Optional<BlockPos> getPosition(
		...
		List<PointOfInterest>
				list =
				Util.copyShuffled(this.getInCircle(typePredicate, pos, radius, occupationStatus), random);
		return list
				.stream()
	...
	public boolean releaseTicket(BlockPos pos) {
		return this.get(ChunkSectionPos.toLong(pos))
		           ...
		           .orElseThrow(() -> Util.getFatalOrPause(new IllegalStateException(
	...
	protected void onUpdate(long pos) {
		...
		this.pointOfInterestDistanceTracker.update(
				pos,
				this.pointOfInterestDistanceTracker.getInitialLevel(pos),
				false
		);
	}
		          ...
		          .forEach(
				          pos -> {
					          ...
					          PointOfInterestTypes
							          ...
							          .ifPresent(poiType -> populator.accept(
									          pos,
	...
	public void preloadChunks(WorldView world, BlockPos pos, int radius) {
		ChunkSectionPos
				...
				.filter(pair ->
						((Optional<PointOfInterestSet>) pair.getSecond()).map(PointOfInterestSet::isValid).orElse(false)
								== false)
	...
	/**
	 * {@code PointOfInterestDistanceTracker}.
	 */
