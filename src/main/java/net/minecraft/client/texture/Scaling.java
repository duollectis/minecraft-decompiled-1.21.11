...
public interface Scaling {
	...
	public record NineSlice(
			int width,
			int height,
			Scaling.NineSlice.Border border,
		...
		private static DataResult<Scaling.NineSlice> validate(Scaling.NineSlice nineSlice) {
			...
			if (border.left() + border.right() >= nineSlice.width()) {
				return DataResult.error(
						() -> "Nine-sliced texture has no horizontal center slice: " + border.left() + " + "
								+ border.right() + " >= " + nineSlice.width()
				);
			}
			else {
				return border.top() + border.bottom() >= nineSlice.height()
				       ? DataResult.error(
						() -> "Nine-sliced texture has no vertical center slice: " + border.top() + " + "
						      + border.bottom() + " >= " + nineSlice.height()
				)
		...
		public record Border(int left, int top, int right, int bottom) {
			...
			private static final Codec<Scaling.NineSlice.Border> UNIFORM_SIDE_SIZES_CODEC = Codecs.POSITIVE_INT
					.flatComapMap(
							size -> new Scaling.NineSlice.Border(size, size, size, size), border -> {
			...
			static final Codec<Scaling.NineSlice.Border>
					CODEC =
					Codec.either(UNIFORM_SIDE_SIZES_CODEC, DIFFERENT_SIDE_SIZES_CODEC)
					     .xmap(
							     Either::unwrap,
							     border -> border.getUniformSideSize().isPresent() ? Either.left(border)
							                                                       : Either.right(border)
					     );

			private OptionalInt getUniformSideSize() {
				return this.left() == this.top() && this.top() == this.right() && this.right() == this.bottom()
				       ? OptionalInt.of(this.left()) : OptionalInt.empty();
			}
		}
	}
	...
	public record Tile(int width, int height) implements Scaling {
		...
		public static final MapCodec<Scaling.Tile> CODEC = RecordCodecBuilder.mapCodec(
				instance -> instance.group(
						                    Codecs.POSITIVE_INT.fieldOf("width").forGetter(Scaling.Tile::width),
						                    Codecs.POSITIVE_INT.fieldOf("height").forGetter(Scaling.Tile::height)
				                    )
