...
public class SpriteLoader {
   ...
   private static CompletableFuture<List<SpriteContents>> loadAll(SpriteOpener opener, List<AtlasSource.SpriteSource> sources, Executor executor) {
      List<CompletableFuture<SpriteContents>> list = sources.stream()
         .map(source -> CompletableFuture.supplyAsync(() -> source.load(opener), executor))
         .toList();
   ...
   public CompletableFuture<SpriteLoader.StitchResult> load(
      ...
      return CompletableFuture.<List<AtlasSource.SpriteSource>>supplyAsync(() -> AtlasLoader.of(resourceManager, path).loadSources(resourceManager), executor)
