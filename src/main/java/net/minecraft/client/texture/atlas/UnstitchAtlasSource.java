...
public record UnstitchAtlasSource(
	...
	public static final MapCodec<UnstitchAtlasSource> CODEC = RecordCodecBuilder.mapCodec(
			instance -> instance.group(
					                    ...
					                    Codecs
							                    .nonEmptyList(UnstitchAtlasSource.Region.CODEC.listOf())
							                    .fieldOf("regions")
							                    .forGetter(UnstitchAtlasSource::regions),
	...
	public void load(ResourceManager resourceManager, AtlasSource.SpriteRegions regions) {
		...
		if (optional.isPresent()) {
			...
			for (UnstitchAtlasSource.Region region : this.regions) {
				regions.add(
						region.sprite,
						new UnstitchAtlasSource.SpriteRegion(atlasSprite, region, this.divisorX, this.divisorY)
				);
			}
		}
		else {
	...
	static class SpriteRegion implements AtlasSource.SpriteRegion {
		...
		public SpriteContents load(SpriteOpener spriteOpener) {
			try {
				...
				return new SpriteContents(this.region.sprite, new SpriteDimensions(k, l), nativeImage2);
			}
