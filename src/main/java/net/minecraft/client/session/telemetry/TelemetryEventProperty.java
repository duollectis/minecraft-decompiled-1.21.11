...
/**
 * Типизированное свойство события телеметрии. Содержит идентификатор,
 * ключ экспорта, codec для сериализации и экспортёр для отправки значения
 * в {@link TelemetryPropertyContainer}.
 *
...
public record TelemetryEventProperty<T>(
	...
	String exportKey,
	Codec<T> codec,
	TelemetryEventProperty.PropertyExporter<T> exporter
) {
	...
	public static final TelemetryEventProperty<UUID> MINECRAFT_SESSION_ID =
		ofUuid("minecraft_session_id", "deviceSessionId");
	...
	public static final TelemetryEventProperty<String> OPERATING_SYSTEM =
		ofString("operating_system", "buildPlatform");
	...
	public static TelemetryEventProperty<UUID> ofUuid(String id, String exportKey) {
		return of(
			...
			exportKey,
			Uuids.STRING_CODEC,
			(container, key, value) -> container.addProperty(key, value.toString())
		);
	}

	public static TelemetryEventProperty<GameLoadTimeEvent.Measurement> ofTimeMeasurement(
		String id,
		String exportKey
	) {
		return of(
			id,
			exportKey,
			GameLoadTimeEvent.Measurement.CODEC,
			(container, key, value) -> container.addProperty(key, value.millis())
		);
	}

	/**
	 * Сериализует значение свойства из длинного списка в строку с разделителем {@code ;}.
	 * Формат экспорта: {@code "val1;val2;val3"}.
	 */
	public static TelemetryEventProperty<LongList> ofLongList(String id, String exportKey) {
		return of(
			...
			(container, key, value) -> container.addProperty(
				key,
				value.longStream().mapToObj(String::valueOf).collect(Collectors.joining(";"))
			)
		);
	}

	/**
	 * Записывает значение этого свойства из {@code map} в {@code container}.
	 * Если значение отсутствует в карте, записывает {@code null}-свойство.
	 *
	 * @param map       карта свойств события
	 * @param container контейнер для экспорта в систему телеметрии
	 */
	public void addTo(PropertyMap map, TelemetryPropertyContainer container) {
		T value = map.get(this);
		if (value != null) {
			exporter.apply(container, exportKey, value);
		} else {
			container.addNullProperty(exportKey);
		}
	}

	public MutableText getTitle() {
		return Text.translatable("telemetry.property." + id + ".title");
	}

	@Override
	public String toString() {
		return "TelemetryProperty[" + id + "]";
	}

	/**
	 * Режим игры для телеметрии. Значение {@code rawId} соответствует
	 * числовому идентификатору режима в протоколе Minecraft.
	 */
	@Environment(EnvType.CLIENT)
	public enum GameMode implements StringIdentifiable {
		...
		public static final Codec<TelemetryEventProperty.GameMode> CODEC =
			StringIdentifiable.createCodec(TelemetryEventProperty.GameMode::values);

		private final String id;
		private final int rawId;

		GameMode(String id, int rawId) {
			this.id = id;
			this.rawId = rawId;
		}
	...
	/**
	 * Функциональный интерфейс для экспорта значения свойства
	 * в {@link TelemetryPropertyContainer}.
	 *
	 * @param <T> тип экспортируемого значения
	 */
	@Environment(EnvType.CLIENT)
	...
	/**
	 * Тип сервера для телеметрии: Realm, локальный или сторонний.
	 */
	@Environment(EnvType.CLIENT)
	public enum ServerType implements StringIdentifiable {
		...
		public static final Codec<TelemetryEventProperty.ServerType> CODEC =
			StringIdentifiable.createCodec(TelemetryEventProperty.ServerType::values);
