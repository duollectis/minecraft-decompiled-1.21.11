...
public class RenderLayers {
   ...
   private static final Function<Identifier, RenderLayer> TEXT_POLYGON_OFFSET = Util.memoize(
      texture -> RenderLayer.of(
         "text_polygon_offset",
         RenderSetup.builder(RenderPipelines.RENDERTYPE_TEXT_POLYGON_OFFSET).texture("Sampler0", texture).useLightmap().translucent().build()
      )
   );
   ...
   private static final RenderLayer DEBUG_FILLED_BOX = RenderLayer.of(
      "debug_filled_box",
      RenderSetup.builder(RenderPipelines.DEBUG_FILLED_BOX).translucent().layeringTransform(LayeringTransform.VIEW_OFFSET_Z_LAYERING).build()
   );
