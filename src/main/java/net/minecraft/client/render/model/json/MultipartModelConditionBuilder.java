...
/**
 * {@code MultipartModelConditionBuilder}.
 */
public class MultipartModelConditionBuilder {
	...
	public final <T extends Comparable<T>> MultipartModelConditionBuilder put(Property<T> property, T value) {
		this.putTerms(
				property,
				new SimpleMultipartModelSelector.Terms(List.of(new SimpleMultipartModelSelector.Term(
						property.name(value),
						false
				)))
		);
	...
	@SafeVarargs
	public final <T extends Comparable<T>> MultipartModelConditionBuilder put(
			Property<T> property,
			T value,
			T... values
	) {
		List<SimpleMultipartModelSelector.Term> list = Stream.concat(Stream.of(value), Stream.of(values))
		                                                     ...
		                                                     .sorted()
		                                                     .distinct()
		                                                     .map(valuex -> new SimpleMultipartModelSelector.Term(
				                                                     valuex,
				                                                     false
		                                                     ))
		                                                     .toList();
