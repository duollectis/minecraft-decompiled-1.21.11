...
/**
 * {@code BakedQuadFactory}.
 */
public class BakedQuadFactory {

	private static final Vector3fc TRANSFORM_CENTER = new Vector3f(0.5F, 0.5F, 0.5F);
	...
	public static BakedQuad bake(
		...
		return new BakedQuad(
				...
				Objects.requireNonNullElse(direction2, Direction.UP),
				sprite,
				bl,
				i
		);
	...
	private static void packVertexData(
		...
		Vector3f vector3f = corner.resolve(vector3fc, vector3fc2).div(16.0F);
		...
		if (affineTransformation != AffineTransformation.identity()) {
			transformVertex(vector3f, TRANSFORM_CENTER, affineTransformation.getMatrix());
		}
		...
		vector3fcs[i] = arg.internVector(vector3f);
	...
	private static @Nullable Direction decodeDirection(Vector3fc[] vector3fcs) {
		Vector3f vector3f = new Vector3f();
		GeometryUtils.normal(vector3fcs[0], vector3fcs[1], vector3fcs[2], vector3f);
		return findDirectionByNormal(vector3f);
	}

	private static @Nullable Direction findDirectionByNormal(Vector3f vector3f) {
	...
	private static void encodeDirection(Vector3fc[] vector3fcs, long[] ls, Direction direction) {
		...
		for (int p = 0; p < 4; p++) {
			...
			float nx = corner.xSide().selectFloat(f, g, h, i, j, k);
			float ox = corner.ySide().selectFloat(f, g, h, i, j, k);
			float q = corner.zSide().selectFloat(f, g, h, i, j, k);
			int r = findVertexIndex(vector3fcs, p, nx, ox, q);
			if (r == -1) {
				throw new IllegalStateException("Can't find vertex to swap");
			}

			if (r != p) {
				swapVertices(vector3fcs, r, p);
				swapVertexData(ls, r, p);
			}
		}
	}

	private static int findVertexIndex(Vector3fc[] vector3fcs, int i, float f, float g, float h) {
		for (int j = i; j < 4; j++) {
	...
	private static void swapVertices(Vector3fc[] vector3fcs, int i, int j) {
		Vector3fc vector3fc = vector3fcs[i];
		vector3fcs[i] = vector3fcs[j];
		vector3fcs[j] = vector3fc;
	}

	private static void swapVertexData(long[] ls, int i, int j) {
		long l = ls[i];
		ls[i] = ls[j];
		ls[j] = l;
	}
