			...
			if (bl || bl2 || bl3) {
				for (Entry<Direction, ModelElementFace> entry : modelElement.faces().entrySet()) {
					...
					if (bl4) {
						Sprite
								sprite =
								baker.getSpriteGetter().get(modelTextures, modelElementFace.textureId(), simpleModel);
						BakedQuad bakedQuad = BakedQuadFactory.bake(
								baker.getVertexInterner(),
								vector3fc,
								vector3fc2,
								modelElementFace,
								sprite,
						...
						if (modelElementFace.cullFace() == null) {
							...
							builder.add(
									Direction.transform(
											settings.getRotation().getMatrix(),
