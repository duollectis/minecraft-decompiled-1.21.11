...
public record ModelTransformation(
	...
	protected static class Deserializer implements JsonDeserializer<ModelTransformation> {
		...
		public ModelTransformation deserialize(
			...
			Transformation
					transformation5 =
					this.parseModelTransformation(jsonDeserializationContext, jsonObject, ItemDisplayContext.HEAD);
			Transformation
					transformation6 =
					this.parseModelTransformation(jsonDeserializationContext, jsonObject, ItemDisplayContext.GUI);
			Transformation
					transformation7 =
					this.parseModelTransformation(jsonDeserializationContext, jsonObject, ItemDisplayContext.GROUND);
			Transformation
					transformation8 =
					this.parseModelTransformation(jsonDeserializationContext, jsonObject, ItemDisplayContext.FIXED);
			Transformation
					transformation9 =
					this.parseModelTransformation(jsonDeserializationContext, jsonObject, ItemDisplayContext.ON_SHELF);
		...
		private Transformation parseModelTransformation(
				JsonDeserializationContext ctx,
				JsonObject json,
				ItemDisplayContext displayContext
		) {
			String string = displayContext.asString();
			return json.has(string) ? (Transformation) ctx.deserialize(json.get(string), Transformation.class)
			                        : Transformation.IDENTITY;
		}
	}
}
