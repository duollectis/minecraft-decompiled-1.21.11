...
/**
 * {@code VillagerClothingFeatureRenderer}.
 */
public class VillagerClothingFeatureRenderer<S extends LivingEntityRenderState & VillagerDataRenderState, M extends EntityModel<S> & ModelWithHat>
	...
	private static final Int2ObjectMap<Identifier> LEVEL_TO_ID = Util.make(
			new Int2ObjectOpenHashMap(), levelToId -> {
	...
	private final Object2ObjectMap<RegistryKey<VillagerType>, VillagerResourceMetadata.HatType>
			villagerTypeToHat =
			new Object2ObjectOpenHashMap();
	private final Object2ObjectMap<RegistryKey<VillagerProfession>, VillagerResourceMetadata.HatType>
			professionToHat =
			new Object2ObjectOpenHashMap();
		...
		if (!livingEntityRenderState.invisible) {
			...
			if (villagerData != null) {
				...
				VillagerResourceMetadata.HatType
						hatType =
						this.getHatType(this.villagerTypeToHat, "type", registryEntry);
				VillagerResourceMetadata.HatType
						hatType2 =
						this.getHatType(this.professionToHat, "profession", registryEntry2);
				...
				boolean bl = hatType2 == VillagerResourceMetadata.HatType.NONE
						|| hatType2 == VillagerResourceMetadata.HatType.PARTIAL
						&& hatType != VillagerResourceMetadata.HatType.FULL;
				M entityModel2 = livingEntityRenderState.baby ? this.babyModel : this.adultModel;
				renderModel(
						bl ? entityModel : entityModel2,
						identifier,
						matrixStack,
				...
				if (!registryEntry2.matchesKey(VillagerProfession.NONE) && !livingEntityRenderState.baby) {
					...
					if (!registryEntry2.matchesKey(VillagerProfession.NITWIT)) {
						Identifier identifier3 = this.getTexture(
								"profession_level",
								(Identifier) LEVEL_TO_ID.get(MathHelper.clamp(
										villagerData.level(),
										1,
										LEVEL_TO_ID.size()
								))
	...
	private Identifier getTexture(String keyType, RegistryEntry<?> entry) {
		return entry
				.getKey()
				.map(key -> this.getTexture(keyType, key.getValue()))
				.orElse(MissingSprite.getMissingSpriteId());
	}

	public <K> VillagerResourceMetadata.HatType getHatType(
			Object2ObjectMap<RegistryKey<K>, VillagerResourceMetadata.HatType> metadataMap,
			String keyType,
			RegistryEntry<K> entry
	) {
		RegistryKey<K> registryKey = entry.getKey().orElse(null);
		return registryKey == null
		       ? VillagerResourceMetadata.HatType.NONE
		       : (VillagerResourceMetadata.HatType) metadataMap.computeIfAbsent(
				       registryKey,
				       object -> this.resourceManager
				                 .getResource(this.getTexture(keyType, registryKey.getValue()))
				                 .flatMap(resource -> {
					                 try {
						                 return resource
						                        .getMetadata()
						                        .decode(VillagerResourceMetadata.SERIALIZER)
						                        .map(VillagerResourceMetadata::hatType);
					                 }
