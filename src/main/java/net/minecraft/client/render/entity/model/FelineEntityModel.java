...
public class FelineEntityModel<T extends FelineEntityRenderState> extends EntityModel<T> {
	...
	private static final float SIT_TAIL_ANGLE = 0.0F;
	private static final float BODY_SIZE_Y = 16.0F;
	private static final float SIT_BACK_LEG_Y = -9.0F;
	...
	public static ModelData getModelData(Dilation dilation) {
		...
		modelPartData.addChild(
				"tail2",
				ModelPartBuilder.create().uv(4, 15).cuboid(-0.5F, 0.0F, 0.0F, 1.0F, 8.0F, 1.0F, dilation2),
				ModelTransform.origin(0.0F, 20.0F, 14.0F)
		);
		ModelPartBuilder
				modelPartBuilder =
	...
	public void setAngles(T felineEntityRenderState) {
		...
		if (felineEntityRenderState.sleepAnimationProgress > 0.0F) {
			this.head.roll =
					MathHelper.lerpAngleDegrees(
							felineEntityRenderState.sleepAnimationProgress,
							this.head.roll,
							-1.2707963F
					);
			this.head.yaw =
			...
			this.upperTail.pitch =
					MathHelper.lerpAngleDegrees(
							felineEntityRenderState.tailCurlAnimationProgress,
							this.upperTail.pitch,
							0.8F
					);
			this.lowerTail.pitch =
					MathHelper.lerpAngleDegrees(
							felineEntityRenderState.tailCurlAnimationProgress,
							this.lowerTail.pitch,
							-0.4F
		...
		if (felineEntityRenderState.headDownAnimationProgress > 0.0F) {
			this.head.pitch =
					MathHelper.lerpAngleDegrees(
							felineEntityRenderState.headDownAnimationProgress,
							this.head.pitch,
							-0.58177644F
					);
		}
