...
public class PandaEntityModel extends QuadrupedEntityModel<PandaEntityRenderState> {
	...
	public static TexturedModelData getTexturedModelData() {
		...
		modelPartData.addChild(
				...
				ModelTransform.origin(0.0F, 11.5F, -17.0F)
		);
		modelPartData.addChild(
				"body",
		...
		ModelPartBuilder
				modelPartBuilder =
				ModelPartBuilder.create().uv(40, 0).cuboid(-3.0F, 0.0F, -3.0F, 6.0F, 9.0F, 6.0F);
	...
	public void setAngles(PandaEntityRenderState pandaEntityRenderState) {
		...
		if (pandaEntityRenderState.sittingAnimationProgress > 0.0F) {
			this.body.pitch =
					MathHelper.lerpAngleRadians(
							pandaEntityRenderState.sittingAnimationProgress,
							this.body.pitch,
							1.7407963F
					);
			this.head.pitch =
		...
		if (pandaEntityRenderState.lieOnBackAnimationProgress > 0.0F) {
			...
			this.head.pitch =
					MathHelper.lerpAngleRadians(
							pandaEntityRenderState.lieOnBackAnimationProgress,
							this.head.pitch,
							(float) (Math.PI / 2)
					);
		}
