...
/**
 * {@code FoxHeldItemFeatureRenderer}.
 */
public class FoxHeldItemFeatureRenderer extends FeatureRenderer<FoxEntityRenderState, FoxEntityModel> {
	...
	public void render(
			...
			int i,
			FoxEntityRenderState foxEntityRenderState,
			float f,
			float g
	) {
		...
		if (!itemRenderState.isEmpty()) {
			...
			matrixStack.translate(
					this.getContextModel().head.originX / 16.0F,
					this.getContextModel().head.originY / 16.0F,
					this.getContextModel().head.originZ / 16.0F
			);
			if (bl2) {
				float h = 0.75F;
			...
			itemRenderState.render(
					...
					foxEntityRenderState.outlineColor
			);
			matrixStack.pop();
		}
	}
