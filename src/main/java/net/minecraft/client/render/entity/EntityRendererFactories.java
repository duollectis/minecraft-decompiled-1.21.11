...
public class EntityRendererFactories {
   ...
   public static Map<EntityType<?>, EntityRenderer<?, ?>> reloadEntityRenderers(EntityRendererFactory.Context ctx) {
      ...
      RENDERER_FACTORIES.forEach((entityType, factory) -> {
         try {
            ...
            throw new IllegalArgumentException("Failed to create model for " + Registries.ENTITY_TYPE.getId((EntityType<?>)entityType), var5);
         }
      });
   ...
   public static boolean isMissingRendererFactories() {
      ...
      for (EntityType<?> entityType : Registries.ENTITY_TYPE) {
         if (entityType != EntityType.PLAYER && entityType != EntityType.MANNEQUIN && !RENDERER_FACTORIES.containsKey(entityType)) {
   ...
   static {
      ...
      register(
         ...
         context -> new PiglinEntityRenderer(
            context, EntityModelLayers.PIGLIN, EntityModelLayers.PIGLIN_BABY, EntityModelLayers.PIGLIN_EQUIPMENT, EntityModelLayers.PIGLIN_BABY_EQUIPMENT
         )
      );
      register(
