...
public class ShulkerEntityRenderer extends MobEntityRenderer<ShulkerEntity, ShulkerEntityRenderState, ShulkerEntityModel> {
	...
	private static final Identifier
			TEXTURE =
			TexturedRenderLayers.SHULKER_TEXTURE_ID.getTextureId().withPath(string -> "textures/" + string + ".png");
	...
	public void updateRenderState(
			...
			ShulkerEntityRenderState shulkerEntityRenderState,
			float f
	) {
		super.updateRenderState(shulkerEntity, shulkerEntityRenderState, f);
		shulkerEntityRenderState.renderPositionOffset =
				Objects.requireNonNullElse(shulkerEntity.getRenderPositionOffset(f), Vec3d.ZERO);
