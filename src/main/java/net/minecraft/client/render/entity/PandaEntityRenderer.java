...
/**
 * {@code PandaEntityRenderer}.
 */
public class PandaEntityRenderer extends AgeableMobEntityRenderer<PandaEntity, PandaEntityRenderState, PandaEntityModel> {

	private static final Map<PandaEntity.Gene, Identifier> TEXTURES = Maps.newEnumMap(
			Map.of(
					PandaEntity.Gene.NORMAL,
					Identifier.ofVanilla("textures/entity/panda/panda.png"),
					PandaEntity.Gene.LAZY,
					Identifier.ofVanilla("textures/entity/panda/lazy_panda.png"),
					PandaEntity.Gene.WORRIED,
					Identifier.ofVanilla("textures/entity/panda/worried_panda.png"),
					PandaEntity.Gene.PLAYFUL,
					Identifier.ofVanilla("textures/entity/panda/playful_panda.png"),
					PandaEntity.Gene.BROWN,
					Identifier.ofVanilla("textures/entity/panda/brown_panda.png"),
					PandaEntity.Gene.WEAK,
					Identifier.ofVanilla("textures/entity/panda/weak_panda.png"),
					PandaEntity.Gene.AGGRESSIVE,
					Identifier.ofVanilla("textures/entity/panda/aggressive_panda.png")
			)
	);
	...
	public PandaEntityRenderer(EntityRendererFactory.Context context) {
		super(
				context,
				new PandaEntityModel(context.getPart(EntityModelLayers.PANDA)),
				new PandaEntityModel(context.getPart(EntityModelLayers.PANDA_BABY)),
				0.9F
		);
	...
	public void updateRenderState(PandaEntity pandaEntity, PandaEntityRenderState pandaEntityRenderState, float f) {
		...
		pandaEntityRenderState.rollOverAnimationProgress =
				pandaEntity.isBaby() ? 0.0F : pandaEntity.getRollOverAnimationProgress(f);
		pandaEntityRenderState.playingTicks = pandaEntity.playingTicks > 0 ? pandaEntity.playingTicks + f : 0.0F;
	}

	protected void setupTransforms(
			PandaEntityRenderState pandaEntityRenderState,
		...
		if (h > 0.0F) {
			...
			matrixStack.multiply(RotationAxis.POSITIVE_X.rotationDegrees(MathHelper.lerp(
					h,
					pandaEntityRenderState.pitch,
					pandaEntityRenderState.pitch + 90.0F
			)));
		...
		if (q > 0.0F) {
			...
			matrixStack.multiply(RotationAxis.POSITIVE_X.rotationDegrees(MathHelper.lerp(
					q,
					pandaEntityRenderState.pitch,
					pandaEntityRenderState.pitch + 180.0F
			)));
		}
	}
