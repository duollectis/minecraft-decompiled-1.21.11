...
/**
 * {@code FallingBlockEntityRenderer}.
 */
public class FallingBlockEntityRenderer extends EntityRenderer<FallingBlockEntity, FallingBlockEntityRenderState> {
	...
	public boolean shouldRender(FallingBlockEntity fallingBlockEntity, Frustum frustum, double d, double e, double f) {
		return !super.shouldRender(fallingBlockEntity, frustum, d, e, f)
		       ? false
		       : fallingBlockEntity.getBlockState() != fallingBlockEntity
		                                               .getEntityWorld()
		                                               .getBlockState(fallingBlockEntity.getBlockPos());
	}
	...
	public void render(
		...
		if (blockState.getRenderType() == BlockRenderType.MODEL) {
			...
			orderedRenderCommandQueue.submitMovingBlock(
					matrixStack,
					fallingBlockEntityRenderState.movingBlockRenderState
			);
			matrixStack.pop();
	...
	public void updateRenderState(
		...
		BlockPos
				...
				BlockPos.ofFloored(
						...
						fallingBlockEntity.getBoundingBox().maxY,
		...
		fallingBlockEntityRenderState.movingBlockRenderState.biome =
				fallingBlockEntity.getEntityWorld().getBiome(blockPos);
