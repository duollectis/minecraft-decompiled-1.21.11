...
public class StriderEntityModel extends EntityModel<StriderEntityRenderState> {
	...
	public static TexturedModelData getTexturedModelData() {
		...
		modelPartData.addChild(
				"right_leg",
				ModelPartBuilder.create().uv(0, 32).cuboid(-2.0F, 0.0F, -2.0F, 4.0F, 16.0F, 4.0F),
				ModelTransform.origin(-4.0F, 8.0F, 0.0F)
		);
		modelPartData.addChild(
				"left_leg",
				ModelPartBuilder.create().uv(0, 55).cuboid(-2.0F, 0.0F, -2.0F, 4.0F, 16.0F, 4.0F),
				ModelTransform.origin(4.0F, 8.0F, 0.0F)
		);
		ModelPartData modelPartData2 = modelPartData.addChild(
				"body",
				ModelPartBuilder.create().uv(0, 0).cuboid(-8.0F, -6.0F, -8.0F, 16.0F, 14.0F, 16.0F),
	...
	public void setAngles(StriderEntityRenderState striderEntityRenderState) {
		...
		if (!striderEntityRenderState.hasPassengers) {
			...
			this.body.yaw = striderEntityRenderState.relativeHeadYaw * (float) (Math.PI / 180.0);
		}
		else {
		...
		this.body.originY = this.body.originY - 2.0F * MathHelper.cos(f * 1.5F) * 2.0F * g;
