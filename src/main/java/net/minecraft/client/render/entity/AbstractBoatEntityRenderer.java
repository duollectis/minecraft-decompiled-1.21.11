...
/**
 * {@code AbstractBoatEntityRenderer}.
 */
public abstract class AbstractBoatEntityRenderer extends EntityRenderer<AbstractBoatEntity, BoatEntityRenderState> {
		...
		if (f > 0.0F) {
			matrixStack.multiply(
					RotationAxis.POSITIVE_X
							.rotationDegrees(MathHelper.sin(f) * f * boatEntityRenderState.damageWobbleStrength / 10.0F
									* boatEntityRenderState.damageWobbleSide)
			);
		}

		if (!boatEntityRenderState.submergedInWater
				&& !MathHelper.approximatelyEquals(boatEntityRenderState.bubbleWobble, 0.0F)) {
			matrixStack.multiply(new Quaternionf().setAngleAxis(
					boatEntityRenderState.bubbleWobble * (float) (Math.PI / 180.0),
					1.0F,
					0.0F,
					1.0F
			));
	 ...
	 *
	 * @return BoatEntityRenderState — результат операции
	 */
