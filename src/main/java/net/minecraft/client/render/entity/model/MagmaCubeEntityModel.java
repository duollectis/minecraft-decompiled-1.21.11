...
public class MagmaCubeEntityModel extends EntityModel<SlimeEntityRenderState> {
	...
	public static TexturedModelData getTexturedModelData() {
		...
		for (int i = 0; i < 8; i++) {
			...
			modelPartData.addChild(
					getSliceName(i),
					ModelPartBuilder.create().uv(j, k).cuboid(-4.0F, 16 + i, -4.0F, 8.0F, 1.0F, 8.0F),
					ModelTransform.NONE
			);
		}
		...
		modelPartData.addChild(
				"inside_cube",
				ModelPartBuilder.create().uv(24, 40).cuboid(-2.0F, 18.0F, -2.0F, 4.0F, 4.0F, 4.0F),
				ModelTransform.NONE
		);
	...
	public void setAngles(SlimeEntityRenderState slimeEntityRenderState) {
		...
		for (int i = 0; i < this.slices.length; i++) {
			this.slices[i].originY = -(4 - i) * f * 1.7F;
		}
	}
}
