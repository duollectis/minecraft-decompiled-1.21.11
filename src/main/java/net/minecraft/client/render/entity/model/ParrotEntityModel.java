...
public class ParrotEntityModel extends EntityModel<ParrotEntityRenderState> {
   ...
   public static TexturedModelData getTexturedModelData() {
      ...
      ModelPartData modelPartData2 = modelPartData.addChild(
         "head", ModelPartBuilder.create().uv(2, 2).cuboid(-1.0F, -1.5F, -1.0F, 2.0F, 3.0F, 2.0F), ModelTransform.origin(0.0F, 15.69F, -2.76F)
      );
      modelPartData2.addChild(
         "head2", ModelPartBuilder.create().uv(10, 0).cuboid(-1.0F, -0.5F, -2.0F, 2.0F, 1.0F, 4.0F), ModelTransform.origin(0.0F, -2.0F, -1.0F)
      );
      modelPartData2.addChild(
         "beak1", ModelPartBuilder.create().uv(11, 7).cuboid(-0.5F, -1.0F, -0.5F, 1.0F, 2.0F, 1.0F), ModelTransform.origin(0.0F, -0.5F, -1.5F)
      );
      modelPartData2.addChild(
         "beak2", ModelPartBuilder.create().uv(16, 7).cuboid(-0.5F, 0.0F, -0.5F, 1.0F, 2.0F, 1.0F), ModelTransform.origin(0.0F, -1.75F, -2.45F)
      );
      modelPartData2.addChild(
         "feather",
   ...
   public void setAngles(ParrotEntityRenderState parrotEntityRenderState) {
      ...
      this.head.yaw = parrotEntityRenderState.relativeHeadYaw * (float) (Math.PI / 180.0);
      switch (parrotEntityRenderState.parrotPose) {
         case STANDING:
            this.leftLeg.pitch = this.leftLeg.pitch
               + MathHelper.cos(parrotEntityRenderState.limbSwingAnimationProgress * 0.6662F) * 1.4F * parrotEntityRenderState.limbSwingAmplitude;
            this.rightLeg.pitch = this.rightLeg.pitch
               + MathHelper.cos(parrotEntityRenderState.limbSwingAnimationProgress * 0.6662F + (float) Math.PI)
                  * 1.4F
                  * parrotEntityRenderState.limbSwingAmplitude;
         case FLYING:
         case ON_SHOULDER:
         default:
            ...
            this.tail.pitch = this.tail.pitch
               + MathHelper.cos(parrotEntityRenderState.limbSwingAnimationProgress * 0.6662F) * 0.3F * parrotEntityRenderState.limbSwingAmplitude;
            this.tail.originY += h;
            this.body.originY += h;
