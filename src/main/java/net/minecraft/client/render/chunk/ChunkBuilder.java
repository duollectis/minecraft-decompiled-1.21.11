...
public class ChunkBuilder {
   ...
   public String getDebugString() {
      return String.format(
         Locale.ROOT, "pC: %03d, pU: %02d, aB: %02d", this.scheduler.size(), this.uploadQueue.size(), this.buffersPool.getAvailableBuilderCount()
      );
   }
   ...
   public class BuiltChunk {
      ...
      public float method_76298(long l) {
         ...
         return this.field_64453 == 0L || m >= this.field_64453
            ? 1.0F
            : (float)m / (float)this.field_64453;
      }
      ...
      VertexSorter getVertexSorter(ChunkSectionPos sectionPos) {
         ...
         return VertexSorter.byDistance(
            (float)(vec3d.x - sectionPos.getMinX()), (float)(vec3d.y - sectionPos.getMinY()), (float)(vec3d.z - sectionPos.getMinZ())
         );
      }
               ...
               if (this.cancelled.get()) {
                  ...
                  try (ScopedProfiler scopedProfiler = Profilers.get().scoped("Compile Section")) {
                     renderData = ChunkBuilder.this.sectionBuilder
                        .build(chunkSectionPos, this.region, BuiltChunk.this.getVertexSorter(chunkSectionPos), buffers);
                  }
