...
/**
 * {@code EntityHitboxDebugRenderer}.
 */
						...
						if (entity2 != null) {
							...
							GizmoDrawing.text(
									...
									entity
											...
											.add(0.0, entity.getBoundingBox().getLengthY() + 1.5, 0.0),
									TextGizmo.Style.left(-65536)
							);
						}
					}
				}
	...
	private void drawHitbox(Entity entity, float tickProgress, boolean inLocalServer) {
		...
		if (entity instanceof LivingEntity) {
			...
			GizmoDrawing.box(
					new Box(
							box.minX,
							box.minY + entity.getStandingEyeHeight() - 0.01F,
							box.minZ,
							box.maxX,
							box.minY + entity.getStandingEyeHeight() + 0.01F,
							box.maxZ
		...
		if (entity instanceof EnderDragonEntity enderDragonEntity) {
			for (EnderDragonPart enderDragonPart : enderDragonEntity.getBodyParts()) {
				...
				GizmoDrawing.box(
						enderDragonPart.getBoundingBox().offset(vec3d7),
						DrawStyle.stroked(ColorHelper.fromFloats(1.0F, 0.25F, 1.0F, 0.0F))
				);
			}
		}
