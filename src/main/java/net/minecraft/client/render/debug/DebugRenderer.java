...
public class DebugRenderer {
	...
	public static Optional<Entity> getTargetedEntity(@Nullable Entity entity, int maxDistance) {
		if (entity == null) {
			...
			EntityHitResult
					entityHitResult =
					ProjectileUtil.raycast(entity, vec3d, vec3d3, box, EntityPredicates.CAN_HIT, i);
			if (entityHitResult == null) {
				return Optional.empty();
			}
			else {
				return vec3d.squaredDistanceTo(entityHitResult.getPos()) > i ? Optional.empty()
				                                                             : Optional.of(entityHitResult.getEntity());
			}
		}
	}
