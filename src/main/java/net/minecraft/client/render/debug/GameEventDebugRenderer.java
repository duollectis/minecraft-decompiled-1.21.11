...
public class GameEventDebugRenderer implements DebugRenderer.Renderer {
	...
	private void forEachEventData(DebugDataStore dataStore, GameEventDebugRenderer.EventConsumer consumer) {
		dataStore.forEachBlockData(
				DebugSubscriptionTypes.GAME_EVENT_LISTENERS,
				(pos, data) -> consumer.accept(pos.toCenterPos(), data.listenerRadius())
		);
		dataStore.forEachEntityData(
				DebugSubscriptionTypes.GAME_EVENT_LISTENERS,
				(entity, data) -> consumer.accept(entity.getEntityPos(), data.listenerRadius())
		);
	}
	...
	public void render(
		...
		this.forEachEventData(
				store, (vec3d, i) -> {
					...
					GizmoDrawing.box(
							Box.of(vec3d, d, d, d),
							DrawStyle.filled(ColorHelper.fromFloats(0.35F, 1.0F, 1.0F, 0.0F))
					);
				}
		);
		this.forEachEventData(
				store,
				(vec3d, i) -> GizmoDrawing.box(
						Box.of(vec3d, 0.5, 1.0, 0.5).offset(0.0, 0.5, 0.0),
						DrawStyle.filled(ColorHelper.fromFloats(0.35F, 1.0F, 1.0F, 0.0F))
				)
		);
		this.forEachEventData(
				store, (vec3d, i) -> {
					...
					GizmoDrawing.text(
							BlockPos.ofFloored(vec3d).toString(),
							vec3d.add(0.0, 1.5, 0.0),
							TextGizmo.Style.left(-6959665).scaled(0.4F)
					);
				}
		);
		store.forEachEvent(
				DebugSubscriptionTypes.GAME_EVENTS, (gameEventDebugData, i, j) -> {
					...
					GizmoDrawing.text(
							gameEventDebugData.event().getIdAsString(),
							vec3d.add(0.0, 0.85, 0.0),
							TextGizmo.Style.left(-7564911).scaled(0.12F)
					);
				}
		);
	}
