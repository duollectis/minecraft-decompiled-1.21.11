...
public class BuiltChunkStorage {
	...
	protected void createChunks(ChunkBuilder chunkBuilder) {
		if (!MinecraftClient.getInstance().isOnThread()) {
			throw new IllegalStateException(
					"createSections called from wrong thread: " + Thread.currentThread().getName());
		}
		else {
			...
			for (int j = 0; j < this.sizeX; j++) {
				for (int k = 0; k < this.sizeY; k++) {
					for (int l = 0; l < this.sizeZ; l++) {
						...
						this.chunks[m] =
								chunkBuilder.new BuiltChunk(
										m,
										ChunkSectionPos.asLong(j, k + this.world.getBottomSectionCoord(), l)
								);
					}
				}
			}
	...
	protected ChunkBuilder.@Nullable BuiltChunk getRenderedChunk(BlockPos blockPos) {
		return this.getRenderedChunk(ChunkSectionPos.toLong(blockPos));
	}

	protected ChunkBuilder.@Nullable BuiltChunk getRenderedChunk(long sectionPos) {
	...
	private ChunkBuilder.@Nullable BuiltChunk getRenderedChunk(int sectionX, int sectionY, int sectionZ) {
	...
	private boolean isSectionWithinViewDistance(int sectionX, int sectionY, int sectionZ) {
		if (sectionY >= this.world.getBottomSectionCoord() && sectionY <= this.world.getTopSectionCoord()) {
			return sectionX < this.sectionPos.getSectionX() - this.viewDistance
					       || sectionX > this.sectionPos.getSectionX() + this.viewDistance
			       ? false
			       : sectionZ >= this.sectionPos.getSectionZ() - this.viewDistance
			         && sectionZ <= this.sectionPos.getSectionZ() + this.viewDistance;
		}
		else {
			return false;
