...
public class WaterFogModifier extends FogModifier {
	...
	public void applyStartEndModifier(
		...
		data.environmentalStart =
				camera
						...
						.get(EnvironmentAttributes.WATER_FOG_START_DISTANCE_VISUAL, g);
		data.environmentalEnd =
				camera
						.getEnvironmentAttributeInterpolator()
						.get(EnvironmentAttributes.WATER_FOG_END_DISTANCE_VISUAL, g);
	...
	public int getFogColor(ClientWorld world, Camera camera, int viewDistance, float skyDarkness) {
		return camera
				.getEnvironmentAttributeInterpolator()
				.get(EnvironmentAttributes.WATER_FOG_COLOR_VISUAL, skyDarkness);
	}
}
