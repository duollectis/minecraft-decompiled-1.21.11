...
/**
 * {@code AtmosphericFogModifier}.
 */
public class AtmosphericFogModifier extends FogModifier {

	private static final int MIN_VIEW_DISTANCE = 8;
	private static final float FOG_START_OFFSET = -160.0F;
	private static final float FOG_END_OFFSET = -256.0F;
	...
	public int getFogColor(ClientWorld world, Camera camera, int viewDistance, float skyDarkness) {
		...
		if (viewDistance >= 4) {
			float
					...
					camera
							...
							.get(EnvironmentAttributes.SUN_ANGLE_VISUAL, skyDarkness) * (float) (Math.PI / 180.0);
			...
			if (h > 0.0F) {
				int
						...
						camera
								.getEnvironmentAttributeInterpolator()
								.get(EnvironmentAttributes.SUNRISE_SUNSET_COLOR_VISUAL, skyDarkness);
		...
		l = applyWeatherToSkyColor(l, world.getRainGradient(skyDarkness), world.getThunderGradient(skyDarkness));
		float g = Math.min(
				camera
						.getEnvironmentAttributeInterpolator()
						.get(EnvironmentAttributes.SKY_FOG_END_DISTANCE_VISUAL, skyDarkness) / 16.0F,
				(float) viewDistance
		);
	...
	private static int applyWeatherToSkyColor(int i, float f, float g) {
		if (f > 0.0F) {
			float h = 1.0F - f * 0.5F;
			float j = 1.0F - f * 0.4F;
	...
	public void applyStartEndModifier(
		...
		this.updateFogMultiplier(camera, clientWorld, renderTickCounter);
		...
		data.skyEnd =
				Math.min(
						...
						camera
								.getEnvironmentAttributeInterpolator()
								.get(EnvironmentAttributes.SKY_FOG_END_DISTANCE_VISUAL, g)
				);
		data.cloudEnd = Math.min(
				(float) (MinecraftClient.getInstance().options.getCloudRenderDistance().getValue() * 16),
	...
	private void updateFogMultiplier(Camera camera, ClientWorld clientWorld, RenderTickCounter renderTickCounter) {
		...
		float
				h =
				MathHelper.clamp(
						(clientWorld.getLightingProvider().get(LightType.SKY).getLightLevel(blockPos) - 8.0F) / 7.0F,
						0.0F,
						1.0F
				);
