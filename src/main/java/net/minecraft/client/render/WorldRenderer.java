...
public class WorldRenderer implements SynchronousResourceReloader, AutoCloseable {
   ...
   public void render(
      ...
      this.worldBorderRendering
         .updateRenderState(
            this.world.getWorldBorder(), f, vec3d, this.client.options.getClampedViewDistance() * 16, this.worldRenderState.worldBorderRenderState
         );
      profiler.pop();
      profiler.swap("debug");
   ...
   private void pushEntityRenders(MatrixStack matrices, WorldRenderState renderStates, OrderedRenderCommandQueue queue) {
      ...
      for (EntityRenderState entityRenderState : renderStates.entityRenderStates) {
         ...
         this.entityRenderManager
            .render(
               entityRenderState, renderStates.cameraRenderState, entityRenderState.x - d, entityRenderState.y - e, entityRenderState.z - f, matrices, queue
            );
      }
   }
         ...
         if (!list.isEmpty() && !(builtChunk.method_76298(Util.getMeasuringTimeMs()) < 0.3F)) {
            for (BlockEntity blockEntity : list) {
               ...
               if (sortedSet != null && !sortedSet.isEmpty()) {
                  ...
                  crumblingOverlayCommand = new ModelCommandRenderer.CrumblingOverlayCommand(sortedSet.last().getStage(), matrixStack.peek());
                  matrixStack.pop();
               } else {
   ...
   private void renderBlockDamage(MatrixStack matrices, VertexConsumerProvider.Immediate immediate, WorldRenderState renderStates) {
      ...
      for (BreakingBlockRenderState breakingBlockRenderState : renderStates.breakingBlockRenderStates) {
         ...
         VertexConsumer vertexConsumer = new OverlayVertexConsumer(
            immediate.getBuffer(ModelBaker.BLOCK_DESTRUCTION_RENDER_LAYERS.get(breakingBlockRenderState.breakProgress)), entry, 1.0F
         );
   ...
   private SectionRenderState renderBlockLayers(Matrix4fc matrix, double cameraX, double cameraY, double cameraZ) {
               ...
               if (m == -1) {
                  ...
                  list.add(
                     new DynamicUniforms.ChunkSectionsValue(
                        new Matrix4f(matrix), blockPos.getX(), blockPos.getY(), blockPos.getZ(), builtChunk.method_76298(l), j, k
                     )
                  );
               }
      ...
      GpuBufferSlice[] gpuBufferSlices = RenderSystem.getDynamicUniforms().writeChunkSections(list.toArray(new DynamicUniforms.ChunkSectionsValue[0]));
                     ...
                     if (skyRenderState.skybox == DimensionType.Skybox.END) {
                        ...
                        if (skyRenderState.endFlashIntensity > 1.0E-5F) {
                           ...
                           skyRendering.drawEndLightFlash(
                              matrixStack, skyRenderState.endFlashIntensity, skyRenderState.endFlashPitch, skyRenderState.endFlashYaw
                           );
                        }
                     } else {
                        MatrixStack matrixStack = new MatrixStack();
