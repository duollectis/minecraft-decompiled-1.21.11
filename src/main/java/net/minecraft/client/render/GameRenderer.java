...
public class GameRenderer implements TrackedWaypoint.PitchProvider, AutoCloseable {
	...
	public static final int WINDOW_FOCUS_DELAY_TICKS = 10;
	private static final Logger LOGGER = LogUtils.getLogger();
	public static final float CAMERA_DEPTH = 0.05F;
	public static final float HUD_FAR_PLANE = 100.0F;
	private static final float NAUSEA_EFFECT_DURATION = 20.0F;
	private static final float NAUSEA_EFFECT_SPEED = 7.0F;
	...
	protected final CubeMapRenderer
			panoramaRenderer =
	...
	protected final RotatingCubeMapRenderer
			...
			new RotatingCubeMapRenderer(this.panoramaRenderer);
	...
	public GameRenderer(
			...
			HeldItemRenderer firstPersonHeldItemRenderer,
	...
	public void renderBlur() {
		PostEffectProcessor
				postEffectProcessor =
				this.client.getShaderLoader().loadPostEffect(BLUR_ID, DefaultFramebufferSet.MAIN_ONLY);
	...
	public void updateCrosshairTarget(float tickProgress) {
		...
		if (entity != null) {
			if (this.client.world != null && this.client.player != null) {
				...
				this.client.crosshairTarget = this.client.player.computeHitResult(tickProgress, entity);
				this.client.targetedEntity =
						this.client.crosshairTarget instanceof EntityHitResult entityHitResult
	...
	private void bobView(MatrixStack matrices, float tickProgress) {
		if (this.client.getCameraEntity() instanceof AbstractClientPlayerEntity abstractClientPlayerEntity) {
			...
			matrices.multiply(RotationAxis.POSITIVE_X.rotationDegrees(
					Math.abs(MathHelper.cos(f * (float) Math.PI - 0.2F) * g) * 5.0F));
		}
	}
	...
	public float getFarPlaneDistance() {
		return Math.max(
				this.viewDistanceBlocks * 4.0F,
				(float) (this.client.options.getCloudRenderDistance().getValue() * 16)
		);
	}
			...
			if (bl && tick && this.client.world != null) {
				...
				if (this.postProcessorId != null && this.postProcessorEnabled) {
					PostEffectProcessor
							...
							this.client
									.getShaderLoader()
									.loadPostEffect(this.postProcessorId, DefaultFramebufferSet.MAIN_ONLY);
					if (postEffectProcessor != null) {
						postEffectProcessor.render(this.client.getFramebuffer(), this.pool);
					}
				}
			...
			if (this.client.getOverlay() != null) {
				try {
					...
					crashReportSection.add(
							"Overlay name",
							() -> this.client.getOverlay().getClass().getCanonicalName()
					);
			...
			if (bl && !((PlayerEntity) entity).getAbilities().allowModifyWorld) {
				...
				if (hitResult != null && hitResult.getType() == HitResult.Type.BLOCK) {
					...
					if (this.client.interactionManager.getCurrentGameMode() == GameMode.SPECTATOR) {
						...
						CachedBlockPosition
								cachedBlockPosition =
								new CachedBlockPosition(this.client.world, blockPos, false);
						Registry<Block>
								registry =
								this.client.world.getRegistryManager().getOrThrow(RegistryKeys.BLOCK);
						bl =
								!itemStack.isEmpty() && (itemStack.canBreak(cachedBlockPosition)
										|| itemStack.canPlaceOn(cachedBlockPosition)
								);
					}
				}
			}
	...
	public void updateCamera(RenderTickCounter renderTickCounter) {
		...
		if (clientPlayerEntity != null && this.client.world != null) {
			...
			Entity
					entity =
					(Entity) (this.client.getCameraEntity() == null ? clientPlayerEntity
	...
	public void renderWorld(RenderTickCounter renderTickCounter) {
		...
		boolean
				bl3 =
				this.client.getCameraEntity() instanceof LivingEntity
						&& ((LivingEntity) this.client.getCameraEntity()).isSleeping();
		RenderSystem.setProjectionMatrix(
				this.hudProjectionMatrix
						.set(
								this.client.getWindow().getFramebufferWidth(),
								this.client.getWindow().getFramebufferHeight(),
								this.getFov(this.camera, f, false)
						),
		...
		RenderSystem
				...
				.clearDepthTexture(this.client.getFramebuffer().getDepthAttachment(), 1.0);
