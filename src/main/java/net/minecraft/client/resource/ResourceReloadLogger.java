...
public class ResourceReloadLogger {
	...
	public void reload(ResourceReloadLogger.ReloadReason reason, List<ResourcePack> packs) {
		...
		this.reloadState =
				new ResourceReloadLogger.ReloadState(
						reason,
						packs.stream().map(ResourcePack::getId).collect(ImmutableList.toImmutableList())
				);
	}
	...
	public void recover(Throwable throwable) {
		if (this.reloadState == null) {
			...
			this.reloadState =
					new ResourceReloadLogger.ReloadState(ResourceReloadLogger.ReloadReason.UNKNOWN, ImmutableList.of());
		}
	...
	static class ReloadState {
		...
		ResourceReloadLogger.@Nullable RecoveryEntry recovery;
		boolean finished;
