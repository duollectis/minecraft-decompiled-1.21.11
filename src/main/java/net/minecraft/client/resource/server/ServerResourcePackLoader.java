...
public class ServerResourcePackLoader implements AutoCloseable {
	...
	private ReloadScheduler.@Nullable ReloadContext reloadContext;
		...
		return new NetworkUtils.DownloadListener() {
			...
			private void showToast() {
				ServerResourcePackLoader.this.client
						.execute(() -> SystemToast.show(
								ServerResourcePackLoader.this.client.getToastManager(),
								this.toastType,
								this.toastTitle,
				...
				if (this.contentLength.isPresent()) {
					this.toastDescription =
							Text.translatable(
									...
									writtenBytes * 100L / this.contentLength.getAsLong()
							);
				}
				else {
					this.toastDescription =
							Text.translatable(
									...
									SizeUnit.getUserFriendlyString(writtenBytes)
							);
				}
	...
	private DownloadQueuer createDownloadQueuer(
		...
		return new DownloadQueuer() {
			...
			private Map<String, String> getHeaders() {
				...
				return Map.of(
						...
						String.valueOf(gameVersion.packVersion(ResourceType.CLIENT_RESOURCES)),
						"User-Agent",
						"Minecraft Java/" + gameVersion.name()
				);
			}
			...
			public void enqueue(
					...
					Consumer<Downloader.DownloadResult> callback
			) {
	...
	private @Nullable List<ResourcePackProfile> toProfiles(List<ReloadScheduler.PackInfo> packs) {
		...
		for (ReloadScheduler.PackInfo packInfo : Lists.reverse(packs)) {
			...
			ResourcePackInfo
					resourcePackInfo =
					new ResourcePackInfo(string, SERVER_NAME_TEXT, this.packSource, Optional.empty());
			...
			ResourcePackProfile.Metadata
					metadata =
					ResourcePackProfile.loadMetadata(
							resourcePackInfo,
							packFactory,
							packVersion,
	...
	private static @Nullable HashCode toHashCode(@Nullable String hash) {
		return hash != null && SHA1_PATTERN.matcher(hash).matches() ? HashCode.fromString(hash.toLowerCase(Locale.ROOT))
		                                                            : null;
	}
				...
				if (id.equals(id)) {
					...
					if (state == PackStateChangeCallback.FinishState.APPLIED) {
						...
						completableFuture.completeExceptionally(new IllegalStateException(
								"Failed to apply pack " + id + ", reason: " + state));
					}
				}
