...
/**
 * {@code AnimationFrameResourceMetadata}.
 */
public record AnimationFrameResourceMetadata(int index, Optional<Integer> time) {
	...
	public static final Codec<AnimationFrameResourceMetadata> CODEC = Codec.either(Codecs.NON_NEGATIVE_INT, BASE_CODEC)
	                                                                       .xmap(
			                                                                       either -> (AnimationFrameResourceMetadata) either.map(
					                                                                       AnimationFrameResourceMetadata::new,
					                                                                       metadata -> metadata
			                                                                       ),
			                                                                       metadatax ->
					                                                                       metadatax.time.isPresent()
					                                                                       ? Either.right(metadatax)
					                                                                       : Either.left(metadatax.index)
	                                                                       );
