...
/**
 * {@code AnimationResourceMetadata}.
 */
public record AnimationResourceMetadata(
		Optional<List<AnimationFrameResourceMetadata>> frames,
	...
	public static final Codec<AnimationResourceMetadata> CODEC = RecordCodecBuilder.create(
			instance -> instance.group(
					                    AnimationFrameResourceMetadata.CODEC
							                    .listOf()
							                    .optionalFieldOf("frames")
							                    .forGetter(AnimationResourceMetadata::frames),
					                    ...
					                    Codecs.POSITIVE_INT
							                    .optionalFieldOf("frametime", 1)
							                    .forGetter(AnimationResourceMetadata::defaultFrameTime),
	...
	public static final ResourceMetadataSerializer<AnimationResourceMetadata>
			SERIALIZER =
			new ResourceMetadataSerializer<>("animation", CODEC);

	public SpriteDimensions getSize(int defaultWidth, int defaultHeight) {
		if (this.width.isPresent()) {
			return this.height.isPresent() ? new SpriteDimensions(this.width.get(), this.height.get())
			                               : new SpriteDimensions(this.width.get(), defaultHeight);
		}
