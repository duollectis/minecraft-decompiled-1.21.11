...
/**
 * {@code WaypointStyleAsset}.
 */
public record WaypointStyleAsset(
		int nearDistance,
		int farDistance,
		List<Identifier> sprites,
		List<Identifier> spriteLocations
) {

	@VisibleForTesting
	public static final String LOCATOR_BAR_DOT_PATH = "hud/locator_bar_dot/";
	...
	public static final Codec<WaypointStyleAsset> CODEC = RecordCodecBuilder.<WaypointStyleAsset>create(
			                                                                        instance -> instance.<Integer, Integer, List<Identifier>>group(
					                                                                                            ...
					                                                                                            Codecs
							                                                                                            ...
							                                                                                            .fieldOf("sprites")
							                                                                                            .forGetter(WaypointStyleAsset::sprites)
			                                                                                            )
			                                                                                            .apply(instance, WaypointStyleAsset::new)
	                                                                        )
	                                                                        .validate(asset -> asset.validate());
	...
	public WaypointStyleAsset(int nearDistance, int farDistance, List<Identifier> sprites) {
		this(
				...
				farDistance,
				sprites,
				sprites.stream().map(id -> id.withPrefixedPath("hud/locator_bar_dot/")).toList()
		);
	}
	...
	public DataResult<WaypointStyleAsset> validate() {
		if (this.sprites.isEmpty()) {
			...
			return this.nearDistance >= this.farDistance
			       ? DataResult.error(() -> "Far distance (" + this.farDistance
			                                + ") cannot be closer or equal to near distance (" + this.nearDistance
			                                + ")")
	...
	public Identifier getSpriteForDistance(float distance) {
		if (distance < this.nearDistance) {
			...
			int
					i =
					MathHelper.lerp(
							(distance - this.nearDistance) / (this.farDistance - this.nearDistance),
							1,
							this.spriteLocations.size() - 1
					);
