...
public record ScreenRect(ScreenPos position, int width, int height) {
	...
	public int getCenter(NavigationAxis axis) {
		return (this.getBoundingCoordinate(axis.getPositiveDirection())
				+ this.getBoundingCoordinate(axis.getNegativeDirection())
		) / 2;
	}
	...
	public ScreenRect transform(Matrix3x2fc matrix) {
		...
		return new ScreenRect(
				MathHelper.floor(vector2f.x),
				MathHelper.floor(vector2f.y),
				MathHelper.floor(vector2f2.x - vector2f.x),
				MathHelper.floor(vector2f2.y - vector2f.y)
		);
	}
