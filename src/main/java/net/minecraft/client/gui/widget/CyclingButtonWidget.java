...
public class CyclingButtonWidget<T> extends PressableWidget implements Updatable {
   ...
   private Text composeText(T value) {
      return (Text)(this.labelType == CyclingButtonWidget.LabelType.VALUE ? this.valueToText.apply(value) : this.composeGenericOptionText(value));
   }
   ...
   public static CyclingButtonWidget.Builder<Boolean> onOffBuilder(Text on, Text off, boolean defaultValue) {
      return new CyclingButtonWidget.Builder<>(value -> value == Boolean.TRUE ? on : off, () -> defaultValue).values(BOOLEAN_VALUES);
   }

   public static CyclingButtonWidget.Builder<Boolean> onOffBuilder(boolean defaultValue) {
      return new CyclingButtonWidget.Builder<>(value -> value == Boolean.TRUE ? ScreenTexts.ON : ScreenTexts.OFF, () -> defaultValue).values(BOOLEAN_VALUES);
   }
