...
public class MerchantScreen extends HandledScreen<MerchantScreenHandler> {
	...
	private static final Identifier
			...
			Identifier.ofVanilla("container/villager/experience_bar_background");
	private static final Identifier
			EXPERIENCE_BAR_CURRENT_TEXTURE =
			Identifier.ofVanilla("container/villager/experience_bar_current");
	private static final Identifier
			EXPERIENCE_BAR_RESULT_TEXTURE =
			Identifier.ofVanilla("container/villager/experience_bar_result");
	...
	private static final Identifier
			...
			Identifier.ofVanilla("container/villager/scroller_disabled");
	private static final Identifier
	...
	private static final Identifier
			...
			Identifier.ofVanilla("container/villager/discount_strikethrough");
	...
	private static final int TRADE_LIST_VISIBLE_ROWS = 6;
	private static final int MAX_TRADE_OFFERS = 7;
	private static final int TRADE_ITEM_PADDING = 5;
	...
	protected void init() {
		...
		for (int l = 0; l < 7; l++) {
			this.offers[l] = this.addDrawableChild(new MerchantScreen.WidgetButtonPage(
					i + 5, k, l, button -> {
	...
	protected void drawForeground(DrawContext context, int mouseX, int mouseY) {
		...
		if (i > 0 && i <= 5 && this.handler.isLeveled()) {
			...
			context.drawText(
					this.textRenderer,
					this.title,
					49 + this.backgroundWidth / 2 - this.textRenderer.getWidth(this.title) / 2,
					6,
					-12566464,
					false
			);
	...
	public void renderMain(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
		...
		if (!tradeOfferList.isEmpty()) {
			...
			TradeOffer tradeOfferx = tradeOfferList.get(o);
			if (this.handler.isLeveled()) {
				this.drawLevelInfo(context, i, j, tradeOfferx);
			}

			if (tradeOfferx.isDisabled() && this.isPointWithinBounds(186, 35, 22, 21, mouseX, mouseY)
		...
		public void renderTooltip(DrawContext context, int x, int y) {
			if (this.hovered && MerchantScreen.this.handler.getRecipes().size()
				...
				if (x < this.getX() + 20) {
					ItemStack
							itemStack =
							MerchantScreen.this.handler
									.getRecipes()
									.get(this.index + MerchantScreen.this.indexStartOffset)
					...
					ItemStack itemStack = MerchantScreen.this.handler
							.getRecipes()
							.get(this.index + MerchantScreen.this.indexStartOffset)
