...
public class LoomScreen extends HandledScreen<LoomScreenHandler> {
	...
	private static final Identifier
			...
			Identifier.ofVanilla("container/loom/scroller_disabled");
	...
	private static final Identifier
			...
			Identifier.ofVanilla("container/loom/pattern_highlighted");
			...
			for (int q = 0; q < 4; q++) {
				for (int r = 0; r < 4; r++) {
					...
					if (t == this.handler.getSelectedPattern()) {
						...
						context.drawTooltip(
								Text.translatable(registryEntry.value().translationKey() + "." + dyeColor2.getId()),
								mouseX,
								mouseY
						);
		...
		if (this.canApplyDyePattern) {
			...
			for (int k = 0; k < 4; k++) {
				for (int l = 0; l < 4; l++) {
					...
					int n = m * 4 + l;
					if (d >= 0.0 && e >= 0.0 && d < 14.0 && e < 14.0 && this.handler.onButtonClick(
							this.client.player,
							n
					)) {
						MinecraftClient
								.getInstance()
								.getSoundManager()
								.play(PositionedSoundInstance.master(SoundEvents.UI_LOOM_SELECT_PATTERN, 1.0F));
	...
	private void onInventoryChanged() {
		...
		BannerPatternsComponent
				bannerPatternsComponent =
				itemStack2.getOrDefault(DataComponentTypes.BANNER_PATTERNS, BannerPatternsComponent.DEFAULT);
		this.hasTooManyPatterns = bannerPatternsComponent.layers().size() >= 6;
		if (this.hasTooManyPatterns) {
			this.bannerPatterns = null;
		}

		if (!ItemStack.areEqual(itemStack2, this.banner) || !ItemStack.areEqual(itemStack3, this.dye)
				|| !ItemStack.areEqual(itemStack4, this.pattern)) {
			this.canApplyDyePattern =
					!itemStack2.isEmpty() && !itemStack3.isEmpty() && !this.hasTooManyPatterns && !this.handler
							.getBannerPatterns()
							.isEmpty();
		}
