...
public class CreativeInventoryScreen extends HandledScreen<CreativeInventoryScreen.CreativeScreenHandler> implements FabricCreativeInventoryScreen {
   ...
   public CreativeInventoryScreen(ClientPlayerEntity player, FeatureSet enabledFeatures, boolean operatorTabEnabled) {
      ...
      this.populateDisplay(
         player.networkHandler.getSearchManager(), enabledFeatures, this.shouldShowOperatorTab(player), player.getEntityWorld().getRegistryManager()
      );
      ...
      if (selectedTab.getType() != ItemGroup.Type.SEARCH) {
         ...
         if (bl && bl2 && this.handleHotbarKeyPressed(input)) {
            ...
            if (this.searchBox.keyPressed(input)) {
               ...
               return this.searchBox.isFocused() && this.searchBox.isVisible() && !input.isEscape() ? true : super.keyPressed(input);
            }
         }
      }
   }
   ...
   protected void drawBackground(DrawContext context, float deltaTicks, int mouseX, int mouseY) {
      ...
      context.drawTexture(
         RenderPipelines.GUI_TEXTURED, selectedTab.getTexture(), this.x, this.y, 0.0F, 0.0F, this.backgroundWidth, this.backgroundHeight, 256, 256
      );
      ...
      if (selectedTab.hasScrollbar()) {
         ...
         context.drawGuiTexture(RenderPipelines.GUI_TEXTURED, identifier, i, j + (int)((k - j - 17) * this.scrollPosition), 12, 15);
      }
