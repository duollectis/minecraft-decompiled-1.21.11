...
public class CreateWorldScreen extends Screen {
   ...
   public static CreateWorldScreen create(
      MinecraftClient client, Runnable runnable, LevelInfo levelInfo, GeneratorOptionsHolder generatorOptionsHolder, @Nullable Path dataPackTempDir
   ) {
            ...
            context -> {
               if (context.worldGenRegistryManager().getOrThrow(RegistryKeys.WORLD_PRESET).streamEntries().findAny().isEmpty()) {
                  ...
                  return new SaveLoading.LoadContext<>(
                     new WorldCreationSettings(worldGenSettings, context.dataConfiguration()), context.dimensionsRegistryManager()
                  );
               }
            },
