...
public class RecoverWorldScreen extends Screen {
	...
	private static final Text
			ISSUE_NONE_TEXT =
			Text.translatable("recover_world.issue.none").formatted(Formatting.GREEN);
	private static final Text
			MISSING_FILE_TEXT =
			Text.translatable("recover_world.issue.missing_file").formatted(Formatting.RED);
	...
	public RecoverWorldScreen(MinecraftClient client, BooleanConsumer callback, LevelStorage.Session session) {
		...
		Text
				...
				Text
						.empty()
						.append(this.toText(session, false, exception))
						.append("\n")
		...
		directionalLayoutWidget.add(
				ButtonWidget
						.builder(RESTORE_TEXT, buttonWidget -> this.tryRestore(client))
						.size(120, 20)
						.tooltip(bl ? null : Tooltip.of(NO_FALLBACK_TEXT))
						.build()
		)
				.active = bl;
	...
	private Text toText(LevelStorage.Session session, boolean old, @Nullable Exception exception) {
		if (old && exception instanceof FileNotFoundException) {
			...
			MutableText mutableText2 = instant != null
			                           ? Text.literal(WorldListWidget.DATE_FORMAT.format(ZonedDateTime.ofInstant(
					instant,
