         ...
         List<LanguageOptionsScreen.LanguageSelectionListWidget.LanguageEntry> list = sortedMap.entrySet()
            ...
            .filter(
               entry -> search.isEmpty()
                  || entry.getValue().name().toLowerCase(Locale.ROOT).contains(search.toLowerCase(Locale.ROOT))
                  || entry.getValue().region().toLowerCase(Locale.ROOT).contains(search.toLowerCase(Locale.ROOT))
            )
            .map(entry -> new LanguageOptionsScreen.LanguageSelectionListWidget.LanguageEntry(entry.getKey(), entry.getValue()))
            .toList();
