...
public class TestInstanceBlockScreen extends Screen {
   ...
   protected void init() {
      ...
      this.addDrawableChild(
         ButtonWidget.builder(ScreenTexts.CANCEL, button -> this.onCancel()).dimensions(this.getX(2, 3), 210, getRoundedWidth(3), 20).build()
      );
      this.refresh(true);
   }
   ...
   public void handleStatus(Text status, Optional<Vec3i> size) {
      ...
      this.testInstanceBlockEntity
         .getErrorMessage()
         .ifPresent(
            message -> mutableText.append(Text.translatable("test_instance.description.failed", Text.empty().formatted(Formatting.RED).append(message)))
               .append("\n\n")
         );
