...
public class AnimatedResultButton extends ClickableWidget {
	...
	public void showResultCollection(
		...
		List<RecipeDisplayEntry> list = resultCollection.filter(
				filteringCraftable ? RecipeResultCollection.RecipeFilterMode.CRAFTABLE
		...
		List<NetworkRecipeId>
				list2 =
				list.stream().map(RecipeDisplayEntry::id).filter(results.getRecipeBook()::isHighlighted).toList();
		if (!list2.isEmpty()) {
