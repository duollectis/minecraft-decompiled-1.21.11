...
public class DrawContext {
   ...
   public void drawText(TextRenderer textRenderer, OrderedText text, int x, int y, int color, boolean shadow) {
      if (ColorHelper.getAlpha(color) != 0) {
         this.state
            .addText(
               new TextGuiElementRenderState(textRenderer, text, new Matrix3x2f(this.matrices), x, y, color, 0, shadow, false, this.scissorStack.peekLast())
            );
      }
   }
   ...
   public void drawGuiTexture(
      RenderPipeline pipeline, Identifier sprite, int textureWidth, int textureHeight, int u, int v, int x, int y, int width, int height
   ) {
   ...
   public void drawOrderedTooltip(TextRenderer textRenderer, List<? extends OrderedText> text, int x, int y, @Nullable Identifier texture) {
      this.drawTooltip(
         textRenderer, text.stream().map(TooltipComponent::of).collect(Collectors.toList()), x, y, HoveredTooltipPositioner.INSTANCE, texture, false
      );
   }
   ...
   public void addBannerResult(
      BannerFlagBlockModel bannerModel, DyeColor baseColor, BannerPatternsComponent resultBannerPatterns, int x1, int y1, int x2, int y2
   ) {
      this.state
   ...
   class TextConsumerImpl implements DrawnTextConsumer, Consumer<Style> {
      ...
      TextConsumerImpl(
         final DrawnTextConsumer.Transformation transformation, final DrawContext.@Nullable HoverType hoverType, final Consumer<Style> styleCallback
      ) {
