...
public class LocatorBar implements Bar {
	...
	public void renderBar(DrawContext context, RenderTickCounter tickCounter) {
		context.drawGuiTexture(
				...
				BACKGROUND,
				this.getCenterX(this.client.getWindow()),
				this.getCenterY(this.client.getWindow()),
				182,
				5
		);
	}
	...
	public void renderAddons(DrawContext context, RenderTickCounter tickCounter) {
		...
		if (entity != null) {
			...
			EntityTickProgress
					entityTickProgress =
					entityx -> tickCounter.getTickProgress(!tickManager.shouldSkipTick(entityx));
			this.client
					.player
					.networkHandler
					.getWaypointHandler()
							...
							waypoint -> {
								if (!waypoint
									...
									if (!(d <= -60.0) && !(d > 60.0)) {
										...
										WaypointStyleAsset
												waypointStyleAsset =
												this.client.getWaypointStyleAssetManager().get(config.style);
										...
										int k = config.color
												.orElseGet(
														() -> (Integer) waypoint.getSource()
														                        .map(
																                        uuid -> ColorHelper.withBrightness(
																                        ...
																                        name -> ColorHelper.withBrightness(
																		                        ColorHelper.withAlpha(
																				                        255,
																				                        name.hashCode()
																		                        ),
										...
										TrackedWaypoint.Pitch
												pitch =
												waypoint.getPitch(world, this.client.gameRenderer, entityTickProgress);
