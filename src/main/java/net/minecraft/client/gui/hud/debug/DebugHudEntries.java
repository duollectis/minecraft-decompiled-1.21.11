...
/**
 * Реестр всех записей отладочного HUD и предустановленных профилей видимости.
 * Записи регистрируются статически при загрузке класса.
 */
...
public class DebugHudEntries {
	...
	/**
	 * Регистрирует запись отладочного HUD под указанным идентификатором.
	 * Используется модами для добавления собственных записей.
	 */
	...
	static {
		Map<Identifier, DebugHudEntryVisibility> defaultProfile = Map.of(
			THREE_DIMENSIONAL_CROSSHAIR, DebugHudEntryVisibility.IN_OVERLAY,
			GAME_VERSION, DebugHudEntryVisibility.IN_OVERLAY,
			TPS, DebugHudEntryVisibility.IN_OVERLAY,
			FPS, DebugHudEntryVisibility.IN_OVERLAY,
			MEMORY, DebugHudEntryVisibility.IN_OVERLAY,
			SYSTEM_SPECS, DebugHudEntryVisibility.IN_OVERLAY,
			PLAYER_POSITION, DebugHudEntryVisibility.IN_OVERLAY,
			PLAYER_SECTION_POSITION, DebugHudEntryVisibility.IN_OVERLAY,
			SIMPLE_PERFORMANCE_IMPACTORS, DebugHudEntryVisibility.IN_OVERLAY
		);
		Map<Identifier, DebugHudEntryVisibility> performanceProfile = Map.of(
			TPS, DebugHudEntryVisibility.IN_OVERLAY,
			FPS, DebugHudEntryVisibility.ALWAYS_ON,
			GPU_UTILIZATION, DebugHudEntryVisibility.IN_OVERLAY,
			MEMORY, DebugHudEntryVisibility.IN_OVERLAY,
			SIMPLE_PERFORMANCE_IMPACTORS, DebugHudEntryVisibility.IN_OVERLAY
		);
		PROFILES = Map.of(DebugProfileType.DEFAULT, defaultProfile, DebugProfileType.PERFORMANCE, performanceProfile);
	}
}
