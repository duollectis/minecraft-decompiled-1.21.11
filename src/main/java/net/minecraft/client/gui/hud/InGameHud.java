...
public class InGameHud {
	...
	private static final Identifier
			...
			Identifier.ofVanilla("hud/effect_background_ambient");
	...
	private static final Identifier
			...
			Identifier.ofVanilla("hud/hotbar_attack_indicator_background");
	private static final Identifier
			HOTBAR_ATTACK_INDICATOR_PROGRESS_TEXTURE =
			Identifier.ofVanilla("hud/hotbar_attack_indicator_progress");
	...
	private static final Identifier
			...
			Identifier.ofVanilla("hud/heart/vehicle_container");
	...
	private static final Comparator<ScoreboardEntry>
	...
	private static final float ATTACK_COOLDOWN_THRESHOLD = 5.0F;
	private static final int SLEEP_FADE_TICKS = 100;
	...
	private static final float AUTOSAVE_LERP_FACTOR = 0.2F;
	private static final int HEART_TEXTURE_SIZE = 9;
	private static final int HEART_SPACING = 8;
	private static final int AIR_BUBBLE_COUNT = 10;
	private static final int AIR_BUBBLE_TEXTURE_SIZE = 9;
	private static final int AIR_BUBBLE_SPACING = 8;
	private static final int AIR_BUBBLE_BURST_RANDOM_OFFSET = 2;
	private static final int SUBMERGED_IN_WATER_AIR_BUBBLE_DELAY = 1;
	private static final float BUBBLE_SOUND_BASE_VOLUME = 0.5F;
	private static final float BUBBLE_SOUND_VOLUME_STEP = 0.1F;
	private static final float BUBBLE_SOUND_BASE_PITCH = 1.0F;
	private static final float BUBBLE_SOUND_PITCH_STEP = 0.1F;
	private static final int BUBBLE_SOUND_VOLUME_THRESHOLD = 3;
	private static final int BUBBLE_SOUND_PITCH_THRESHOLD = 5;
	private static final float NAUSEA_OVERLAY_ALPHA = 0.2F;
	private static final int SCOREBOARD_PADDING_RIGHT = 5;
	private static final int SCOREBOARD_PADDING_TOP = 5;
	...
	public InGameHud(MinecraftClient client) {
		...
		this.bars = ImmutableMap.of(
				...
				(Supplier<Bar>) () -> Bar.EMPTY,
				InGameHud.BarType.EXPERIENCE,
				(Supplier<Bar>) () -> new ExperienceBar(client),
				InGameHud.BarType.LOCATOR,
				(Supplier<Bar>) () -> new LocatorBar(client),
				InGameHud.BarType.JUMPABLE_VEHICLE,
				(Supplier<Bar>) () -> new JumpBar(client)
		);
	...
	public void render(DrawContext context, RenderTickCounter tickCounter) {
		if (!(this.client.currentScreen instanceof LevelLoadingScreen)) {
			...
			if (!this.client.options.hudHidden) {
				...
				this.renderSubtitlesHud(
						context,
						this.client.currentScreen == null || this.client.currentScreen.deferSubtitles()
				);
			}
			...
			if (clientPlayerEntity.isUsingSpyglass()) {
				...
				for (EquipmentSlot equipmentSlot : EquipmentSlot.values()) {
					...
					if (equippableComponent != null && equippableComponent.slot() == equipmentSlot
							&& equippableComponent.cameraOverlay().isPresent()) {
						this.renderOverlay(
								context,
								equippableComponent
										.cameraOverlay()
										.get()
										.withPath(overlayTexture -> "textures/" + overlayTexture + ".png"),
								1.0F
						);
					}
				}
	...
	private void renderOverlayMessage(DrawContext context, RenderTickCounter tickCounter) {
		...
		if (this.overlayMessage != null && this.overlayRemaining > 0) {
			...
			if (i > 0) {
				...
				context
						.getMatrices()
						.translate(context.getScaledWindowWidth() / 2, context.getScaledWindowHeight() - 68);
				int j;
	...
	private void renderTitleAndSubtitle(DrawContext context, RenderTickCounter tickCounter) {
		if (this.title != null && this.titleRemainTicks > 0) {
			...
			if (i > 0) {
				...
				context
						.getMatrices()
						.translate(context.getScaledWindowWidth() / 2, context.getScaledWindowHeight() / 2);
				context.getMatrices().pushMatrix();
	...
	private void renderCrosshair(DrawContext context, RenderTickCounter tickCounter) {
		...
		if (gameOptions.getPerspective().isFirstPerson()) {
			if (this.client.interactionManager.getCurrentGameMode() != GameMode.SPECTATOR
					|| this.shouldRenderSpectatorCrosshair(this.client.crosshairTarget)) {
					...
					if (this.client.options.getAttackIndicator().getValue() == AttackIndicator.CROSSHAIR) {
						...
						if (this.client.targetedEntity != null && this.client.targetedEntity instanceof LivingEntity
							...
							AttackRangeComponent
									attackRangeComponent =
									this.client.player.getActiveOrMainHandStack().get(DataComponentTypes.ATTACK_RANGE);
							bl &=
									attackRangeComponent == null || attackRangeComponent.isWithinRange(
											this.client.player,
	...
	private void renderStatusEffectOverlay(DrawContext context, RenderTickCounter tickCounter) {
		...
		if (!collection.isEmpty() && (this.client.currentScreen == null
				|| !this.client.currentScreen.showsStatusEffects()
		)) {
			int i = 0;
			int j = 0;
					...
					if (statusEffectInstance.isAmbient()) {
						...
						if (statusEffectInstance.isDurationBelow(200)) {
							...
							f = MathHelper.clamp(m / 10.0F / 5.0F * 0.5F, 0.0F, 0.5F)
									+ MathHelper.cos(m * (float) Math.PI / 5.0F) * MathHelper.clamp(
									n / 10.0F * 0.25F,
									0.0F,
									0.25F
							);
	...
	private void renderHotbar(DrawContext context, RenderTickCounter tickCounter) {
		...
		if (playerEntity != null) {
			...
			context.drawGuiTexture(
					...
					HOTBAR_TEXTURE,
					i - 91,
					context.getScaledWindowHeight() - 22,
					182,
					22
			);
			context.drawGuiTexture(
			...
			for (int m = 0; m < 9; m++) {
				...
				this.renderHotbarItem(
						...
						tickCounter,
						playerEntity,
						playerEntity.getInventory().getStack(m),
						l++
				);
			}
			...
			if (this.client.world.getTime() >= 120500L) {
				...
				text = Text.translatable(
						...
						StringHelper.formatTicks(
								(int) (120500L - this.client.world.getTime()),
	...
	private void renderScoreboardSidebar(DrawContext context, ScoreboardObjective objective) {
		...
		for (SidebarEntry sidebarEntry : sidebarEntrys) {
			j =
					Math.max(
							j,
							this.getTextRenderer().getWidth(sidebarEntry.name) + (sidebarEntry.scoreWidth > 0 ? k
							                                                                                    + sidebarEntry.scoreWidth
							                                                                                  : 0
							)
					);
		}
		...
		context.drawText(this.getTextRenderer(), text, q + j / 2 - i / 2, u - 9, -1, false);
	...
	private void renderStatusBars(DrawContext context) {
		...
		if (playerEntity != null) {
			...
			float
					f =
					Math.max(
							(float) playerEntity.getAttributeValue(EntityAttributes.MAX_HEALTH),
							(float) Math.max(j, i)
					);
		...
		for (int l = i + j - 1; l >= 0; l--) {
			...
			if (bl2) {
				...
				if (r < absorption) {
					...
					this.drawHeart(
							context,
							heartType == InGameHud.HeartType.WITHERED ? heartType : InGameHud.HeartType.ABSORBING,
							o,
							p,
							bl,
							false,
	...
	private void updateVignetteDarkness(Entity entity) {
		...
		float
				...
				LightmapTextureManager.getBrightness(
						entity.getEntityWorld().getDimension(),
						entity.getEntityWorld().getLightLevel(blockPos)
				);
	...
	private void renderVignetteOverlay(DrawContext context, @Nullable Entity entity) {
		...
		if (entity != null) {
			...
			double
					d =
					Math.min(
							worldBorder.getShrinkingSpeed() * worldBorder.getWarningTime(),
							Math.abs(worldBorder.getSizeLerpTarget() - worldBorder.getSize())
					);
	...
	private void renderNauseaOverlay(DrawContext context, float nauseaStrength) {
		...
		context.drawTexture(
				...
				i,
				j,
				ColorHelper.fromFloats(1.0F, g, h, k)
		);
		context.getMatrices().popMatrix();
	}
	...
	private void tick() {
		...
		if (this.client.player != null) {
			...
			else if (this.currentStack.isEmpty() || !itemStack.isOf(this.currentStack.getItem()) || !itemStack
					.getName()
					.equals(this.currentStack.getName())) {
					...
					255.0F
							* MathHelper.clamp(
							MathHelper.lerp(
									...
									this.lastAutosaveIndicatorAlpha,
									this.autosaveIndicatorAlpha
							), 0.0F, 1.0F
					)
			);
			if (i > 0) {
	...
	private boolean shouldShowJumpBar() {
		return this.client.player.getMountJumpStrength() > 0.0F
				|| Nullables.mapOrElse(this.client.player.getJumpingMount(), JumpingMount::getJumpCooldown, 0) > 0;
	}
