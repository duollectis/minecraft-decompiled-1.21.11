...
public class SubtitlesHud implements SoundInstanceListener {
			...
			if (!this.audibleEntries.isEmpty()) {
				...
				for (SubtitlesHud.SubtitleEntry subtitleEntry2 : this.audibleEntries) {
					...
					if (soundEntry != null) {
						...
						int
								p =
								MathHelper.floor(MathHelper.clampedLerp(
										(float) (Util.getMeasuringTimeMs() - soundEntry.time) / (float) (3000.0 * d),
										255.0F,
										75.0F
								));
						...
						context
								.getMatrices()
								.translate(
										context.getScaledWindowWidth() - l * 1.0F - 2.0F,
										context.getScaledWindowHeight() - 35 - i * (m + 1) * 1.0F
								);
						...
						if (!bl) {
							if (e > 0.0) {
								context.drawTextWithShadow(
										...
										">",
										l - this.client.textRenderer.getWidth(">"),
										-n,
										q
								);
							}
	...
	public void onSoundPlayed(SoundInstance sound, WeightedSoundSet soundSet, float range) {
		if (soundSet.getSubtitle() != null) {
			...
			this.entries.add(new SubtitlesHud.SubtitleEntry(
					text,
					range,
	...
	static class SubtitleEntry {
		...
		public SubtitlesHud.@Nullable SoundEntry getNearestSound(Vec3d pos) {
			if (this.sounds.isEmpty()) {
				...
				return this.sounds.size() == 1
				       ...
				       : this.sounds
				         .stream()
				         .min(Comparator.comparingDouble(soundPos -> soundPos.location().distanceTo(pos)))
				         .orElse(null);
			}
		}
