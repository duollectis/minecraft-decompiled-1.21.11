...
public class SuffixArray<T> {
	...
	private static final boolean
			PRINT_COMPARISONS =
			Boolean.parseBoolean(System.getProperty("SuffixArray.printComparisons", "false"));
	private static final boolean
			PRINT_ARRAY =
			Boolean.parseBoolean(System.getProperty("SuffixArray.printArray", "false"));
	...
	public void build() {
		...
		IntComparator
				intComparator =
				(a, b) -> js[a] == js[b] ? Integer.compare(ks[a], ks[b]) : Integer.compare(js[a], js[b]);
		...
		while (j < k) {
			...
			if (PRINT_COMPARISONS) {
				LOGGER.debug(
						...
						new Object[]{text, l, this.getDebugString(l), m}
				);
			}
			...
			while (j < k) {
				...
				if (PRINT_COMPARISONS) {
					LOGGER.debug(
							...
							new Object[]{text, mx, this.getDebugString(mx), n}
					);
				}
