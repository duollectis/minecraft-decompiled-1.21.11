...
import net.minecraft.client.realms.gui.screen.*;
...
/**
 * {@code RealmsPrepareConnectionTask}.
 */
public class RealmsPrepareConnectionTask extends LongRunningTask {
	...
	public void run() {
				...
				case 6006:
					...
					setScreen(
							(Screen) (bl
									...
									this.lastScreen,
									this.server.id,
									this.server.isMinigame()
							)
							          : new RealmsGenericErrorScreen(
									          Text.translatable("mco.brokenworld.nonowner.title"),
									          Text.translatable("mco.brokenworld.nonowner.error"),
									          this.lastScreen
							          )
							)
					);
		...
		if (realmsServerAddress.address() == null) {
			...
			boolean
					bl2 =
					realmsServerAddress.resourcePackUrl() != null && realmsServerAddress.resourcePackHash() != null;
			Screen screen = (Screen) (bl2
			                          ? this.createResourcePackConfirmationScreen(
					realmsServerAddress,
					getResourcePackId(this.server),
					this::createConnectingScreen
			)
	...
	private static UUID getResourcePackId(RealmsServer server) {
		return server.minigameName != null
		       ...
		       : UUID.nameUUIDFromBytes(("realms:" + Objects.requireNonNullElse(server.name, "") + ":"
		                                 + server.activeSlot
	...
	public RealmsLongRunningMcoTaskScreen createConnectingScreen(RealmsServerAddress address) {
		return new RealmsConnectingScreen(
				this.lastScreen,
				address,
				new RealmsConnectTask(this.lastScreen, this.server, address)
		);
	}

	private PopupScreen createResourcePackConfirmationScreen(
			RealmsServerAddress address,
			UUID id,
			Function<RealmsServerAddress, Screen> connectingScreenCreator
	) {
		Text text = Text.translatable("mco.configure.world.resourcepack.question");
		return RealmsPopups.createInfoPopup(
				this.lastScreen, text, popup -> {
					setScreen(new MessageScreen(APPLYING_PACK_TEXT));
					this
							.downloadResourcePack(address, id)
							.thenRun(() -> setScreen(connectingScreenCreator.apply(address)))
							.exceptionally(throwable -> {
								...
								setScreen(new RealmsGenericErrorScreen(
										Text.translatable("mco.download.resourcePack.fail"), this.lastScreen));
								return null;
							});
				}
		);
		...
		try {
			if (address.resourcePackUrl() == null) {
				...
				ServerResourcePackLoader
						serverResourcePackLoader =
						MinecraftClient.getInstance().getServerResourcePackProvider();
				CompletableFuture<Void> completableFuture = serverResourcePackLoader.getPackLoadFuture(id);
				serverResourcePackLoader.acceptAll();
				serverResourcePackLoader.addResourcePack(
						id,
						new URL(address.resourcePackUrl()),
						address.resourcePackHash()
				);
