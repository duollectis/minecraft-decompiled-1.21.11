...
public class RealmsClient {
	...
	public static final RealmsClient.Environment ENVIRONMENT = Optional.ofNullable(System.getenv("realms.environment"))
	                                                                   .or(() -> Optional.ofNullable(System.getProperty(
			                                                                   "realms.environment")))
	...
	private RealmsClient(String sessionId, String username, MinecraftClient client) {
		...
		this.featureFlagsFuture =
				CompletableFuture.supplyAsync(this::fetchFeatureFlags, Util.getDownloadWorkerExecutor());
	}
	...
	private Set<String> fetchFeatureFlags() {
		if (MinecraftClient.getInstance().isOfflineDeveloperMode()) {
			...
			try {
				...
				Set<String>
						set =
						jsonArray.asList().stream().<String>map(JsonElement::getAsString).collect(Collectors.toSet());
	...
	public RealmsServer createPrereleaseServer(Long parentWorldId) throws RealmsServiceException {
		...
		String
				string2 =
				this.url("worlds" + "/$PARENT_WORLD_ID/createPrereleaseRealm".replace("$PARENT_WORLD_ID", string));
	...
	public RealmsRegionDataList getRegionDataList() throws RealmsServiceException {
		...
		try {
			...
			if (realmsRegionDataList == null) {
				...
				Set<RealmsRegion>
						set =
						realmsRegionDataList.regionData().stream().map(RegionData::region).collect(Collectors.toSet());
	...
	public void uninvite(long worldId, UUID profileUuid) throws RealmsServiceException {
		String string = this.url(
				"invites" + "/$WORLD_ID/invite/$UUID"
						.replace("$WORLD_ID", String.valueOf(worldId))
						.replace("$UUID", UndashedUuid.toString(profileUuid))
		);
	...
	public void configure(
		...
		RealmsRegionSelectionPreference realmsRegionSelectionPreference = regionSelectionPreference != null
		                                                                  ...
		                                                                  : new RealmsRegionSelectionPreference(
		...
		RealmsOptionsDto
				realmsOptionsDto =
				new RealmsOptionsDto(slotId, options, RealmsSettingDto.isHardcore(settings));
		RealmsConfigurationDto realmsConfigurationDto = new RealmsConfigurationDto(
				realmsOptionsDto, settings, realmsRegionSelectionPreference, realmsDescriptionDto
		);
	...
	public void updateSlot(long worldId, int slot, RealmsWorldOptions options, List<RealmsSettingDto> settings)
	throws RealmsServiceException {
		String
				string =
				this.url("worlds" + "/$WORLD_ID/slot/$SLOT_ID"
						.replace("$WORLD_ID", String.valueOf(worldId))
	...
	public void restoreWorld(long worldId, String backupId) throws RealmsServiceException {
		String
				string =
				this.url(
						"worlds" + "/$WORLD_ID/backups".replace("$WORLD_ID", String.valueOf(worldId)),
						"backupId=" + backupId
				);
		this.execute(Request.put(string, "", 40000, 600000));
	}

	public WorldTemplatePaginatedList fetchWorldTemplates(int page, int pageSize, RealmsServer.WorldType type)
	throws RealmsServiceException {
		String string = this.url(
				"worlds" + "/templates/$WORLD_TYPE".replace("$WORLD_TYPE", type.toString()),
	...
	public Boolean putIntoMinigameMode(long worldId, String minigameId) throws RealmsServiceException {
		String
				...
				"/minigames/$MINIGAME_ID/$WORLD_ID"
						.replace("$MINIGAME_ID", minigameId)
						.replace("$WORLD_ID", String.valueOf(worldId));
	...
	public Ops op(long worldId, UUID profileUuid) throws RealmsServiceException {
		String
				string =
				"/$WORLD_ID/$PROFILE_UUID"
						.replace("$WORLD_ID", String.valueOf(worldId))
						.replace("$PROFILE_UUID", UndashedUuid.toString(profileUuid));
	...
	public Boolean resetWorldWithTemplate(long worldId, String worldTemplateId) throws RealmsServiceException {
		RealmsWorldResetDto
				realmsWorldResetDto =
				new RealmsWorldResetDto(null, Long.valueOf(worldTemplateId), -1, false, Set.of());
	...
	public @Nullable UploadInfo upload(long worldId) throws RealmsServiceException {
		...
		UploadInfo
				uploadInfo =
				UploadInfo.parse(this.execute(Request.put(string, UploadInfo.createRequestContent(string2))));
		if (uploadInfo != null) {
	...
	private static String url(String path, @Nullable String queryString, boolean aks) {
		try {
			return new URI(
					...
					aks ? ENVIRONMENT.aksUrl : ENVIRONMENT.baseUrl,
					"/" + path,
					queryString,
					null
			).toASCIIString();
	...
	@net.fabricmc.api.Environment(EnvType.CLIENT)
	/**
	 * {@code CompatibleVersionResponse}.
	 */
