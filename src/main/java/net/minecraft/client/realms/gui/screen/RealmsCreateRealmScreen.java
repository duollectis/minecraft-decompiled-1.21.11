...
public class RealmsCreateRealmScreen extends RealmsScreen {
	...
	private static final int DESCRIPTION_MAX_LENGTH = 210;
	...
	private void createWorld(RealmsServer realmsServer, boolean prerelease) {
		if (!realmsServer.isPrerelease() && prerelease) {
			...
			CompletableFuture
					.<RealmsServer>supplyAsync(() -> createPrereleaseServer(realmsServer), Util.getMainWorkerExecutor())
					.thenAcceptAsync(
							prereleaseServer -> {
	...
	private void createWorld(RealmsServer server) {
		WorldCreationTask
				worldCreationTask =
				new WorldCreationTask(server.id, this.nameBox.getText(), this.descriptionBox.getText());
		RealmsCreateWorldScreen realmsCreateWorldScreen = RealmsCreateWorldScreen.newRealm(
				this, server, worldCreationTask, () -> this.client.execute(() -> {
