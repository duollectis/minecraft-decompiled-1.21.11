...
/**
 * {@code RealmsSelectWorldTemplateScreen}.
 */
public class RealmsSelectWorldTemplateScreen extends RealmsScreen {
	...
	private final List<Text> headerTexts;
	...
	public RealmsSelectWorldTemplateScreen(
			...
			Consumer<WorldTemplate> callback,
			RealmsServer.WorldType worldType,
			@Nullable WorldTemplatePaginatedList templateList,
			List<Text> list
	) {
		super(title);
		...
		if (templateList == null) {
			...
			this.templateList =
					new RealmsSelectWorldTemplateScreen.WorldTemplateObjectSelectionList(Lists.newArrayList(templateList.templates()));
	...
	@Override
	public void init() {
		this.layout.setHeaderHeight(33 + this.headerTexts.size() * (9 + 4));
		...
		this.headerTexts.forEach(text -> directionalLayoutWidget.add(new TextWidget(text, this.textRenderer)));
		this.templateList =
				this.layout.addBody(new RealmsSelectWorldTemplateScreen.WorldTemplateObjectSelectionList(this.templateList.getValues()));
		DirectionalLayoutWidget
		...
		(new Thread("realms-template-fetcher") {
			...
			public void run() {
				...
				while (worldTemplatePaginatedList != null) {
					Either<WorldTemplatePaginatedList, Exception>
							either =
							RealmsSelectWorldTemplateScreen.this.fetchWorldTemplates(
									worldTemplatePaginatedList, realmsClient
							);
					worldTemplatePaginatedList = RealmsSelectWorldTemplateScreen.this.client
							.submit(
									() -> {
										if (either.right().isPresent()) {
											RealmsSelectWorldTemplateScreen.LOGGER.error(
													"Couldn't fetch templates",
													(Throwable) either.right().get()
											);
											if (RealmsSelectWorldTemplateScreen.this.templateList.isEmpty()) {
												RealmsSelectWorldTemplateScreen.this.noTemplatesMessage =
											...
											WorldTemplatePaginatedList
													worldTemplatePaginatedListx =
													(WorldTemplatePaginatedList) either.left().get();
												...
												if (RealmsSelectWorldTemplateScreen.this.templateList.isEmpty()) {
													...
													TextRenderingUtils.LineSegment
															...
															TextRenderingUtils.LineSegment.link(
																	I18n.translate("mco.template.select.none.linkTitle"),
																	Urls.REALMS_CONTENT_CREATOR.toString()
															);
													RealmsSelectWorldTemplateScreen.this.noTemplatesMessage =
															TextRenderingUtils.decompose(string, lineSegment);
												}
	...
	Either<WorldTemplatePaginatedList, Exception> fetchWorldTemplates(
			WorldTemplatePaginatedList templateList,
			RealmsClient realms
	) {
		try {
			return Either.left(realms.fetchWorldTemplates(
					templateList.page() + 1,
					templateList.size(),
					this.worldType
			));
	...
	private void renderMessages(DrawContext context, int x, int y, List<TextRenderingUtils.Line> messages) {
		for (int i = 0; i < messages.size(); i++) {
			...
			int
					...
					line.segments
							.stream()
							.mapToInt(segment -> this.textRenderer.getWidth(segment.renderedText()))
							.sum();
	...
	@Environment(EnvType.CLIENT)
	/**
	 * {@code WorldTemplateObjectSelectionList}.
	 */
	class WorldTemplateObjectSelectionList extends AlwaysSelectedEntryListWidget<RealmsSelectWorldTemplateScreen.WorldTemplateObjectSelectionListEntry> {
		...
		public boolean mouseClicked(Click click, boolean doubled) {
			if (RealmsSelectWorldTemplateScreen.this.currentLink != null) {
				ConfirmLinkScreen.open(
						RealmsSelectWorldTemplateScreen.this,
						RealmsSelectWorldTemplateScreen.this.currentLink
				);
				return true;
			}
			else {
		...
		public void setSelected(RealmsSelectWorldTemplateScreen.@Nullable WorldTemplateObjectSelectionListEntry worldTemplateObjectSelectionListEntry) {
	...
	class WorldTemplateObjectSelectionListEntry
		...
		private static final ButtonTextures
				LINK_TEXTURES =
				new ButtonTextures(Identifier.ofVanilla("icon/link"), Identifier.ofVanilla("icon/link_highlighted"));
		...
		public WorldTemplateObjectSelectionListEntry(final WorldTemplate template) {
			...
			if (!template.link().isBlank()) {
				this.infoButton = new TexturedButtonWidget(
						...
						15,
						LINK_TEXTURES,
						ConfirmLinkScreen.opening(RealmsSelectWorldTemplateScreen.this, template.link()),
						INFO_TOOLTIP_TEXT
				);
			...
			if (!template.trailer().isBlank()) {
				this.trailerButton = new TexturedButtonWidget(
						...
						15,
						VIDEO_LINK_TEXTURES,
						ConfirmLinkScreen.opening(RealmsSelectWorldTemplateScreen.this, template.trailer()),
		...
		public void render(DrawContext context, int mouseX, int mouseY, boolean hovered, float deltaTicks) {
			...
			if (this.infoButton != null) {
				this.infoButton.setPosition(
						this.getContentRightEnd() - j - this.infoButton.getWidth() - 10,
						this.getContentY()
				);
			...
			if (this.trailerButton != null) {
				this.trailerButton.setPosition(
						this.getContentRightEnd() - j - this.trailerButton.getWidth() * 2 - 15,
						this.getContentY()
				);
			...
			context.drawTextWithShadow(
					RealmsSelectWorldTemplateScreen.this.textRenderer,
					this.mTemplate.version(),
					this.getContentRightEnd() - j - 5,
					l,
					-6250336
			);
			context.drawTextWithShadow(
					RealmsSelectWorldTemplateScreen.this.textRenderer,
					this.mTemplate.author(),
					k,
					l + 9 + 5,
					-6250336
			);
			if (!this.mTemplate.recommendedPlayers().isBlank()) {
				context.drawTextWithShadow(
						RealmsSelectWorldTemplateScreen.this.textRenderer,
						this.mTemplate.recommendedPlayers(),
						k,
						this.getContentBottomEnd() - 9 / 2 - 5,
						-8355712
				);
			}
		}
