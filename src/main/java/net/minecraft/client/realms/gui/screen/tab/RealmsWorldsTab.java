...
public class RealmsWorldsTab extends GridScreenTab implements RealmsUpdatableTab {
	...
	public RealmsWorldsTab(RealmsConfigureWorldScreen screen, MinecraftClient client, RealmsServer server) {
				...
				ButtonWidget.builder(
						            ...
						            button -> client.setScreen(
								            new RealmsSlotOptionsScreen(
										            screen,
										            server.slots.get(server.activeSlot).copy(),
										            server.worldType,
										            server.activeSlot
								            )
						            )
		...
		this.backupButton = adder3.add(
				ButtonWidget.builder(
						            ...
						            button -> client.setScreen(new RealmsBackupScreen(
								            screen,
								            server.copy(),
								            server.activeSlot
						            ))
		...
		this.resetButton =
				adder3.add(ButtonWidget
						.builder(Text.empty(), button -> this.reset())
						.dimensions(0, 0, 150, 20)
						.build());
			...
			this.client
					.setScreen(
							RealmsCreateWorldScreen.resetWorld(
									this.screen,
									this.server.copy(),
									() -> this.client.execute(() -> this.client.setScreen(this.screen.getNewScreen()))
							)
					);
		}
	}
			...
			this.client
					.setScreen(
							new RealmsLongRunningMcoTaskScreen(
									...
									new SwitchMinigameTask(this.server.id, template, realmsConfigureWorldScreen)
							)
					);
		}
		else {
	...
	public void update(RealmsServer server) {
		...
		if (this.isMinigame()) {
			...
			boolean
					bl =
					server.slots.containsKey(server.activeSlot) && server.slots.get(server.activeSlot).options.empty;
			if (bl) {
	...
	private RealmsWorldSlotButton createSlotButton(int slotIndex) {
		return new RealmsWorldSlotButton(
				0, 0, 80, 80, slotIndex, this.server, button -> {
	...
	private void showSwitchMinigameScreen() {
		RealmsSelectWorldTemplateScreen realmsSelectWorldTemplateScreen = new RealmsSelectWorldTemplateScreen(
				...
				null,
				List.of(
						Text.translatable("mco.minigame.world.info.line1").withColor(-4539718),
						Text.translatable("mco.minigame.world.info.line2").withColor(-4539718)
				)
		);
											...
											.setScreen(
													new RealmsLongRunningMcoTaskScreen(
															...
															new SwitchSlotTask(
																	server.id,
																	slotId,
																	() -> this.client.execute(() -> this.client.setScreen(
																			realmsConfigureWorldScreen))
															)
													)
											);
