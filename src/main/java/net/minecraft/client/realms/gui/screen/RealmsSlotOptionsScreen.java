...
/**
 * {@code RealmsSlotOptionsScreen}.
 */
public class RealmsSlotOptionsScreen extends RealmsScreen {

	private static final int MIN_DIFFICULTY = 2;
	public static final List<Difficulty>
			DIFFICULTIES =
			ImmutableList.of(Difficulty.PEACEFUL, Difficulty.EASY, Difficulty.NORMAL, Difficulty.HARD);
	private static final int DEFAULT_DIFFICULTY = 0;
	public static final List<GameMode>
			GAME_MODES =
			ImmutableList.of(GameMode.SURVIVAL, GameMode.CREATIVE, GameMode.ADVENTURE);
	...
	public RealmsSlotOptionsScreen(
			RealmsConfigureWorldScreen parent,
			RealmsSlot slot,
	...
	public void init() {
		...
		this.nameEdit = this.addSelectableChild(
				new TextFieldWidget(
						...
						20,
						null,
						Text.translatable("mco.configure.world.edit.slot.name")
				)
		);
		...
		CyclingButtonWidget<Difficulty> cyclingButtonWidget = this.addDrawableChild(
				CyclingButtonWidget.builder(Difficulty::getTranslatableName, this.difficulty)
				                   ...
				                   .build(
						                   ...
						                   (cyclingButtonWidgetx, difficulty) -> this.difficulty = difficulty
				                   )
		);
		CyclingButtonWidget<GameMode> cyclingButtonWidget2 = this.addDrawableChild(
				CyclingButtonWidget.builder(GameMode::getSimpleTranslatableName, this.gameMode)
				                   ...
				                   .build(
						                   ...
						                   this.column2_x,
						                   20,
						                   Text.translatable("selectWorld.gameMode"),
						                   (button, gameModeIndex) -> this.gameMode = gameModeIndex
				                   )
		);
		...
		this.spawnProtectionButton = this.addDrawableChild(
				new RealmsSlotOptionsScreen.SettingsSlider(
						this.column1_x,
						row(5),
						this.column2_x,
		...
		this.addDrawableChild(
				ButtonWidget
						.builder(Text.translatable("mco.configure.world.buttons.done"), button -> this.saveSettings())
						.dimensions(this.column1_x, row(13), this.column2_x, 20)
						.build()
		);
		this.addDrawableChild(ButtonWidget
				.builder(ScreenTexts.CANCEL, button -> this.close())
				.dimensions(i, row(13), this.column2_x, 20)
				.build());
	}

	private CyclingButtonWidget.UpdateCallback<Boolean> getSpawnToggleButtonCallback(
			Text text,
		...
		return (button, value) -> {
			if (value) {
				...
				this.client.setScreen(RealmsPopups.createContinuableWarningPopup(
						this, text, popup -> {
	...
	public void render(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
		...
		context.drawTextWithShadow(
				this.textRenderer,
				EDIT_SLOT_NAME,
				this.column1_x + this.column2_x / 2 - this.textRenderer.getWidth(EDIT_SLOT_NAME) / 2,
				row(0) - 5,
				-1
		);
		...
		public void applyValue() {
			if (RealmsSlotOptionsScreen.this.spawnProtectionButton.active) {
				RealmsSlotOptionsScreen.this.spawnProtection =
						(int) MathHelper.lerp(MathHelper.clamp(this.value, 0.0, 1.0), this.min, this.max);
			}
		}
