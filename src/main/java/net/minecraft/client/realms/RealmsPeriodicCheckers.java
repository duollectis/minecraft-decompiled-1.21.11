...
/**
 * {@code RealmsPeriodicCheckers}.
 */
public class RealmsPeriodicCheckers {

	public final PeriodicRunnerFactory
			runnerFactory =
			new PeriodicRunnerFactory(Util.getIoWorkerExecutor(), TimeUnit.MILLISECONDS, Util.nanoTimeSupplier);
	...
	public RealmsPeriodicCheckers(RealmsClient client) {
				...
				.create(
						...
						() -> {
							...
							return RealmsMainScreen.isSnapshotRealmsEligible()
							       ? new RealmsPeriodicCheckers.AvailableServers(
									realmsServerList.servers(),
									client.getPrereleaseEligibleServers()
							)
		...
		this.pendingInvitesCount = this.runnerFactory
				.create(
						...
						client::pendingInvitesCount,
						Duration.ofSeconds(10L),
						Backoff.exponential(360)
				);
		this.trialAvailability =
				this.runnerFactory.create(
						"trial availablity",
						client::trialAvailable,
						Duration.ofSeconds(60L),
						Backoff.exponential(60)
				);
		this.news =
				this.runnerFactory.create("unread news", client::getNews, Duration.ofMinutes(5L), Backoff.ONE_CYCLE);
		this.notifications =
				this.runnerFactory.create(
						"notifications",
