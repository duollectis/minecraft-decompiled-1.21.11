...
public class GlCommandEncoder implements CommandEncoder {
            ...
            if (biConsumer != null) {
               biConsumer.accept(object, (name, gpuBufferSlice) -> {
                  if (pass.pipeline.program().getUniform(name) instanceof GlUniform.UniformBuffer(int i)) {
                     GL32.glBindBufferRange(35345, i, ((GlGpuBuffer)gpuBufferSlice.buffer()).id, gpuBufferSlice.offset(), gpuBufferSlice.length());
                  }
               });
            }
   ...
   protected void drawBoundObjectWithRenderPass(
      RenderPassImpl pass, int baseVertex, int firstIndex, int count, VertexFormat.@Nullable IndexType indexType, int instanceCount
   ) {
         ...
         switch ((GlUniform)entry2.getValue()) {
            case GlUniform.UniformBuffer(int var63):
               ...
               if (bl2) {
                  ...
                  GL32.glBindBufferRange(35345, var41, ((GlGpuBuffer)gpuBufferSlice2.buffer()).id, gpuBufferSlice2.offset(), gpuBufferSlice2.length());
               }
               break;
