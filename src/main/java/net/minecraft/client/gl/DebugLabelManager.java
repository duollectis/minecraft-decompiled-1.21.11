	...
	static class EXTDebugLabelManager extends DebugLabelManager {
		...
		public void labelCompiledShader(CompiledShader shader) {
			EXTDebugLabel.glLabelObjectEXT(
					35656,
					shader.getHandle(),
					StringHelper.truncate(shader.getDebugLabel(), 256, true)
			);
		}
		...
		public void labelShaderProgram(ShaderProgram program) {
			EXTDebugLabel.glLabelObjectEXT(
					35648,
					program.getGlRef(),
					StringHelper.truncate(program.getDebugLabel(), 256, true)
			);
		}
		...
		public void labelAllocatedBuffer(VertexBufferManager.AllocatedBuffer buffer) {
			EXTDebugLabel.glLabelObjectEXT(
					32884,
					buffer.glId,
					StringHelper.truncate(buffer.vertexFormat.toString(), 256, true)
			);
		}
	...
	static class KHRDebugLabelManager extends DebugLabelManager {
		...
		public void labelGlGpuBuffer(GlGpuBuffer buffer) {
			...
			if (supplier != null) {
				KHRDebug.glObjectLabel(
						33504,
						buffer.id,
						StringHelper.truncate(supplier.get(), this.maxLabelLength, true)
				);
			}
		}
		...
		public void labelGlTexture(GlTexture texture) {
			KHRDebug.glObjectLabel(
					5890,
					texture.glId,
					StringHelper.truncate(texture.getLabel(), this.maxLabelLength, true)
			);
		}
		...
		public void labelCompiledShader(CompiledShader shader) {
			KHRDebug.glObjectLabel(
					33505,
					shader.getHandle(),
					StringHelper.truncate(shader.getDebugLabel(), this.maxLabelLength, true)
			);
		}
		...
		public void labelShaderProgram(ShaderProgram program) {
			KHRDebug.glObjectLabel(
					33506,
					program.getGlRef(),
					StringHelper.truncate(program.getDebugLabel(), this.maxLabelLength, true)
			);
		}
		...
		public void labelAllocatedBuffer(VertexBufferManager.AllocatedBuffer buffer) {
			KHRDebug.glObjectLabel(
					32884,
					buffer.glId,
					StringHelper.truncate(buffer.vertexFormat.toString(), this.maxLabelLength, true)
			);
		}
