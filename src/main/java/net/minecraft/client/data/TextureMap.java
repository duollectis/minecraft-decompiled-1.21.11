...
/**
 * Хранилище соответствий между слотами текстур ({@link TextureKey}) и их идентификаторами.
 * Используется при генерации JSON-моделей блоков и предметов для подстановки текстур в шаблоны.
 */
...
public class TextureMap {
	...
	public TextureMap copy(TextureKey parent, TextureKey child) {
		entries.put(child, entries.get(parent));
		return this;
	}
	...
	/**
	 * Ищет текстуру для указанного слота, поднимаясь по цепочке родительских ключей,
	 * пока не найдёт зарегистрированный идентификатор.
	 *
	 * @param key слот текстуры
	 * @return идентификатор текстуры
	 * @throws IllegalStateException если текстура не найдена ни в одном из родительских слотов
	 */
	public Identifier getTexture(TextureKey key) {
		for (TextureKey current = key; current != null; current = current.getParent()) {
			Identifier id = entries.get(current);
	...
	public TextureMap copyAndAdd(TextureKey key, Identifier id) {
		...
		copy.inherited.addAll(inherited);
		copy.put(key, id);
	...
	public static TextureMap flowerbed(Block block) {
		return new TextureMap()
				.put(TextureKey.FLOWERBED, getId(block))
				.put(TextureKey.STEM, getSubId(block, "_stem"));
	}
	...
	public static TextureMap stemAndUpper(Block stem, Block upper) {
		return new TextureMap()
				.put(TextureKey.STEM, getId(stem))
				.put(TextureKey.UPPERSTEM, getId(upper));
	}
	...
	public static TextureMap paneAndTopForEdge(Block block, Block top) {
		return new TextureMap()
				.put(TextureKey.PANE, getId(block))
				.put(TextureKey.EDGE, getSubId(top, "_top"));
	}
	...
	public static TextureMap wallSideTopBottom(Block block) {
		Identifier blockId = getId(block);

		return new TextureMap()
				.put(TextureKey.WALL, blockId)
				.put(TextureKey.SIDE, blockId)
				.put(TextureKey.TOP, getSubId(block, "_top"))
				.put(TextureKey.BOTTOM, getSubId(block, "_bottom"));
	}
	...
	public static TextureMap sculkShrieker(boolean canSummon) {
		String innerTopSuffix = canSummon ? "_can_summon" : "";
		...
		return new TextureMap()
				...
				.put(TextureKey.INNER_TOP, getSubId(Blocks.SCULK_SHRIEKER, innerTopSuffix + "_inner_top"))
	...
	public static TextureMap bars(Block block) {
		...
		return new TextureMap()
				.put(TextureKey.BARS, blockId)
				.put(TextureKey.EDGE, blockId);
	}
	...
	public static Identifier getSubId(Block block, String suffix) {
		return Registries.BLOCK.getId(block).withPath(path -> "block/" + path + suffix);
	}
	...
	public static Identifier getSubId(Item item, String suffix) {
		return Registries.ITEM.getId(item).withPath(path -> "item/" + path + suffix);
	}
}
