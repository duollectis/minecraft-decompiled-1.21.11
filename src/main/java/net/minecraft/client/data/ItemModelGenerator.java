...
import net.minecraft.client.render.item.tint.*;
...
public class ItemModelGenerator {
	...
	public final List<RangeDispatchItemModel.Entry> createCompassRangeDispatchEntries(Item item) {
		...
		for (int i = 1; i < 32; i++) {
			...
			ItemModel.Unbaked
					unbaked2 =
					ItemModels.basic(this.registerSubModel(
							item,
							String.format(Locale.ROOT, "_%02d", j),
							Models.GENERATED
					));
				...
				.accept(
						...
						ItemModels.condition(
								...
								ItemModels.rangeDispatch(
										new CompassProperty(true, CompassState.Target.LODESTONE),
										32.0F,
										list
								),
								ItemModels.rangeDispatch(
										new CompassProperty(true, CompassState.Target.SPAWN),
										32.0F,
										list
								)
						)
	...
	public final void registerRecoveryCompass(Item item) {
		this.output
				.accept(
						item,
						ItemModels.rangeDispatch(
								new CompassProperty(true, CompassState.Target.RECOVERY),
								32.0F,
								this.createCompassRangeDispatchEntries(item)
						)
				);
	}
		...
		this.output
				.accept(
						...
						ItemModels.overworldSelect(
								ItemModels.rangeDispatch(
										new TimeProperty(true, TimeProperty.Source.DAYTIME),
										64.0F,
										list
								),
								ItemModels.rangeDispatch(
										new TimeProperty(true, TimeProperty.Source.RANDOM),
										64.0F,
										list
								)
						)
	...
	public final void registerBundle(Item item) {
		...
		ItemModel.Unbaked
				...
				ItemModels.composite(
						...
						new BundleSelectedItemModel.Unbaked(),
						ItemModels.basic(identifier2)
				);
		...
		this.output.accept(
				...
				ItemModels.select(
						...
						unbaked,
						ItemModels.switchCase(ItemDisplayContext.GUI, unbaked3)
				)
		);
	}
	...
	public final void registerShield(Item item) {
		ItemModel.Unbaked
				unbaked =
				ItemModels.special(ModelIds.getItemModelId(item), new ShieldModelRenderer.Unbaked());
		ItemModel.Unbaked
				unbaked2 =
				ItemModels.special(ModelIds.getItemSubModelId(item, "_blocking"), new ShieldModelRenderer.Unbaked());
		this.registerCondition(item, ItemModels.usingItemProperty(), unbaked2, unbaked);
	}

	public static ItemModel.Unbaked createModelWithInHandVariant(
			ItemModel.Unbaked model,
			ItemModel.Unbaked inHandModel
	) {
	...
	public final void registerTrident(Item item) {
		...
		ItemModel.Unbaked
				unbaked2 =
				ItemModels.special(ModelIds.getItemSubModelId(item, "_in_hand"), new TridentModelRenderer.Unbaked());
		ItemModel.Unbaked
				unbaked3 =
				ItemModels.special(ModelIds.getItemSubModelId(item, "_throwing"), new TridentModelRenderer.Unbaked());
	...
	public final void registerSpear(Item item) {
		...
		ItemModel.Unbaked unbaked2 = ItemModels.basic(
				Models.SPEAR_IN_HAND.upload(
						item,
						TextureMap.layer0(TextureMap.getSubId(item, "_in_hand")),
						this.modelCollector
				)
		);
		this.output.accept(
				item,
				createModelWithInHandVariant(unbaked, unbaked2),
				new ItemAsset.Properties(true, false, 1.95F)
		);
	}
	...
	public final void registerPotion(Item item) {
		Identifier
				identifier =
				this.uploadTwoLayers(
						item,
						ModelIds.getMinecraftNamespacedItem("potion_overlay"),
	...
	public final void registerTippedArrow(Item item) {
		Identifier
				...
				this.uploadTwoLayers(
						item,
						ModelIds.getItemSubModelId(item, "_head"),
						ModelIds.getItemSubModelId(item, "_base")
				);
		this.registerPotionTinted(item, identifier);
	}
