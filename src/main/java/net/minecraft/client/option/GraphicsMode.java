...
public enum GraphicsMode implements StringIdentifiable {
	...
	public void apply(MinecraftClient client) {
		GameOptionsScreen
				gameOptionsScreen =
				client.currentScreen instanceof GameOptionsScreen ? (GameOptionsScreen) client.currentScreen : null;
		...
		switch (this) {
			...
			case FANCY: {
				...
				this.applyOption(
						gameOptionsScreen,
						client.options.getChunkBuilderMode(),
						ChunkBuilderMode.PLAYER_AFFECTED
				);
			...
			case FABULOUS: {
				...
				this.applyOption(
						gameOptionsScreen,
						client.options.getImprovedTransparency(),
						Util.getOperatingSystem() != Util.OperatingSystem.OSX
				);
				this.applyOption(gameOptionsScreen, client.options.getWeatherRadius(), 10);
				this.applyOption(gameOptionsScreen, client.options.getMaxAnisotropy(), 2);
				if (GpuDeviceInfo.get(gpuDevice).isAmdGpu()) {
					this.applyOption(
							gameOptionsScreen,
							client.options.getTextureFiltering(),
							TextureFilteringMode.RGSS
					);
				}
				else {
					this.applyOption(
