...
public class ParticleSpriteManager implements ResourceReloader {
	...
	private final Map<Identifier, ParticleSpriteManager.SimpleSpriteProvider>
	...
	private <T extends ParticleEffect> void register(
			ParticleType<T> type,
			ParticleSpriteManager.SpriteAwareFactory<T> factory
	) {
		ParticleSpriteManager.SimpleSpriteProvider
				simpleSpriteProvider =
				new ParticleSpriteManager.SimpleSpriteProvider();
	...
	public CompletableFuture<Void> reload(
		...
		CompletableFuture<List<ReloadResult>>
				completableFuture =
				                 ...
				                 .thenCompose(resources -> {
					                 ...
					                 resources.forEach((resourceId, resource) -> {
						                 ...
						                 list.add(CompletableFuture.supplyAsync(
								                 () -> new ReloadResult(
										                 identifier,
										                 this.load(identifier, resource)
								                 ), executor
						                 ));
					                 });
					                 return Util.combineSafe(list);
				                 });
		CompletableFuture<SpriteLoader.StitchResult>
				completableFuture2 =
				store.getOrThrow(AtlasManager.stitchKey).getPreparations(Atlases.PARTICLES);
		return CompletableFuture
				.allOf(completableFuture, completableFuture2)
				...
				.thenAcceptAsync(
						void_ -> {
							...
							completableFuture.join().forEach(reloadResult -> {
							...
							if (!set.isEmpty()) {
								LOGGER.warn(
										...
										set.stream().sorted().map(Identifier::toString).collect(Collectors.joining(","))
								);
							}
