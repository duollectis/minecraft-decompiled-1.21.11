...
public class InterpolatedNoiseSampler implements DensityFunction.Base {
	...
	private static final MapCodec<InterpolatedNoiseSampler> MAP_CODEC = RecordCodecBuilder.mapCodec(
			instance -> instance.group(
					                    SCALE_AND_FACTOR_RANGE
							                    .fieldOf("xz_scale")
							                    .forGetter(interpolatedNoiseSampler -> interpolatedNoiseSampler.xzScale),
					                    SCALE_AND_FACTOR_RANGE
							                    .fieldOf("y_scale")
							                    .forGetter(interpolatedNoiseSampler -> interpolatedNoiseSampler.yScale),
					                    SCALE_AND_FACTOR_RANGE
							                    .fieldOf("xz_factor")
							                    .forGetter(interpolatedNoiseSampler -> interpolatedNoiseSampler.xzFactor),
					                    SCALE_AND_FACTOR_RANGE
							                    .fieldOf("y_factor")
							                    .forGetter(interpolatedNoiseSampler -> interpolatedNoiseSampler.yFactor),
					                    Codec
							                    .doubleRange(1.0, 8.0)
							                    .fieldOf("smear_scale_multiplier")
							                    .forGetter(interpolatedNoiseSampler -> interpolatedNoiseSampler.smearScaleMultiplier)
			                    )
	...
	private InterpolatedNoiseSampler(
		...
		this.maxValue = lowerInterpolatedNoise.getMaxValueForScale(this.scaledYScale);
	}
	...
	public double sample(DensityFunction.NoisePos pos) {
		...
		return MathHelper.clampedLerp(q, l / 512.0, m / 512.0) / 128.0;
	}
