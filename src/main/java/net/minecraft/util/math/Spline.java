      ...
      Codec<Spline.Implementation<C, I>> codec2 = RecordCodecBuilder.create(
         instance -> instance.group(
               ...
               Codecs.nonEmptyList(codec.listOf())
                  .fieldOf("points")
                  .forGetter(
                     (Spline.Implementation<C, I> spline) -> IntStream.range(0, spline.locations.length)
                        .mapToObj(index -> new Serialized<C, I>(spline.locations()[index], (Spline<C, I>)spline.values().get(index), spline.derivatives()[index]))
                        .toList()
                  )
            )
