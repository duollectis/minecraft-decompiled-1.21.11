...
/**
 * Тип отчёта об ошибке Minecraft. Содержит заголовок и список случайных «самородков» —
 * юмористических фраз, которые выводятся в шапке отчёта для снижения стресса пользователя.
 */
public record ReportType(String header, List<String> nuggets) {
	...
	/**
	 * Выбирает случайный «самородок» на основе текущего времени в наносекундах.
	 * При любой ошибке возвращает запасную фразу.
	 */
	public String chooseNugget() {
		try {
			return nuggets.get((int) (Util.getMeasuringTimeNano() % nuggets.size()));
	...
	/**
	 * Добавляет в {@code reportBuilder} заголовок типа отчёта, случайный самородок
	 * и дополнительные строки {@code extraInfo}.
	 *
	 * @param reportBuilder построитель строки отчёта
	 * @param extraInfo дополнительные строки, добавляемые после самородка
	 */
	public void addHeaderAndNugget(StringBuilder reportBuilder, List<String> extraInfo) {
		...
		reportBuilder.append(" ----\n");
		reportBuilder.append("// ");
		reportBuilder.append(chooseNugget());
		reportBuilder.append('\n');
