...
public class SuppressedExceptionsTracker {
   ...
   public synchronized void onSuppressedException(String location, Throwable exception) {
      ...
      this.queue.add(new SuppressedExceptionsTracker.Entry(l, location, (Class<? extends Throwable>)exception.getClass(), string));

      while (this.queue.size() > 8) {
         this.queue.remove();
      }

      SuppressedExceptionsTracker.Key key = new SuppressedExceptionsTracker.Key(location, (Class<? extends Throwable>)exception.getClass());
   ...
   public synchronized String collect() {
      ...
      if (!this.keyToCount.isEmpty()) {
         ...
         while (var6.hasNext()) {
            it.unimi.dsi.fastutil.objects.Object2IntMap.Entry<SuppressedExceptionsTracker.Key> entry2 = (it.unimi.dsi.fastutil.objects.Object2IntMap.Entry<SuppressedExceptionsTracker.Key>)var6.next();
            stringBuilder.append("\t\t\t")
