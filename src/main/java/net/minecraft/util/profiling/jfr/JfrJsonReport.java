   ...
   private JsonElement collectStructureGenSection(List<StructureGenerationSample> structureGenerationSamples) {
      ...
      if (optional.isEmpty()) {
         ...
         structureGenerationSamples.stream()
            ...
            .forEach(
               (name, samples) -> {
                  Optional<LongRunningSampleStatistics<StructureGenerationSample>> optionalx = LongRunningSampleStatistics.fromSamples(
                     (List<StructureGenerationSample>)samples
                  );
                  if (!optionalx.isEmpty()) {
                     LongRunningSampleStatistics<StructureGenerationSample> longRunningSampleStatistics2 = optionalx.get();
