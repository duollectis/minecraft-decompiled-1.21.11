...
public record LongRunningSampleStatistics<T extends LongRunningSample>(
	...
	public static <T extends LongRunningSample> Optional<LongRunningSampleStatistics<T>> fromSamples(List<T> samples) {
		if (samples.isEmpty()) {
			...
			Duration
					duration =
					list.stream().map(LongRunningSample::duration).reduce(Duration::plus).orElse(Duration.ZERO);
			...
			Map<Integer, Double>
					map =
					Quantiles.create(list.stream().mapToLong(sample -> sample.duration().toNanos()).toArray());
			return Optional.of(new LongRunningSampleStatistics<>(
					longRunningSample,
					longRunningSample2,
					longRunningSample3,
					i,
