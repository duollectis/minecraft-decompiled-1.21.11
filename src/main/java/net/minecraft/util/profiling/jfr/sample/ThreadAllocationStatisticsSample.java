...
/**
 * {@code ThreadAllocationStatisticsSample}.
 */
public record ThreadAllocationStatisticsSample(Instant time, String threadName, long allocated) {
	...
	public static ThreadAllocationStatisticsSample fromEvent(RecordedEvent event) {
		...
		String
				string =
				recordedThread == null ? "unknown"
				                       : (String) MoreObjects.firstNonNull(recordedThread.getJavaName(), "unknown");
	...
	public static ThreadAllocationStatisticsSample.AllocationMap toAllocationMap(List<ThreadAllocationStatisticsSample> samples) {
		...
		Map<String, List<ThreadAllocationStatisticsSample>>
				map2 =
				samples.stream().collect(Collectors.groupingBy(sample -> sample.threadName));
		map2.forEach((threadName, groupedSamples) -> {
			if (groupedSamples.size() >= 2) {
				ThreadAllocationStatisticsSample threadAllocationStatisticsSample = groupedSamples.get(0);
				ThreadAllocationStatisticsSample
						threadAllocationStatisticsSample2 =
						groupedSamples.get(groupedSamples.size() - 1);
				long
						l =
						Duration
								.between(threadAllocationStatisticsSample.time, threadAllocationStatisticsSample2.time)
								.getSeconds();
				long m = threadAllocationStatisticsSample2.allocated - threadAllocationStatisticsSample.allocated;
				map.put(threadName, (double) m / l);
			}
		});
