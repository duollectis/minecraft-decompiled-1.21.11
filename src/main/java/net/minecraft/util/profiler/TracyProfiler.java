...
public class TracyProfiler implements Profiler {
   ...
   public void push(String location) {
      ...
      if (SharedConstants.isDevelopment) {
         Optional<StackFrame> optional = STACK_WALKER.walk(
            stream -> stream.filter(frame -> frame.getDeclaringClass() != TracyProfiler.class && frame.getDeclaringClass() != Profiler.UnionProfiler.class)
               .findFirst()
         );
   ...
   public void visit(String marker, int num) {
      this.markers.computeIfAbsent(marker, markerName -> new TracyProfiler.Marker(this.threadName + " " + marker)).increment(num);
   }
