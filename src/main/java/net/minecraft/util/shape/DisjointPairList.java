...
public class DisjointPairList extends AbstractDoubleList implements PairList {
	...
	public boolean forEachPair(PairList.Consumer predicate) {
		return this.inverted ? this.iterateSections((x, y, index) -> predicate.merge(y, x, index))
	...
	public double getDouble(int position) {
		return position < this.first.size() ? this.first.getDouble(position)
		                                    : this.second.getDouble(position - this.first.size());
	}
