...
public abstract class VoxelShape {
	...
	protected int getCoordIndex(Direction.Axis axis, double coord) {
		return MathHelper.binarySearch(0, this.voxels.getSize(axis) + 1, i -> coord < this.getPointPosition(axis, i))
				- 1;
	}
		...
		if (this.isEmpty()) {
			...
			if (vec3d.lengthSquared() < 1.0E-7) {
				...
				return this.voxels
				       ...
				       ? new BlockHitResult(
						vec3d2,
						Direction.getFacing(vec3d.x, vec3d.y, vec3d.z).getOpposite(),
						pos,
						true
				)
	...
	public Optional<Vec3d> getClosestPointTo(Vec3d target) {
		if (this.isEmpty()) {
			...
			MutableObject<Vec3d> mutableObject = new MutableObject();
			this.forEachBox((minX, minY, minZ, maxX, maxY, maxZ) -> {
				...
				if (vec3d2 == null || target.squaredDistanceTo(d, e, f) < target.squaredDistanceTo(vec3d2)) {
					mutableObject.setValue(new Vec3d(d, e, f));
				}
			});
