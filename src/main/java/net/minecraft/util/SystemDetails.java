...
public class SystemDetails {
	...
	public SystemDetails() {
		...
		this.addSection(
				"Memory", () -> {
					...
					long p = m / 1048576L;
					long q = n / 1048576L;
					return n + " bytes (" + q + " MiB) / " + m + " bytes (" + p + " MiB) up to " + l + " bytes (" + o
							+ " MiB)";
				}
		);
		...
		this.addSection("JVM Flags", () -> collectJvmArguments(flag -> flag.startsWith("-X")));
		this.addSection("Debug Flags", () -> collectJvmArguments(flag -> flag.startsWith("-DMC_DEBUG_")));
	}
	...
	private void addPhysicalMemoryGroup(List<PhysicalMemory> memories) {
		...
		for (PhysicalMemory physicalMemory : memories) {
			...
			this.addSection(
					string + "capacity (MiB)",
					() -> String.format(Locale.ROOT, "%.2f", toMebibytes(physicalMemory.getCapacity()))
			);
			this.addSection(
					string + "clockSpeed (GHz)",
					() -> String.format(Locale.ROOT, "%.2f", (float) physicalMemory.getClockSpeed() / 1.0E9F)
			);
	...
	private void addVirtualMemoryGroup(VirtualMemory virtualMemory) {
		this.addSection(
				...
				() -> String.format(Locale.ROOT, "%.2f", toMebibytes(virtualMemory.getVirtualMax()))
		);
		this.addSection(
				"Virtual memory used (MiB)",
				() -> String.format(Locale.ROOT, "%.2f", toMebibytes(virtualMemory.getVirtualInUse()))
		);
		this.addSection(
				"Swap memory total (MiB)",
				() -> String.format(Locale.ROOT, "%.2f", toMebibytes(virtualMemory.getSwapTotal()))
		);
		this.addSection(
				"Swap memory used (MiB)",
				() -> String.format(Locale.ROOT, "%.2f", toMebibytes(virtualMemory.getSwapUsed()))
		);
	}
	...
	private void addGraphicsCardGroup(List<GraphicsCard> graphicsCards) {
		...
		for (GraphicsCard graphicsCard : graphicsCards) {
			...
			this.addSection(
					string + "VRAM (MiB)",
					() -> String.format(Locale.ROOT, "%.2f", toMebibytes(graphicsCard.getVRam()))
			);
	...
	private void addProcessorGroup(CentralProcessor centralProcessor) {
		...
		this.addSection(
				"Frequency (GHz)",
				() -> String.format(Locale.ROOT, "%.2f", (float) processorIdentifier.getVendorFreq() / 1.0E9F)
		);
		this.addSection(
				"Number of physical packages",
				() -> String.valueOf(centralProcessor.getPhysicalPackageCount())
		);
