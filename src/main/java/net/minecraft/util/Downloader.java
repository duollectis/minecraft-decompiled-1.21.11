...
public class Downloader implements AutoCloseable {
	...
	private final SimpleConsecutiveExecutor
			executor =
			new SimpleConsecutiveExecutor(Util.getDownloadWorkerExecutor(), "download-queue");
	...
	record FileInfo(String name, long size) {
		...
		public static final Codec<Downloader.FileInfo> CODEC = RecordCodecBuilder.create(
				instance -> instance.group(
						                    Codec.STRING.fieldOf("name").forGetter(Downloader.FileInfo::name),
						                    Codec.LONG.fieldOf("size").forGetter(Downloader.FileInfo::size)
				                    )
	...
	record LogEntry(
		...
		public static final Codec<Downloader.LogEntry> CODEC = RecordCodecBuilder.create(
				instance -> instance.group(
						                    ...
						                    Codec
								                    .mapEither(Codec.STRING.fieldOf("error"), Downloader.FileInfo.CODEC.fieldOf("file"))
								                    .forGetter(Downloader.LogEntry::errorOrFileInfo)
				                    )
