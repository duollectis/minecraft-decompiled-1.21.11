...
public class Codecs {
	...
	public static final Codec<Vector2fc> VECTOR_2F = Codec.FLOAT
			...
			.comapFlatMap(
					list -> Util
							...
							.map(listx -> new Vector2f((Float) listx.get(0), (Float) listx.get(1))),
	...
	public static final Codec<AxisAngle4f> AXIS_ANGLE_4F = RecordCodecBuilder.create(
			instance -> instance.group(
					                    ...
					                    VECTOR_3F
							                    .fieldOf("axis")
							                    .forGetter(axisAngle -> new Vector3f(axisAngle.x, axisAngle.y, axisAngle.z))
			                    )
			                    .apply(instance, AxisAngle4f::new)
	);
	public static final Codec<Quaternionfc>
			ROTATION =
			Codec.withAlternative(QUATERNION_F, AXIS_ANGLE_4F.xmap(Quaternionf::new, AxisAngle4f::new));
	public static final Codec<Matrix4fc> MATRIX_4F = Codec.FLOAT.listOf().comapFlatMap(
			list -> Util.decodeFixedLengthList(list, 16).map(listx -> {
	...
	public static final Codec<Integer>
			RGB =
			Codec.withAlternative(Codec.INT, VECTOR_3F, vec -> ColorHelper.fromFloats(1.0F, vec.x(), vec.y(), vec.z()));
	public static final Codec<Integer>
			ARGB =
			Codec.withAlternative(
					Codec.INT,
					VECTOR_4F,
					vec -> ColorHelper.fromFloats(vec.w(), vec.x(), vec.y(), vec.z())
			);
	public static final Codec<Integer>
			HEX_RGB =
			Codec.withAlternative(hexColor(6).xmap(ColorHelper::fullAlpha, ColorHelper::zeroAlpha), RGB);
	...
	public static final Codec<Integer> UNSIGNED_BYTE = Codec.BYTE
			.flatComapMap(
					UnsignedBytes::toInt,
					value -> value > 255 ? DataResult.error(() -> "Unsigned byte was too large: " + value + " > 255")
					                     : DataResult.success(value.byteValue())
			);
	public static final Codec<Integer>
			NON_NEGATIVE_INT =
			rangedInt(0, Integer.MAX_VALUE, v -> "Value must be non-negative: " + v);
	public static final Codec<Integer>
			POSITIVE_INT =
			rangedInt(1, Integer.MAX_VALUE, v -> "Value must be positive: " + v);
	public static final Codec<Long>
			NON_NEGATIVE_LONG =
			rangedLong(0L, Long.MAX_VALUE, v -> "Value must be non-negative: " + v);
	...
	public static final Codec<Float>
			NON_NEGATIVE_FLOAT =
			rangedInclusiveFloat(0.0F, Float.MAX_VALUE, v -> "Value must be non-negative: " + v);
	public static final Codec<Float>
			POSITIVE_FLOAT =
			rangedFloat(0.0F, Float.MAX_VALUE, v -> "Value must be positive: " + v);
	...
	public static final Codec<Instant>
			INSTANT =
			formattedTime(DateTimeFormatter.ISO_INSTANT).xmap(Instant::from, Function.identity());
	...
	public static final Codec<String> ESCAPED_STRING = Codec.STRING
			.comapFlatMap(
					string -> DataResult.success(StringEscapeUtils.unescapeJava(string)),
	...
	public static final Function<Optional<Long>, OptionalLong>
			...
			optional -> optional.map(OptionalLong::of)
	...
	public static final Function<OptionalLong, Optional<Long>>
			...
			optionalLong -> optionalLong.isPresent()
			                ...
			                : Optional.empty();
	public static final Codec<BitSet>
			BIT_SET =
			Codec.LONG_STREAM.xmap(stream -> BitSet.valueOf(stream.toArray()), set -> Arrays.stream(set.toLongArray()));
	...
	private static final Codec<Property> GAME_PROFILE_PROPERTY = RecordCodecBuilder.create(
			instance -> instance.group(
					                    ...
					                    Codec
							                    .sizeLimitedString(1024)
							                    .optionalFieldOf("signature")
							                    .forGetter(property -> Optional.ofNullable(property.signature()))
			                    )
			                    .apply(
					                    instance,
					                    (key, value, signature) -> new Property(
							                    key,
							                    value,
							                    (String) signature.orElse(null)
					                    )
			                    )
	);
	public static final Codec<PropertyMap> GAME_PROFILE_PROPERTY_MAP = Codec.either(
			                                                                        Codec.unboundedMap(Codec.STRING, Codec.STRING.listOf())
			                                                                             .validate(map -> map.size() > 16 ? DataResult.error(() ->
			                                                                                                                                 "Cannot have more than 16 properties, but was "
			                                                                                                                                 + map.size()) : DataResult.success(map)),
	...
	public static final Codec<String> IDENTIFIER_PATH = Codec.STRING
			.validate(
					path -> !Identifier.isPathValid(path) ? DataResult.error(() ->
					                                                         "Invalid string to use as a resource path element: "
					                                                         + path) : DataResult.success(path)
			);
		...
		return Codec.STRING.comapFlatMap(
				value -> {
					if (!value.startsWith("#")) {
						...
						if (j != hexDigits) {
							return DataResult.error(() -> "Hex color is wrong size, expected " + hexDigits
									+ " digits but got " + j);
						}
						else {
							try {
								long m = HexFormat.fromHexDigitsToLong(value, "#".length(), value.length());
								return m >= 0L && m <= l ? DataResult.success((int) m)
								                         : DataResult.error(() -> "Color value out of range: " + value);
							}
	...
	public static <P, I> Codec<I> createCodecForPairObject(
		...
		Codec<I> codec2 = Codec.list(codec).comapFlatMap(
				list -> Util.decodeFixedLengthList(list, 2).flatMap(listx -> {
		...
		Codec<I> codec3 = RecordCodecBuilder.<Pair<P, P>>create(
				                                    instance -> instance.group(
						                                                        codec.fieldOf(leftFieldName).forGetter((Pair<P, P> p) -> p.getFirst()),
						                                                        codec.fieldOf(rightFieldName).forGetter((Pair<P, P> p) -> p.getSecond())
				                                                        )
				                                                        .apply(instance, Pair::of)
		                                    )
		                                    .comapFlatMap(
				                                    (Pair<P, P> pair) -> combineFunction.apply(
						                                    pair.getFirst(),
						                                    pair.getSecond()
				                                    ),
				                                    (I pair) -> Pair.of(
						                                    leftFunction.apply(pair),
						                                    rightFunction.apply(pair)
				                                    )
		                                    );
		Codec<I> codec4 = Codec.withAlternative(codec2, codec3);
		return Codec.either(codec, codec4)
		            .comapFlatMap(
				            (Either<P, I> either) -> either.map(
						            (P object) -> combineFunction.apply(object, object),
						            DataResult::success
				            ),
				            (I pair) -> {
	...
	public static <E> Codec<E> rawIdChecked(
			ToIntFunction<E> elementToRawId,
			IntFunction<@Nullable E> rawIdToElement,
			int errorRawId
	) {
		return Codec.INT
				.flatXmap(
						...
						element -> {
							...
							return j == errorRawId ? DataResult.error(() -> "Element with unknown id: " + element)
	...
	public static <I, E> Codec<E> idChecked(
			Codec<I> idCodec,
			Function<I, @Nullable E> idToElement,
			Function<E, @Nullable I> elementToId
	) {
		return idCodec.flatXmap(
				id -> {
					E object = idToElement.apply((I) id);
					return object == null ? DataResult.error(() -> "Unknown element id: " + id)
					                      : DataResult.success(object);
				}, element -> {
					I object = elementToId.apply((E) element);
					return object == null ? DataResult.error(() -> "Element with unknown id: " + element)
					                      : DataResult.success(object);
				}
		);
	}
	...
	public static <E> Codec<E> withLifecycle(
			Codec<E> originalCodec,
			Function<E, Lifecycle> entryLifecycleGetter,
		...
		return originalCodec.mapResult(new ResultFunction<E>() {
			...
			public <T> DataResult<Pair<E, T>> apply(DynamicOps<T> ops, T input, DataResult<Pair<E, T>> result) {
				return result
						.result()
						.map(pair -> result.setLifecycle(entryLifecycleGetter.apply((E) pair.getFirst())))
						.orElse(result);
			}
	...
	public static <K, V> Codecs.StrictUnboundedMapCodec<K, V> strictUnboundedMap(
			Codec<K> keyCodec,
	...
	public static <E> Codec<List<E>> listOrSingle(Codec<E> entryCodec, Codec<List<E>> listCodec) {
		return Codec.either(listCodec, entryCodec)
		            .xmap(
				            either -> (List) either.map(list -> list, List::of),
	...
	private static Codec<Long> rangedLong(long min, long max, Function<Long, String> messageFactory) {
		return Codec.LONG
				.validate(
						value -> value.compareTo(min) >= 0L && value.compareTo(max) <= 0L ? DataResult.success(value)
						                                                                  : DataResult.error(() -> messageFactory.apply(
								                                                                  value))
				);
	}
	...
	private static Codec<Float> rangedInclusiveFloat(
			float minInclusive,
			float maxInclusive,
	...
	private static Codec<Float> rangedFloat(
			float minExclusive,
			float maxInclusive,
			Function<Float, String> messageFactory
	) {
	...
	public static Codec<Float> rangedInclusiveFloat(float minInclusive, float maxInclusive) {
		return rangedInclusiveFloat(
				minInclusive,
				maxInclusive,
				value -> "Value must be within range [" + minInclusive + ";" + maxInclusive + "]: " + value
		);
	}
		...
		return collection -> {
			...
			if (iterator.hasNext()) {
				...
				while (iterator.hasNext()) {
					...
					if (object3 != object) {
						return DataResult.error(() -> "Mixed type list: element " + object2 + " had type " + object3
								+ ", but list is of type " + object);
					}
				}
			}
				...
				codec, new Decoder<A>() {
					...
					public <T> DataResult<Pair<A, T>> decode(DynamicOps<T> ops, T input) {
						try {
							...
							return DataResult.error(() -> "Caught exception decoding " + input + ": "
									+ var4.getMessage());
						}
					}
				}
	...
	public record StrictUnboundedMapCodec<K, V>(
		...
		public <T> DataResult<Map<K, V>> decode(DynamicOps<T> ops, MapLike<T> input) {
			...
			for (Pair<T, T> pair : input.entries().toList()) {
				...
				if (optional.isPresent()) {
					...
					return DataResult.error(() -> dataResult.result().isPresent() ? "Map entry '" + dataResult
					                                                                                .result()
					                                                                                .get() + "' : "
					                                                                + string : string);
				}
		...
		public <T> DataResult<Pair<Map<K, V>, T>> decode(DynamicOps<T> ops, T input) {
			return ops
					...
					.flatMap(map -> this.decode(ops, map))
