...
/**
 * Отображение рецепта крафта с формой (shaped). Хранит размеры сетки,
 * список слотов ингредиентов (размер = width * height), результат и станок.
 */
public record ShapedCraftingRecipeDisplay(
	...
	public static final PacketCodec<RegistryByteBuf, ShapedCraftingRecipeDisplay> PACKET_CODEC = PacketCodec.tuple(
			PacketCodecs.VAR_INT, ShapedCraftingRecipeDisplay::width,
			PacketCodecs.VAR_INT, ShapedCraftingRecipeDisplay::height,
			SlotDisplay.PACKET_CODEC.collect(PacketCodecs.toList()), ShapedCraftingRecipeDisplay::ingredients,
			SlotDisplay.PACKET_CODEC, ShapedCraftingRecipeDisplay::result,
			SlotDisplay.PACKET_CODEC, ShapedCraftingRecipeDisplay::craftingStation,
			ShapedCraftingRecipeDisplay::new
	);
	public static final RecipeDisplay.Serializer<ShapedCraftingRecipeDisplay> SERIALIZER = new RecipeDisplay.Serializer<>(
			CODEC, PACKET_CODEC
	);
