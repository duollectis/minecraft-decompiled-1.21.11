...
public class SnbtParsing {
   ...
   public static <T> PackratParser<T> createParser(DynamicOps<T> ops) {
            ...
            Term.sequence(
               ...
               Term.anyOf(
                  Term.sequence(
                     ...
                     Term.always(symbol2, new SnbtParsing.NumberSuffix(SnbtParsing.Signedness.UNSIGNED, SnbtParsing.NumericType.BYTE))
                  ),
                  Term.sequence(
                     Literals.character('s', 'S'),
                     Term.always(symbol2, new SnbtParsing.NumberSuffix(SnbtParsing.Signedness.UNSIGNED, SnbtParsing.NumericType.SHORT))
                  ),
                  Term.sequence(
                     Literals.character('i', 'I'),
                     Term.always(symbol2, new SnbtParsing.NumberSuffix(SnbtParsing.Signedness.UNSIGNED, SnbtParsing.NumericType.INT))
                  ),
                  Term.sequence(
                     Literals.character('l', 'L'),
                     Term.always(symbol2, new SnbtParsing.NumberSuffix(SnbtParsing.Signedness.UNSIGNED, SnbtParsing.NumericType.LONG))
                  )
               )
            ),
            Term.sequence(
               ...
               Term.anyOf(
                  Term.sequence(
                     Literals.character('b', 'B'),
                     Term.always(symbol2, new SnbtParsing.NumberSuffix(SnbtParsing.Signedness.SIGNED, SnbtParsing.NumericType.BYTE))
                  ),
                  Term.sequence(
                     Literals.character('s', 'S'),
                     Term.always(symbol2, new SnbtParsing.NumberSuffix(SnbtParsing.Signedness.SIGNED, SnbtParsing.NumericType.SHORT))
                  ),
                  Term.sequence(
                     Literals.character('i', 'I'),
                     Term.always(symbol2, new SnbtParsing.NumberSuffix(SnbtParsing.Signedness.SIGNED, SnbtParsing.NumericType.INT))
                  ),
                  Term.sequence(
                     Literals.character('l', 'L'),
                     Term.always(symbol2, new SnbtParsing.NumberSuffix(SnbtParsing.Signedness.SIGNED, SnbtParsing.NumericType.LONG))
                  )
               )
            ),
      ...
      ParsingRuleEntry<StringReader, Entry<String, T>> parsingRuleEntry4 = parsingRules.set(
         symbol30, Term.sequence(parsingRules.term(symbol29), Literals.character(':'), parsingRules.term(symbol26)), (ParsingRule.RuleAction<StringReader, Entry<String, T>>) state -> {
           ParseResults parseResults = state.getResults();
   ...
   static enum ArrayType {
      ...
      public abstract <T> @Nullable T decode(DynamicOps<T> ops, List<SnbtParsing.IntValue> values, ParsingState<?> state);
