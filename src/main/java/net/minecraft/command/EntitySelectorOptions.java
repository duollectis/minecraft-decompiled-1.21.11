...
public class EntitySelectorOptions {
		...
		if (OPTIONS.isEmpty()) {
			...
			putOption(
					...
					reader -> {
						...
						if ((!doubleRange.getMin().isPresent() || !((Double) doubleRange.getMin().get() < 0.0))
								&& (!doubleRange.getMax().isPresent() || !((Double) doubleRange.getMax().get() < 0.0
						)
						)) {
					...
					reader -> reader.getDistance() == null,
			...
			putOption(
					...
					reader -> {
						...
						if ((!intRange.getMin().isPresent() || (Integer) intRange.getMin().get() >= 0)
								&& (!intRange.getMax().isPresent() || (Integer) intRange.getMax().get() >= 0)) {
					...
					reader -> reader.getLevelRange() == null,
			...
			putOption(
					"x_rotation",
					reader -> reader.setPitchRange(NumberRange.AngleRange.parse(reader.getReader())),
					reader -> reader.getPitchRange() == null,
					Text.translatable("argument.entity.options.x_rotation.description")
			);
			putOption(
					"y_rotation",
					reader -> reader.setYawRange(NumberRange.AngleRange.parse(reader.getReader())),
					reader -> reader.getYawRange() == null,
			...
			putOption(
					...
					reader -> {
						...
						reader.setSuggestionProvider(
								(builder, consumer) -> CommandSource.suggestMatching(
										Arrays.asList(
												"nearest",
												"furthest",
												"random",
					...
					reader -> {
						reader.setSuggestionProvider((builder, consumer) -> {
							...
							if (!reader.excludesEntityType()) {
								...
								CommandSource.suggestIdentifiers(
										Registries.ENTITY_TYPE
												.streamTags()
												.map(tag -> tag.getTag().id()), builder, String.valueOf('#')
								);
							}
						...
						if (reader.excludesEntityType() && !bl) {
							...
							if (reader.readTagCharacter()) {
								...
								EntityType<?>
										entityType =
										Registries.ENTITY_TYPE.getOptionalValue(identifier).orElseThrow(() -> {
											reader.getReader().setCursor(i);
			...
			putOption(
					"tag", reader -> {
						boolean bl = reader.readNegationCharacter();
						String string = reader.getReader().readUnquotedString();
						reader.addPredicate(entity -> "".equals(string) ? entity.getCommandTags().isEmpty() != bl
						                                                : entity.getCommandTags().contains(string)
						                                                  != bl);
						...
						if (!map.isEmpty()) {
							reader.addPredicate(entity -> {
								...
								for (Entry<String, NumberRange.IntRange> entry : map.entrySet()) {
									ScoreboardObjective
											scoreboardObjective =
											scoreboard.getNullableObjective(entry.getKey());
									...
									ReadableScoreboardScore
											readableScoreboardScore =
											scoreboard.getScore(entity, scoreboardObjective);
			...
			putOption(
					...
					reader -> {
						...
						while (stringReader.canRead() && stringReader.peek() != '}') {
							...
							if (stringReader.canRead() && stringReader.peek() == '{') {
								...
								stringReader.skipWhitespace();
								map.put(
										identifier, advancementProgress -> {
											for (Entry<String, Predicate<CriterionProgress>> entry : map2.entrySet()) {
												CriterionProgress
														criterionProgress =
														advancementProgress.getCriterionProgress(entry.getKey());
												if (criterionProgress == null || !entry
														.getValue()
							...
							reader.addPredicate(entity -> {
								if (!(entity instanceof ServerPlayerEntity serverPlayerEntity)) {
									...
									ServerAdvancementLoader
											serverAdvancementLoader =
											serverPlayerEntity.getEntityWorld().getServer().getAdvancementLoader();
									...
									for (Entry<Identifier, Predicate<AdvancementProgress>> entry : map.entrySet()) {
										...
										if (advancementEntry == null || !entry
												.getValue()
												.test(playerAdvancementTracker.getProgress(advancementEntry))) {
											return false;
										}
									}
					...
					reader -> !reader.selectsAdvancements(),
			...
			putOption(
					...
					reader -> {
						boolean bl = reader.readNegationCharacter();
						RegistryKey<LootCondition>
								registryKey =
								RegistryKey.of(RegistryKeys.PREDICATE, Identifier.fromCommandInput(reader.getReader()));
						reader.addPredicate(
								entity -> {
									if (entity.getEntityWorld() instanceof ServerWorld serverWorld) {
										...
										if (optional.isEmpty()) {
											...
											LootWorldContext
													lootWorldContext =
													new LootWorldContext.Builder(serverWorld)
															.add(LootContextParameters.THIS_ENTITY, entity)
															.add(LootContextParameters.ORIGIN, entity.getEntityPos())
															.build(LootContextTypes.SELECTOR);
											LootContext
													lootContext =
													new LootContext.Builder(lootWorldContext).build(Optional.empty());
	...
	public static EntitySelectorOptions.SelectorHandler getHandler(
			EntitySelectorReader reader,
			String option,
			int restoreCursor
