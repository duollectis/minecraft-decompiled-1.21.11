...
/**
 * {@code ItemPredicateArgumentType}.
 */
public class ItemPredicateArgumentType extends ParserBackedArgumentType<ItemPredicateArgumentType.ItemStackPredicateArgument> {
	...
	private static final Collection<String>
			EXAMPLES =
			Arrays.asList("stick", "minecraft:stick", "#stick", "#stick{foo:'bar'}");
	...
	static final Dynamic2CommandExceptionType MALFORMED_ITEM_COMPONENT_EXCEPTION = new Dynamic2CommandExceptionType(
			(componentId, component) -> Text.stringifiedTranslatable(
	...
	static final Dynamic2CommandExceptionType MALFORMED_ITEM_PREDICATE_EXCEPTION = new Dynamic2CommandExceptionType(
			(predicateId, predicate) -> Text.stringifiedTranslatable(
	...
	static final Map<Identifier, ItemPredicateArgumentType.ComponentCheck> SPECIAL_COMPONENT_CHECKS = Stream.of(
			                                                                                                        new ItemPredicateArgumentType.ComponentCheck(
					                                                                                                        COUNT_ID,
					                                                                                                        stack -> true,
					                                                                                                        NumberRange.IntRange.CODEC.map(range -> stack -> range.test(stack.getCount()))
			                                                                                                        )
	                                                                                                        )
	                                                                                                        .collect(
			                                                                                                        Collectors.toUnmodifiableMap(
					                                                                                                        ItemPredicateArgumentType.ComponentCheck::id,
					                                                                                                        check -> (ItemPredicateArgumentType.ComponentCheck) check
			                                                                                                        ));
	static final Map<Identifier, ItemPredicateArgumentType.SubPredicateCheck> SPECIAL_SUB_PREDICATE_CHECKS = Stream.of(
			                                                                                                               new ItemPredicateArgumentType.SubPredicateCheck(
					                                                                                                               COUNT_ID,
					                                                                                                               NumberRange.IntRange.CODEC.map(range -> stack -> range.test(stack.getCount()))
			                                                                                                               )
	                                                                                                               )
	                                                                                                               .collect(
			                                                                                                               Collectors.toUnmodifiableMap(
					                                                                                                               ...
					                                                                                                               check -> (ItemPredicateArgumentType.SubPredicateCheck) check
			                                                                                                               ));
	...
	private static ItemPredicateArgumentType.SubPredicateCheck hasComponentCheck(RegistryEntry.Reference<ComponentType<?>> type) {
		...
		return new ItemPredicateArgumentType.SubPredicateCheck(
				type.registryKey().getValue(),
				Unit.CODEC.map(v -> predicate)
		);
	}

	public ItemPredicateArgumentType(CommandRegistryAccess commandRegistryAccess) {
		super(ItemPredicateParsing
				.createParser(new ItemPredicateArgumentType.Context(commandRegistryAccess))
				.map(predicates -> Util.allOf(predicates)::test));
	}
	...
	public static ItemPredicateArgumentType.ItemStackPredicateArgument getItemStackPredicate(
			CommandContext<ServerCommandSource> context,
			String name
	) {
		return (ItemPredicateArgumentType.ItemStackPredicateArgument) context.getArgument(
				name,
				ItemPredicateArgumentType.ItemStackPredicateArgument.class
		);
	}
	 ...
	 * {@code ComponentCheck}.
	 */
	record ComponentCheck(
			Identifier id,
			Predicate<ItemStack> presenceChecker,
			Decoder<? extends Predicate<ItemStack>> valueChecker
	) {

		public static <T> ItemPredicateArgumentType.ComponentCheck read(
			...
			if (codec == null) {
				...
				return new ItemPredicateArgumentType.ComponentCheck(
						id, stack -> stack.contains(type), codec.map(expected -> stack -> {
		...
		public Predicate<ItemStack> createPredicate(ImmutableStringReader reader, Dynamic<?> value)
		throws CommandSyntaxException {
			...
			return (Predicate<ItemStack>) dataResult.getOrThrow(
					error -> ItemPredicateArgumentType.MALFORMED_ITEM_COMPONENT_EXCEPTION.createWithContext(
							reader,
							this.id.toString(),
							error
					)
	...
	static class Context
		...
		Context(RegistryWrapper.WrapperLookup registries) {
			...
			this.itemSubPredicateTypeRegistryWrapper =
					registries.getOrThrow(RegistryKeys.DATA_COMPONENT_PREDICATE_TYPE);
		}

		public Predicate<ItemStack> itemMatchPredicate(
			...
			RegistryEntry.Reference<Item> reference = this.itemRegistryWrapper
					...
					.orElseThrow(() -> ItemPredicateArgumentType.INVALID_ITEM_ID_EXCEPTION.createWithContext(
		...
		public Predicate<ItemStack> tagMatchPredicate(
				ImmutableStringReader immutableStringReader,
				Identifier identifier
		) throws CommandSyntaxException {
			RegistryEntryList<Item> registryEntryList = this.itemRegistryWrapper
					.getOptional(TagKey.of(RegistryKeys.ITEM, identifier))
					.orElseThrow(() -> ItemPredicateArgumentType.UNKNOWN_ITEM_TAG_EXCEPTION.createWithContext(
							immutableStringReader,
							identifier
					));
		...
		public ItemPredicateArgumentType.ComponentCheck componentCheck(
				ImmutableStringReader immutableStringReader,
				Identifier identifier
		) throws CommandSyntaxException {
			ItemPredicateArgumentType.ComponentCheck
					componentCheck =
					ItemPredicateArgumentType.SPECIAL_COMPONENT_CHECKS.get(identifier);
			if (componentCheck != null) {
				...
				ComponentType<?> componentType = this.dataComponentTypeRegistryWrapper
						...
						.orElseThrow(() -> ItemPredicateArgumentType.UNKNOWN_ITEM_COMPONENT_EXCEPTION.createWithContext(
								immutableStringReader,
								identifier
						));
		...
		public Predicate<ItemStack> componentMatchPredicate(
				ImmutableStringReader immutableStringReader,
				ItemPredicateArgumentType.ComponentCheck componentCheck,
				Dynamic<?> dynamic
		) throws CommandSyntaxException {
			return componentCheck.createPredicate(
					immutableStringReader,
					RegistryOps.withRegistry(dynamic, this.registries)
			);
		}
		...
		public ItemPredicateArgumentType.SubPredicateCheck subPredicateCheck(
				ImmutableStringReader immutableStringReader,
				Identifier identifier
		) throws CommandSyntaxException {
			ItemPredicateArgumentType.SubPredicateCheck
					subPredicateCheck =
					ItemPredicateArgumentType.SPECIAL_SUB_PREDICATE_CHECKS.get(identifier);
			return subPredicateCheck != null
			       ...
			       : this.itemSubPredicateTypeRegistryWrapper
			         ...
			         .orElseThrow(() -> ItemPredicateArgumentType.UNKNOWN_ITEM_PREDICATE_EXCEPTION.createWithContext(
					         immutableStringReader,
					         identifier
			         ));
		}
		...
		public Predicate<ItemStack> subPredicatePredicate(
				ImmutableStringReader immutableStringReader,
				ItemPredicateArgumentType.SubPredicateCheck subPredicateCheck,
				Dynamic<?> dynamic
		) throws CommandSyntaxException {
			return subPredicateCheck.createPredicate(
					immutableStringReader,
					RegistryOps.withRegistry(dynamic, this.registries)
			);
		}
	...
	record SubPredicateCheck(Identifier id, Decoder<? extends Predicate<ItemStack>> type) {
		...
		throws CommandSyntaxException {
			...
			return (Predicate<ItemStack>) dataResult.getOrThrow(
					error -> ItemPredicateArgumentType.MALFORMED_ITEM_PREDICATE_EXCEPTION.createWithContext(
							reader,
							this.id.toString(),
							error
					)
