...
public record CollectionPredicate<T, P extends Predicate<T>>(
   ...
   public static <T, P extends Predicate<T>> Codec<CollectionPredicate<T, P>> createCodec(Codec<P> predicateCodec) {
      return RecordCodecBuilder.create(
         instance -> instance.group(
               CollectionContainsPredicate.createCodec(predicateCodec).optionalFieldOf("contains").forGetter(CollectionPredicate::contains),
               CollectionCountsPredicate.createCodec(predicateCodec).optionalFieldOf("count").forGetter(CollectionPredicate::counts),
   ...
   public boolean test(Iterable<T> iterable) {
      if (this.contains.isPresent() && !this.contains.get().test(iterable)) {
         ...
         return this.counts.isPresent() && !this.counts.get().test(iterable) ? false : !this.size.isPresent() || this.size.get().test(Iterables.size(iterable));
      }
   }
}
