...
public interface NumberRange<T extends Number & Comparable<T>> {
	...
	SimpleCommandExceptionType
			EXCEPTION_EMPTY =
			new SimpleCommandExceptionType(Text.translatable("argument.range.empty"));

	SimpleCommandExceptionType
			EXCEPTION_SWAPPED =
			new SimpleCommandExceptionType(Text.translatable("argument.range.swapped"));
	...
	public record AngleRange(NumberRange.Bounds<Float> bounds) implements NumberRange<Float> {
		...
		public static final Codec<NumberRange.AngleRange> CODEC = NumberRange.Bounds.createCodec(Codec.FLOAT)
		                                                                            .xmap(
				                                                                            NumberRange.AngleRange::new,
				                                                                            NumberRange.AngleRange::bounds
		                                                                            );
		public static final PacketCodec<ByteBuf, NumberRange.AngleRange>
				PACKET_CODEC =
				NumberRange.Bounds.createPacketCodec(PacketCodecs.FLOAT)
	...
	public record Bounds<T extends Number & Comparable<T>>(Optional<T> min, Optional<T> max) {
		...
		public static <T extends Number & Comparable<T>> NumberRange.Bounds<T> any() {
			return new NumberRange.Bounds<T>(Optional.empty(), Optional.empty());
		}
		...
		static <T extends Number & Comparable<T>> Codec<NumberRange.Bounds<T>> createCodec(Codec<T> valueCodec) {
			Codec<NumberRange.Bounds<T>> codec = RecordCodecBuilder.create(
					instance -> instance.group(
							                    valueCodec.optionalFieldOf("min").forGetter(NumberRange.Bounds::min),
							                    valueCodec.optionalFieldOf("max").forGetter(NumberRange.Bounds::max)
					                    )
					                    .apply(instance, NumberRange.Bounds::new)
			);
			return Codec.either(codec, valueCodec).xmap(
					either -> (NumberRange.Bounds) either.map(bounds -> bounds, value -> exactly((T) value)),
					bounds -> {
						Optional<T> optional = bounds.getPoint();
						return optional.isPresent() ? Either.right(optional.get()) : Either.left(bounds);
					}
			);
		}

		static <B extends ByteBuf, T extends Number & Comparable<T>> PacketCodec<B, NumberRange.Bounds<T>> createPacketCodec(
				PacketCodec<B, T> valuePacketCodec
		) {
			return new PacketCodec<B, NumberRange.Bounds<T>>() {
				...
				public NumberRange.Bounds<T> decode(B byteBuf) {
					...
					Optional<T>
							optional =
							(b & 1) != 0 ? Optional.of(valuePacketCodec.decode(byteBuf)) : Optional.empty();
					Optional<T>
							optional2 =
							(b & 2) != 0 ? Optional.of(valuePacketCodec.decode(byteBuf)) : Optional.empty();
		...
		private static <T extends Number> Optional<T> parseNumber(
			...
			if (string.isEmpty()) {
				...
				try {
					return Optional.of(parsingFunction.apply(string));
				}
	...
	public record DoubleRange(
			...
			NumberRange.Bounds<Double> boundsSqr
		...
		public static final Codec<NumberRange.DoubleRange> CODEC = NumberRange.Bounds.createCodec(Codec.DOUBLE)
		                                                                             .validate(NumberRange.Bounds::validate)
		                                                                             .xmap(
				                                                                             NumberRange.DoubleRange::new,
				                                                                             NumberRange.DoubleRange::bounds
		                                                                             );
		public static final PacketCodec<ByteBuf, NumberRange.DoubleRange>
				PACKET_CODEC =
				NumberRange.Bounds.createPacketCodec(PacketCodecs.DOUBLE)
		...
		public boolean test(double value) {
			return this.bounds.min.isPresent() && this.bounds.min.get() > value ? false
			                                                                    : this.bounds.max.isEmpty() || !(
					                                                                    this.bounds.max.get() < value
			                                                                    );
		}
	...
	public record IntRange(
		...
		public static final Codec<NumberRange.IntRange> CODEC = NumberRange.Bounds.createCodec(Codec.INT)
		                                                                          .validate(NumberRange.Bounds::validate)
		                                                                          .xmap(
				                                                                          NumberRange.IntRange::new,
				                                                                          NumberRange.IntRange::bounds
		                                                                          );
		public static final PacketCodec<ByteBuf, NumberRange.IntRange>
				PACKET_CODEC =
				NumberRange.Bounds.createPacketCodec(PacketCodecs.INTEGER)
		...
		public boolean test(int value) {
			return this.bounds.min.isPresent() && this.bounds.min.get() > value ? false : this.bounds.max.isEmpty()
			                                                                              || this.bounds.max.get()
			                                                                                 >= value;
		}

		public boolean testSqrt(long value) {
			return this.boundsSqr.min.isPresent() && this.boundsSqr.min.get() > value ? false
			                                                                          : this.boundsSqr.max.isEmpty()
			                                                                            || this.boundsSqr.max.get()
			                                                                               >= value;
		}
