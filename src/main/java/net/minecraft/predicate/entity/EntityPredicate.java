      ...
      if (entity == null) {
         ...
         if (this.location.located.isPresent() && !this.location.located.get().test(world, entity.getX(), entity.getY(), entity.getZ())) {
            ...
            if (this.effects.isPresent() && !this.effects.get().test(entity)) {
               ...
               return false;
            } else if (this.passenger.isPresent() && entity.getPassengerList().stream().noneMatch(entityx -> this.passenger.get().test(world, pos, entityx))) {
               return false;
            } else if (this.targetedEntity.isPresent()
               && !this.targetedEntity.get().test(world, pos, entity instanceof MobEntity ? ((MobEntity)entity).getTarget() : null)) {
               return false;
               ...
               if (this.slots.isPresent() && !this.slots.get().matches(entity)) {
                  ...
                  return !this.components.test((ComponentsAccess)entity) ? false : this.nbt.isEmpty() || this.nbt.get().test(entity);
               }
            }
         }
      }
