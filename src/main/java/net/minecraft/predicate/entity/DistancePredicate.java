...
public record DistancePredicate(
   ...
   public static DistancePredicate horizontal(NumberRange.DoubleRange horizontal) {
      return new DistancePredicate(
         NumberRange.DoubleRange.ANY, NumberRange.DoubleRange.ANY, NumberRange.DoubleRange.ANY, horizontal, NumberRange.DoubleRange.ANY
      );
   }
   ...
   public boolean test(double x0, double y0, double z0, double x1, double y1, double z1) {
      ...
      if (!this.x.test(MathHelper.abs(f)) || !this.y.test(MathHelper.abs(g)) || !this.z.test(MathHelper.abs(h))) {
         ...
         return !this.horizontal.testSqrt(f * f + h * h) ? false : this.absolute.testSqrt(f * f + g * g + h * h);
      }
   }
}
