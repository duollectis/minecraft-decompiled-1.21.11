...
public record StatePredicate(List<StatePredicate.Condition> conditions) {
   private static final Codec<List<StatePredicate.Condition>> CONDITION_LIST_CODEC = Codec.unboundedMap(Codec.STRING, StatePredicate.ValueMatcher.CODEC)
      .xmap(
         states -> states.entrySet()
            ...
            .toList(),
         conditions -> conditions.stream().collect(Collectors.toMap(StatePredicate.Condition::key, StatePredicate.Condition::valueMatcher))
      );
   ...
   public static class Builder {
      private final com.google.common.collect.ImmutableList.Builder<StatePredicate.Condition> conditions = ImmutableList.builder();
