...
public interface EnchantmentEffectComponentTypes {
	...
	Codec<ComponentType<?>>
			...
			Codec.lazyInitialized(() -> Registries.ENCHANTMENT_EFFECT_COMPONENT_TYPE.getCodec());
	...
	ComponentType<List<EnchantmentEffectEntry<EnchantmentValueEffect>>> DAMAGE_PROTECTION = register(
			"damage_protection",
			builder -> builder.codec(EnchantmentEffectEntry
					.createCodec(EnchantmentValueEffect.CODEC, LootContextTypes.ENCHANTED_DAMAGE)
					.listOf())
	);
	...
	ComponentType<List<EnchantmentEffectEntry<DamageImmunityEnchantmentEffect>>> DAMAGE_IMMUNITY = register(
			"damage_immunity",
			builder -> builder.codec(EnchantmentEffectEntry
					.createCodec(DamageImmunityEnchantmentEffect.CODEC, LootContextTypes.ENCHANTED_DAMAGE)
					.listOf())
	);
	...
	ComponentType<List<TargetedEnchantmentEffect<EnchantmentEntityEffect>>> POST_ATTACK = register(
			"post_attack",
			builder -> builder.codec(TargetedEnchantmentEffect
					.createPostAttackCodec(EnchantmentEntityEffect.CODEC, LootContextTypes.ENCHANTED_DAMAGE)
					.listOf())
	);

	ComponentType<List<EnchantmentEffectEntry<EnchantmentEntityEffect>>> POST_PIERCING_ATTACK = register(
			"post_piercing_attack",
			builder -> builder.codec(EnchantmentEffectEntry
					.createCodec(EnchantmentEntityEffect.CODEC, LootContextTypes.ENCHANTED_DAMAGE)
					.listOf())
	);
	...
	ComponentType<List<EnchantmentEffectEntry<EnchantmentEntityEffect>>> HIT_BLOCK = register(
			"hit_block",
			builder -> builder.codec(EnchantmentEffectEntry
					.createCodec(EnchantmentEntityEffect.CODEC, LootContextTypes.HIT_BLOCK)
					.listOf())
	);
	...
	ComponentType<List<EnchantmentEffectEntry<EnchantmentValueEffect>>> ITEM_DAMAGE = register(
			"item_damage",
			builder -> builder.codec(EnchantmentEffectEntry
					.createCodec(EnchantmentValueEffect.CODEC, LootContextTypes.ENCHANTED_ITEM)
					.listOf())
	);
	...
	ComponentType<List<TargetedEnchantmentEffect<EnchantmentValueEffect>>> EQUIPMENT_DROPS = register(
			"equipment_drops",
			builder -> builder.codec(TargetedEnchantmentEffect
					.createEquipmentDropsCodec(EnchantmentValueEffect.CODEC, LootContextTypes.ENCHANTED_DAMAGE)
					.listOf())
	);
	...
	ComponentType<List<EnchantmentEffectEntry<EnchantmentLocationBasedEffect>>> LOCATION_CHANGED = register(
			"location_changed",
			builder -> builder.codec(EnchantmentEffectEntry
					.createCodec(EnchantmentLocationBasedEffect.CODEC, LootContextTypes.ENCHANTED_LOCATION)
					.listOf())
	);
	...
	ComponentType<List<EnchantmentEffectEntry<EnchantmentEntityEffect>>> TICK = register(
			"tick",
			builder -> builder.codec(EnchantmentEffectEntry
					.createCodec(EnchantmentEntityEffect.CODEC, LootContextTypes.ENCHANTED_ENTITY)
					.listOf())
	);
	...
	ComponentType<List<EnchantmentEffectEntry<EnchantmentValueEffect>>> PROJECTILE_SPREAD = register(
			"projectile_spread",
			builder -> builder.codec(EnchantmentEffectEntry
					.createCodec(EnchantmentValueEffect.CODEC, LootContextTypes.ENCHANTED_ENTITY)
					.listOf())
	);
	...
	ComponentType<EnchantmentValueEffect>
			CROSSBOW_CHARGE_TIME =
			register("crossbow_charge_time", builder -> builder.codec(EnchantmentValueEffect.CODEC));
	...
	ComponentType<List<RegistryEntry<SoundEvent>>>
			TRIDENT_SOUND =
			register("trident_sound", builder -> builder.codec(SoundEvent.ENTRY_CODEC.listOf()));

	ComponentType<Unit>
			PREVENT_EQUIPMENT_DROP =
			register("prevent_equipment_drop", builder -> builder.codec(Unit.CODEC));
