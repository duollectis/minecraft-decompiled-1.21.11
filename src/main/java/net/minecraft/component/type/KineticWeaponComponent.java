      ...
      this.sound
         .ifPresent(
            sound -> entity.getEntityWorld()
               .playSound(entity, entity.getX(), entity.getY(), entity.getZ(), (RegistryEntry<SoundEvent>)sound, entity.getSoundCategory(), 1.0F, 1.0F)
         );
   }

   public void playHitSound(Entity entity) {
      this.hitSound.ifPresent(hitSound -> entity.getEntityWorld().playSoundFromEntityClient(entity, hitSound.value(), entity.getSoundCategory(), 1.0F, 1.0F));
   }

   public int getUseTicks() {
      return this.delayTicks + this.damageConditions.map(KineticWeaponComponent.Condition::maxDurationTicks).orElse(0);
   }
      ...
      if (i >= this.delayTicks) {
         ...
         for (EntityHitResult entityHitResult : (Collection<EntityHitResult>)ProjectileUtil.collectPiercingCollisions(
            ...
            if (!bl2) {
               ...
               boolean bl5 = this.damageConditions.isPresent() && this.damageConditions.get().isSatisfied(i, d, h, f);
               if (bl3 || bl4 || bl5) {
