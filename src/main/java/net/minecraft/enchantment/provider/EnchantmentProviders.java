...
/**
 * Реестровые ключи и bootstrap-метод для встроенных провайдеров зачарований.
 */
public interface EnchantmentProviders {
	...
	static void bootstrap(Registerable<EnchantmentProvider> registry) {
		...
		registry.register(
			...
			new SingleEnchantmentProvider(enchantments.getOrThrow(Enchantments.PIERCING), ConstantIntProvider.create(1))
		);
		registry.register(
			PILLAGER_POST_WAVE_3_RAID,
			new SingleEnchantmentProvider(enchantments.getOrThrow(Enchantments.QUICK_CHARGE), ConstantIntProvider.create(1))
		);
		registry.register(
			PILLAGER_POST_WAVE_5_RAID,
			new SingleEnchantmentProvider(enchantments.getOrThrow(Enchantments.QUICK_CHARGE), ConstantIntProvider.create(2))
		);
		registry.register(
			VINDICATOR_RAID,
			new SingleEnchantmentProvider(enchantments.getOrThrow(Enchantments.SHARPNESS), ConstantIntProvider.create(1))
		);
		registry.register(
			VINDICATOR_POST_WAVE_5_RAID,
			new SingleEnchantmentProvider(enchantments.getOrThrow(Enchantments.SHARPNESS), ConstantIntProvider.create(2))
		);
		registry.register(
			ENDERMAN_LOOT_DROP,
			new SingleEnchantmentProvider(enchantments.getOrThrow(Enchantments.SILK_TOUCH), ConstantIntProvider.create(1))
		);
	}
