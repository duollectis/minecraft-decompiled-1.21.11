	...
	static void bootstrap(Registerable<EnchantmentProvider> registry) {
		...
		registry.register(
				...
				new ByCostWithDifficultyEnchantmentProvider(
						registryEntryLookup.getOrThrow(EnchantmentTags.ON_MOB_SPAWN_EQUIPMENT),
						5,
						17
				)
		);
		registry.register(
				...
				new SingleEnchantmentProvider(
						registryEntryLookup.getOrThrow(Enchantments.PIERCING),
		...
		registry.register(
				...
				new SingleEnchantmentProvider(
						registryEntryLookup.getOrThrow(Enchantments.QUICK_CHARGE),
						ConstantIntProvider.create(1)
				)
		);
		registry.register(
		...
		registry.register(VINDICATOR_RAID,
				new SingleEnchantmentProvider(
						registryEntryLookup.getOrThrow(Enchantments.SHARPNESS),
						ConstantIntProvider.create(1)
				)
		);
		registry.register(
		...
		registry.register(
				ENDERMAN_LOOT_DROP,
				new SingleEnchantmentProvider(
						registryEntryLookup.getOrThrow(Enchantments.SILK_TOUCH),
						ConstantIntProvider.create(1)
				)
		);
	}
