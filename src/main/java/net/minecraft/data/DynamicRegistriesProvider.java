            ...
            registries -> {
               ...
               return CompletableFuture.allOf(
                  RegistryLoader.DYNAMIC_REGISTRIES
                     .stream()
                     .flatMap(entry -> this.writeRegistryEntries(writer, registries, dynamicOps, (RegistryLoader.Entry<?>)entry).stream())
            ...
            wrapper -> {
               ...
               return CompletableFuture.allOf(
                  wrapper.streamEntries()
                     .map(
                        entryx -> writeToPath(pathResolver.resolveJson(entryx.registryKey().getValue()), writer, ops, registry.elementCodec(), entryx.value())
                     )
                     .toArray(CompletableFuture[]::new)
               );
            }
