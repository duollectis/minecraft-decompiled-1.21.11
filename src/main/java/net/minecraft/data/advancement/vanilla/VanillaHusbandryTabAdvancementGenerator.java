...
/**
 * {@code VanillaHusbandryTabAdvancementGenerator}.
 */
public class VanillaHusbandryTabAdvancementGenerator implements AdvancementTabGenerator {
	...
	public static final List<EntityType<?>>
			EGG_LAYING_ANIMALS =
			List.of(EntityType.TURTLE, EntityType.FROG, EntityType.SNIFFER);
	...
	private static final Item[]
			FISH_BUCKET_ITEMS =
			new Item[]{Items.COD_BUCKET, Items.TROPICAL_FISH_BUCKET, Items.PUFFERFISH_BUCKET, Items.SALMON_BUCKET};
	...
	private static final Comparator<RegistryEntry.Reference<?>>
			REGISTRY_ENTRY_COMPARATOR =
			Comparator.comparing(entry -> entry.registryKey().getValue());
	...
	public void accept(RegistryWrapper.WrapperLookup registries, Consumer<AdvancementEntry> exporter) {
		...
		AdvancementEntry advancementEntry3 = Advancement.Builder.create()
		                                                        ...
		                                                        .criterion(
				                                                        "bred",
				                                                        BredAnimalsCriterion.Conditions.any()
		                                                        )
		...
		Advancement.Builder.create()
		                   ...
		                   .criterion(
				                   "kill_axolotl_target",
				                   EffectsChangedCriterion.Conditions.create(EntityPredicate.Builder
						                   .create()
						                   .type(registryEntryLookup, EntityType.AXOLOTL))
		                   )
		                   .display(
		...
		Advancement.Builder.create()
		                   ...
		                   .criterion(
				                   "torchflower",
				                   ItemCriterion.Conditions.createPlacedBlock(Blocks.TORCHFLOWER_CROP)
		                   )
		...
		Advancement.Builder.create()
		                   ...
		                   .display(
				                   Items.DRIED_GHAST,
				                   Text.translatable("advancements.husbandry.place_dried_ghast_in_water.title"),
				                   Text.translatable("advancements.husbandry.place_dried_ghast_in_water.description"),
				                   null,
				                   AdvancementFrame.TASK,
				                   true,
				                   true,
		                   ...
		                   .build(exporter, "husbandry/place_dried_ghast_in_water");
	}
	...
	public static AdvancementEntry createBreedAllAnimalsAdvancement(
		...
		return requireListedAnimalsBred(
				Advancement.Builder.create(),
				breedableAnimals,
				entityTypeLookup,
				eggLayingAnimals
	...
	private static Advancement.Builder requireFoodItemsEaten(
			Advancement.Builder builder,
			RegistryEntryLookup<Item> itemLookup
	) {
		for (Item item : FOOD_ITEMS) {
			builder.criterion(
					...
					ConsumeItemCriterion.Conditions.item(itemLookup, item)
			);
		}
	...
	private static Advancement.Builder requireListedAnimalsBred(
		...
		breedableAnimals.forEach(
				entityType -> builder.criterion(
						...
						BredAnimalsCriterion.Conditions.create(EntityPredicate.Builder
								.create()
								.type(entityTypeLookup, (EntityType<?>) entityType))
				)
		);
	...
	private static Advancement.Builder requireListedFishBucketsFilled(
			Advancement.Builder builder,
			RegistryEntryLookup<Item> itemLookup
	) {
		for (Item item : FISH_BUCKET_ITEMS) {
			builder.criterion(
					Registries.ITEM.getId(item).getPath(),
					FilledBucketCriterion.Conditions.create(ItemPredicate.Builder.create().items(itemLookup, item))
			);
		}
	...
	private static Advancement.Builder requireListedFishCaught(
			Advancement.Builder builder,
			RegistryEntryLookup<Item> itemLookup
	) {
		for (Item item : FISH_ITEMS) {
			builder.criterion(
					...
					FishingRodHookedCriterion.Conditions.create(
							Optional.empty(),
							Optional.empty(),
							Optional.of(ItemPredicate.Builder.create().items(itemLookup, item).build())
					)
			);
		}
	...
	private static Advancement.Builder requireAllCatsTamed(
			Advancement.Builder builder,
			RegistryWrapper<CatVariant> catVariantRegistry
	) {
	...
	private static Advancement.Builder requireAllWolvesTamed(
			Advancement.Builder builder,
			RegistryWrapper<WolfVariant> wolfVariantRegistry
	) {
