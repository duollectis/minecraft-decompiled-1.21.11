...
public class VanillaHusbandryTabAdvancementGenerator implements AdvancementTabGenerator {
   ...
   public void accept(RegistryWrapper.WrapperLookup registries, Consumer<AdvancementEntry> exporter) {
      ...
      Advancement.Builder.create()
         ...
         .criterion("place_dried_ghast_in_water", ItemCriterion.Conditions.createPlacedWithState(Blocks.DRIED_GHAST, Properties.WATERLOGGED, true))
   ...
   private static Advancement.Builder requireListedAnimalsBred(
      ...
      breedableAnimals.forEach(
         entityType -> builder.criterion(
            ...
            BredAnimalsCriterion.Conditions.create(EntityPredicate.Builder.create().type(entityTypeLookup, (EntityType<?>)entityType))
         )
      );
      eggLayingAnimals.forEach(
         entityType -> builder.criterion(
            ...
            BredAnimalsCriterion.Conditions.create(
               Optional.of(EntityPredicate.Builder.create().type(entityTypeLookup, (EntityType<?>)entityType).build()),
               Optional.of(EntityPredicate.Builder.create().type(entityTypeLookup, (EntityType<?>)entityType).build()),
               Optional.empty()
            )
         )
   ...
   private static Advancement.Builder requireListedFishBucketsFilled(Advancement.Builder builder, RegistryEntryLookup<Item> itemLookup) {
      for (Item item : FISH_BUCKET_ITEMS) {
         builder.criterion(
            Registries.ITEM.getId(item).getPath(), FilledBucketCriterion.Conditions.create(ItemPredicate.Builder.create().items(itemLookup, item))
         );
      }
      ...
      for (Item item : FISH_ITEMS) {
         builder.criterion(
            ...
            FishingRodHookedCriterion.Conditions.create(
               Optional.empty(), Optional.empty(), Optional.of(ItemPredicate.Builder.create().items(itemLookup, item).build())
            )
         );
      }
