...
public class DependencyTracker<K, V extends DependencyTracker.Dependencies<K>> {
	...
	private void traverse(Multimap<K, K> parentChild, Set<K> visited, K rootKey, BiConsumer<K, V> callback) {
		if (visited.add(rootKey)) {
			...
			V dependencies = this.underlying.get(rootKey);
	...
	private static <K> boolean containsReverseDependency(Multimap<K, K> dependencies, K key, K dependency) {
		...
		return collection.contains(key) ? true : collection
		                                         .stream()
		                                         .anyMatch(subdependency -> containsReverseDependency(
				                                         dependencies,
				                                         key,
				                                         (K) subdependency
		                                         ));
	...
	public void traverse(BiConsumer<K, V> callback) {
		...
		this.underlying.forEach((key, value) -> value.forDependencies(dependency -> addDependency(
				multimap,
				(K) key,
				dependency
		)));
		this.underlying.forEach((key, value) -> value.forOptionalDependencies(dependency -> addDependency(
				multimap,
				(K) key,
				dependency
		)));
