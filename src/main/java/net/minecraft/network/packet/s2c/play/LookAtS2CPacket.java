...
public class LookAtS2CPacket implements Packet<ClientPlayPacketListener> {
	...
	public static final PacketCodec<PacketByteBuf, LookAtS2CPacket>
			CODEC =
			Packet.createCodec(LookAtS2CPacket::write, LookAtS2CPacket::new);
	...
	public LookAtS2CPacket(
			EntityAnchorArgumentType.EntityAnchor selfAnchor,
			double targetX,
			double targetY,
			double targetZ
	) {
	...
	public @Nullable Vec3d getTargetPosition(World world) {
		if (this.lookAtEntity) {
			...
			return entity == null ? new Vec3d(this.targetX, this.targetY, this.targetZ)
			                      : this.targetAnchor.positionAt(entity);
		}
		else {
