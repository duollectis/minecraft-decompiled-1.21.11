...
public record CommandSuggestionsS2CPacket(int id, int start, int length, List<CommandSuggestionsS2CPacket.Suggestion> suggestions)
   ...
   public CommandSuggestionsS2CPacket(int completionId, Suggestions suggestions) {
      this(
         ...
         suggestions.getList()
            .stream()
            .map(
               suggestion -> new CommandSuggestionsS2CPacket.Suggestion(suggestion.getText(), Optional.ofNullable(suggestion.getTooltip()).map(Texts::toText))
            )
            .toList()
      );
   }
   ...
   public Suggestions getSuggestions() {
      ...
      return new Suggestions(
         ...
         this.suggestions
            .stream()
            .map(suggestion -> new com.mojang.brigadier.suggestion.Suggestion(stringRange, suggestion.text(), suggestion.tooltip().orElse(null)))
            .toList()
      );
   }
