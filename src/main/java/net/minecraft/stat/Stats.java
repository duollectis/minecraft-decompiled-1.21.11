...
/**
 * Реестр всех типов статистики и пользовательских (custom) статистик игры.
 *
 * <p>Содержит девять типов статистики, сгруппированных по категориям объектов
 * (блоки, предметы, сущности), а также набор именованных {@link Identifier}-констант
 * для пользовательских статистик (перемещение, урон, взаимодействие и т.д.).
 *
 * <p>Все поля инициализируются при загрузке класса через статические методы
 * {@link #registerType} и {@link #register}, которые одновременно регистрируют
 * объекты в соответствующих реестрах Minecraft.
 */
public class Stats {

	// -------------------------------------------------------------------------
	// Типы статистики по категориям объектов
	// -------------------------------------------------------------------------
	...
	// -------------------------------------------------------------------------
	// Пользовательские статистики — общие действия
	// -------------------------------------------------------------------------
	...
	// -------------------------------------------------------------------------
	// Пользовательские статистики — перемещение
	// -------------------------------------------------------------------------
	...
	public static final Identifier HAPPY_GHAST_ONE_CM = register("happy_ghast_one_cm", StatFormatter.DISTANCE);
	...
	public static final Identifier NAUTILUS_ONE_CM = register("nautilus_one_cm", StatFormatter.DISTANCE);

	// -------------------------------------------------------------------------
	// Пользовательские статистики — боевые и прочие действия
	// -------------------------------------------------------------------------
	...
	// -------------------------------------------------------------------------
	// Пользовательские статистики — взаимодействие с блоками
	// -------------------------------------------------------------------------
	...
	/**
	 * Регистрирует пользовательскую статистику в реестре {@code CUSTOM_STAT}
	 * и создаёт для неё {@link Stat} с указанным форматтером.
	 *
	 * @param id        строковый идентификатор статистики (без namespace)
	 * @param formatter форматтер для отображения значения
	 * @return {@link Identifier} зарегистрированной статистики
	 */
	...
	/**
	 * Регистрирует новый тип статистики в реестре {@code STAT_TYPE}.
	 *
	 * @param id       строковый идентификатор типа (без namespace)
	 * @param registry реестр объектов, к которым привязан тип
	 * @param <T>      тип объектов реестра
	 * @return зарегистрированный {@link StatType}
	 */
	private static <T> StatType<T> registerType(String id, Registry<T> registry) {
		Text name = Text.translatable("stat_type.minecraft." + id);
		return Registry.register(Registries.STAT_TYPE, id, new StatType<>(registry, name));
	}
}
