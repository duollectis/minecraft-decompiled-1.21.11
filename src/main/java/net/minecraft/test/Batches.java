...
public class Batches {
	...
	public static List<GameTestBatch> batch(
		...
		Map<RegistryEntry<TestEnvironmentDefinition>, List<GameTestState>> map = instances.stream()
		                                                                                  .flatMap(instance -> decorator.decorate(
		...
		return map.entrySet().stream().flatMap(entry -> {
			...
			return Streams.mapWithIndex(
					...
					(states, index) -> create(states, registryEntry, (int) index)
			);
		}).toList();
	}
	...
	public static TestRunContext.Batcher batcher(int batchSize) {
		return states -> {
			...
			return map.entrySet().stream().flatMap(entry -> {
				...
				List<GameTestState> list = entry.getValue();
				return Streams.mapWithIndex(
						Lists.partition(list, batchSize).stream(),
						(statesx, index) -> create(List.copyOf(statesx), registryEntry, (int) index)
				);
			}).toList();
		};
	}
