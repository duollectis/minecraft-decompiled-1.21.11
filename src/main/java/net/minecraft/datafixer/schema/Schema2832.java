...
/**
 * Схема версии 2832 (Minecraft 1.18 — Caves & Cliffs, часть II).
 * <p>
 * Вводит обновлённый формат чанка без вложенного тега {@code Level}: данные сущностей,
 * блок-сущностей, тиков блоков, секций с биомами и состояниями блоков, а также
 * структур теперь хранятся напрямую в корне чанка (строчные ключи вместо PascalCase).
 * Также регистрирует тип {@code MULTI_NOISE_BIOME_SOURCE_PARAMETER_LIST} и обновляет
 * {@code WORLD_GEN_SETTINGS} с поддержкой пресетов для источника биомов {@code multi_noise}.
 */
									...
									DSL.taggedChoiceLazy(
										...
										ImmutableMap.of(
											...
											(Supplier<TypeTemplate>) () -> DSL.or(
												DSL.fields(
													"preset",
													TypeReferences.MULTI_NOISE_BIOME_SOURCE_PARAMETER_LIST.in(schema)
												),
												DSL.list(DSL.fields(
													"biome",
													TypeReferences.BIOME.in(schema)
