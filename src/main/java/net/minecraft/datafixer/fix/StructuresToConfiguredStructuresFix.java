...
public class StructuresToConfiguredStructuresFix extends DataFix {
	...
	private static final Map<String, StructuresToConfiguredStructuresFix.Mapping>
			STRUCTURE_TO_CONFIGURED_STRUCTURES_MAPPING =
			ImmutableMap.<String, StructuresToConfiguredStructuresFix.Mapping>builder()
			            .put(
					            "mineshaft",
			            ...
			            .put(
					            ...
					            StructuresToConfiguredStructuresFix.Mapping.create(
							            Map.of(
									            List.of("minecraft:beach", "minecraft:snowy_beach"),
			            ...
			            .put(
					            "pillager_outpost",
					            StructuresToConfiguredStructuresFix.Mapping.create("minecraft:pillager_outpost")
			            )
			            .put("mansion", StructuresToConfiguredStructuresFix.Mapping.create("minecraft:mansion"))
			            .put(
					            "jungle_pyramid",
					            StructuresToConfiguredStructuresFix.Mapping.create("minecraft:jungle_pyramid")
			            )
			            .put(
					            "desert_pyramid",
					            StructuresToConfiguredStructuresFix.Mapping.create("minecraft:desert_pyramid")
			            )
			            ...
			            .put(
					            "buried_treasure",
					            StructuresToConfiguredStructuresFix.Mapping.create("minecraft:buried_treasure")
			            )
			            .put(
					            "nether_fossil",
					            StructuresToConfiguredStructuresFix.Mapping.create("minecraft:nether_fossil")
			            )
			            .put(
					            "bastion_remnant",
					            StructuresToConfiguredStructuresFix.Mapping.create("minecraft:bastion_remnant")
			            )
			            .build();
	...
	private Dynamic<?> fixChunk(Dynamic<?> chunkDynamic) {
		return chunkDynamic.update(
				"structures",
				structuresDynamic -> structuresDynamic
						.update("starts", startsDynamic -> this.fixStructureStarts(startsDynamic, chunkDynamic))
						.update(
								"References",
								referencesDynamic -> this.fixStructureReferences(referencesDynamic, chunkDynamic)
						)
		);
	}
	...
	private Dynamic<?> fixStructureStarts(Dynamic<?> startsDynamic, Dynamic<?> chunkDynamic) {
		...
		map.forEach((structureId, startDynamic) -> {
			if (!startDynamic.get("id").asString("INVALID").equals("INVALID")) {
				...
				if (dynamic2 == null) {
					LOGGER.warn(
							"Encountered unknown structure in datafixer: {}",
							structureId.asString("<missing key>")
					);
				}
				else {
		...
		map.forEach(
				(structureId, referenceDynamic) -> {
					if (referenceDynamic.asLongStream().count() != 0L) {
						Dynamic<?>
								dynamic2 =
								this.mapStructureToConfiguredStructure((Dynamic<?>) structureId, chunkDynamic);
						if (dynamic2 == null) {
							LOGGER.warn(
									"Encountered unknown structure in datafixer: {}",
									structureId.asString("<missing key>")
							...
							hashMap.compute(
									...
									(configuredStructureId, referenceDynamicx) -> referenceDynamicx == null
									                                              ? referenceDynamic
									                                              : referenceDynamic.createLongList(
											                                              LongStream.concat(
													                                              referenceDynamicx.asLongStream(),
													                                              referenceDynamic.asLongStream()
											                                              ))
							);
						}
					}
	...
	private @Nullable Dynamic<?> mapStructureToConfiguredStructure(
			Dynamic<?> structureIdDynamic,
			Dynamic<?> chunkDynamic
	) {
	...
	private Optional<String> getBiomeRepresentativeStructure(
			Dynamic<?> chunkDynamic,
			StructuresToConfiguredStructuresFix.Mapping mappingForStructure
	) {
		Object2IntArrayMap<String> object2IntArrayMap = new Object2IntArrayMap();
		chunkDynamic.get("sections")
		            .asList(Function.identity())
		            .forEach(sectionDynamic -> sectionDynamic
				            .get("biomes")
				            .get("palette")
				            .asList(Function.identity())
				            .forEach(biomePaletteDynamic -> {
					            String
							            string =
							            mappingForStructure.biomeMapping().get(biomePaletteDynamic.asString(""));
					            if (string != null) {
 ...
 */
	record Mapping(Map<String, String> biomeMapping, String fallback) {
		...
		public static StructuresToConfiguredStructuresFix.Mapping create(
				Map<List<String>, String> biomeMapping,
				String fallback
		) {
