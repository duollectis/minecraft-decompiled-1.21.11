...
/**
 * Конвертирует устаревшие двухбуквенные коды паттернов баннера (например, {@code "b"}, {@code "bl"})
 * в полные namespace-идентификаторы (например, {@code "minecraft:base"}, {@code "minecraft:square_bottom_left"}).
 * Также переименовывает поле {@code Patterns} → {@code patterns} и конвертирует цвет.
 */
public class BannerPatternFormatFix extends ChoiceFix {
	...
	private static Dynamic<?> replacePatterns(Dynamic<?> dynamic) {
		return dynamic.renameAndFixField(
			"Patterns",
			"patterns",
			patterns -> patterns.createList(patterns.asStream().map(BannerPatternFormatFix::replacePatternAndColor))
		);
	}
	...
	private static Dynamic<?> replacePatternAndColor(Dynamic<?> dynamic) {
		dynamic = dynamic.renameAndFixField(
			"Pattern",
			"pattern",
			patternValue -> (Dynamic<?>) DataFixUtils.orElse(
				patternValue.asString()
					.map(code -> OLD_TO_NEW_PATTERNS.getOrDefault(code, code))
					.map(patternValue::createString)
					.result(),
				patternValue
			)
		);
