...
public class BannerPatternFormatFix extends ChoiceFix {
	...
	private static Dynamic<?> replacePatterns(Dynamic<?> dynamic) {
		return dynamic.renameAndFixField(
				"Patterns",
				"patterns",
				dynamicx -> dynamicx.createList(dynamicx.asStream().map(BannerPatternFormatFix::replacePatternAndColor))
		);
	}
	...
	private static Dynamic<?> replacePatternAndColor(Dynamic<?> dynamic) {
		dynamic = dynamic.renameAndFixField(
				...
				dynamicx -> (Dynamic) DataFixUtils.orElse(
						dynamicx
								.asString()
								.map(string -> OLD_TO_NEW_PATTERNS.getOrDefault(string, string))
								.map(dynamicx::createString)
								.result(), dynamicx
				)
		);
