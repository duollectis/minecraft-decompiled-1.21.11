      ...
      return this.writeFixAndRead(
         ...
         dynamic -> {
            ...
            if (optional.isPresent()) {
               ...
               dynamic = Dynamic.copyField(optional.get(), "using_converts_to", dynamic, "minecraft:use_remainder");
               dynamic = dynamic.set("minecraft:food", optional.get().remove("eat_seconds").remove("effects").remove("using_converts_to"));
               return dynamic.set(
                  "minecraft:consumable",
                  dynamic.emptyMap().set("consume_seconds", dynamic.createFloat(f)).set("on_consume_effects", dynamic.createList(stream2))
               );
            } else {
               return dynamic;
            }
