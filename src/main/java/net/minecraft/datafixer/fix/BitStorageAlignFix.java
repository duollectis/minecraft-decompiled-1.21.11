...
public class BitStorageAlignFix extends DataFix {
	...
	protected TypeRewriteRule makeRule() {
		...
		return this.fixTypeEverywhereTyped(
				...
				chunk -> chunk.updateTyped(
						opticFinder,
						level -> this.fixHeightmaps(fixLevel(opticFinder2, opticFinder3, opticFinder4, level))
				)
		);
	}
		...
		return fixedLevel.update(
				...
				levelDynamic -> levelDynamic.update(
						...
						heightmapsDynamic -> heightmapsDynamic.updateMapValues(
								heightmap -> heightmap.mapSecond(heightmapDynamic -> fixBitStorageArray(
										levelDynamic,
										heightmapDynamic,
										256,
										9
	...
	private static Typed<?> fixLevel(
			OpticFinder<?> levelSectionsFinder,
				...
				levelSection -> levelSection.updateTyped(
						...
						section -> {
							int
									i =
									section
											.getOptional(paletteFinder)
											.map(palette -> Math.max(4, DataFixUtils.ceillog2(palette.size())))
											.orElse(0);
							return i != 0 && !MathHelper.isPowerOfTwo(i)
							       ? section.update(
									...
									sectionDynamic -> sectionDynamic.update(
											"BlockStates",
											statesDynamic -> fixBitStorageArray(sectionDynamic, statesDynamic, 4096, i)
									)
							)
							       : section;
						}
