...
public class ParticleUnflatteningFix extends DataFix {
	...
	private <T> Dynamic<T> fixParticle(Dynamic<T> dynamic) {
		...
		if (optional.isEmpty()) {
			...
			Dynamic<T>
					dynamic2 =
					dynamic.createMap(Map.of(dynamic.createString("type"), dynamic.createString(string2)));
			...
			return switch (string2) {
				...
				case "minecraft:dust_color_transition" ->
						strings.length > 1 ? this.fixDustColorTransitionParticle(dynamic2, strings[1]) : dynamic2;
				case "minecraft:sculk_charge" ->
						strings.length > 1 ? this.fixSculkChargeParticle(dynamic2, strings[1]) : dynamic2;
				case "minecraft:vibration" ->
						strings.length > 1 ? this.fixVibrationParticle(dynamic2, strings[1]) : dynamic2;
	...
	private <T> Dynamic<T> fixBlockParticle(Dynamic<T> dynamic, String params) {
		...
		if (i == -1) {
			...
			dynamic2 =
					dynamic2.set(
							"Name",
							dynamic.createString(IdentifierNormalizingSchema.normalize(params.substring(0, i)))
					);
	...
	private static <T> Dynamic<T> parseColor(Dynamic<T> dynamic, StringReader paramsReader)
	...
	private <T> Dynamic<T> fixVibrationParticle(Dynamic<T> dynamic, String params) {
		try {
			...
			Dynamic<T>
					dynamic2 =
					(Dynamic<T>) dynamic.createIntList(IntStream.of(
							MathHelper.floor(f),
							MathHelper.floor(g),
							MathHelper.floor(h)
					));
			...
			Dynamic<T> dynamic3 = dynamic.createMap(
					(Map<Dynamic<T>, Dynamic<T>>) (Map<?, ?>) Map.of(
							dynamic.createString("type"),
							dynamic.createString("minecraft:block"),
