...
public class StatusEffectDurationFix extends DataFix {
						...
						this.fixTypeEverywhereTyped(
								"EffectDurationItem", type, itemStackTyped -> {
									if (itemStackTyped
											...
											.filter(pair -> POTION_ITEM_IDS.contains(pair.getSecond()))
											.isPresent()) {
										Optional<? extends Typed<?>>
												optional =
												itemStackTyped.getOptionalTyped(opticFinder2);
										if (optional.isPresent()) {
											Dynamic<?> dynamic = (Dynamic<?>) optional.get().get(DSL.remainderFinder());
											Typed<?>
													typed =
													optional
															.get()
	...
	private Dynamic<?> fixPotionEffect(Dynamic<?> effectDynamic) {
		return effectDynamic.update(
				"FactorCalculationData", factorCalculationDataDynamic -> {
