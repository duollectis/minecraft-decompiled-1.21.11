...
public class ItemRemoveBlockEntityTagFix extends DataFix {
		...
		return TypeRewriteRule.seq(
				this.fixTypeEverywhereTyped(
						...
						typed -> typed.updateTyped(
								opticFinder,
								typedx -> this.removeBlockEntityTag(typedx, opticFinder2, opticFinder8, "BlockEntityTag")
						)
				),
				new TypeRewriteRule[]{
						this.fixTypeEverywhereTyped(
								...
								typed -> typed.updateTyped(
										opticFinder3,
										typedx -> this.removeBlockEntityTag(
												typedx,
												opticFinder4,
												opticFinder8,
												"TileEntityData"
						...
						this.fixTypeEverywhereTyped(
								...
								typed -> typed.updateTyped(
										...
										typedx -> typedx.updateTyped(
												opticFinder6,
												typedxx -> this.removeBlockEntityTag(typedxx, opticFinder7, opticFinder8, "nbt")
										)
								)
						),
	...
	private Typed<?> removeBlockEntityTag(
			Typed<?> typed,
		...
		if (optional.isEmpty()) {
			...
			return !this.itemIds.contains(string2) ? typed : Util.apply(
					typed,
					typed.getType(),
