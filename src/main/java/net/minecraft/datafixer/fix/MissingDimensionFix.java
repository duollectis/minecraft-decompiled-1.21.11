...
public class MissingDimensionFix extends DataFix {
	...
	protected static <A1, A2> Type<Pair<Either<A1, Unit>, Pair<Either<A2, Unit>, Dynamic<?>>>> extract2Opt(
		...
		return DSL.and(
				DSL.optional(DSL.field(field1, type1)),
				DSL.optional(DSL.field(field2, type2)),
				DSL.remainderType()
		);
	}
	...
	protected TypeRewriteRule makeRule() {
		...
		CompoundListType<String, ?>
				compoundListType =
				DSL.compoundList(IdentifierNormalizingSchema.getIdentifierType(), extract1("generator", type));
		...
		if (!type3.findFieldType("dimensions").equals(type2)) {
			...
			return fixTypeEverywhereTyped(
					...
					worldGenSettingsTyped -> worldGenSettingsTyped.updateTyped(
							fieldFinder, dimensionsTyped -> dimensionsTyped.updateTyped(
									opticFinder, dimensionsListTyped -> {
										if (!(dimensionsListTyped.getValue() instanceof List)) {
											...
											Dynamic<?>
													dynamic =
													(Dynamic<?>) worldGenSettingsTyped.get(DSL.remainderFinder());
											Dynamic<?> dynamic2 = this.createDefaultDimensionSettings(dynamic);
											return (Typed) DataFixUtils.orElse(
													compoundListType
															.readTyped(dynamic2)
															.result()
															.map(Pair::getFirst), dimensionsListTyped
											);
										}
										else {
	...
	protected static Type<? extends Pair<? extends Either<? extends Pair<? extends Either<?, Unit>, ? extends Pair<? extends Either<? extends List<? extends Pair<? extends Either<?, Unit>, Dynamic<?>>>, Unit>, Dynamic<?>>>, Unit>, Dynamic<?>>> flatGeneratorType(
		...
		return extract1Opt(
				...
				extract2Opt(
						"biome",
						schema.getType(TypeReferences.BIOME),
						"layers",
						DSL.list(extract1Opt("block", schema.getType(TypeReferences.BLOCK_NAME)))
				)
		);
	}

	private <T> Dynamic<T> createDefaultDimensionSettings(Dynamic<T> worldGenSettingsDynamic) {
		...
		return new Dynamic(
				...
				StructureSeparationDataFix.createDimensionSettings(
						...
						l,
						StructureSeparationDataFix.createDefaultOverworldGeneratorSettings(worldGenSettingsDynamic, l),
						false
				)
		);
	}
