...
public class ItemIdFix extends DataFix {
	...
	public TypeRewriteRule makeRule() {
		...
		return fixTypeEverywhereTyped(
				...
				itemStackTyped -> itemStackTyped.update(
						...
						type2,
						id -> (Pair) id.map(
								ordinal -> Pair.of(TypeReferences.ITEM_NAME.typeName(), fromId(ordinal)),
								named -> named
						)
				)
		);
	...
	private static Int2ObjectMap<String> buildNumericalIdToStringIdMap() {
