...
public class OptionsKeyLwjgl3Fix extends DataFix {
	...
	private static final Int2ObjectMap<String>
			...
			buildNumericalKeyIdsToKeyNamesMap();
										...
										.map(entry -> {
											if (((Dynamic) entry.getKey()).asString("").startsWith("key_")) {
												...
												if (i < 0) {
													...
													return Pair.of(
															(Dynamic) entry.getKey(),
															((Dynamic) entry.getValue()).createString(string)
													);
												}
												else {
													String
															string2 =
															(String) NUMERICAL_KEY_IDS_TO_KEY_NAMES.getOrDefault(
																	i,
																	"key.unknown"
															);
													return Pair.of(
															(Dynamic) entry.getKey(),
															((Dynamic) entry.getValue()).createString(string2)
													);
												}
											}
											else {
	...
	private static Int2ObjectMap<String> buildNumericalKeyIdsToKeyNamesMap() {
