            ...
            .map(
               enchantments -> enchantments.map(
                  enchantmentDynamic -> enchantmentDynamic.update(
                     "id",
                     idDynamic -> (Dynamic)idDynamic.asString()
                        .map(oldId -> enchantmentDynamic.createString(this.oldToNewIds.getOrDefault(IdentifierNormalizingSchema.normalize(oldId), oldId)))
