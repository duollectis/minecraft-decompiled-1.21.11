   ...
   private static <T> Dynamic<T> updateEntity(
      ...
      return entityDynamic.map(
         object -> {
            ...
            Function<T, T> function2 = objectx -> (T)variantIntToId.apply(new Dynamic(dynamicOps, objectx)).getValue();
            return dynamicOps.get(object, oldVariantKey)
               .map(object2 -> dynamicOps.set(object, newVariantKey, function2.apply((T)object2)))
               .result()
               .orElse(object);
         }
      );
      ...
      return inputTyped.update(
         ...
         entityDynamic -> updateEntity(
            ...
            variantDynamic -> (Dynamic)DataFixUtils.orElse(
               variantDynamic.asNumber().map(variantInt -> variantDynamic.createString(this.variantIntToId.apply(variantInt.intValue()))).result(),
               variantDynamic
            )
         )
      );
