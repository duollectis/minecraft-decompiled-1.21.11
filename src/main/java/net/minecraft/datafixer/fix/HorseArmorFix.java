				...
				dynamic2 = dynamic2.update(
						...
						armorItemsDynamic -> armorItemsDynamic.createList(
								Streams.mapWithIndex(
										armorItemsDynamic.asStream(),
										(itemDynamic, slot) -> slot == 2L ? itemDynamic.emptyMap() : itemDynamic
								)
						)
				);
				dynamic2 = dynamic2.update(
						...
						armorDropChancesDynamic -> armorDropChancesDynamic.createList(
								Streams.mapWithIndex(
										...
										(dropChanceDynamic, slot) -> slot == 2L ? dropChanceDynamic.createFloat(0.085F)
										                                        : dropChanceDynamic
								)
						)
				);
