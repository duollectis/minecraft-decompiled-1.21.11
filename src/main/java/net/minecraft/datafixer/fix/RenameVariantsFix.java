   ...
   protected Typed<?> transform(Typed<?> inputTyped) {
      return inputTyped.update(
         ...
         dynamic -> dynamic.update(
            "variant",
            variant -> (Dynamic)DataFixUtils.orElse(
               variant.asString().map(variantName -> variant.createString(this.oldToNewNames.getOrDefault(variantName, variantName))).result(), variant
            )
         )
      );
   }
