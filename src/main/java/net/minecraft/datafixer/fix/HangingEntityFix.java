		...
		if ((isPainting || isItemFrame) && entityDynamic.get("Facing").asNumber().result().isEmpty()) {
			...
			if (entityDynamic.get("Direction").asNumber().result().isPresent()) {
				...
				entityDynamic =
						entityDynamic.set(
								"TileX",
								entityDynamic.createInt(entityDynamic.get("TileX").asInt(0) + is[0])
						);
				entityDynamic =
						entityDynamic.set(
								"TileY",
								entityDynamic.createInt(entityDynamic.get("TileY").asInt(0) + is[1])
						);
				entityDynamic =
						entityDynamic.set(
								"TileZ",
								entityDynamic.createInt(entityDynamic.get("TileZ").asInt(0) + is[2])
						);
				...
				if (isItemFrame && entityDynamic.get("ItemRotation").asNumber().result().isPresent()) {
					entityDynamic =
							entityDynamic.set(
									"ItemRotation",
									entityDynamic.createByte((byte) (entityDynamic.get("ItemRotation").asByte((byte) 0)
											* 2
									))
							);
				}
	...
	public TypeRewriteRule makeRule() {
		...
		TypeRewriteRule typeRewriteRule = this.fixTypeEverywhereTyped(
				...
				entityTyped -> entityTyped.updateTyped(
						opticFinder,
						type,
						paintingTyped -> paintingTyped.update(
								DSL.remainderFinder(),
								paintingDynamic -> this.fixDecorationPosition(paintingDynamic, true, false)
						)
				)
		);
		TypeRewriteRule typeRewriteRule2 = this.fixTypeEverywhereTyped(
				...
				entityTyped -> entityTyped.updateTyped(
						opticFinder2,
						type2,
						itemFrameTyped -> itemFrameTyped.update(
								DSL.remainderFinder(),
								itemFrameDynamic -> this.fixDecorationPosition(itemFrameDynamic, false, true)
						)
				)
		);
