...
/**
 * {@code UpdateSignTextFormatFix}.
 */
public class UpdateSignTextFormatFix extends ChoiceWriteReadFix {

	public static final List<String> SIGN_SIDES = List.of(
			"Text1",
			"Text2",
			"Text3",
			"Text4",
	...
	private static <T> Dynamic<T> updateFront(Dynamic<T> signData) {
		...
		Dynamic<T> dynamic2 = signData.emptyMap()
		                              ...
		                              .set(
				                              "color",
				                              signData.get("Color").result().orElse(signData.createString("black"))
		                              )
		                              .set(
				                              "has_glowing_text",
				                              signData.get("GlowingText").result().orElse(signData.createBoolean(false))
		                              );
		...
		if (list2.stream().anyMatch(Optional::isPresent)) {
			dynamic2 = dynamic2.set(
					"filtered_messages", signData.createList(Streams.mapWithIndex(
							list2.stream(), (message, index) -> {
	...
	private static <T> Stream<Optional<Dynamic<T>>> streamKeys(Dynamic<T> signData, String prefix) {
		return Stream.of(
				signData.get(prefix + "1").result(),
				signData.get(prefix + "2").result(),
				signData.get(prefix + "3").result(),
				signData.get(prefix + "4").result()
		);
	}
