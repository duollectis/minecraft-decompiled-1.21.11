...
public class ItemStackComponentizationFix extends DataFix {
   ...
   private static Dynamic<?> fixDisplay(ItemStackComponentizationFix.StackData data, Dynamic<?> dynamic, int hideFlags) {
      dynamic.get("Name").result().filter(TextFixes::method_74084).ifPresent(dynamicx -> data.setComponent("minecraft:custom_name", dynamicx));
      OptionalDynamic<?> optionalDynamic = dynamic.get("Lore");
      if (optionalDynamic.result().isPresent()) {
         data.setComponent("minecraft:lore", dynamic.createList(dynamic.get("Lore").asStream().filter(TextFixes::method_74084)));
      }
   ...
   private static <T> Dynamic<T> fixBlockEntityData(ItemStackComponentizationFix.StackData data, Dynamic<T> dynamic, String blockEntityId) {
      ...
      return switch (blockEntityId) {
         ...
         case "minecraft:banner" -> {
            ...
            if (optional2.isPresent()) {
               data.setComponent("minecraft:base_color", dynamic.createString(FixUtil.getColorName(optional2.get().intValue())));
            }
