...
public class ItemStackComponentizationFix extends DataFix {
	...
	private static void fixStack(ItemStackComponentizationFix.StackData data, Dynamic<?> dynamic) {
		...
		data.getAndRemove("BlockStateTag")
		    .result()
		    .ifPresent(blockStateTagDynamic -> data.setComponent(
				    "minecraft:block_state",
				    fixBlockStateTag((Dynamic<?>) blockStateTagDynamic)
		    ));
		data.moveToComponent("EntityTag", "minecraft:entity_data");
		data.applyFixer(
				"BlockEntityTag", false, blockEntityTagDynamic -> {
		...
		if (data.itemEquals("minecraft:crossbow")) {
			...
			data.moveToComponent(
					"ChargedProjectiles",
					"minecraft:charged_projectiles",
					dynamic.createList(Stream.empty())
			);
		}
		...
		if (data.itemEquals("minecraft:player_head")) {
			data.getAndRemove("SkullOwner")
			    .result()
			    .ifPresent(skullOwnerDynamic -> data.setComponent(
					    "minecraft:profile",
					    createProfileDynamic((Dynamic<?>) skullOwnerDynamic)
			    ));
		}
	}
	...
	private static Dynamic<?> fixBlockStateTag(Dynamic<?> dynamic) {
		return (Dynamic<?>) DataFixUtils.orElse(
				dynamic.asMapOpt().result().map(stream -> stream.collect(Collectors.toMap(
						Pair::getFirst, pair -> {
	...
	private static Dynamic<?> fixDisplay(
		...
		dynamic
				.get("Name")
				.result()
				.filter(TextFixes::isValidStrictJson)
				.ifPresent(dynamicx -> data.setComponent("minecraft:custom_name", dynamicx));
		OptionalDynamic<?> optionalDynamic = dynamic.get("Lore");
		if (optionalDynamic.result().isPresent()) {
			data.setComponent(
					"minecraft:lore",
					dynamic.createList(dynamic.get("Lore").asStream().filter(TextFixes::isValidStrictJson))
			);
		}
	...
	private static <T> Dynamic<T> fixBlockEntityData(
			ItemStackComponentizationFix.StackData data,
			Dynamic<T> dynamic,
		...
		return switch (blockEntityId) {
			...
			case "minecraft:decorated_pot" -> {
				...
				if (optional2.isPresent()) {
					data.setComponent(
							"minecraft:container",
							dynamic.createList(Stream.of(dynamic
									.emptyMap()
									.set("slot", dynamic.createInt(0))
									.set("item", optional2.get())))
					);
				}
			...
			case "minecraft:banner" -> {
				...
				if (optional2.isPresent()) {
					data.setComponent(
							...
							dynamic.createString(FixUtil.getColorName(optional2.get().intValue()))
					);
				}
	...
	private static Optional<Pair<String, Integer>> getEnchantmentAndLevelPair(Dynamic<?> dynamic) {
		return dynamic.get("id")
		              .asString()
		              .apply2stable(
				              (enchantmentId, level) -> Pair.of(
						              enchantmentId,
						              MathHelper.clamp(level.intValue(), 0, 255)
				              ), dynamic.get("lvl").asNumber()
		              )
		              .result();
	}

	private static void fixAdventureModePredicates(
			ItemStackComponentizationFix.StackData data,
			Dynamic<?> dynamic,
			int hideFlags
	) {
			                             ...
			                             .set(
					                             ...
					                             dynamic.createList(
							                             optional.get()
							                                     .asStream()
							                                     .map(
									                                     (Dynamic<?> predicatesDynamic) -> (Dynamic<?>) DataFixUtils.orElse(
											                                     predicatesDynamic
													                                     .asString()
													                                     .map(string -> createBlockPredicateListDynamic(
															                                     predicatesDynamic,
															                                     string
													                                     ))
													                                     .result(),
	...
	private static void fixAttributeModifiers(
		...
		if (!optionalDynamic.result().isEmpty()) {
			...
			List<? extends Dynamic<?>>
					list =
					optionalDynamic.asList(ItemStackComponentizationFix::fixAttributeModifier);
	...
	private static Dynamic<?> fixAttributeModifier(Dynamic<?> dynamic) {
		...
		return Dynamic.copyAndFixField(
				dynamic, "Operation", dynamic2, "operation", operationDynamic -> {
	...
	private static Pair<Dynamic<?>, Dynamic<?>> fixMapDecorations(Dynamic<?> dynamic) {
		Dynamic<?>
				dynamic2 =
				(Dynamic<?>) DataFixUtils.orElseGet(dynamic.get("id").result(), () -> dynamic.createString(""));
		Dynamic<?> dynamic3 = dynamic.emptyMap()
		                             .set(
				                             "type",
				                             dynamic.createString(getMapDecorationName(dynamic.get("type").asInt(0)))
		                             )
	...
	private static void fixPotionContents(ItemStackComponentizationFix.StackData data, Dynamic<?> dynamic) {
		...
		Optional<String>
				optional =
				data.getAndRemove("Potion").asString().result().filter(potionId -> !potionId.equals("minecraft:empty"));
		if (optional.isPresent()) {
	...
	private static @Nullable Dynamic<?> fixBookPages(ItemStackComponentizationFix.StackData data, Dynamic<?> dynamic) {
		...
		Map<String, String> map = data.getAndRemove("filtered_pages")
		                              .asMap(
				                              filteredPagesKeyDynamic -> filteredPagesKeyDynamic.asString("0"),
				                              filteredPagesValueDynamic -> filteredPagesValueDynamic.asString("")
		                              );
		if (list.isEmpty()) {
			return null;
		}
	...
	private static Dynamic<?> createFilterableTextDynamic(
			Dynamic<?> dynamic,
			String unfiltered,
		...
		if (!optional.isEmpty() || !optional2.isEmpty()) {
			...
			if (optional.isPresent() && optional2.isPresent()) {
				dynamic2 =
						dynamic2.set(
								"target",
								dynamic.emptyMap().set("pos", optional.get()).set("dimension", optional2.get())
						);
			}
	...
	private static void fixFireworks(ItemStackComponentizationFix.StackData data) {
		data.applyFixer(
				...
				true,
				fireworksDynamic -> {
					Stream<? extends Dynamic<?>>
							stream =
							fireworksDynamic
									.get("Explosions")
									.asStream()
									.map(ItemStackComponentizationFix::fixExplosion);
	...
	private static Dynamic<?> fixExplosion(Dynamic<?> dynamic) {
		dynamic = dynamic.set(
				"shape", dynamic.createString(switch (dynamic.get("Type").asInt(0)) {
					case 1 -> "large_ball";
					case 2 -> "star";
					case 3 -> "creeper";
					case 4 -> "burst";
	...
	public static Dynamic<?> createProfileDynamic(Dynamic<?> dynamic) {
		...
		if (optional.isPresent()) {
			return isValidUsername(optional.get()) ? dynamic
			                                         .emptyMap()
			                                         .set("name", dynamic.createString(optional.get()))
			                                       : dynamic.emptyMap();
		}
		else {
						                        ...
						                        .map(
								                        pair -> {
									                        Dynamic<?> dynamic = propertiesDynamic.emptyMap()
									                                                              .set(
											                                                              "name",
											                                                              propertiesDynamic.createString(
													                                                              entry.getKey())
									                                                              )
									                                                              .set(
											                                                              "value",
									                        ...
									                        Optional<String>
									                        optional =
									                        (Optional<String>) pair.getSecond();
									                        return optional.isPresent() ? dynamic.set(
											                        "signature",
											                        propertiesDynamic.createString(optional.get())
									                        ) : dynamic;
								                        }
						                        )
				          )
		...
		return writeFixAndRead(
				...
				dynamic -> {
					Optional<? extends Dynamic<?>>
							optional =
							ItemStackComponentizationFix.StackData.fromDynamic(dynamic).map(data -> {
	...
	static class StackData {
		...
		public static Optional<ItemStackComponentizationFix.StackData> fromDynamic(Dynamic<?> dynamic) {
			return dynamic.get("id")
			              .asString()
			              .apply2stable(
					              (itemId, count) -> new ItemStackComponentizationFix.StackData(
							              itemId,
							              count.intValue(),
							              dynamic.remove("id").remove("Count")
					              ),
		...
		private static <T> Dynamic<T> mergeLeftoverNbt(Dynamic<T> data, Dynamic<?> leftoverNbt) {
			...
			return dynamicOps.getMap(data.getValue())
			                 .flatMap(mapLike -> dynamicOps.mergeToMap(
