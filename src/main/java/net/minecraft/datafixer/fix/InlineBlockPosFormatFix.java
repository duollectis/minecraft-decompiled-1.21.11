            ...
            entityTyped -> {
               ...
               for (OpticFinder<?> opticFinder4 : list) {
                  entityTyped = entityTyped.updateTyped(
                     opticFinder4, decorationTyped -> decorationTyped.update(DSL.remainderFinder(), this::fixDecorationFields)
                  );
               }
   ...
   private Dynamic<?> fixPlayerFields(Dynamic<?> dynamic) {
      ...
      if (optional.isPresent() && optional2.isPresent() && optional3.isPresent()) {
         Dynamic<?> dynamic2 = dynamic.createMap(
            Map.of(
               dynamic.createString("pos"), FixUtil.createBlockPos(dynamic, optional.get().intValue(), optional2.get().intValue(), optional3.get().intValue())
            )
         );
