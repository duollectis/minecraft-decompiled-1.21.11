...
public class WallPropertyFix extends DataFix {
		...
		return propertiesDynamic.update(
				...
				propertyValue -> (Dynamic) DataFixUtils.orElse(
						propertyValue
								.asString()
								.result()
								.map(WallPropertyFix::booleanToWallType)
								.map(propertyValue::createString), propertyValue
				)
		);
	}
	...
	private static <T> Dynamic<T> updateWallProperties(Dynamic<T> blockStateDynamic) {
		...
		return !bl ? blockStateDynamic : blockStateDynamic.update(
				"Properties", propertiesDynamic -> {
