...
/**
 * Переименовывает сущности, блоки и предметы, получившие новые идентификаторы
 * в рамках «The Renaming» — масштабного переименования ресурсов в Minecraft 1.13.
 * Также удаляет устаревший префикс {@code minecraft:bred_} у сущностей.
 */
public class EntityTheRenameningBlockFix extends EntityRenameFix {
	...
	public EntityTheRenameningBlockFix(Schema schema, boolean changesType) {
		super("EntityTheRenameningBlock", schema, changesType);
	}

	@Override
	protected String rename(String oldName) {
		String name = oldName.startsWith(BRED_PREFIX)
			? "minecraft:" + oldName.substring(BRED_PREFIX.length())
			: oldName;

		return ENTITIES.getOrDefault(name, name);
	}
}
