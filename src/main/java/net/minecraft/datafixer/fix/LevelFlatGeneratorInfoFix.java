		...
		return levelDynamic.get("generatorName").asString("").equalsIgnoreCase("flat")
		       ? levelDynamic.update(
				...
				generatorOptionsDynamic -> (Dynamic) DataFixUtils.orElse(
						...
						.asString()
						.map(this::fixFlatGeneratorOptions)
						.map(generatorOptionsDynamic::createString)
						.result(),
						generatorOptionsDynamic
				)
		)
		...
		if (generatorOptions.isEmpty()) {
			...
			if (i >= 0 && i <= 3) {
				...
				stringBuilder.append(StreamSupport
						.<String>stream(SPLIT_ON_COMMA.split(string2).spliterator(), false)
						.map(stringx -> {
							...
							int
									l =
									i == 3 ? EntityBlockStateFix.getNumericalBlockId("minecraft:" + string3)
									       : NumberUtils.toInt(string3, 0);
							int m = k + 1;
							int n = list2.size() > m ? NumberUtils.toInt(list2.get(m), 0) : 0;
							return (j == 1 ? "" : j + "*") + BlockStateFlattening
									.lookupState(l << 4 | n)
									.get("Name")
									.asString("");
						})
