...
public class EntityUuidFix extends AbstractUuidFix {
	...
	protected TypeRewriteRule makeRule() {
		return this.fixTypeEverywhereTyped(
				"EntityUUIDFixes", this.getInputSchema().getType(this.typeReference), typed -> {
	...
	private static Dynamic<?> updateAngryAtMemory(Dynamic<?> dynamic) {
		return dynamic.update(
				...
				dynamicx -> dynamicx.update(
						"memories", dynamicxx -> dynamicxx.update(
								"minecraft:angry_at",
								dynamicxxx -> updateStringUuid(dynamicxxx, "value", "value").orElseGet(() -> {
	...
	private static Dynamic<?> updateFox(Dynamic<?> dynamic) {
		Optional<Dynamic<?>> optional = dynamic.get("TrustedUUIDs")
		                                       .result()
		                                       .map(dynamic2 -> dynamic.createList(dynamic2
				                                       .asStream()
				                                       .map(dynamicxx -> (Dynamic<?>) createArrayFromCompoundUuid(
						                                       dynamicxx).orElseGet(() -> {
					                                       LOGGER.warn("Trusted contained invalid data.");
					                                       return dynamicxx;
				                                       }))));
		return (Dynamic<?>) DataFixUtils.orElse(
				optional.map(dynamic2 -> dynamic
						.remove("TrustedUUIDs")
						.set("Trusted", dynamic2)), dynamic
		);
	}
	...
	private static Dynamic<?> updateLeashable(Dynamic<?> dynamic) {
		return updateLiving(dynamic).update(
				"Leash",
				dynamicx -> updateRegularMostLeast(dynamicx, "UUID", "UUID").orElse(dynamicx)
		);
	}
						        ...
						        .map(
								        dynamicxx -> dynamicxx.update(
										        ...
										        dynamic2x -> dynamicxx.createList(
												        dynamic2x
														        .asStream()
														        .map(dynamicxxxx -> (Dynamic<?>) updateRegularMostLeast(
																        dynamicxxxx,
																        "UUID",
																        "UUID"
														        ).orElse(dynamicxxxx))
	...
	private static Dynamic<?> updateProjectile(Dynamic<?> dynamic) {
		return (Dynamic<?>) DataFixUtils.orElse(
				dynamic
						.get("OwnerUUID")
						.result()
						.map(dynamic2 -> dynamic.remove("OwnerUUID").set("Owner", dynamic2)), dynamic
		);
	}
