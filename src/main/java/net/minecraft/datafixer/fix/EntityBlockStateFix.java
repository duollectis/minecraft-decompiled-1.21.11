...
/**
 * {@code EntityBlockStateFix}.
 */
public class EntityBlockStateFix extends DataFix {

	private static final Map<String, Integer> BLOCK_NAME_TO_ID = DataFixUtils.make(
			Maps.<String, Integer>newHashMap(), map -> {
	...
	public TypeRewriteRule makeRule() {
		...
		Function<Typed<?>, Typed<?>>
				function =
				minecart -> this.mergeIdAndData(minecart, "DisplayTile", "DisplayData", "DisplayState");
		Function<Typed<?>, Typed<?>>
				function2 =
				arrow -> this.mergeIdAndData(arrow, "inTile", "inData", "inBlockState");
		...
		Function<Typed<?>, Typed<?>>
				function3 =
				projectile -> projectile.update(type.finder(), DSL.remainderType(), Pair::getSecond);
		return this.fixTypeEverywhereTyped(
				"EntityBlockStateFix",
				schema.getType(TypeReferences.ENTITY),
				schema2.getType(TypeReferences.ENTITY),
				entity -> {
	...
	private Typed<?> fixFallingBlock(Typed<?> fallingBlock) {
		...
		return fallingBlock.update(
				type.finder(), type2, state -> {
					int i = (Integer) state.map(
	...
	private Typed<?> mergeIdAndData(Typed<?> entity, String oldIdKey, String oldDataKey, String newStateKey) {
		...
		return entity.update(
				type.finder(), type2, state -> {
					int
							i =
							(Integer) ((Either<?, ?>) state.getSecond()).map(
									id -> id,
									id -> EntityBlockStateFix.getNumericalBlockId((String) id)
							);
