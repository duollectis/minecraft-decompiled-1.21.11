...
public class ChunkPalettedStorageFix extends DataFix {
								        ...
								        if (this.blockEntities.put(l, blockEntityTag) != null) {
									        ChunkPalettedStorageFix.LOGGER
											        .warn(
													        ...
													        new Object[]{this.x, this.z, ix, jx, k}
											        );
								        }
							        }
					        )
							...
							case 2:
								...
								while (var30.hasNext()) {
									...
									if ("minecraft:grass_block".equals(ChunkPalettedStorageFix.getName(dynamic))) {
										String
												string =
												ChunkPalettedStorageFix.getName(this.getBlock(adjacentTo(
														j,
														ChunkPalettedStorageFix.Facing.UP
												)));
									...
									if (dynamic != null) {
										...
										this.setBlock(
												...
												ChunkPalettedStorageFix.Mapping.NOTE_BLOCK_IDS_TO_STATES
														.getOrDefault(
																string,
																ChunkPalettedStorageFix.Mapping.NOTE_BLOCK_IDS_TO_STATES.get(
																		"false0")
														)
										);
									}
									...
									if (dynamic != null) {
										...
										if (k != 14 && k >= 0 && k < 16) {
											...
											if (ChunkPalettedStorageFix.Mapping.BED_IDS_TO_STATES.containsKey(string2)) {
												this.setBlock(
														j,
														ChunkPalettedStorageFix.Mapping.BED_IDS_TO_STATES.get(string2)
												);
											}
										}
									}
									...
									if (ChunkPalettedStorageFix.getName(dynamic).endsWith("_door")) {
										...
										if ("lower".equals(ChunkPalettedStorageFix.getProperty(dynamic2, "half"))) {
											...
											if (string4.equals(ChunkPalettedStorageFix.getName(dynamic3))) {
												String
														string5 =
														ChunkPalettedStorageFix.getProperty(dynamic2, "facing");
												String string6 = ChunkPalettedStorageFix.getProperty(dynamic2, "open");
												String
														string7 =
														bl ? "left"
														   : ChunkPalettedStorageFix.getProperty(dynamic3, "hinge");
												String
														string8 =
														bl ? "false"
														   : ChunkPalettedStorageFix.getProperty(dynamic3, "powered");
												this.setBlock(
														j,
														ChunkPalettedStorageFix.Mapping.DOOR_IDS_TO_STATES.get(
																string4 + string5 + "lower" + string7 + string6
																		+ string8)
												);
												this.setBlock(
														k,
														ChunkPalettedStorageFix.Mapping.DOOR_IDS_TO_STATES.get(
																string4 + string5 + "upper" + string7 + string6
																		+ string8)
												);
											}
										}
								...
								while (var25.hasNext()) {
									...
									if ("minecraft:carved_pumpkin".equals(ChunkPalettedStorageFix.getName(dynamic))) {
										String
												...
												ChunkPalettedStorageFix.getName(this.getBlock(adjacentTo(
														j,
														ChunkPalettedStorageFix.Facing.DOWN
												)));
									...
									if (dynamic != null) {
										...
										this.setBlock(
												...
												ChunkPalettedStorageFix.Mapping.PLANT_TO_FLOWER_POT_STATES
														.getOrDefault(
																string,
																ChunkPalettedStorageFix.Mapping.PLANT_TO_FLOWER_POT_STATES.get(
																		"minecraft:air0")
														)
										);
									}
							...
							case 144:
								...
								while (var22.hasNext()) {
									...
									if (dynamic != null) {
										...
										String
												string3 =
												ChunkPalettedStorageFix.getProperty(this.getBlock(j), "facing");
										String string2;
										...
										this.setBlock(
												...
												ChunkPalettedStorageFix.Mapping.SKULL_IDS_TO_STATES
														.getOrDefault(
																string2,
																ChunkPalettedStorageFix.Mapping.SKULL_IDS_TO_STATES.get(
																		"0north")
														)
										);
									}
							...
							case 175:
								...
								while (var21.hasNext()) {
									...
									if ("upper".equals(ChunkPalettedStorageFix.getProperty(dynamic, "half"))) {
										Dynamic<?>
												dynamic2 =
												this.getBlock(adjacentTo(j, ChunkPalettedStorageFix.Facing.DOWN));
										String string3 = ChunkPalettedStorageFix.getName(dynamic2);
										switch (string3) {
											case "minecraft:sunflower":
												this.setBlock(
														j,
														ChunkPalettedStorageFix.Mapping.UPPER_HALF_SUNFLOWER_STATE
												);
												break;
											case "minecraft:lilac":
												this.setBlock(
														j,
														ChunkPalettedStorageFix.Mapping.UPPER_HALF_LILAC_STATE
												);
												break;
											case "minecraft:tall_grass":
												this.setBlock(
														j,
														ChunkPalettedStorageFix.Mapping.UPPER_HALF_TALL_GRASS_STATE
												);
												break;
											case "minecraft:large_fern":
												this.setBlock(
														j,
														ChunkPalettedStorageFix.Mapping.UPPER_HALF_LARGE_FERN_STATE
												);
												break;
											case "minecraft:rose_bush":
												this.setBlock(
														j,
														ChunkPalettedStorageFix.Mapping.UPPER_HALF_ROSE_BUSH_STATE
												);
												break;
											case "minecraft:peony":
												this.setBlock(
														j,
														ChunkPalettedStorageFix.Mapping.UPPER_HALF_PEONY_STATE
												);
										}
									}
								}
								...
								while (var10.hasNext()) {
									...
									if (dynamic != null) {
										...
										if (k != 15 && k >= 0 && k < 16) {
											String
													string2 =
													ChunkPalettedStorageFix.getProperty(
															dynamic2,
											...
											if (ChunkPalettedStorageFix.Mapping.BANNER_IDS_TO_STATES.containsKey(string2)) {
												this.setBlock(
														j,
														ChunkPalettedStorageFix.Mapping.BANNER_IDS_TO_STATES.get(string2)
												);
											}
										}
									}
	...
	static class Mapping {
		...
		static final Map<String, Dynamic<?>> PLANT_TO_FLOWER_POT_STATES = DataFixUtils.make(
				Maps.<String, Dynamic<?>>newHashMap(), map -> {
		...
		static final Map<String, Dynamic<?>> SKULL_IDS_TO_STATES = DataFixUtils.make(
				Maps.<String, Dynamic<?>>newHashMap(), map -> {
		...
		static final Map<String, Dynamic<?>> DOOR_IDS_TO_STATES = DataFixUtils.make(
				Maps.<String, Dynamic<?>>newHashMap(), map -> {
					door(map, "oak_door");
					door(map, "iron_door");
		...
		static final Map<String, Dynamic<?>> NOTE_BLOCK_IDS_TO_STATES = DataFixUtils.make(
				Maps.<String, Dynamic<?>>newHashMap(), map -> {
					for (int i = 0; i < 26; i++) {
						map.put(
								"true" + i,
								FixUtil.createBlockState(
										...
										Map.of("powered", "true", "note", String.valueOf(i))
								)
						);
						map.put(
								"false" + i,
								FixUtil.createBlockState(
										"minecraft:note_block",
										Map.of("powered", "false", "note", String.valueOf(i))
								)
						);
					}
				}
		);
		private static final Int2ObjectMap<String> COLORS_BY_IDS = (Int2ObjectMap<String>) DataFixUtils.make(
				new Int2ObjectOpenHashMap(), map -> {
					map.put(0, "white");
					map.put(1, "orange");
					map.put(2, "magenta");
		...
		static final Map<String, Dynamic<?>> BED_IDS_TO_STATES = DataFixUtils.make(
				Maps.<String, Dynamic<?>>newHashMap(), map -> {
		...
		static final Map<String, Dynamic<?>> BANNER_IDS_TO_STATES = DataFixUtils.make(
				Maps.<String, Dynamic<?>>newHashMap(), map -> {
					ObjectIterator var1 = COLORS_BY_IDS.int2ObjectEntrySet().iterator();
		...
		public int visit(int sidesToUpgrade) {
			if (!this.hasBlocks) {
				...
				ChunkPalettedStorageFix.ChunkNibbleArray chunkNibbleArray = this.section
						.get("Data")
						.asByteBufferOpt()
						.map(byteBufferx -> new ChunkPalettedStorageFix.ChunkNibbleArray(DataFixUtils.toArray(
								byteBufferx)))
						.result()
				...
				for (int i = 0; i < 4096; i++) {
					...
					int
							m =
							chunkNibbleArray2.get(j, k, l) << 12 | (byteBuffer.get(i) & 255) << 4
									| chunkNibbleArray.get(j, k, l);
					if (ChunkPalettedStorageFix.Mapping.DOOR_BLOCKS.get(m >> 4)) {
						this.addInPlaceUpdate(m >> 4, i);
					}

					if (ChunkPalettedStorageFix.Mapping.STAIR_BLOCKS.get(m >> 4)) {
		...
		public Dynamic<?> transform() {
			...
			if (!this.hasBlocks) {
				...
				dynamic =
						dynamic.set(
								"BlockStates",
								dynamic.createLongList(Arrays.stream(wordPackedArray.getAlignedArray()))
						);
