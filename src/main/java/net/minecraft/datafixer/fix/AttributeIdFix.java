...
/**
 * Мигрирует атрибуты сущностей и предметов из устаревшего формата (UUID + Name)
 * в новый формат с текстовым идентификатором {@code id}.
 * Также переименовывает поля модификаторов и конвертирует числовые операции в строковые.
 */
public class AttributeIdFix extends DataFix {
	...
	protected TypeRewriteRule makeRule() {
		Type<?> itemStackType = getInputSchema().getType(TypeReferences.ITEM_STACK);
		...
		return TypeRewriteRule.seq(
			fixTypeEverywhereTyped(
				...
				typed -> typed.updateTyped(
					componentsFinder,
					components -> components.update(DSL.remainderFinder(), AttributeIdFix::fixItemStack)
				)
			),
			new TypeRewriteRule[]{
				fixTypeEverywhereTyped(
					"AttributeIdFix (Entity)",
					getInputSchema().getType(TypeReferences.ENTITY),
					AttributeIdFix::fixEntity
				),
				fixTypeEverywhereTyped(
					"AttributeIdFix (Player)",
	...
	private static Stream<Dynamic<?>> castAndFixModifiers(Stream<?> stream) {
		return fixModifiers((Stream<Dynamic<?>>) stream);
	}

	private static Stream<Dynamic<?>> fixModifiers(Stream<Dynamic<?>> modifierStream) {
		Map<String, Dynamic<?>> result = new Object2ObjectArrayMap<>();

		modifierStream.forEach(modifier -> {
			UUID uuid = getUuidFromIntArray(modifier.get("uuid").asIntStream().toArray());
			String name = modifier.get("name").asString("");
			String idByUuid = uuid != null ? UUID_TO_ID.get(uuid) : null;
			String idByName = NAME_TO_ID.get(name);

			if (idByUuid != null) {
				modifier = modifier.set("id", modifier.createString(idByUuid));
				result.put(idByUuid, modifier.remove("uuid").remove("name"));
			} else if (idByName != null) {
				Dynamic<?> existing = result.get(idByName);

				if (existing == null) {
					modifier = modifier.set("id", modifier.createString(idByName));
					result.put(idByName, modifier.remove("uuid").remove("name"));
				} else {
					double existingAmount = existing.get("amount").asDouble(0.0);
					double newAmount = modifier.get("amount").asDouble(0.0);
					result.put(idByName, existing.set("amount", modifier.createDouble(existingAmount + newAmount)));
				}
			} else {
				String fallbackId = "minecraft:" + (uuid != null ? uuid.toString().toLowerCase(Locale.ROOT) : "unknown");
				modifier = modifier.set("id", modifier.createString(fallbackId));
				result.put(fallbackId, modifier.remove("uuid").remove("name"));
			}
		});
	...
	private static Dynamic<?> renameOtherFields(Dynamic<?> modifier) {
		return modifier.renameField("UUID", "uuid")
			.renameField("Name", "name")
			.renameField("Amount", "amount")
			.renameAndFixField(
				"Operation", "operation",
				op -> op.createString(switch (op.asInt(0)) {
					case 0 -> "add_value";
	...
	private static Dynamic<?> fixItemStack(Dynamic<?> stackData) {
		return stackData.update(
			...
			attributeModifiers -> attributeModifiers.update(
				...
				modifiers -> (Dynamic<?>) DataFixUtils.orElse(
					modifiers.asStreamOpt()
						.result()
						.map(AttributeIdFix::castAndFixModifiers)
						.map(modifiers::createList),
					modifiers
				)
			)
		);
			...
			remainder -> remainder.renameAndFixField(
				...
				attributes -> (Dynamic<?>) DataFixUtils.orElse(
					attributes.asStreamOpt()
						.result()
						.map(stream -> stream.map(AttributeIdFix::fixAttribute))
	...
	/**
	 * Восстанавливает UUID из массива из 4 int-значений (big-endian упаковка).
	 * Возвращает null, если массив имеет неверную длину.
	 */
	public static @Nullable UUID getUuidFromIntArray(int[] uuidArray) {
		return uuidArray.length != 4
			? null
			: new UUID(
				(long) uuidArray[0] << 32 | uuidArray[1] & INT_MASK,
				(long) uuidArray[2] << 32 | uuidArray[3] & INT_MASK
			);
	}
}
