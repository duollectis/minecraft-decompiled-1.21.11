...
public class AttributeIdFix extends DataFix {
	...
	private static final Map<UUID, String> UUID_TO_ID = ImmutableMap.<UUID, String>builder()
	                                                                ...
	                                                                .put(
			                                                                UUID.fromString(
					                                                                "6555be74-63b3-41f1-a245-77833b3c2562"),
			                                                                "minecraft:evil"
	                                                                )
	                                                                .put(
			                                                                UUID.fromString(
		...
		return TypeRewriteRule.seq(
				this.fixTypeEverywhereTyped(
						...
						typed -> typed.updateTyped(
								opticFinder,
								typedx -> typedx.update(DSL.remainderFinder(), AttributeIdFix::fixItemStack)
						)
				),
				new TypeRewriteRule[]{
						this.fixTypeEverywhereTyped(
	...
	private static Dynamic<?> renameOtherFields(Dynamic<?> attributeDynamic) {
		return attributeDynamic.renameField("UUID", "uuid")
		                       ...
		                       .renameAndFixField(
				                       "Operation", "operation", operationDynamic -> {
				...
				attributeModifiersDynamic -> attributeModifiersDynamic.update(
						...
						modifiersDynamic -> (Dynamic) DataFixUtils.orElse(
								modifiersDynamic
										...
										.map(modifiersDynamic::createList), modifiersDynamic
						)
				)
		);
	}
	...
	public static @Nullable UUID getUuidFromIntArray(int[] uuidArray) {
		return uuidArray.length != 4
		       ? null
		       : new UUID(
				       (long) uuidArray[0] << 32 | uuidArray[1] & 4294967295L,
				       (long) uuidArray[2] << 32 | uuidArray[3] & 4294967295L
		       );
	}
}
