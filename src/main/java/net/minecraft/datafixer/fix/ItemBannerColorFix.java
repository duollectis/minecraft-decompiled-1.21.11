							...
							if (optional3.isPresent()) {
								...
								if (dynamic3.get("Base").asNumber().result().isPresent()) {
									dynamic =
											dynamic.set(
													"Damage",
													dynamic.createShort((short) (dynamic3.get("Base").asInt(0) & 15))
											);
									...
									if (optional4.isPresent()) {
										...
										Dynamic<?> dynamic5 = dynamic4.createMap(
												ImmutableMap.of(
														dynamic4.createString("Lore"),
														dynamic4.createList(Stream.of(dynamic4.createString("(+NBT")))
												)
										);
									...
									return itemStackTyped.set(DSL.remainderFinder(), dynamic)
									                     .set(
											                     ...
											                     typed.set(
													                     opticFinder3,
													                     typed2.set(DSL.remainderFinder(), dynamic3)
											                     )
									                     );
								}
							}
