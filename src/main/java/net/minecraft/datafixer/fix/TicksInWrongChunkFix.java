...
public class TicksInWrongChunkFix extends DataFix {
		...
		return this.fixTypeEverywhereTyped(
				"Handle ticks saved in the wrong chunk", type, chunkTyped -> {
					...
					Optional<? extends Dynamic<?>>
							optional2 =
							optional.isPresent() ? optional.get().write().result() : Optional.empty();
					return chunkTyped.update(
							DSL.remainderFinder(), chunkTag -> {
	...
	private static Dynamic<?> putNeighborTicks(
			Dynamic<?> chunkTag,
			int chunkX,
			int chunkZ,
			Optional<? extends Dynamic<?>> ticks,
			String upgradeDataKey
	) {
		if (ticks.isPresent()) {
				...
				chunkTag =
						chunkTag.set(
								...
								chunkTag
										.get("UpgradeData")
										.orElseEmptyMap()
										.set(upgradeDataKey, chunkTag.createList(list.stream()))
						);
			}
		}
