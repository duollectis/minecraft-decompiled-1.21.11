...
/**
 * Глобальный реестр атрибутов по умолчанию для всех типов живых сущностей.
 * Инициализируется один раз при загрузке класса и остаётся неизменяемым.
 * <p>
 * Метод {@link #checkMissing()} используется при старте сервера для обнаружения
 * типов сущностей без зарегистрированных атрибутов.
 */
public class DefaultAttributeRegistry {
	...
	private static final Map<EntityType<? extends LivingEntity>, DefaultAttributeContainer> DEFAULT_ATTRIBUTE_REGISTRY =
			ImmutableMap.<EntityType<? extends LivingEntity>, DefaultAttributeContainer>builder()
					...
					.put(EntityType.CAMEL_HUSK, CamelEntity.createCamelAttributes().build())
					...
					.put(EntityType.COPPER_GOLEM, CopperGolemEntity.createCopperGolemAttributes().build())
					...
					.put(EntityType.HAPPY_GHAST, HappyGhastEntity.createHappyGhastAttributes().build())
					...
					.put(EntityType.NAUTILUS, NautilusEntity.createNautilusAttributes().build())
					...
					.put(EntityType.PARCHED, ParchedEntity.createParchedAttributes().build())
					...
					.put(EntityType.ZOMBIE_NAUTILUS, ZombieNautilusEntity.createZombieNautilusAttributes().build())
	...
	/**
	 * Проверяет, что все не-MISC сущности имеют зарегистрированные атрибуты.
	 * Вызывается при старте сервера; отсутствующие записи логируются или вызывают паузу.
	 */
