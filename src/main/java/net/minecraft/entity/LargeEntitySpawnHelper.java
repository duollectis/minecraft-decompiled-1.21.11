...
public class LargeEntitySpawnHelper {
	...
	public static <T extends MobEntity> Optional<T> trySpawnAt(
		...
		for (int i = 0; i < tries; i++) {
			...
			if (world.getWorldBorder().contains(mutable)
					...
					&& (!requireEmptySpace || world.isSpaceEmpty(entityType.getSpawnBox(
					mutable.getX() + 0.5,
					mutable.getY(),
					mutable.getZ() + 0.5
			))
	...
	public interface Requirements {
		...
		LargeEntitySpawnHelper.Requirements
				IRON_GOLEM =
				(world, pos, state, abovePos, aboveState) -> !state.isOf(Blocks.COBWEB)
		...
		LargeEntitySpawnHelper.Requirements
				WARDEN =
				(world, pos, state, abovePos, aboveState) -> aboveState.getCollisionShape(world, abovePos).isEmpty()
