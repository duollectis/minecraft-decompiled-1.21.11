...
public class EntityType<T extends Entity> implements ToggleableFeature, TypeFilter<Entity, T> {
   ...
   public static final EntityType<BlazeEntity> BLAZE = register(
      "blaze",
      EntityType.Builder.create(BlazeEntity::new, SpawnGroup.MONSTER).makeFireImmune().dimensions(0.6F, 1.8F).maxTrackingRange(8).notAllowedInPeaceful()
   );
   ...
   public static final EntityType<BreezeEntity> BREEZE = register(
      "breeze",
      EntityType.Builder.create(BreezeEntity::new, SpawnGroup.MONSTER).dimensions(0.6F, 1.77F).eyeHeight(1.3452F).maxTrackingRange(10).notAllowedInPeaceful()
   );
   ...
   public static final EntityType<CamelHuskEntity> CAMEL_HUSK = register(
      "camel_husk", EntityType.Builder.create(CamelHuskEntity::new, SpawnGroup.MONSTER).dimensions(1.7F, 2.375F).eyeHeight(2.275F).maxTrackingRange(10)
   );
   ...
   public static final EntityType<CaveSpiderEntity> CAVE_SPIDER = register(
      "cave_spider",
      EntityType.Builder.create(CaveSpiderEntity::new, SpawnGroup.MONSTER).dimensions(0.7F, 0.5F).eyeHeight(0.45F).maxTrackingRange(8).notAllowedInPeaceful()
   );
   ...
   public static final EntityType<CopperGolemEntity> COPPER_GOLEM = register(
      "copper_golem", EntityType.Builder.create(CopperGolemEntity::new, SpawnGroup.MISC).dimensions(0.49F, 0.98F).eyeHeight(0.8125F).maxTrackingRange(10)
   );
   ...
   public static final EntityType<CreakingEntity> CREAKING = register(
      "creaking",
      EntityType.Builder.create(CreakingEntity::new, SpawnGroup.MONSTER).dimensions(0.9F, 2.7F).eyeHeight(2.3F).maxTrackingRange(8).notAllowedInPeaceful()
   );
   public static final EntityType<CreeperEntity> CREEPER = register(
      "creeper", EntityType.Builder.create(CreeperEntity::new, SpawnGroup.MONSTER).dimensions(0.6F, 1.7F).maxTrackingRange(8).notAllowedInPeaceful()
   );
   ...
   public static final EntityType<HappyGhastEntity> HAPPY_GHAST = register(
      ...
      EntityType.Builder.create(HappyGhastEntity::new, SpawnGroup.CREATURE)
         .dimensions(4.0F, 4.0F)
         .eyeHeight(2.6F)
         .passengerAttachments(new Vec3d(0.0, 4.0, 1.7), new Vec3d(-1.7, 4.0, 0.0), new Vec3d(0.0, 4.0, -1.7), new Vec3d(1.7, 4.0, 0.0))
         .vehicleAttachment(0.5F)
         .maxTrackingRange(10)
   );
   ...
   public static <T extends Entity> Consumer<T> nbtCopier(Consumer<T> chained, World world, ItemStack stack, @Nullable LivingEntity spawner) {
      ...
      return typedEntityData != null ? chained.andThen(entity -> loadFromEntityNbt(world, spawner, entity, typedEntityData)) : chained;
   }
      ...
      if (minecraftServer != null && entity != null) {
         if (entity.getType() == data.getType()) {
            if (world.isClient()
               ...
               || spawner instanceof PlayerEntity playerEntity && minecraftServer.getPlayerManager().isOperator(playerEntity.getPlayerConfigEntry())) {
   ...
   public static @Nullable Entity loadEntityWithPassengers(NbtCompound nbt, World world, SpawnReason reason, LoadedEntityProcessor processor) {
      ...
      try (ErrorReporter.Logging logging = new ErrorReporter.Logging(LOGGER)) {
         var5 = loadEntityWithPassengers(NbtReadView.create(logging, world.getRegistryManager(), nbt), world, reason, processor);
      }
   ...
   public static @Nullable Entity loadEntityWithPassengers(
      ...
      Entity var6;
      try (ErrorReporter.Logging logging = new ErrorReporter.Logging(LOGGER)) {
         var6 = loadEntityWithPassengers(type, NbtReadView.create(logging, world.getRegistryManager(), data), world, reason, processor);
      }
   ...
   public static Stream<Entity> streamFromData(ReadView.ListReadView view, World world, SpawnReason reason) {
      return view.stream().mapMulti((viewx, callback) -> loadEntityWithPassengers(viewx, world, reason, entity -> {
