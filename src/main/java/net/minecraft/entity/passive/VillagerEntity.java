...
public class VillagerEntity extends MerchantEntity implements InteractionObserver, VillagerDataContainer {
   ...
   public static final Map<MemoryModuleType<GlobalPos>, BiPredicate<VillagerEntity, RegistryEntry<PointOfInterestType>>> POINTS_OF_INTEREST = ImmutableMap.of(
      MemoryModuleType.HOME,
      (BiPredicate<VillagerEntity, RegistryEntry<PointOfInterestType>>) (villager, poi) -> poi.matchesKey(PointOfInterestTypes.HOME),
      MemoryModuleType.JOB_SITE,
      (BiPredicate<VillagerEntity, RegistryEntry<PointOfInterestType>>) (villager, poi) -> villager.getVillagerData().profession().value().heldWorkstation().test(poi),
      MemoryModuleType.POTENTIAL_JOB_SITE,
      (BiPredicate<VillagerEntity, RegistryEntry<PointOfInterestType>>) (villager, poi) -> VillagerProfession.IS_ACQUIRABLE_JOB_SITE.test(poi),
      MemoryModuleType.MEETING_POINT,
      (BiPredicate<VillagerEntity, RegistryEntry<PointOfInterestType>>) (villager, poi) -> poi.matchesKey(PointOfInterestTypes.MEETING)
   );
   ...
   protected void readCustomData(ReadView view) {
      ...
      this.dataTracker.set(VILLAGER_DATA, view.<VillagerData>read("VillagerData", VillagerData.CODEC).orElseGet(VillagerEntity::createVillagerData));
   ...
   protected void fillRecipes(ServerWorld world) {
      ...
      if (registryKey != null) {
         ...
         if (this.getEntityWorld().getEnabledFeatures().contains(FeatureFlags.TRADE_REBALANCE)) {
            Int2ObjectMap<TradeOffers.Factory[]> int2ObjectMap = TradeOffers.REBALANCED_PROFESSION_TO_LEVELED_TRADE.get(registryKey);
            int2ObjectMap2 = int2ObjectMap != null ? int2ObjectMap : TradeOffers.PROFESSION_TO_LEVELED_TRADE.get(registryKey);
         } else {
