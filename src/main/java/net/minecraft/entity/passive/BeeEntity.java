...
public class BeeEntity extends AnimalEntity implements Angerable, Flutterer {
   ...
   private void addParticle(World world, double lastX, double x, double lastZ, double z, double y, ParticleEffect effect) {
      world.addParticleClient(
         effect, MathHelper.lerp(world.random.nextDouble(), lastX, x), y, MathHelper.lerp(world.random.nextDouble(), lastZ, z), 0.0, 0.0, 0.0
      );
   }
   ...
   @Nullable BeehiveBlockEntity getHive() {
      if (this.hivePos == null) {
         ...
         return this.isTooFar(this.hivePos) ? null : this.getEntityWorld().getBlockEntity(this.hivePos, BlockEntityType.BEEHIVE).orElse(null);
      }
   }
   ...
   public void registerTracking(ServerWorld world, DebugTrackable.Tracker tracker) {
      ...
      tracker.track(
         ...
         () -> new BeeDebugData(
            Optional.ofNullable(this.getHivePos()), Optional.ofNullable(this.getFlowerPos()), this.getMoveGoalTicks(), this.getPossibleHives()
         )
      );
   }
