...
public class BeeEntity extends AnimalEntity implements Angerable, Flutterer {
	...
	protected void readCustomData(ReadView view) {
		...
		this.hivePos = view.<BlockPos>read("hive_pos", BlockPos.CODEC).orElse(null);
		this.flowerPos = view.<BlockPos>read("flower_pos", BlockPos.CODEC).orElse(null);
	...
	boolean canEnterHive() {
		if (this.cannotEnterHiveTicks <= 0 && !this.pollinateGoal.isRunning() && !this.hasStung()
			...
			boolean bl = this.hasNectar()
					...
					.getAttributeValue(EnvironmentAttributes.BEES_STAY_IN_HIVE_GAMEPLAY, this.getEntityPos());
	...
	protected void mobTick(ServerWorld world) {
		...
		if (bl) {
			...
			if (this.ticksSinceSting % 5 == 0
					&& this.random.nextInt(MathHelper.clamp(1200 - this.ticksSinceSting, 1, 1200)) == 0) {
	...
	public void tickMovement() {
		...
		if (!this.getEntityWorld().isClient()) {
			...
			boolean
					bl =
					this.hasAngerTime() && !this.hasStung() && this.getTarget() != null
							&& this.getTarget().squaredDistanceTo(this) < 4.0;
	...
	@Nullable BeehiveBlockEntity getHive() {
		if (this.hivePos == null) {
			...
			return this.isTooFar(this.hivePos) ? null : this
			                                            .getEntityWorld()
			                                            .getBlockEntity(this.hivePos, BlockEntityType.BEEHIVE)
			                                            .orElse(null);
		}
	}
	...
	public ActionResult interactMob(PlayerEntity player, Hand hand) {
		...
		if (this.isBreedingItem(itemStack) && itemStack.getItem() instanceof BlockItem blockItem
	...
	public boolean isFlappingWings() {
		return this.isInAir() && this.age % WING_FLAP_TICKS == 0;
	}
	...
	public void registerTracking(ServerWorld world, DebugTrackable.Tracker tracker) {
		...
		tracker.track(
				...
				() -> new BeeDebugData(
						Optional.ofNullable(this.getHivePos()),
						Optional.ofNullable(this.getFlowerPos()),
						this.getMoveGoalTicks(),
						this.getPossibleHives()
				)
		);
	}
	...
	class BeeWanderAroundGoal extends Goal {
		...
		public void start() {
			...
			if (vec3d != null) {
				BeeEntity.this.navigation.startMovingAlong(
						BeeEntity.this.navigation.findPathTo(
		...
		private @Nullable Vec3d getRandomLocation() {
			Vec3d vec3d2;
			if (BeeEntity.this.hasValidHive() && !BeeEntity.this.isWithinDistance(
					BeeEntity.this.hivePos,
					this.getMaxWanderDistance()
			)) {
			...
			Vec3d
					vec3d3 =
					AboveGroundTargeting.find(BeeEntity.this, 8, 7, vec3d2.x, vec3d2.z, (float) (Math.PI / 2), 3, 1);
			return vec3d3 != null ? vec3d3 : NoPenaltySolidTargeting.find(
					BeeEntity.this,
					8,
					4,
					-2,
	...
	class EnterHiveGoal extends BeeEntity.NotAngryGoal {
		...
		public boolean canBeeStart() {
			if (BeeEntity.this.hivePos != null && BeeEntity.this.canEnterHive()
	...
	class FindHiveGoal extends BeeEntity.NotAngryGoal {
		...
		@Override
		public boolean canBeeStart() {
			return BeeEntity.this.ticksLeftToFindHive == 0 && !BeeEntity.this.hasHivePos()
		...
		private List<BlockPos> getNearbyFreeHives() {
			...
			PointOfInterestStorage
					pointOfInterestStorage =
					((ServerWorld) BeeEntity.this.getEntityWorld()).getPointOfInterestStorage();
	...
	class GrowCropsGoal extends BeeEntity.NotAngryGoal {
		...
		public boolean canBeeStart() {
			if (BeeEntity.this.getCropsGrownSincePollination() >= 10) {
				...
				return BeeEntity.this.random.nextFloat() < 0.3F ? false : BeeEntity.this.hasNectar()
	...
	public class MoveToHiveGoal extends BeeEntity.NotAngryGoal {
			...
			if (BeeEntity.this.hivePos != null) {
				...
				if (this.ticks > this.getTickCount(2400)) {
					...
					if (!BeeEntity.this.isWithinDistance(BeeEntity.this.hivePos, 16)) {
						...
						else if (this.path != null && BeeEntity.this.navigation
								.getCurrentPath()
								.equalsPath(this.path)) {
							this.ticksUntilLost++;
		...
		private boolean startMovingToFar(BlockPos pos) {
			...
			return BeeEntity.this.navigation.getCurrentPath() != null && BeeEntity.this.navigation
					.getCurrentPath()
					.reachesTarget();
		}
	...
	class PollinateGoal extends BeeEntity.NotAngryGoal {
						...
						if (!bl && this.ticks > 600) {
							...
							if (bl) {
								...
								if (bl3) {
									this.nextTarget =
											new Vec3d(
													vec3d.getX() + this.getRandomOffset(),
													vec3d.getY(),
													vec3d.getZ() + this.getRandomOffset()
											);
									BeeEntity.this.navigation.stop();
								}
								else {
		...
		private void moveToNextTarget() {
			BeeEntity.this
					.getMoveControl()
					.moveTo(this.nextTarget.getX(), this.nextTarget.getY(), this.nextTarget.getZ(), 0.35F);
		}
	...
	class ValidateFlowerGoal extends BeeEntity.NotAngryGoal {
		...
		public void start() {
			if (BeeEntity.this.flowerPos != null
					...
					&& !this.isFlower(BeeEntity.this.flowerPos)) {
				BeeEntity.this.clearFlowerPos();
			}
	...
	class ValidateHiveGoal extends BeeEntity.NotAngryGoal {
		...
		@Override
		public void start() {
			if (BeeEntity.this.hivePos != null && BeeEntity.this.getEntityWorld().isPosLoaded(BeeEntity.this.hivePos)
					&& !BeeEntity.this.hasValidHive()) {
				BeeEntity.this.clearHivePos();
			}
