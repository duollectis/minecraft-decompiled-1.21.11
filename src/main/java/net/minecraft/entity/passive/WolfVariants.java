...
public class WolfVariants {
	...
	private static void register(
			Registerable<WolfVariant> registry,
			RegistryKey<WolfVariant> key,
			String textureName,
		...
		register(
				...
				key,
				textureName,
				createSpawnConditions(RegistryEntryList.of(registry
						.getRegistryLookup(RegistryKeys.BIOME)
						.getOrThrow(biome)))
		);
	}
	...
	private static void register(
		...
		register(
				...
				key,
				textureName,
				createSpawnConditions(registry.getRegistryLookup(RegistryKeys.BIOME).getOrThrow(biomeTag))
		);
	}
		...
		registry.register(
				...
				new WolfVariant(
						new WolfVariant.WolfAssetInfo(
								new AssetInfo.TextureAssetInfo(identifier),
								new AssetInfo.TextureAssetInfo(identifier2),
								new AssetInfo.TextureAssetInfo(identifier3)
						),
						spawnConditions
				)
		);
