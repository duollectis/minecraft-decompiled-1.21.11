...
public class AllayEntity extends PathAwareEntity implements InventoryOwner, Vibrations {
	...
	private static final int MAX_ITEM_HOLD_ANIMATION_TICKS = 5;
	private static final float DANCE_CYCLE_TICKS = 55.0F;
	private static final float SPIN_ANIMATION_TICKS = 15.0F;
	...
	public static final int NOTE_BLOCK_RANGE = 1024;
	private static final TrackedData<Boolean>
			DANCING =
			DataTracker.registerData(AllayEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
	private static final TrackedData<Boolean>
			CAN_DUPLICATE =
			DataTracker.registerData(AllayEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
	...
	public AllayEntity(EntityType<? extends AllayEntity> entityType, World world) {
		...
		this.jukeboxEventHandler = new EntityGameEventHandler<>(
				new AllayEntity.JukeboxEventListener(
						this.vibrationCallback.getPositionSource(),
	...
	private boolean isItemPickupCoolingDown() {
		return this
				.getBrain()
				.isMemoryInState(MemoryModuleType.ITEM_PICKUP_COOLDOWN_TICKS, MemoryModuleState.VALUE_PRESENT);
	}
	...
	private boolean shouldStopDancing() {
		return this.jukeboxPos == null
				|| !this.jukeboxPos.isWithinDistance(
				this.getEntityPos(),
				GameEvent.JUKEBOX_PLAY.value().notificationRadius()
		)
	...
	class VibrationCallback implements Vibrations.Callback {
		...
		private final PositionSource
				positionSource =
				new EntityPositionSource(AllayEntity.this, AllayEntity.this.getStandingEyeHeight());
		...
		public boolean accepts(
			...
			if (AllayEntity.this.isAiDisabled()) {
				...
				Optional<GlobalPos>
						optional =
						AllayEntity.this.getBrain().getOptionalRegisteredMemory(MemoryModuleType.LIKED_NOTEBLOCK);
				if (optional.isEmpty()) {
					return true;
				}
				else {
