...
public class TropicalFishEntity extends SchoolingFishEntity {
	...
	public static final TropicalFishEntity.Variant DEFAULT_VARIANT = new TropicalFishEntity.Variant(
			TropicalFishEntity.Pattern.KOB, DyeColor.WHITE, DyeColor.WHITE
	);
	private static final TrackedData<Integer>
			VARIANT =
			DataTracker.registerData(TropicalFishEntity.class, TrackedDataHandlerRegistry.INTEGER);
	...
	static int getVariantId(TropicalFishEntity.Pattern variety, DyeColor baseColor, DyeColor patternColor) {
		return variety.getIndex() & 65535 | (baseColor.getIndex() & 0xFF) << 16
				| (patternColor.getIndex() & 0xFF) << 24;
	}
	...
	protected void writeCustomData(WriteView view) {
		...
		view.put(
				"Variant",
				TropicalFishEntity.Variant.CODEC,
				new TropicalFishEntity.Variant(this.getTropicalFishVariant())
		);
	}
	...
	protected void readCustomData(ReadView view) {
		...
		TropicalFishEntity.Variant
				variant =
				view
						.<TropicalFishEntity.Variant>read("Variant", TropicalFishEntity.Variant.CODEC)
	...
	public <T> @Nullable T get(ComponentType<? extends T> type) {
		if (type == DataComponentTypes.TROPICAL_FISH_PATTERN) {
			...
			return type == DataComponentTypes.TROPICAL_FISH_PATTERN_COLOR ? castComponentValue(
	...
	public static boolean canTropicalFishSpawn(
			EntityType<TropicalFishEntity> type,
			WorldAccess world,
			SpawnReason reason,
			BlockPos pos,
			Random random
		...
		return world.getFluidState(pos.down()).isIn(FluidTags.WATER)
				...
				&& (world.getBiome(pos).isIn(BiomeTags.ALLOWS_TROPICAL_FISH_SPAWNS_AT_ANY_HEIGHT)
				|| WaterCreatureEntity.canSpawn(type, world, reason, pos, random)
		);
	}
 ...
 */
	public static enum Pattern implements StringIdentifiable, TooltipAppender {
		...
		public static final Codec<TropicalFishEntity.Pattern>
				CODEC =
				StringIdentifiable.createCodec(TropicalFishEntity.Pattern::values);
		...
		public static final PacketCodec<ByteBuf, TropicalFishEntity.Pattern> PACKET_CODEC = PacketCodecs.indexed(
				INDEX_MAPPER, TropicalFishEntity.Pattern::getIndex
		);
		...
		public void appendTooltip(
			...
			DyeColor
					...
					components.getOrDefault(
							...
							TropicalFishEntity.DEFAULT_VARIANT.baseColor()
					);
			DyeColor
					dyeColor2 =
					components.getOrDefault(
							DataComponentTypes.TROPICAL_FISH_PATTERN_COLOR,
							TropicalFishEntity.DEFAULT_VARIANT.patternColor()
					);
			Formatting[] formattings = new Formatting[]{Formatting.ITALIC, Formatting.GRAY};
			int
					i =
					TropicalFishEntity.COMMON_VARIANTS.indexOf(new TropicalFishEntity.Variant(
							this,
							dyeColor,
							dyeColor2
					));
			if (i != -1) {
				textConsumer.accept(Text
						.translatable(TropicalFishEntity.getToolTipForVariant(i))
 ...
 */
	public record Variant(TropicalFishEntity.Pattern pattern, DyeColor baseColor, DyeColor patternColor) {
		...
		public static final Codec<TropicalFishEntity.Variant>
				CODEC =
				Codec.INT.xmap(TropicalFishEntity.Variant::new, TropicalFishEntity.Variant::getId);

		public Variant(int id) {
			this(
					TropicalFishEntity.getVariety(id),
					TropicalFishEntity.getBaseColor(id),
					TropicalFishEntity.getPatternColor(id)
			);
		}
