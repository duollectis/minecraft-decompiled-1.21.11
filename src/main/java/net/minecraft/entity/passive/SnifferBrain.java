...
public class SnifferBrain {
	...
	private static final int SNIFF_SEARCH_RADIUS = 6;
	...
	private static void addCoreActivities(Brain<SnifferEntity> brain) {
		brain.setTaskList(
				Activity.CORE,
				0,
				ImmutableList.<Task<? super SnifferEntity>>of(
						new StayAboveWaterTask(0.8F),
						new FleeTask<SnifferEntity>(2.0F) {
							@Override
	...
	private static void addDigActivities(Brain<SnifferEntity> brain) {
		brain.setTaskList(
				Activity.DIG,
				ImmutableList.of(
						Pair.of(0, new SnifferBrain.DiggingTask(160, 180)),
						Pair.of(0, new SnifferBrain.FinishDiggingTask(40))
				),
				Set.of(
						Pair.of(MemoryModuleType.IS_PANICKING, MemoryModuleState.VALUE_ABSENT),
	...
	private static void addIdleActivities(Brain<SnifferEntity> brain) {
		brain.setTaskList(
				...
				ImmutableList.of(
						Pair.of(
								0, new BreedTask(EntityType.SNIFFER) {
									@Override
	...
	static class SearchingTask extends MultiTickTask<SnifferEntity> {
		...
		protected boolean shouldKeepRunning(ServerWorld serverWorld, SnifferEntity snifferEntity, long l) {
			if (!snifferEntity.canTryToDig()) {
				...
				Optional<BlockPos>
						optional2 =
						snifferEntity.getBrain().getOptionalRegisteredMemory(MemoryModuleType.SNIFFER_SNIFFING_TARGET);
