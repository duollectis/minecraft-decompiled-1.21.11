...
public class DolphinEntity extends WaterAnimalEntity {
	...
	public static final Predicate<ItemEntity>
			CAN_TAKE =
			item -> !item.cannotPickup() && item.isAlive() && item.isTouchingWater();
	...
	public void tick() {
		...
		if (this.isAiDisabled()) {
			...
			if (this.getEntityWorld().isClient() && this.isTouchingWater()
					&& this.getVelocity().lengthSquared() > 0.03) {
				...
				for (int i = 0; i < 2; i++) {
					this.getEntityWorld()
					    .addParticleClient(
							    ...
							    this.getX() - vec3d.x * h + f,
							    this.getY() - vec3d.y,
							    this.getZ() - vec3d.z * h + g,
							    0.0,
							    0.0,
							    0.0
					    );
			...
			if (this.dolphin.treasurePos != null) {
				...
				if (this.dolphin.isNearTarget() || this.dolphin.getNavigation().isIdle()) {
					...
					if (vec3d2 != null) {
						...
						if (!world.getFluidState(blockPos).isIn(FluidTags.WATER) || !world
								.getBlockState(blockPos)
								.canPathfindThrough(NavigationType.WATER)) {
					...
					this.dolphin
							...
							.lookAt(
									...
									vec3d2.y,
									vec3d2.z,
									this.dolphin.getMaxHeadRotation() + 20,
									this.dolphin.getMaxLookPitchChange()
							);
	...
	class PlayWithItemsGoal extends Goal {
		...
		public boolean canStart() {
			if (this.nextPlayingTime > DolphinEntity.this.age) {
				...
				List<ItemEntity> list = DolphinEntity.this.getEntityWorld()
				                                          .getEntitiesByClass(
						                                          ItemEntity.class,
						                                          DolphinEntity.this
								                                          .getBoundingBox()
		...
		public void start() {
			List<ItemEntity> list = DolphinEntity.this.getEntityWorld()
			                                          .getEntitiesByClass(
					                                          ItemEntity.class,
					                                          DolphinEntity.this.getBoundingBox().expand(8.0, 8.0, 8.0),
					                                          DolphinEntity.CAN_TAKE
			                                          );
			if (!list.isEmpty()) {
		...
		private void spitOutItem(ItemStack stack) {
			if (!stack.isEmpty()) {
				...
				ItemEntity
						itemEntity =
						new ItemEntity(
								DolphinEntity.this.getEntityWorld(),
								DolphinEntity.this.getX(),
								d,
								DolphinEntity.this.getZ(),
								stack
						);
	...
	static class SwimWithPlayerGoal extends Goal {
		...
		public boolean shouldContinue() {
			return this.closestPlayer != null && this.closestPlayer.isSwimming()
					&& this.dolphin.squaredDistanceTo(this.closestPlayer) < 256.0;
		}

		@Override
		public void start() {
			this.closestPlayer.addStatusEffect(
					new StatusEffectInstance(StatusEffects.DOLPHINS_GRACE, 100),
					this.dolphin
			);
		}
