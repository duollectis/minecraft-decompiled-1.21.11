...
public class GoatEntity extends AnimalEntity {
	...
	public static final double HORN_LOSS_CHANCE = 0.1F;
	private static final TrackedData<Boolean>
			SCREAMING =
			DataTracker.registerData(GoatEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
	private static final TrackedData<Boolean>
			LEFT_HORN =
			DataTracker.registerData(GoatEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
	private static final TrackedData<Boolean>
	...
	public ItemStack getGoatHornStack() {
		...
		TagKey<Instrument>
				tagKey =
				this.isScreaming() ? InstrumentTags.SCREAMING_GOAT_HORNS : InstrumentTags.REGULAR_GOAT_HORNS;
		return this.getEntityWorld()
		           .getRegistryManager()
		           .getOrThrow(RegistryKeys.INSTRUMENT)
		           .getRandomEntry(tagKey, random)
		           .map(instrument -> GoatHornItem.getStackForInstrument(
				           Items.GOAT_HORN,
				           (RegistryEntry<Instrument>) instrument
		           ))
	...
	public @Nullable GoatEntity createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
		...
		if (goatEntity != null) {
			...
			PassiveEntity
					passiveEntity2 =
					(PassiveEntity) (serverWorld.getRandom().nextBoolean() ? this : passiveEntity);
			boolean
					bl =
	...
	public EntityDimensions getBaseDimensions(EntityPose pose) {
		return pose == EntityPose.LONG_JUMPING ? LONG_JUMPING_DIMENSIONS.scaled(this.getScaleFactor())
	...
	public boolean dropHorn() {
		...
		if (!bl && !bl2) {
			...
			ItemEntity
					itemEntity =
					new ItemEntity(this.getEntityWorld(), vec3d.getX(), vec3d.getY(), vec3d.getZ(), itemStack, d, e, f);
	...
	public static boolean canSpawn(
		...
		return world.getBlockState(pos.down()).isIn(BlockTags.GOATS_SPAWNABLE_ON) && isLightLevelValidForNaturalSpawn(
				world,
				pos
		);
	}
