...
public class FoxEntity extends AnimalEntity {
	...
	private static final TrackedData<Integer>
			VARIANT =
			DataTracker.registerData(FoxEntity.class, TrackedDataHandlerRegistry.INTEGER);
	private static final TrackedData<Byte>
			FOX_FLAGS =
			DataTracker.registerData(FoxEntity.class, TrackedDataHandlerRegistry.BYTE);
	...
	private static final EntityDimensions
			...
			EntityType.FOX.getDimensions().scaled(0.5F).withEyeHeight(0.2975F);
	...
	protected void initGoals() {
		...
		this.goalSelector
				.add(
						...
						new FleeEntityGoal<>(
								...
								1.6,
								1.4,
								entity -> NOTICEABLE_PLAYER_FILTER.test(entity) && !this.canTrust(entity)
										&& !this.isAggressive()
						)
				);
		this.goalSelector.add(
				...
				new FleeEntityGoal<>(
						...
						1.6,
						1.4,
						entity -> !((WolfEntity) entity).isTamed() && !this.isAggressive()
				)
		);
		this.goalSelector.add(
				4,
				new FleeEntityGoal<>(this, PolarBearEntity.class, 8.0F, 1.6, 1.4, entity -> !this.isAggressive())
		);
		...
		this.targetSelector
				.add(
						...
						new FoxEntity.DefendFriendGoal(
								...
								false,
								false,
								(entity, world) -> JUST_ATTACKED_SOMETHING_FILTER.test(entity) && !this.canTrust(entity)
						)
				);
	}
		...
		if (status == 45) {
			...
			if (!itemStack.isEmpty()) {
				for (int i = 0; i < 8; i++) {
					Vec3d
							vec3d =
							new Vec3d((this.random.nextFloat() - 0.5) * 0.1, this.random.nextFloat() * 0.1 + 0.1, 0.0)
	...
	public @Nullable FoxEntity createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
		...
		if (foxEntity != null) {
			foxEntity.setVariant(
					this.random.nextBoolean() ? this.getVariant() : ((FoxEntity) passiveEntity).getVariant());
		}
	...
	public static boolean canSpawn(
		...
		return world.getBlockState(pos.down()).isIn(BlockTags.FOXES_SPAWNABLE_ON) && isLightLevelValidForNaturalSpawn(
				world,
				pos
		);
	}
	...
	public <T> @Nullable T get(ComponentType<? extends T> type) {
		return type == DataComponentTypes.FOX_VARIANT ? castComponentValue((ComponentType<T>) type, this.getVariant())
		                                              : super.get(type);
	}
	...
	protected void readCustomData(ReadView view) {
		...
		this.setVariant(view
				.<FoxEntity.Variant>read("Type", FoxEntity.Variant.CODEC)
				.orElse(FoxEntity.Variant.DEFAULT));
	...
	public boolean canPickupItem(ItemStack stack) {
		ItemStack itemStack = this.getEquippedStack(EquipmentSlot.MAINHAND);
		return itemStack.isEmpty() || this.eatingTime > 0 && this.isConsumableFood(stack) && !this.isConsumableFood(
				itemStack);
	}
	...
	public float getHeadRoll(float tickProgress) {
		return MathHelper.lerp(tickProgress, this.lastHeadRollProgress, this.headRollProgress) * 0.11F
				* (float) Math.PI;
	}
	...
	protected @Nullable SoundEvent getAmbientSound() {
		if (this.isSleeping()) {
			...
			if (!this.getEntityWorld().isDay() && this.random.nextFloat() < 0.1F) {
				List<PlayerEntity> list = this.getEntityWorld()
				                              .getEntitiesByClass(
						                              PlayerEntity.class,
						                              this.getBoundingBox().expand(16.0, 16.0, 16.0),
	...
	public static boolean canJumpChase(FoxEntity fox, LivingEntity chasedEntity) {
		...
		for (int j = 0; j < 6; j++) {
			...
			for (int k = 1; k < 4; k++) {
				if (!fox
						.getEntityWorld()
						.getBlockState(BlockPos.ofFloored(fox.getX() + h, fox.getY() + k, fox.getZ() + g))
						.isReplaceable()) {
					return false;
				}
			}
	...
	class AvoidDaylightGoal extends EscapeSunlightGoal {
		...
		public boolean canStart() {
			if (!FoxEntity.this.isSleeping() && this.mob.getTarget() == null) {
				if (FoxEntity.this.getEntityWorld().isThundering() && FoxEntity.this
						.getEntityWorld()
						.isSkyVisible(this.mob.getBlockPos())) {
	...
	abstract class CalmDownGoal extends Goal {
		...
		protected boolean canCalmDown() {
			return !castToServerWorld(FoxEntity.this.getEntityWorld())
					.getTargets(
							LivingEntity.class,
							this.WORRIABLE_ENTITY_PREDICATE,
							FoxEntity.this,
	...
	class DefendFriendGoal extends ActiveTargetGoal<LivingEntity> {
		...
		public DefendFriendGoal(
				...
				final @Nullable boolean checkCanNavigate,
		...
		public boolean canStart() {
			if (this.reciprocalChance > 0 && this.mob.getRandom().nextInt(this.reciprocalChance) != 0) {
				...
				for (LazyEntityReference<LivingEntity> lazyEntityReference : FoxEntity.this
						.getTrustedEntities()
						.toList()) {
	...
	class DelayedCalmDownGoal extends FoxEntity.CalmDownGoal {
		...
		@Override
		public boolean canStart() {
			return FoxEntity.this.sidewaysSpeed == 0.0F && FoxEntity.this.upwardSpeed == 0.0F
		...
		private boolean canNotCalmDown() {
			if (this.timer > 0) {
				...
				return FoxEntity.this.getEntityWorld().isDay() && this.isAtFavoredLocation() && !this.canCalmDown()
		...
		public void start() {
			...
			FoxEntity.this
					.getMoveControl()
					.moveTo(FoxEntity.this.getX(), FoxEntity.this.getY(), FoxEntity.this.getZ(), 0.0);
		}
	}
	...
	public class EatBerriesGoal extends MoveToTargetPosGoal {
		...
		private void pickSweetBerries(BlockState state) {
			...
			FoxEntity.this.getEntityWorld().setBlockState(this.targetPos, state.with(SweetBerryBushBlock.AGE, 1), 2);
			FoxEntity.this
					.getEntityWorld()
					.emitGameEvent(GameEvent.BLOCK_CHANGE, this.targetPos, GameEvent.Emitter.of(FoxEntity.this));
		}
	...
	/**
	 * {@code EscapeWhenNotAggressiveGoal}.
	 */
	...
	public class FoxLookControl extends LookControl {
		...
		protected boolean shouldStayHorizontal() {
			return !FoxEntity.this.isChasing() && !FoxEntity.this.isInSneakingPose() && !FoxEntity.this.isRollingHead()
					&& !FoxEntity.this.isWalking();
		}
	}
	...
	class GoToVillageGoal extends net.minecraft.entity.ai.goal.GoToVillageGoal {
		...
		private boolean canGoToVillage() {
			return !FoxEntity.this.isSleeping() && !FoxEntity.this.isSitting() && !FoxEntity.this.isAggressive()
					&& FoxEntity.this.getTarget() == null;
		}
	}

	/**
	 * {@code JumpChasingGoal}.
	 */
	public class JumpChasingGoal extends DiveJumpingGoal {
		...
		public boolean shouldContinue() {
			...
			if (livingEntity != null && livingEntity.isAlive()) {
				...
				return (!(d * d < 0.05F) || !(Math.abs(FoxEntity.this.getPitch()) < 15.0F)
		...
		public void start() {
			...
			if (livingEntity != null) {
				...
				Vec3d vec3d = new Vec3d(
						livingEntity.getX() - FoxEntity.this.getX(),
						livingEntity.getY() - FoxEntity.this.getY(),
						livingEntity.getZ() - FoxEntity.this.getZ()
				)
						.normalize();
		...
		public void tick() {
			...
			if (livingEntity != null && FoxEntity.this.distanceTo(livingEntity) <= 2.0F) {
				FoxEntity.this.tryAttack(castToServerWorld(FoxEntity.this.getEntityWorld()), livingEntity);
			}
	...
	class MateGoal extends AnimalMateGoal {
		...
		protected void breed() {
			...
			if (foxEntity != null) {
				...
				foxEntity.refreshPositionAndAngles(
						this.animal.getX(),
						this.animal.getY(),
						this.animal.getZ(),
						0.0F,
	...
	class PickupItemGoal extends Goal {
		...
		public boolean canStart() {
			if (!FoxEntity.this.getEquippedStack(EquipmentSlot.MAINHAND).isEmpty()) {
				...
				List<ItemEntity> list = FoxEntity.this.getEntityWorld()
				                                      .getEntitiesByClass(
						                                      ItemEntity.class,
	...
	public static enum Variant implements StringIdentifiable {
		...
		public static final StringIdentifiable.EnumCodec<FoxEntity.Variant>
				CODEC =
		...
		public static final PacketCodec<ByteBuf, FoxEntity.Variant>
				PACKET_CODEC =
	...
	public class WorriableEntityFilter implements TargetPredicate.EntityPredicate {
		...
		public boolean test(LivingEntity livingEntity, ServerWorld serverWorld) {
			...
			else if (livingEntity instanceof ChickenEntity || livingEntity instanceof RabbitEntity
			...
			else if (livingEntity instanceof PlayerEntity playerEntity && (playerEntity.isSpectator()
					|| playerEntity.isCreative()
			)) {
				return false;
			}
			else {
				return FoxEntity.this.canTrust(livingEntity) ? false
				                                             : !livingEntity.isSleeping() && !livingEntity.isSneaky();
			}
		}
	}
}
