...
public class FoxEntity extends AnimalEntity {
      ...
      this.targetSelector
         .add(
            ...
            new FoxEntity.DefendFriendGoal(
               LivingEntity.class, false, false, (entity, world) -> JUST_ATTACKED_SOMETHING_FILTER.test(entity) && !this.canTrust(entity)
            )
         );
   }
   ...
   protected void readCustomData(ReadView view) {
      ...
      this.clearTrusted();
      view.<List<LazyEntityReference<LivingEntity>>>read("Trusted", TRUSTED_ENTITIES_CODEC).orElse(List.of()).forEach(this::trust);
      this.setSleeping(view.getBoolean("Sleeping", false));
      this.setVariant(view.<FoxEntity.Variant>read("Type", FoxEntity.Variant.CODEC).orElse(FoxEntity.Variant.DEFAULT));
   ...
   private void spit(ItemStack stack) {
      if (!stack.isEmpty() && !this.getEntityWorld().isClient()) {
         ItemEntity itemEntity = new ItemEntity(
            this.getEntityWorld(), this.getX() + this.getRotationVector().x, this.getY() + 1.0, this.getZ() + this.getRotationVector().z, stack
         );
   ...
   public static boolean canJumpChase(FoxEntity fox, LivingEntity chasedEntity) {
      ...
      for (int j = 0; j < 6; j++) {
         ...
         for (int k = 1; k < 4; k++) {
            if (!fox.getEntityWorld().getBlockState(BlockPos.ofFloored(fox.getX() + h, fox.getY() + k, fox.getZ() + g)).isReplaceable()) {
               return false;
            }
         }
      }
