...
public class StriderEntity extends AnimalEntity implements ItemSteerable {
	...
	private static final TrackedData<Integer>
			BOOST_TIME =
			DataTracker.registerData(StriderEntity.class, TrackedDataHandlerRegistry.INTEGER);
	private static final TrackedData<Boolean>
			COLD =
			DataTracker.registerData(StriderEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
	...
	public Vec3d updatePassengerForDismount(LivingEntity passenger) {
		...
		for (BlockPos blockPos : set) {
			if (!this.getEntityWorld().getFluidState(blockPos).isIn(FluidTags.LAVA)) {
				double g = this.getEntityWorld().getDismountHeight(blockPos);
				if (Dismounting.canDismountInBlock(g)) {
					...
					while (var14.hasNext()) {
						...
						if (Dismounting.canPlaceEntityAt(this.getEntityWorld(), passenger, box.offset(vec3d2))) {
	...
	protected void playStepSound(BlockPos pos, BlockState state) {
		this.playSound(
				this.isInLava() ? SoundEvents.ENTITY_STRIDER_STEP_LAVA : SoundEvents.ENTITY_STRIDER_STEP,
				1.0F,
				1.0F
		);
	}
	...
	private void updateFloating() {
		if (this.isInLava()) {
			...
			if (shapeContext.isAbove(FluidBlock.COLLISION_SHAPE, this.getBlockPos(), true)
					&& !this.getEntityWorld().getFluidState(this.getBlockPos().up()).isIn(FluidTags.LAVA)) {
	...
	public ActionResult interactMob(PlayerEntity player, Hand hand) {
		...
		if (!bl && this.hasSaddleEquipped() && !this.hasPassengers() && !player.shouldCancelInteraction()) {
			...
			if (!actionResult.isAccepted()) {
				...
				return (ActionResult) (this.canEquip(itemStack, EquipmentSlot.SADDLE) ? itemStack.useOnEntity(
						player,
						this,
						hand
				) : ActionResult.PASS
				...
				if (mobEntity != null) {
					entityData =
							this.initializeRider(
									...
									difficulty,
									mobEntity,
									new ZombieEntity.ZombieData(ZombieEntity.shouldBeBaby(random), false)
							);
	...
	private EntityData initializeRider(
			ServerWorldAccess world,
			LocalDifficulty difficulty,
			MobEntity rider,
			@Nullable EntityData entityData
	...
	static class GoBackToLavaGoal extends MoveToTargetPosGoal {
		...
		protected boolean isTargetPos(WorldView world, BlockPos pos) {
			return world.getBlockState(pos).isOf(Blocks.LAVA) && world
	...
	static class Navigation extends MobNavigation {
		...
		protected boolean canWalkOnPath(PathNodeType pathType) {
			return pathType != PathNodeType.LAVA && pathType != PathNodeType.DAMAGE_FIRE
