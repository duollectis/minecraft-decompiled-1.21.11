...
public class ArmadilloBrain {
	...
	private static final float BREED_SPEED = 1.25F;
	private static final float WALK_TO_ADULT_SPEED = 1.0F;
	...
	private static final SingleTickTask<ArmadilloEntity> UNROLL_TASK = TaskTriggerer.task(
			context -> context.group(context.queryMemoryAbsent(MemoryModuleType.DANGER_DETECTED_RECENTLY))
			                  .apply(
					                  context, memoryQueryResult -> (serverWorld, armadillo, l) -> {
	...
	private static void addCoreActivities(Brain<ArmadilloEntity> brain) {
		brain.setTaskList(
				Activity.CORE,
				0,
				ImmutableList.<Task<? super ArmadilloEntity>>of(
						new StayAboveWaterTask(0.8F),
						new ArmadilloBrain.UnrollAndFleeTask(2.0F),
						new UpdateLookControlTask(45, 90),
						new MoveToTargetTask() {
							@Override
							protected boolean shouldRun(ServerWorld serverWorld, MobEntity mobEntity) {
								return mobEntity instanceof ArmadilloEntity armadilloEntity
										       && armadilloEntity.isNotIdle()
								       ? false
								       : super.shouldRun(serverWorld, mobEntity);
							}
						},
	...
	public static class RollUpTask extends MultiTickTask<ArmadilloEntity> {
		...
		static final int MIN_PEEK_TICKS = 5;
		static final int MAX_PEEK_TICKS = 75;
		...
		protected void keepRunning(ServerWorld serverWorld, ArmadilloEntity armadilloEntity, long l) {
			...
			if (armadilloEntity.shouldSwitchToScaredState()) {
				...
				else if (state == ArmadilloEntity.State.UNROLLING
						&& m > ArmadilloEntity.State.UNROLLING.getLengthInTicks()) {
					armadilloEntity.setState(ArmadilloEntity.State.SCARED);
				}
			}
		}
