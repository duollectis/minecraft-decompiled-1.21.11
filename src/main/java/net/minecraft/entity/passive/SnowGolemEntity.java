...
public class SnowGolemEntity extends GolemEntity implements Shearable, RangedAttackMob {
	...
	private static final TrackedData<Byte>
			SNOW_GOLEM_FLAGS =
			DataTracker.registerData(SnowGolemEntity.class, TrackedDataHandlerRegistry.BYTE);
	...
	protected void initGoals() {
		...
		this.targetSelector.add(
				...
				new ActiveTargetGoal<>(
						...
						true,
						false,
						(entity, serverWorld) -> entity instanceof Monster
				)
		);
	}
	...
	public void tickMovement() {
		...
		if (this.getEntityWorld() instanceof ServerWorld serverWorld) {
			if (serverWorld
					...
					.getAttributeValue(EnvironmentAttributes.SNOW_GOLEM_MELTS_GAMEPLAY, this.getEntityPos())) {
			...
			for (int i = 0; i < 4; i++) {
				...
				if (this.getEntityWorld().getBlockState(blockPos).isAir()
					...
					this
							.getEntityWorld()
							.emitGameEvent(GameEvent.BLOCK_PLACE, blockPos, GameEvent.Emitter.of(this, blockState));
				}
			}
		}
	}
	...
	public void shootAt(LivingEntity target, float pullProgress) {
		...
		if (this.getEntityWorld() instanceof ServerWorld serverWorld) {
			...
			ProjectileEntity.spawn(
					new SnowballEntity(serverWorld, this, itemStack),
					serverWorld,
					itemStack,
					entity -> entity.setVelocity(d, e + g - entity.getY(), f, 1.6F, 12.0F)
			);
		}
	...
	public void sheared(ServerWorld world, SoundCategory shearedSoundCategory, ItemStack shears) {
		...
		this.forEachShearedItem(
				...
				shears,
				(serverWorld, itemStack) -> this.dropStack(serverWorld, itemStack, this.getStandingEyeHeight())
		);
	}
