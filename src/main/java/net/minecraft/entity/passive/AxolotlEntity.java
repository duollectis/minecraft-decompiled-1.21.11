...
public class AxolotlEntity extends AnimalEntity implements Bucketable {
	...
	private static final int ANIMATION_INTERPOLATION_TICKS = 10;
	protected static final ImmutableList<? extends SensorType<? extends Sensor<? super AxolotlEntity>>>
			SENSORS =
			ImmutableList.of(
	...
	private static final TrackedData<Integer>
			VARIANT =
			DataTracker.registerData(AxolotlEntity.class, TrackedDataHandlerRegistry.INTEGER);
	private static final TrackedData<Boolean>
			PLAYING_DEAD =
			DataTracker.registerData(AxolotlEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
	private static final TrackedData<Boolean>
			FROM_BUCKET =
			DataTracker.registerData(AxolotlEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
	...
	protected void readCustomData(ReadView view) {
		...
		this.setVariant(view
				.<AxolotlEntity.Variant>read("Variant", AxolotlEntity.Variant.INDEX_CODEC)
				.orElse(AxolotlEntity.Variant.DEFAULT));
	...
	public EntityData initialize(
		...
		if (spawnReason == SpawnReason.BUCKET) {
			...
			if (entityData instanceof AxolotlEntity.AxolotlData) {
				...
				entityData =
						new AxolotlEntity.AxolotlData(
								AxolotlEntity.Variant.getRandomNatural(random),
								AxolotlEntity.Variant.getRandomNatural(random)
						);
			}
	...
	private void tickClient() {
		...
		boolean
				bl =
				this.limbAnimator.isLimbMoving() || this.getPitch() != this.lastPitch || this.getYaw() != this.lastYaw;
	...
	public <T> @Nullable T get(ComponentType<? extends T> type) {
		return type == DataComponentTypes.AXOLOTL_VARIANT ? castComponentValue(
	...
	public boolean damage(ServerWorld world, DamageSource source, float amount) {
		...
		if (!this.isAiDisabled()
				&& this.getEntityWorld().random.nextInt(3) == 0
				&& (this.getEntityWorld().random.nextInt(3) < amount || f / this.getMaxHealth() < 0.5F)
				&& amount < f
	...
	public void copyDataFromNbt(NbtCompound nbt) {
		...
		nbt.getLong("HuntingCooldown")
		   .ifPresentOrElse(
				   huntingCooldown -> this
						   .getBrain()
						   .remember(MemoryModuleType.HAS_HUNTING_COOLDOWN, true, nbt.getLong("HuntingCooldown", 0L)),
 ...
 */
	public static enum Variant implements StringIdentifiable {
		...
		public static final PacketCodec<ByteBuf, AxolotlEntity.Variant>
				PACKET_CODEC =
				PacketCodecs.indexed(INDEX_MAPPER, AxolotlEntity.Variant::getIndex);
		public static final Codec<AxolotlEntity.Variant>
				CODEC =
				StringIdentifiable.createCodec(AxolotlEntity.Variant::values);
		@Deprecated
		public static final Codec<AxolotlEntity.Variant>
				INDEX_CODEC =
				Codec.INT.xmap(INDEX_MAPPER::apply, AxolotlEntity.Variant::getIndex);
