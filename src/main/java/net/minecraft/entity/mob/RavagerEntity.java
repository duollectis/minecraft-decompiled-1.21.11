...
public class RavagerEntity extends RaiderEntity {
   ...
   private void spawnStunnedParticles() {
      if (this.random.nextInt(6) == 0) {
         ...
         this.getEntityWorld()
            .addParticleClient(TintedParticleEffect.create(ParticleTypes.ENTITY_EFFECT, 0.49803922F, 0.5137255F, 0.57254905F), d, e, f, 0.0, 0.0, 0.0);
      }
   }
   ...
   private void roar() {
      if (this.isAlive() && this.getEntityWorld() instanceof ServerWorld serverWorld) {
         ...
         for (LivingEntity livingEntity : this.getEntityWorld().getEntitiesByClass(LivingEntity.class, this.getBoundingBox().expand(4.0), predicate)) {
