...
public class WardenBrain {
	...
	private static final int DISTURBANCE_MEMORY_DURATION = 100;
	...
	private static void addCoreActivities(Brain<WardenEntity> brain) {
		brain.setTaskList(
				Activity.CORE,
				0,
				ImmutableList.<Task<? super WardenEntity>>of(
	...
	private static void addEmergeActivities(Brain<WardenEntity> brain) {
		brain.setTaskList(
				Activity.EMERGE,
				5,
				ImmutableList.<Task<? super WardenEntity>>of(new EmergeTask(EMERGE_DURATION)),
				MemoryModuleType.IS_EMERGING
		);
	}
	...
	private static void addDigActivities(Brain<WardenEntity> brain) {
		brain.setTaskList(
				...
				ImmutableSet.of(
						Pair.of(MemoryModuleType.ROAR_TARGET, MemoryModuleState.VALUE_ABSENT),
		...
		brain.setTaskList(
				...
				ImmutableList.of(
						...
						ForgetAttackTargetTask.create(
								(world, target) -> !warden.getAngriness().isAngry() || !warden.isValidTarget(target),
