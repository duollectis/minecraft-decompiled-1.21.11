...
public class GhastEntity extends MobEntity implements Monster {
   ...
   public static class GhastMoveControl extends MoveControl {
         ...
         if (this.state == MoveControl.State.MOVE_TO) {
            if (this.collisionCheckCooldown-- <= 0) {
               ...
               if (this.willCollide(vec3d)) {
                  this.ghast
                     .setVelocity(
                        this.ghast.getVelocity().add(vec3d.normalize().multiply(this.ghast.getAttributeValue(EntityAttributes.FLYING_SPEED) * 5.0 / 3.0))
                     );
               } else {
                  this.state = MoveControl.State.WAIT;
               }
      ...
      private boolean canPassThrough(
         BlockView world, @Nullable Vec3d oldPos, @Nullable Vec3d newPos, BlockPos blockPos, boolean waterAllowed, boolean lavaAllowed
      ) {
