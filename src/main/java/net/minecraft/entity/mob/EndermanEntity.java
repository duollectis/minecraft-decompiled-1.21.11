...
public class EndermanEntity extends HostileEntity implements Angerable {
   ...
   public void playAngrySound() {
      if (this.age >= this.lastAngrySoundAge + 400) {
         ...
         if (!this.isSilent()) {
            this.getEntityWorld()
               .playSoundClient(this.getX(), this.getEyeY(), this.getZ(), SoundEvents.ENTITY_ENDERMAN_STARE, this.getSoundCategory(), 2.5F, 1.0F, false);
         }
      }
   }
   ...
   protected void readCustomData(ReadView view) {
      ...
      this.setCarriedBlock(view.<BlockState>read("carriedBlockState", BlockState.CODEC).filter(blockState -> !blockState.isAir()).orElse(null));
   ...
   private boolean teleportTo(double x, double y, double z) {
      ...
      if (bl && !bl2) {
         ...
         if (bl3) {
            ...
            if (!this.isSilent()) {
               this.getEntityWorld()
                  .playSound(null, this.lastX, this.lastY, this.lastZ, SoundEvents.ENTITY_ENDERMAN_TELEPORT, this.getSoundCategory(), 1.0F, 1.0F);
   ...
   static class TeleportTowardsPlayerGoal extends ActiveTargetGoal<PlayerEntity> {
      ...
      public TeleportTowardsPlayerGoal(EndermanEntity enderman, TargetPredicate.@Nullable EntityPredicate targetPredicate) {
