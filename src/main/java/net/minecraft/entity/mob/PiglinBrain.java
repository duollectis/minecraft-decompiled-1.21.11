...
/**
 * Мозг (Brain) для пиглина. Управляет поведением, торговлей и памятью.
 */
public class PiglinBrain {

	public static final int DESIRED_DISTANCE_FROM_ENTITY = 8;
	public static final int DESIRED_DISTANCE_FROM_TARGET = 4;
	public static final Item BARTERING_ITEM = Items.GOLD_INGOT;
	private static final int GOLD_ITEM_PICKUP_RANGE = 16;
	private static final int BARTERING_DURATION = 600;
	private static final int ADMIRE_ITEM_DURATION = 119;
	private static final int WANTED_ITEM_PICKUP_RANGE = 9;
	private static final int ADMIRE_ITEM_TIME_LIMIT_MIN = 200;
	private static final int ADMIRE_ITEM_TIME_LIMIT_MAX = 200;
	private static final int DEFEAT_TARGET_DURATION = 300;
	...
	private static final int RIDE_TARGET_RANGE = 8;
	...
	private static final int MELEE_ATTACK_DURATION = 20;
	private static final UniformIntProvider AVOID_MEMORY_DURATION = TimeHelper.betweenSeconds(5, MELEE_ATTACK_DURATION);
	private static final int CROSSBOW_ATTACK_RANGE = 200;
	private static final int AVOID_TARGET_RANGE = 12;
	private static final int SOUL_FIRE_REPELLENT_RANGE = 8;
	private static final int GOLD_HOLDING_PLAYER_RANGE = 14;
	private static final int HOGLIN_RIDE_RANGE = 8;
	private static final int HOGLIN_RIDE_MEMORY_RANGE = 5;
	private static final float CROSSBOW_ATTACK_FORWARD_MOVEMENT = 0.75F;
	private static final int ZOMBIFIED_PIGLIN_RANGE = 6;
	...
	private static final float BARTER_SPEED = 0.1F;
	private static final float WALK_SPEED = 1.0F;
	private static final float FIGHT_SPEED = 1.0F;
	private static final float START_RIDING_SPEED = 0.8F;
	private static final float CELEBRATE_SPEED = 1.0F;
	private static final float AVOID_SPEED = 1.0F;
	...
	protected static void setHuntedRecently(PiglinEntity piglin, Random random) {
		int duration = HUNT_MEMORY_DURATION.get(random);
		piglin.getBrain().remember(MemoryModuleType.HUNTED_RECENTLY, true, duration);
	}
	...
	private static void addCoreActivities(Brain<PiglinEntity> brain) {
		brain.setTaskList(
				Activity.CORE,
				0,
				ImmutableList.<Task<? super PiglinEntity>>of(
						...
						AdmireItemTask.create(ADMIRE_ITEM_DURATION),
						DefeatTargetTask.create(DEFEAT_TARGET_DURATION, PiglinBrain::isHuntingTarget),
	...
	private static void addFightActivities(PiglinEntity piglin, Brain<PiglinEntity> brain) {
		brain.setTaskList(
				...
				10,
				ImmutableList.<Task<? super PiglinEntity>>of(
						ForgetAttackTargetTask.create((world, target) -> !isPreferredAttackTarget(
								world,
								piglin,
								target
						)),
						TaskTriggerer.runIf(
								...
								LookTowardsAttackTargetTask.create(5, CROSSBOW_ATTACK_FORWARD_MOVEMENT)
						),
						RangedApproachTask.create(1.0F),
						new SpearAttackTask(1.0, 10.0F),
						new SpearChargeTask(1.0, 1.0, 10.0F, 2.0F),
						new SpearFleeTask(1.0),
						MeleeAttackTask.create(MELEE_ATTACK_DURATION),
	...
	private static void addCelebrateActivities(Brain<PiglinEntity> brain) {
		brain.setTaskList(
				...
				ImmutableList.of(
						...
						TaskTriggerer.runIf(
								piglin -> !piglin.isDancing(),
								WalkTowardsFuzzyPosTask.create(MemoryModuleType.CELEBRATE_LOCATION, 2, 1.0F)
						),
						TaskTriggerer.runIf(
								PiglinEntity::isDancing,
								WalkTowardsFuzzyPosTask.create(MemoryModuleType.CELEBRATE_LOCATION, 4, WANDER_SPEED)
						),
						new RandomTask<PiglinEntity>(
								ImmutableList.of(
										Pair.of(LookAtMobTask.create(EntityType.PIGLIN, 8.0F), 1),
										Pair.of(StrollTask.create(WANDER_SPEED, 2, 1), 1),
										Pair.of(new WaitTask(10, MELEE_ATTACK_DURATION), 1)
								)
						)
				),
	...
	private static void addAdmireItemActivities(Brain<PiglinEntity> brain) {
		brain.setTaskList(
				...
				10,
				ImmutableList.of(
						WalkTowardsNearestVisibleWantedItemTask.create(
						...
						AdmireItemTimeLimitTask.create(ADMIRE_ITEM_TIME_LIMIT_MIN, 200)
				),
	...
	private static void addAvoidActivities(Brain<PiglinEntity> brain) {
		brain.setTaskList(
				...
				10,
				ImmutableList.of(
						GoToRememberedPositionTask.createEntityBased(MemoryModuleType.AVOID_TARGET, 1.0F, AVOID_TARGET_RANGE, true),
						makeRandomFollowTask(),
						makeRandomWanderTask(),
	...
	private static void addRideActivities(Brain<PiglinEntity> brain) {
		brain.setTaskList(
				...
				10,
				ImmutableList.<Task<? super PiglinEntity>>of(
						StartRidingTask.create(START_RIDING_SPEED),
						LookAtMobTask.create(PiglinBrain::isGoldHoldingPlayer, 8.0F),
						TaskTriggerer.runIf(
								TaskTriggerer.predicate(Entity::hasVehicle),
								Tasks.<PiglinEntity>pickRandomly(
										ImmutableList
												.<Pair<? extends TaskRunnable<? super PiglinEntity>, Integer>>builder()
												.addAll(makeFollowTasks())
												.add(Pair.of(TaskTriggerer.predicate(piglin -> true), 1))
												.build()
								)
						),
	...
	private static ImmutableList<Pair<SingleTickTask<LivingEntity>, Integer>> makeFollowTasks() {
		return ImmutableList.<Pair<SingleTickTask<LivingEntity>, Integer>>of(
	...
	private static RandomTask<LivingEntity> makeRandomFollowTask() {
		return new RandomTask<LivingEntity>(
	...
	private static RandomTask<PiglinEntity> makeRandomWanderTask() {
		return new RandomTask<>(
				ImmutableList.of(
						Pair.of(StrollTask.create(WANDER_SPEED), 2),
						Pair.of(
						...
						Pair.of(TaskTriggerer.runIf(PiglinBrain::canWander, GoToLookTargetTask.create(WANDER_SPEED, 3)), 2),
	...
	private static boolean canRideHoglin(PiglinEntity piglin) {
		...
		return vehicle instanceof PiglinEntity riddenPiglin && riddenPiglin.isBaby()
				|| vehicle instanceof HoglinEntity riddenHoglin && riddenHoglin.isBaby();
	}

	/**
	 * Обрабатывает подбор предмета пиглином: золотые самородки поглощаются целиком,
	 * остальные предметы — по одному с перемещением в инвентарь.
	 */
	...
	private static void doBarter(PiglinEntity piglin, List<ItemStack> items) {
		Optional<PlayerEntity>
				optional =
				piglin.getBrain().getOptionalRegisteredMemory(MemoryModuleType.NEAREST_VISIBLE_PLAYER);
	...
	private static void dropBarteredItem(PiglinEntity piglin, PlayerEntity player, List<ItemStack> items) {
		drop(piglin, items, player.getEntityPos());
	}
	...
	private static List<ItemStack> getBarteredItem(PiglinEntity piglin) {
		LootTable
				...
				piglin
						...
						.getLootTable(LootTables.PIGLIN_BARTERING_GAMEPLAY);
		List<ItemStack> list = lootTable.generateLoot(
				new LootWorldContext.Builder((ServerWorld) piglin.getEntityWorld())
	...
	private static boolean isHuntingTarget(LivingEntity piglin, LivingEntity target) {
		return target.getType() == EntityType.HOGLIN
				&& Random.create(piglin.getEntityWorld().getTime()).nextFloat() < BARTER_SPEED;
	}
	...
	private static boolean getNearestZombifiedPiglin(PiglinEntity piglin) {
		...
		if (brain.hasMemoryModule(MemoryModuleType.NEAREST_VISIBLE_ZOMBIFIED)) {
			LivingEntity
					livingEntity =
					brain.getOptionalRegisteredMemory(MemoryModuleType.NEAREST_VISIBLE_ZOMBIFIED).get();
	...
	private static Optional<? extends LivingEntity> getPreferredTarget(ServerWorld world, PiglinEntity piglin) {
		...
		if (getNearestZombifiedPiglin(piglin)) {
			...
			if (optional.isPresent() && Sensor.testAttackableTargetPredicateIgnoreVisibility(
					world,
					piglin,
					optional.get()
			)) {
				...
				Optional<MobEntity>
						optional2 =
						brain.getOptionalRegisteredMemory(MemoryModuleType.NEAREST_VISIBLE_NEMESIS);
				if (optional2.isPresent()) {
					return optional2;
				}
				else {
					Optional<PlayerEntity>
							optional3 =
							brain.getOptionalRegisteredMemory(MemoryModuleType.NEAREST_TARGETABLE_PLAYER_NOT_WEARING_GOLD);
					return optional3.isPresent() && Sensor.testAttackableTargetPredicate(world, piglin, optional3.get())
	...
	public static void onGuardedBlockInteracted(ServerWorld world, PlayerEntity player, boolean blockOpen) {
		List<PiglinEntity>
				...
				player
						.getEntityWorld()
						.getNonSpectatingEntities(PiglinEntity.class, player.getBoundingBox().expand(16.0));
		list
				.stream()
				.filter(PiglinBrain::hasIdleActivity)
				.filter(piglin -> !blockOpen || TargetUtil.isVisibleInMemory(piglin, player))
				.forEach(nearbyPiglin -> {
					if (world.getGameRules().getValue(GameRules.UNIVERSAL_ANGER)) {
	...
	protected static boolean isWillingToTrade(PiglinEntity piglin, ItemStack nearbyItems) {
		return !hasBeenHitByPlayer(piglin) && !isAdmiringItem(piglin) && piglin.isAdult() && acceptsForBarter(
				nearbyItems);
	}
		...
		if (!piglin.getBrain().hasActivity(Activity.AVOID)) {
			if (Sensor.testAttackableTargetPredicateIgnoreVisibility(world, piglin, target)) {
				if (!TargetUtil.isNewTargetTooFar(piglin, target, 4.0)) {
					if (target.getType() == EntityType.PLAYER && world
							.getGameRules()
	...
	protected static List<AbstractPiglinEntity> getNearbyVisiblePiglins(PiglinEntity piglin) {
		return piglin
				.getBrain()
				.getOptionalRegisteredMemory(MemoryModuleType.NEAREST_VISIBLE_ADULT_PIGLINS)
	...
	private static List<AbstractPiglinEntity> getNearbyPiglins(AbstractPiglinEntity piglin) {
		return piglin
				.getBrain()
				.getOptionalRegisteredMemory(MemoryModuleType.NEARBY_ADULT_PIGLINS)
				.orElse(ImmutableList.of());
	}
	...
	private static Task<LivingEntity> makeRememberRideableHoglinTask() {
		LookAtMobWithIntervalTask.Interval
				interval =
				new LookAtMobWithIntervalTask.Interval(MEMORY_TRANSFER_TASK_DURATION);
		return MemoryTransferTask.create(
				entity -> entity.isBaby() && interval.shouldRun(entity.getEntityWorld().random),
	...
	public static void angerAtCloserTargets(ServerWorld world, AbstractPiglinEntity piglin, LivingEntity target) {
		getNearbyPiglins(piglin).forEach(nearbyPiglin -> {
			if (target.getType() != EntityType.HOGLIN
					|| nearbyPiglin.canHunt() && ((HoglinEntity) target).canBeHunted()) {
	...
	public static void becomeAngryWith(ServerWorld world, AbstractPiglinEntity piglin, LivingEntity target) {
		if (Sensor.testAttackableTargetPredicateIgnoreVisibility(world, piglin, target)) {
			...
			piglin.getBrain().remember(MemoryModuleType.ANGRY_AT, target.getUuid(), BARTERING_DURATION);
			if (target.getType() == EntityType.HOGLIN && piglin.canHunt()) {
				rememberHunting(piglin);
			}

			if (target.getType() == EntityType.PLAYER && world.getGameRules().getValue(GameRules.UNIVERSAL_ANGER)) {
				piglin.getBrain().remember(MemoryModuleType.UNIVERSAL_ANGER, true, BARTERING_DURATION);
			}
		}
	}
	...
	private static void runAwayFromClosestTarget(PiglinEntity piglin, LivingEntity target) {
		...
		LivingEntity
				...
				TargetUtil.getCloserEntity(
						piglin,
						brain.getOptionalRegisteredMemory(MemoryModuleType.AVOID_TARGET),
						target
				);
		livingEntity =
				TargetUtil.getCloserEntity(
	...
	private static boolean shouldRunAwayFromHoglins(PiglinEntity piglin) {
		...
		if (!brain.hasMemoryModule(MemoryModuleType.AVOID_TARGET)) {
			...
			if (entityType == EntityType.HOGLIN) {
				...
				return isZombified(entityType)
				       ? !brain.hasMemoryModuleWithValue(MemoryModuleType.NEAREST_VISIBLE_ZOMBIFIED, livingEntity)
				       : false;
			}
		}
	}
	...
	private static boolean hasOutnumberedHoglins(PiglinEntity piglins) {
		int
				i =
				piglins.getBrain().getOptionalRegisteredMemory(MemoryModuleType.VISIBLE_ADULT_PIGLIN_COUNT).orElse(0)
						+ 1;
	...
	private static void runAwayFrom(PiglinEntity piglin, LivingEntity target) {
		...
		piglin
				...
				.remember(
						MemoryModuleType.AVOID_TARGET,
						target,
						AVOID_MEMORY_DURATION.get(piglin.getEntityWorld().random)
				);
		rememberHunting(piglin);
	}
	...
	public static void rememberHunting(AbstractPiglinEntity piglin) {
		piglin
				...
				.remember(
						...
						true,
						HUNT_MEMORY_DURATION.get(piglin.getEntityWorld().random)
				);
	}

	private static void setEatenRecently(PiglinEntity piglin) {
		piglin.getBrain().remember(MemoryModuleType.ATE_RECENTLY, true, ADMIRE_ITEM_TIME_LIMIT_MIN);
	}
	...
	private static void setAdmiringItem(LivingEntity entity) {
		entity.getBrain().remember(MemoryModuleType.ADMIRING_ITEM, true, ADMIRE_ITEM_DURATION);
	}
