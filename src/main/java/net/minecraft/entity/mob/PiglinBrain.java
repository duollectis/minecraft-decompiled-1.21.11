...
public class PiglinBrain {
   ...
   private static void addFightActivities(PiglinEntity piglin, Brain<PiglinEntity> brain) {
      brain.setTaskList(
         ...
         ImmutableList.<Task<? super PiglinEntity>>of(
            ...
            TaskTriggerer.runIf(PiglinBrain::isHoldingCrossbow, LookTowardsAttackTargetTask.create(5, 0.75F)),
         ...
         ImmutableList.<Task<? super PiglinEntity>>of(
            ...
            TaskTriggerer.runIf(
               ...
               Tasks.<PiglinEntity>pickRandomly(
                  ImmutableList.<Pair<? extends TaskRunnable<? super PiglinEntity>, Integer>>builder()
   ...
   private static List<ItemStack> getBarteredItem(PiglinEntity piglin) {
      ...
      List<ItemStack> list = lootTable.generateLoot(
         new LootWorldContext.Builder((ServerWorld)piglin.getEntityWorld()).add(LootContextParameters.THIS_ENTITY, piglin).build(LootContextTypes.BARTER)
      );
      return list;
   }

   private static boolean isHuntingTarget(LivingEntity piglin, LivingEntity target) {
      return target.getType() != EntityType.HOGLIN ? false : Random.create(piglin.getEntityWorld().getTime()).nextFloat() < 0.1F;
   }
