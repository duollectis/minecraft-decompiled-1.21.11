   ...
   public void tick() {
      ...
      if (this.isOnGround() && !this.onGroundLastTick) {
         ...
         for (int i = 0; i < f * 16.0F; i++) {
            ...
            this.getEntityWorld().addParticleClient(this.getParticles(), this.getX() + k, this.getY(), this.getZ() + l, 0.0, 0.0, 0.0);
         }
   ...
   public static boolean canSpawn(EntityType<SlimeEntity> type, WorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
      if (world.getDifficulty() != Difficulty.PEACEFUL) {
         ...
         if (world.getBiome(pos).isIn(BiomeTags.ALLOWS_SURFACE_SLIME_SPAWNS) && pos.getY() > 50 && pos.getY() < 70) {
            float f = world.getEnvironmentAttributes().getAttributeValue(EnvironmentAttributes.SURFACE_SLIME_SPAWN_CHANCE_GAMEPLAY, pos);
