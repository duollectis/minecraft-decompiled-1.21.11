...
public class HoglinBrain {
	...
	public static final int AVOID_DISTANCE = 8;
	public static final int NEAREST_ADULT_DISTANCE = 4;
	private static final UniformIntProvider AVOID_MEMORY_DURATION = TimeHelper.betweenSeconds(5, 20);
	private static final int PACIFY_COOLDOWN = 200;
	private static final int LOOK_DISTANCE = 8;
	...
	private static final int FIGHT_PACIFY_COOLDOWN = 200;
	...
	private static final float BREED_MOVE_SPEED = 0.6F;
	...
	private static void avoid(HoglinEntity hoglin, LivingEntity target) {
		...
		hoglin
				...
				.remember(
						MemoryModuleType.AVOID_TARGET,
						target,
						AVOID_MEMORY_DURATION.get(hoglin.getEntityWorld().random)
				);
	}

	private static Optional<? extends LivingEntity> getNearestVisibleTargetablePlayer(
			ServerWorld world,
			HoglinEntity hoglin
	) {
	...
	private static List<HoglinEntity> getAdultHoglinsAround(HoglinEntity hoglin) {
		return hoglin
				.getBrain()
				.getOptionalRegisteredMemory(MemoryModuleType.NEAREST_VISIBLE_ADULT_HOGLINS)
