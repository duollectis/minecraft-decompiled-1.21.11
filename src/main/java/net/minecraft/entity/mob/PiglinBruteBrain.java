...
public class PiglinBruteBrain {
	...
	protected static void setCurrentPosAsHome(PiglinBruteEntity piglinBrute) {
		GlobalPos
				globalPos =
				GlobalPos.create(piglinBrute.getEntityWorld().getRegistryKey(), piglinBrute.getBlockPos());
	...
	private static Optional<? extends LivingEntity> getTarget(ServerWorld world, AbstractPiglinEntity piglin) {
		...
		if (optional.isPresent() && Sensor.testAttackableTargetPredicateIgnoreVisibility(
			...
			return optional2.isPresent() ? optional2 : piglin
			                                           .getBrain()
			                                           .getOptionalRegisteredMemory(MemoryModuleType.NEAREST_VISIBLE_NEMESIS);
		}
	}
