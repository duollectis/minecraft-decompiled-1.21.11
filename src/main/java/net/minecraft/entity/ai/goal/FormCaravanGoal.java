...
public class FormCaravanGoal extends Goal {
	...
	public boolean canStart() {
		if (!this.llama.isLeashed() && !this.llama.isFollowing()) {
			List<Entity> list = this.llama.getEntityWorld().getOtherEntities(
					this.llama, this.llama.getBoundingBox().expand(9.0, 4.0, 9.0), entityx -> {
		...
		if (this.llama.isFollowing()) {
			if (!(this.llama.getLeashHolder() instanceof LeashKnotEntity)) {
				...
				Vec3d
						vec3d =
						new Vec3d(
								llamaEntity.getX() - this.llama.getX(),
								llamaEntity.getY() - this.llama.getY(),
								llamaEntity.getZ() - this.llama.getZ()
						)
								.normalize()
				...
				this.llama
						.getNavigation()
						.startMovingTo(
								this.llama.getX() + vec3d.x,
								this.llama.getY() + vec3d.y,
								this.llama.getZ() + vec3d.z,
								this.speed
						);
			}
		}
