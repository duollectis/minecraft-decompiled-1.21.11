...
/**
 * Фабричный класс задачи мозга, направляющей сущность к ближайшей кровати (HOME POI) для укрытия.
 * Сначала ищет кровать в предпочтительном радиусе, затем в максимальном, затем использует запомненный дом.
 */
				                  ...
				                  .apply(
						                  ...
						                  (walkTarget, home, hidingPlace, path, lookTarget, breedTarget, interactionTarget) -> (world, entity, time) -> {
							                  world.getPointOfInterestStorage()
							                       ...
							                       .filter(pos -> pos.isWithinDistance(
									                       entity.getEntityPos(),
									                       preferredDistance
							                       ))
							                       .or(
							                       ...
							                       .or(() -> context
									                       .<GlobalPos>getOptionalValue(home)
									                       .map(GlobalPos::pos))
							                       .ifPresent(pos -> {
								                       path.forget();
								                       lookTarget.forget();
								                       ...
								                       hidingPlace.remember(GlobalPos.create(
