...
public abstract class AbstractBoatEntity extends VehicleEntity implements Leashable {
	...
	public static final int UNDERWATER_TICKS_MAX = 60;
	...
	protected Vec3d getPassengerAttachmentPos(Entity passenger, EntityDimensions dimensions, float scaleFactor) {
		...
		return new Vec3d(0.0, this.getPassengerAttachmentY(dimensions), f).rotateY(
	...
	public void tick() {
		...
		if (this.location != AbstractBoatEntity.Location.UNDER_WATER
				&& this.location != AbstractBoatEntity.Location.UNDER_FLOWING_WATER) {
		...
		if (this.isLogicalSideForUpdatingMovement()) {
			...
			if (this.getEntityWorld().isClient()) {
				...
				this
						.getEntityWorld()
						.sendPacket(new BoatPaddleStateC2SPacket(this.isPaddleMoving(0), this.isPaddleMoving(1)));
			}
		...
		for (int i = 0; i <= 1; i++) {
			if (this.isPaddleMoving(i)) {
				if (!this.isSilent()
						...
						&& (this.paddlePhases[i] + (float) (Math.PI / 8)) % (float) (Math.PI * 2) >= (float) (Math.PI
						/ 4
				)) {
		...
		List<Entity>
				...
				this
						...
						.getOtherEntities(
								this,
								this.getBoundingBox().expand(0.2F, -0.01F, 0.2F),
			...
			if (i > 0) {
				...
				if (j > 0 && i == 0) {
					...
					if (this.bubbleColumnIsDrag) {
						...
						this.setVelocity(
								vec3d.x,
								this.hasPassenger(passenger -> passenger instanceof PlayerEntity) ? 2.7 : 0.6,
								vec3d.z
						);
					}
				}
	...
	public float lerpPaddlePhase(int paddle, float tickProgress) {
		return this.isPaddleMoving(paddle)
		       ? MathHelper.clampedLerp(
				tickProgress,
				this.paddlePhases[paddle] - (float) (Math.PI / 8),
				this.paddlePhases[paddle]
		)
		       : 0.0F;
	}
					...
					for (int s = k; s < l; s++) {
						if (r <= 0 || s != k && s != l - 1) {
							...
							if (!(blockState.getBlock() instanceof LilyPadBlock)
									...
									blockState.getCollisionShape(this.getEntityWorld(), mutable).offset(mutable),
									voxelShape,
		...
		for (int o = i; o < j; o++) {
			for (int p = k; p < l; p++) {
				for (int q = m; q < n; q++) {
					...
					FluidState fluidState = this.getEntityWorld().getFluidState(mutable);
					if (fluidState.isIn(FluidTags.WATER)) {
						float f = p + fluidState.getHeight(this.getEntityWorld(), mutable);
	...
	private AbstractBoatEntity.@Nullable Location getUnderWaterLocation() {
		Box box = this.getBoundingBox();
		...
		for (int o = i; o < j; o++) {
			for (int p = k; p < l; p++) {
				for (int q = m; q < n; q++) {
					...
					FluidState fluidState = this.getEntityWorld().getFluidState(mutable);
					if (fluidState.isIn(FluidTags.WATER)
							&& d < mutable.getY() + fluidState.getHeight(this.getEntityWorld(), mutable)) {
	...
	public Vec3d updatePassengerForDismount(LivingEntity passenger) {
		...
		if (!this.getEntityWorld().isWater(blockPos2)) {
			...
			double g = this.getEntityWorld().getDismountHeight(blockPos2);
			...
			while (var14.hasNext()) {
				...
				for (Vec3d vec3d2 : list) {
					if (Dismounting.canPlaceEntityAt(this.getEntityWorld(), vec3d2, passenger, entityPose)) {
	...
	public ActionResult interact(PlayerEntity player, Hand hand) {
		...
		if (actionResult != ActionResult.PASS) {
			...
			return (ActionResult) (player.shouldCancelInteraction()
					                       || !(this.ticksUnderwater < 60.0F)
					                       || !this.getEntityWorld().isClient() && !player.startRiding(this)
			                       ? ActionResult.PASS
	...
	protected void fall(double heightDifference, boolean onGround, BlockState state, BlockPos landedPosition) {
		...
		if (!this.hasVehicle()) {
			...
			else if (!this.getEntityWorld().getFluidState(this.getBlockPos().down()).isIn(FluidTags.WATER)
	...
	public boolean isPaddleMoving(int paddle) {
		return this.dataTracker.get(paddle == 0 ? LEFT_PADDLE_MOVING : RIGHT_PADDLE_MOVING)
	...
	public @Nullable LivingEntity getControllingPassenger() {
		return this.getFirstPassenger() instanceof LivingEntity livingEntity ? livingEntity
	...
	public boolean isSubmergedInWater() {
		return this.location == AbstractBoatEntity.Location.UNDER_WATER
				|| this.location == AbstractBoatEntity.Location.UNDER_FLOWING_WATER;
	}
