...
public abstract class AbstractBoatEntity extends VehicleEntity implements Leashable {
   ...
   public float lerpPaddlePhase(int paddle, float tickProgress) {
      return this.isPaddleMoving(paddle)
         ? MathHelper.clampedLerp(tickProgress, this.paddlePhases[paddle] - (float) (Math.PI / 8), this.paddlePhases[paddle])
         : 0.0F;
   }
                  ...
                  if (r <= 0 || s != k && s != l - 1) {
                     ...
                     if (!(blockState.getBlock() instanceof LilyPadBlock)
                        && VoxelShapes.matchesAnywhere(
                           blockState.getCollisionShape(this.getEntityWorld(), mutable).offset(mutable), voxelShape, BooleanBiFunction.AND
                        )) {
      ...
      for (int o = i; o < j; o++) {
         for (int p = k; p < l; p++) {
            for (int q = m; q < n; q++) {
               mutable.set(o, p, q);
               FluidState fluidState = this.getEntityWorld().getFluidState(mutable);
               if (fluidState.isIn(FluidTags.WATER) && d < mutable.getY() + fluidState.getHeight(this.getEntityWorld(), mutable)) {
