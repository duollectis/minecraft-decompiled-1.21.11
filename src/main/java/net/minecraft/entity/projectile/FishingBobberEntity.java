...
public class FishingBobberEntity extends ProjectileEntity {
   ...
   public void onTrackedDataSet(TrackedData<?> data) {
      if (HOOK_ENTITY_ID.equals(data)) {
         ...
         this.hookedEntity = i > 0 ? this.getEntityWorld().getEntityById(i - 1) : null;
      }
   ...
   public void tick() {
      ...
      if (playerEntity == null) {
         this.discard();
      } else if (this.getEntityWorld().isClient() || !this.removeIfInvalid(playerEntity)) {
         if (this.isOnGround()) {
            this.removalTimer++;
            ...
            if (this.state == FishingBobberEntity.State.HOOKED_IN_ENTITY) {
               if (this.hookedEntity != null) {
                  if (!this.hookedEntity.isRemoved()
                     && this.hookedEntity.isInteractable()
                     && this.hookedEntity.getEntityWorld().getRegistryKey() == this.getEntityWorld().getRegistryKey()) {
   ...
   private FishingBobberEntity.PositionType getPositionType(BlockPos pos) {
      ...
      if (!blockState.isAir() && !blockState.isOf(Blocks.LILY_PAD)) {
         ...
         return fluidState.isIn(FluidTags.WATER) && fluidState.isStill() && blockState.getCollisionShape(this.getEntityWorld(), pos).isEmpty()
   ...
   public int use(ItemStack usedItem) {
      ...
      if (!this.getEntityWorld().isClient() && playerEntity != null && !this.removeIfInvalid(playerEntity)) {
         ...
         if (this.hookedEntity != null) {
            ...
            LootWorldContext lootWorldContext = new LootWorldContext.Builder((ServerWorld)this.getEntityWorld())
               ...
               playerEntity.getEntityWorld()
                  .spawnEntity(
                     new ExperienceOrbEntity(
                        playerEntity.getEntityWorld(), playerEntity.getX(), playerEntity.getY() + 0.5, playerEntity.getZ() + 0.5, this.random.nextInt(6) + 1
                     )
                  );
   ...
   public void handleStatus(byte status) {
      if (status == 31 && this.getEntityWorld().isClient() && this.hookedEntity instanceof PlayerEntity playerEntity && playerEntity.isMainPlayer()) {
         this.pullHookedEntity(this.hookedEntity);
      }
   ...
   public void onSpawnPacket(EntitySpawnS2CPacket packet) {
      ...
      if (this.getPlayerOwner() == null) {
         ...
         LOGGER.error("Failed to recreate fishing hook on client. {} (id: {}) is not a valid owner.", this.getEntityWorld().getEntityById(i), i);
         this.discard();
      }
   }
