...
public class FishingBobberEntity extends ProjectileEntity {
				...
				if (this.state == FishingBobberEntity.State.HOOKED_IN_ENTITY) {
					if (this.hookedEntity != null) {
						if (!this.hookedEntity.isRemoved()
								...
								&& this.hookedEntity.getEntityWorld().getRegistryKey() == this
								.getEntityWorld()
								.getRegistryKey()) {
							this.setPosition(
					...
					if (bl) {
						...
						if (this.caughtFish) {
							this.setVelocity(this
									...
									.add(
											0.0,
											-0.1 * this.velocityRandom.nextFloat() * this.velocityRandom.nextFloat(),
											0.0
									));
						}
	...
	private FishingBobberEntity.PositionType getPositionType(BlockPos pos) {
		...
		if (!blockState.isAir() && !blockState.isOf(Blocks.LILY_PAD)) {
			...
			return fluidState.isIn(FluidTags.WATER) && fluidState.isStill() && blockState
		...
		if (!this.getEntityWorld().isClient() && playerEntity != null && !this.removeIfInvalid(playerEntity)) {
			...
			if (this.hookedEntity != null) {
				...
				for (ItemStack itemStack : list) {
					ItemEntity
							itemEntity =
							new ItemEntity(this.getEntityWorld(), this.getX(), this.getY(), this.getZ(), itemStack);
	...
	public void handleStatus(byte status) {
		if (status == 31 && this.getEntityWorld().isClient() && this.hookedEntity instanceof PlayerEntity playerEntity
	...
	public void onSpawnPacket(EntitySpawnS2CPacket packet) {
		...
		if (this.getPlayerOwner() == null) {
			...
			LOGGER.error(
					...
					this.getEntityWorld().getEntityById(i),
					i
			);
			this.discard();
		}
