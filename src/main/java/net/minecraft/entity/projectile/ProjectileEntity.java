...
public abstract class ProjectileEntity extends Entity implements Ownable {
	...
	protected ProjectileDeflection hitOrDeflect(HitResult hitResult) {
		if (hitResult.getType() == HitResult.Type.ENTITY) {
			...
			if (projectileDeflection != ProjectileDeflection.NONE) {
				if (entity != this.lastDeflectedEntity && this.deflect(
		...
		else if (this.deflectsAgainstWorldBorder() && hitResult instanceof BlockHitResult blockHitResult
				&& blockHitResult.isAgainstWorldBorder()) {
	...
	protected void onCollision(HitResult hitResult) {
		...
		if (type == HitResult.Type.ENTITY) {
			...
			this
					.getEntityWorld()
					.emitGameEvent(GameEvent.PROJECTILE_LAND, hitResult.getPos(), GameEvent.Emitter.of(this, null));
		}
			...
			this
					...
					.emitGameEvent(
							...
							blockPos,
							GameEvent.Emitter.of(this, this.getEntityWorld().getBlockState(blockPos))
					);
		}
	}
	...
	protected void updateRotation() {
		...
		double d = vec3d.horizontalLength();
		this.setPitch(updateRotation(
				this.lastPitch,
				(float) (MathHelper.atan2(vec3d.y, d) * 180.0F / (float) Math.PI)
		));
		this.setYaw(updateRotation(
				this.lastYaw,
				(float) (MathHelper.atan2(vec3d.x, vec3d.z) * 180.0F / (float) Math.PI)
		));
	}
	...
	public boolean canModifyAt(ServerWorld world, BlockPos pos) {
		Entity entity = this.getOwner();
		return entity instanceof PlayerEntity ? entity.canModifyAt(world, pos) : entity == null || world
		                                                                                           .getGameRules()
	...
	public boolean canBreakBlocks(ServerWorld world) {
		return this.getType().isIn(EntityTypeTags.IMPACT_PROJECTILES) && world
				.getGameRules()
				.getValue(GameRules.PROJECTILES_CAN_BREAK_BLOCKS);
	}
