...
public class FireworkRocketEntity extends ProjectileEntity implements FlyingItemEntity {
   ...
   public void tick() {
      ...
      if (this.life == 0 && !this.isSilent()) {
         this.getEntityWorld()
            .playSound(null, this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_FIREWORK_ROCKET_LAUNCH, SoundCategory.AMBIENT, 3.0F, 1.0F);
      }
   ...
   private void explode(ServerWorld world) {
      ...
      if (f > 0.0F) {
         ...
         for (LivingEntity livingEntity : this.getEntityWorld().getNonSpectatingEntities(LivingEntity.class, this.getBoundingBox().expand(5.0))) {
