...
/**
 * Реестр всех типов фаз дракона. Каждый тип хранит ссылку на класс фазы
 * и умеет создавать её экземпляр через рефлексию.
 */
public class PhaseType<T extends Phase> {
	...
	public Phase create(EnderDragonEntity dragon) {
		try {
			return getConstructor().newInstance(dragon);
		} catch (Exception ex) {
			throw new Error(ex);
		}
	}
	...
	protected Constructor<? extends Phase> getConstructor() throws NoSuchMethodException {
		return phaseClass.getConstructor(EnderDragonEntity.class);
	}
