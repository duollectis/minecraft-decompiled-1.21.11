...
public class EnderDragonEntity extends MobEntity implements Monster {
   ...
   public void tickMovement() {
      ...
      if (this.isDead()) {
         ...
         this.getEntityWorld().addParticleClient(ParticleTypes.EXPLOSION, this.getX() + f, this.getY() + 2.0 + g, this.getZ() + h, 0.0, 0.0, 0.0);
      } else {
         ...
         for (int p = j; p <= m; p++) {
            for (int q = k; q <= n; q++) {
               ...
               if (!blockState.isAir() && !blockState.isIn(BlockTags.DRAGON_TRANSPARENT)) {
                  if (world.getGameRules().getValue(GameRules.DO_MOB_GRIEFING) && !blockState.isIn(BlockTags.DRAGON_IMMUNE)) {
   ...
   protected void updatePostDeath() {
      ...
      if (this.ticksSinceDeath >= 180 && this.ticksSinceDeath <= 200) {
         ...
         float g = (this.random.nextFloat() - 0.5F) * 4.0F;
         float h = (this.random.nextFloat() - 0.5F) * 8.0F;
         this.getEntityWorld().addParticleClient(ParticleTypes.EXPLOSION_EMITTER, this.getX() + f, this.getY() + 2.0 + g, this.getZ() + h, 0.0, 0.0, 0.0);
      }
