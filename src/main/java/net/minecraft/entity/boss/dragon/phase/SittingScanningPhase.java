...
public class SittingScanningPhase extends AbstractSittingPhase {
	...
	private static final TargetPredicate
			...
			TargetPredicate.createAttackable().setBaseMaxDistance(150.0);
	...
	public SittingScanningPhase(EnderDragonEntity enderDragonEntity) {
		...
		this.CLOSE_PLAYER_PREDICATE = TargetPredicate.createAttackable()
		                                             ...
		                                             .setPredicate((player, world) ->
				                                             Math.abs(player.getY() - enderDragonEntity.getY())
						                                             <= 10.0);
	}
		...
		if (livingEntity != null) {
			if (this.ticks > 25) {
				...
				Vec3d
						vec3d =
						new Vec3d(
								livingEntity.getX() - this.dragon.getX(),
								0.0,
								livingEntity.getZ() - this.dragon.getZ()
						).normalize();
				...
				if (g < 0.0F || g > 10.0F) {
					...
					double h = MathHelper.clamp(
							MathHelper.wrapDegrees(
									180.0 - MathHelper.atan2(d, e) * 180.0F / (float) Math.PI - this.dragon.getYaw()),
							-100.0,
							100.0
					);
			...
			if (livingEntity != null) {
				...
				this.dragon
						...
						.setPathTarget(new Vec3d(livingEntity.getX(), livingEntity.getY(), livingEntity.getZ()));
			}
		}
	}
