...
public record EquipmentDropChances(Map<EquipmentSlot, Float> byEquipment) {
	...
	public static final EquipmentDropChances
			DEFAULT =
			new EquipmentDropChances(Util.mapEnum(EquipmentSlot.class, slot -> 0.085F));
	public static final Codec<EquipmentDropChances>
			CODEC =
	...
	public EquipmentDropChances withChance(EquipmentSlot slot, float chance) {
		if (chance < 0.0F) {
			...
			return this.get(slot) == chance
			       ? this
			       : new EquipmentDropChances(Util.mapEnum(
					       EquipmentSlot.class,
					       slotx -> slotx == slot ? chance : this.get(slotx)
			       ));
		}
	}
