...
/**
 * {@code SingleOptionInputControl}.
 */
public record SingleOptionInputControl(
		int width,
		List<SingleOptionInputControl.Entry> entries,
		Text label,
		boolean labelVisible
) implements InputControl {

	public static final MapCodec<SingleOptionInputControl>
			CODEC =
			RecordCodecBuilder.<SingleOptionInputControl>mapCodec(
					                  instance -> instance.group(
							                                      Dialog.WIDTH_CODEC.optionalFieldOf("width", 200).forGetter(SingleOptionInputControl::width),
							                                      Codecs
									                                      .nonEmptyList(SingleOptionInputControl.Entry.CODEC.listOf())
									                                      .fieldOf("options")
									                                      .forGetter(SingleOptionInputControl::entries),
							                                      TextCodecs.CODEC.fieldOf("label").forGetter(SingleOptionInputControl::label),
							                                      Codec.BOOL
									                                      .optionalFieldOf("label_visible", true)
									                                      .forGetter(SingleOptionInputControl::labelVisible)
					                                      )
			                  ...
			                  .validate(inputControl -> {
				                  long
						                  l =
						                  inputControl.entries
								                  .stream()
								                  .filter(SingleOptionInputControl.Entry::initial)
								                  .count();
				                  return l > 1L ? DataResult.error(() -> "Multiple initial values")
