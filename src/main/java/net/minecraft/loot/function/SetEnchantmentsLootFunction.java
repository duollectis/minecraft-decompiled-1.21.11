...
public class SetEnchantmentsLootFunction extends ConditionalLootFunction {
               ...
               this.enchantments
                  .forEach(
                     (enchantment, level) -> builder.set(
                        ...
                        MathHelper.clamp(builder.getLevel((RegistryEntry<Enchantment>)enchantment) + level.nextInt(context), 0, 255)
                     )
                  );
            } else {
               this.enchantments
                  .forEach((enchantment, level) -> builder.set((RegistryEntry<Enchantment>)enchantment, MathHelper.clamp(level.nextInt(context), 0, 255)));
            }
         }
      );
      return stack;
   ...
   public static class Builder extends ConditionalLootFunction.Builder<SetEnchantmentsLootFunction.Builder> {
      private final com.google.common.collect.ImmutableMap.Builder<RegistryEntry<Enchantment>, LootNumberProvider> enchantments = ImmutableMap.builder();
      private final boolean add;
