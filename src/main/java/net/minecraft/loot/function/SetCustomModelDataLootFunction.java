...
/**
 * Функция лута, устанавливающая компонент {@code custom_model_data} предмета.
 * Поддерживает операции над списками вещественных чисел, флагов, строк и цветов.
 */
public class SetCustomModelDataLootFunction extends ConditionalLootFunction {
	...
	private static final Codec<LootNumberProvider> COLOR_CODEC = Codec.withAlternative(
		LootNumberProviderTypes.CODEC, Codecs.RGB, value -> new ConstantLootNumberProvider((float) (int) value)
	);
	...
	public Set<ContextParameter<?>> getAllowedParameters() {
		return Stream.concat(floats.stream(), colors.stream())
	...
	private static <T> List<T> applyValues(Optional<ListOperation.Values<T>> values, List<T> current) {
		return values.<List<T>>map(operation -> operation.apply(current)).orElse(current);
	}

	private static <T, E> List<E> applyMapped(
		Optional<ListOperation.Values<T>> values,
		List<E> current,
		Function<T, E> mapper
	) {
		return values.<List<E>>map(operation -> {
			List<E> mapped = operation.value().stream().map(mapper).toList();
			return operation.operation().apply(current, mapped);
		}).orElse(current);
	}

	@Override
	public ItemStack process(ItemStack stack, LootContext context) {
		CustomModelDataComponent current = stack.getOrDefault(
		...
		stack.set(
			...
			new CustomModelDataComponent(
				applyMapped(floats, current.floats(), provider -> provider.nextFloat(context)),
				applyValues(flags, current.flags()),
				applyValues(strings, current.strings()),
				applyMapped(colors, current.colors(), provider -> provider.nextInt(context))
			)
		);
		return stack;
	}
