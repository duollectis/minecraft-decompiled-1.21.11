...
/** Реестр всех типов функций лута и утилиты для их объединения. */
public class LootFunctionTypes {
	...
	public static final Codec<LootFunction> CODEC = Codec.lazyInitialized(
		() -> Codec.withAlternative(BASE_CODEC, AndLootFunction.INLINE_CODEC)
	);

	public static final Codec<RegistryEntry<LootFunction>> ENTRY_CODEC =
		RegistryElementCodec.of(RegistryKeys.ITEM_MODIFIER, CODEC);

	public static final LootFunctionType<SetCountLootFunction> SET_COUNT =
		register("set_count", SetCountLootFunction.CODEC);

	public static final LootFunctionType<SetItemLootFunction> SET_ITEM =
		register("set_item", SetItemLootFunction.CODEC);

	public static final LootFunctionType<EnchantWithLevelsLootFunction> ENCHANT_WITH_LEVELS =
		register("enchant_with_levels", EnchantWithLevelsLootFunction.CODEC);
	...
	/**
	 * Объединяет список функций лута в одну составную функцию.
	 * Оптимизирует случаи 0, 1 и 2 функций без создания лямбды-цикла.
	 *
	 * @param terms список функций для объединения
	 * @return составная функция, последовательно применяющая все переданные функции
	 */
	public static BiFunction<ItemStack, LootContext, ItemStack> join(
		List<? extends BiFunction<ItemStack, LootContext, ItemStack>> terms
	) {
		List<BiFunction<ItemStack, LootContext, ItemStack>> functions = List.copyOf(terms);

		return switch (functions.size()) {
			case 0 -> NOOP;
			case 1 -> (BiFunction) functions.get(0);
			case 2 -> {
				BiFunction<ItemStack, LootContext, ItemStack> first = functions.get(0);
				BiFunction<ItemStack, LootContext, ItemStack> second = functions.get(1);
				yield (stack, context) -> second.apply(first.apply(stack, context), context);
			}
			default -> (stack, context) -> {
				for (BiFunction<ItemStack, LootContext, ItemStack> function : functions) {
