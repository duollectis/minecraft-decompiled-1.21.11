...
/** Реестр типов провайдеров числовых значений для лут-таблиц. */
public class LootNumberProviderTypes {
	...
	public static final Codec<LootNumberProvider> CODEC = Codec.lazyInitialized(() -> {
		Codec<LootNumberProvider> withUniform = Codec.withAlternative(BASE_CODEC, UniformLootNumberProvider.CODEC.codec());
		return Codec.either(ConstantLootNumberProvider.INLINE_CODEC, withUniform)
			.xmap(
				Either::unwrap,
				provider -> provider instanceof ConstantLootNumberProvider constantProvider
					? Either.left(constantProvider)
	...
	public static final LootNumberProviderType ENCHANTMENT_LEVEL =
		register("enchantment_level", EnchantmentLevelLootNumberProvider.CODEC);
	...
	private static LootNumberProviderType register(String id, MapCodec<? extends LootNumberProvider> codec) {
		return Registry.register(
			...
			new LootNumberProviderType(codec)
		);
	}
}
